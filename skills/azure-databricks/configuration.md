...
> This document is a reference companion to the main [SKILL.md](SKILL.md). Fetching documentation content requires **network access** for this skill:
- **Preferred**: Invoke `mcp_microsoftdocs:microsoft_docs_fetch` with the query string `from=learn-agent-skill`. The response is Markdown.
- **Fallback**: Invoke `fetch_webpage` with the query string `from=learn-agent-skill&accept=text/markdown`. The response is Markdown.
