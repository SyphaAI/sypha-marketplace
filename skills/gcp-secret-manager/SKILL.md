...
description: >-
  Store and protect secrets in Google Cloud Secret Manager. Configure IAM
  policies, integrate with GKE, and manage secret versions. Use when handling
...
Securely store and manage secrets within Google Cloud Platform.

## When to Use This Skill

Apply this skill when:
- Working with secrets in GCP environments
- Integrating secrets into GKE workloads via Workload Identity
- Storing API keys, database credentials, or TLS certificates
- Implementing secret versioning and rotation workflows
- Meeting compliance requirements that mandate centralized secret management
...
- Prefer Workload Identity for GKE over exported service account keys
- Enforce IAM least-privilege at the individual secret level rather than the project level
- Enable audit logging for all secret access via Cloud Audit Logs
- Leverage secret versions to allow safe rollback when rotation problems arise
- Apply expiration dates or TTLs to temporary secrets
- Integrate with Cloud KMS for customer-managed encryption keys
- Apply labels consistently to support organization and automation
- Use Cloud Monitoring to track secret access patterns
- Establish rotation schedules for all long-lived credentials
- Apply conditional IAM bindings to restrict access by resource name pattern
