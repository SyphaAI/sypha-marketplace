...
When a user reports a problem with the project, inspect the `target/run_results.json` file to determine which resources failed and the reason why.

Verify the `metadata.generated_at` key to confirm the data is current.
