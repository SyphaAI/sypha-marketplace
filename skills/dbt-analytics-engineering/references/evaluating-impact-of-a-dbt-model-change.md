...
Evaluate downstream dependencies prior to modifying a dbt model. Establishes the scope of impact and recommends suitable build selectors.

## When to Use

- Before altering SQL logic in an existing model
- Before renaming, removing, or updating column types
- Before switching model materialization
...
Look for these tools first — they deliver richer lineage data:
...
When modifying or dropping a column, determine which downstream models reference it:
...
With MCP tools, use `get_column_lineage` for accurate tracking.
...
**Skipping the pre-change check** - Always perform the impact assessment first, even for changes that appear minor.

**Overlooking column-level impact** - Dropping a column breaks downstream models that depend on it. Verify column usage, not just model dependencies.

**Building without scope limits** - Use `--select` to constrain the build. Never run `dbt build` without selectors on large projects.
