...
This document tracks ES|QL language features, commands, and functions across Elasticsearch versions. Consult it to
determine compatibility when authoring queries for a specific Elasticsearch deployment.
...
Available under `TS ... | STATS`. See [time-series-queries.md](time-series-queries.md) for the full reference. All time
...
- Use `WHERE` to narrow results to the relevant subset
- Use `STATS` to aggregate at query time
- For exports, use the Search API with `search_after` instead

### Time Zone Support (Limited before Serverless / 9.4)

ES|QL has **limited timezone support** on self-managed clusters prior to 9.4. All dates are processed internally in UTC
and there is no per-function timezone argument.

On **Serverless**, ES|QL supports a query-wide timezone via the `SET time_zone` directive (GA on Serverless). This accepts
...
- Nested paths like `nested_field.sub_field` cannot be used
- Data must be flattened at index time for ES|QL access

### Unsupported Field Types

The following field types are unsupported or have limitations:
