...
Reusable patterns for translating natural language requests into ES|QL queries.
...
When converting natural language into ES|QL, look for these key elements:
...
**Warning:** ES|QL applies three-valued logic. `!=` silently excludes rows where the field is `NULL` (missing). Add an explicit
`IS NULL` check to prevent silent false negatives.
...
Prefer `LOOKUP JOIN` over `ENRICH` — it requires no policy setup and reflects changes immediately.
...
Use `ENRICH` when a pre-existing enrich policy is already in place (e.g., GeoIP) or when running on versions that predate LOOKUP JOIN.
...
Use when the user wants to identify when a metric spiked, dipped, or shifted trend. Requires a time-ordered series (e.g.
...
Use when the user wants to cluster log messages by similar format or view the distinct "types" of log lines.
...
Use when the user wants to detect when counts per log category spiked or shifted over time. Combines CATEGORIZE with time
...
- **Omitting LIMIT** - Always include `LIMIT` to avoid returning too many rows
- **Wrong time field** - Commonly used names: `@timestamp`, `timestamp`, `time`, `date`
- **Case sensitivity** - Field names are case-sensitive: `host.Name` ≠ `host.name`
- **String vs Keyword** - Use the `.keyword` suffix for exact matches on text fields: `WHERE status.keyword == "active"`
- **Type mismatches** - Explicitly convert types when needed: `EVAL num = TO_INTEGER(string_field)`
- **STATS without aggregation** - STATS requires aggregate functions (`STATS count = COUNT(*) BY host`, not
  `STATS BY host`)
- **Missing FROM or TS** - Every query must begin with a source command
- **Pipe placement** - Every command except FROM must be preceded by a pipe
- **NULL exclusion in negation** - `!=` silently drops rows where the field is `NULL` (missing). This is the most
  frequent source of silent false negatives.
- **CATEGORIZE grouping order** - `CATEGORIZE(field)` must appear as the first grouping in `STATS ... BY`. The form
  `BY host.name, category = CATEGORIZE(message)` is not allowed.
- **CHANGE_POINT requires ordered input** - Sort the sequence on the key before applying CHANGE_POINT if needed.
- **LOOKUP JOIN must precede STATS** - Fields from a joined index are dropped after aggregation. Always join first,
  then aggregate:
...
- **DATE_EXTRACT parameter order** - The date part string must come first, followed by the date expression:
...
- **Datetime subtraction** - ES|QL does not allow direct datetime arithmetic. Use `DATE_DIFF` to compute time intervals:
...
- **STD_DEV, not STDDEV** - The standard deviation function must be spelled `STD_DEV` (with an underscore):
