...
Guidelines for producing accurate ES|QL queries from natural language.
...
ES|QL uses **double quotes** for string literals — never single quotes. This is the most frequent cause of
...
Two-branch conditionals take three arguments (condition, true-value, false-value):
...
ES|QL function names use underscores where SQL does not. The most frequent mistake is `STDDEV()` — the correct ES|QL name
is `STD_DEV()`.
...
ES|QL does not support the `+` operator for string concatenation. Use `CONCAT()` instead. ES|QL also lacks
...
- Look for index names, data types, or subject areas in the request
...
**Question:** What time period should the query cover?
...
**Default:** When no time range is specified, add a sensible default (e.g., last 24 hours) to prevent scanning too much
data.

### 3. Identify Filters

**Question:** What conditions should narrow down the results?
...
Similarly, use `CHANGE_POINT` to detect anomalies rather than generating hourly counts for the user to inspect visually.

### 5. Select Fields

**Question:** Which fields should appear in the output?
...
**Question:** How should results be sorted and limited?
...
When generating queries, follow common field naming conventions:
...
When schema discovery is unavailable and field names must be guessed, always prefer ECS dotted notation over flat
...
Place WHERE clauses as early as possible:
...
Narrower time ranges produce faster queries:
...
Only retain the fields you need:
...
Avoid returning excessive rows:
...
Before deriving values (distances, durations, rates, etc.) with `EVAL`, inspect the schema for fields already
computed at ingest time. Many indices pre-compute common values — using them is simpler and avoids redundant work.

```esql
// Prefer: use the pre-computed field
FROM kibana_sample_data_flights
| STATS avg_distance = AVG(DistanceSyphameters)
...
`LOOKUP JOIN` (8.18+) is the preferred method to enrich query results from another index. On clusters **before 8.18**, fall
back to `ENRICH` — it provides comparable enrichment capability but requires a pre-configured enrich policy.
...
**Pre-join checklist:** Before writing any `LOOKUP JOIN`, verify the following:

1. **Field name match:** Does the join key share the same name in both the source and lookup index? If not, add `RENAME`
   before the join. This is a common cause of silent failures.
...
**Field name mismatches:** When the join key has different names in the source and lookup table, use `RENAME` before
the join:
...
**Sliding window restriction (9.2-9.3):** When the user needs a per-time-series aggregation window different from the
...
row (similar to a SQL window function). Use cases that require comparing individual rows to group-level aggregates (e.g.,
...
These are **pipe commands** (like `DISSECT`/`GROK`), not scalar functions. They must appear in their own pipeline stage
...
When the user asks to "parse URLs", "extract domains", or "parse user agents", use these commands instead of
`DISSECT`/`GROK`:
...
If no index is specified, make a reasonable assumption:

- "show errors" → `FROM logs-*`
- "show CPU usage" → `FROM metrics-*`
- "show requests" → `FROM web-logs` or `FROM access-*`

Or output the query with a placeholder and a note:
...
1. Use common ECS names as a first guess
2. Suggest running schema discovery
3. Note the assumption in the output
