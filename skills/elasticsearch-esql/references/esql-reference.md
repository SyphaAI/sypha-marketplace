...
Elasticsearch. Commands are chained together using pipes (`|`).
...
An ES|QL query begins with a **source command** followed by zero or more **processing commands** joined by pipes.

---

## Query Directives

Query directives alter the behavior of an ES|QL query. They must appear before the source command.

### SET (9.3+, tech preview)

Configures query-level settings. Each `SET` directive must end with a semicolon before the source command.
...
**`unmapped_fields`** (9.3+ preview) -- governs how unmapped fields are handled:

- `"default"` / `"fail"` -- the query fails when it references unmapped fields
- `"nullify"` -- treats unmapped fields as null values
- `"load"` -- dynamically loads unmapped fields. **Limitation:** `"load"` is incompatible with subqueries and views. Use
  `"nullify"` when composing subqueries or querying views.

**`time_zone`** (Serverless GA; self-managed planned) -- sets the default timezone for the query, overriding the UTC
...
> **When to use:** `unmapped_fields` is helpful when querying across multiple indices where some may not have all fields
> mapped. `time_zone` shifts date functions and display to a non-UTC zone. There is no per-function timezone argument —
> `DATE_TRUNC(1 hour, @timestamp, "America/Los_Angeles")` does **not** work.
>
> **Restriction:** `SET` directives cannot appear inside view definitions. The caller must apply `SET` when querying the
> view.
...
is a complete ES|QL pipeline. Columns present in one branch but absent in another are filled with `null`.
...
**Note:** Without an explicit `LIMIT`, queries default to 1000 rows (or the value of the cluster setting
esql.query.result_truncation_default_size).

### ROW

Produces a row from literal values. Useful for testing.
...
Retrieves data from time series data streams (TSDS). Behaves like `FROM` but enables time series aggregation functions
in `STATS` and targets only time series indices. **Preview from 9.2 to 9.3, GA since 9.4**; GA on Elastic Cloud
Serverless.
...
Queries time series data streams (TSDS) using **Prometheus Query Language (PromQL)** rather than ES|QL syntax. Like
`TS`, it produces a table that the remaining ES|QL pipeline can process. Available since **9.4 (preview)** and on
Elastic Cloud Serverless. See [promql-command.md](promql-command.md) for the full reference.
...
`rate(http_requests_total[5m])`); the engine uses `max(step, scrape_interval)` as the window. This makes the query
scale with the date picker.
...
To retain `NULL` rows in negation filters, add an explicit `IS NULL` check:
...
Appends or replaces columns with computed values.
...
Aggregates data, optionally grouped by one or more columns.
...
Aggregates data like `STATS`, but retains all original columns and attaches the aggregated values as new columns. The
output row count matches the input. Tech preview in 9.2, GA in 9.3.
...
- `INLINE STATS` preserves every input row and appends the aggregated values as new columns
...
- Calculate percentages of group totals without issuing a separate query
- Replaces certain patterns that would require subqueries in SQL
...
Restricts the number of rows returned. Supports an optional grouped top-N with `BY` (Serverless).
...
Joins data from a lookup index into the current results. This is the recommended approach for enriching query results
with data from another index. GA in 8.19/9.1.
...
- **Name collisions:** If a lookup field shares a name with an existing source column, the lookup value overwrites it.
...
Enriches data using a pre-configured enrich policy. On clusters running 8.18+, prefer `LOOKUP JOIN` — it requires no
policy setup and reflects changes immediately. On clusters **before 8.18**, `ENRICH` is the only option for data
enrichment. If no enrich policy exists, suggest the user create one (see
[Generation Tips](generation-tips.md#lookup-join-and-enrich) for setup steps).
...
Splits the query into multiple execution branches that each operate on the same input data, then merges the results into
a single output table. A `_fork` column identifies which branch produced each row. Tech preview in 9.1.
...
Merges rows from multiple result sets (typically FORK branches) and assigns updated relevance scores. Tech preview in
9.2.
...
Uses an inference model to re-score an initial candidate set of documents. Tech preview in 9.2 (GA on Serverless).
Since 9.3, defaults to 1000 rows; configurable via `esql.command.rerank.limit` and `esql.command.rerank.enabled`
cluster settings.
...
Forwards prompts and context to a Large Language Model (LLM) via a `completion` inference endpoint. Tech preview in
...
- `probability` -- value between 0 and 1 (exclusive), the likelihood each row is included
...
Expands multivalued fields into individual rows.
...
Returns one row per distinct metric available in the targeted time series data stream(s), along with applicable
dimensions and metadata. Use it to explore the metric catalogue without inspecting index mappings or calling the field
capabilities API. **GA since 9.4** (and on Elastic Cloud Serverless).
...
> **Note:** `URI_PARTS`, `USER_AGENT`, and `REGISTERED_DOMAIN` are **pipe commands** (like `DISSECT`/`GROK`), not
> scalar functions. The syntax `URI_PARTS(field)` does not work — use `| URI_PARTS target = field`.
...
ES|QL does not natively access `flattened` field sub-keys. Use `METADATA _source` with `JSON_EXTRACT` to reach into
...
This approach also works for any field that exists in the raw document but lacks an explicit mapping.
...
For handling fields that contain multiple values.
...
Extracts the best matching snippets from text fields (9.3+).
...
Field names that do not start with a letter, `_`, or `@` must be enclosed in backticks. A literal backtick inside a
...
Access document metadata with the `METADATA` directive on the `FROM` command. Once declared, metadata fields behave
