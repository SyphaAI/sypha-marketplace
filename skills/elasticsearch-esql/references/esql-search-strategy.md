...
This guide instructs an agent on how to perform high-quality relevance search on content indices using ES|QL.
...
Agents must always follow a **multi-stage retrieval pattern**:

```text
retrieve → (optional fuse) → rerank
```

Begin with inexpensive retrieval and apply costly ranking only to a bounded candidate set.
...
Use the most specific index pattern that satisfies the request.
...
Before writing queries, identify which fields are searchable.
...
Short fields deliver **precision**. Long fields deliver **recall**.

### 3. Choose Retrieval Strategy

Consult the [Quick Strategy Rules](#quick-strategy-rules) table to select the right approach based on available field
types. Then apply the matching retrieval pattern below.
...
- a single `MATCH` on the primary content field is often sufficient
- add extra fields (title, summary) only when recall is poor or the user explicitly requests broader search
...
Prefer the semantic field representing the **primary document body**.
...
If semantic search is requested but the index has no `semantic_text` field:

1. retrieve candidates using lexical search
...
**Never** stop at "semantic search unavailable" without first attempting lexical retrieval.

## Reranking Stage

> **Version:** `RERANK` is 9.2+ (preview). On clusters below 9.2, skip the reranking stage and rely on initial
> retrieval scoring. For clusters below 9.2, sorting by `_score DESC` after `MATCH` provides BM25 or vector-based
> ordering.
...
- lexical retrieval produced quality candidates
- semantic ranking improves result ordering
...
Use when exact wording is required.
...
> the `FROM` clause includes it via `METADATA _index` (typically `METADATA _id, _index, _score` for `FORK`/`FUSE`). If
> `_index` is omitted from `METADATA`, the column does not exist in the pipeline — the query fails with an unknown-field
> error. Prefer the [compatible-schemas](#compatible-schemas) pattern when per-index branching is not needed.
...
Keep the pattern straightforward:
...
Do not jump directly to expensive ranking.
...
Apply the hybrid pattern whenever both lexical and semantic fields are available.
