...
Full-text search in ES|QL relies on analyzer-aware functions for fast, relevance-ranked text retrieval. Prefer these over
`LIKE`/`RLIKE` when searching natural language content — they are substantially faster on large datasets.

> **Version:** `MATCH` and `QSTR` debuted in 8.17 (preview) and reached GA in 8.19/9.1. `KQL` and scoring via
> `METADATA _score` arrived in 8.18/9.0 (GA in 8.19/9.1). `MATCH_PHRASE` requires 8.19/9.1+. Refer to the version column in the
...
Reach for full-text search functions (`MATCH`, `QSTR`, `KQL`, `MATCH_PHRASE`) when:

- Querying natural-language text (log messages, descriptions, titles, comments)
- Relevance ranking is important (most relevant results first)
- You require analyzer features on `text` fields: case-insensitive matching, stemming, synonyms, fuzzy matching (note:
  analyzer features do **not** apply to `semantic_text` fields)
- Searching multivalued text fields
- Performance on large datasets is a concern

Use `LIKE`/`RLIKE` instead when:

- Pattern-matching against exact (keyword) values: file paths, URLs, status codes
- Structural regex matching is required and not covered by query string syntax
- Working with small datasets where analyzer support is unnecessary
...
> invalid** — that is not a real field name. To search across several text fields, use separate calls combined with `OR` (for
...
> **Semantic search:** `MATCH` is **required** for querying `semantic_text` fields — it automatically performs semantic
> (vector) search rather than lexical search. No syntax change is needed. Other full-text functions (`QSTR`, `KQL`,
...
Shorthand for `MATCH()` with default parameters. Use it for concise, simple searches.
...
Matches documents where words appear in the exact specified order. Equivalent to the Query DSL `match_phrase` query.
...
Multi-field search using Lucene query string syntax. Equivalent to the Query DSL `query_string` query. Use this when
complex boolean logic, wildcards, or field-specific searches are needed within a single expression.
...
Execute KQL queries within ES|QL. Useful for porting existing Kibana search bar queries without rewriting them.
...
Full-text functions produce relevance scores. For lexical search on `text` fields, scoring is based on BM25. For semantic
search on `semantic_text` fields, scoring derives from vector similarity. For hybrid search via `FUSE`, scores are
merged using the chosen fusion method (RRF or linear). Scoring must be explicitly requested.
...
Query `semantic_text` fields using `MATCH` or `:` for vector-based semantic matching. No separate function is required —
MATCH automatically switches to semantic search when the target field is `semantic_text`.
...
Runs multiple search strategies in parallel and merges results with relevance scoring.
...
1. `METADATA _id, _index, _score` must appear in the `FROM` clause — FUSE requires all three. The same `METADATA _index`
   declaration is required **before** filtering on `_index` inside a branch (for example `WHERE _index LIKE "logs-*"`).
...
3. Each branch should `SORT _score DESC` to provide ranked results to FUSE
...
Joins search results against a pre-built lookup index to enrich them with additional fields.
...
- After `STATS`, only aggregated columns remain — original source fields are no longer present
...
- Parameters are **values only** — `?field_name` cannot serve as a dynamic column reference
- Duration syntax (`30d`) cannot be a parameter directly — use `?days::integer * 1d` instead
- Optional parameters should carry defaults to avoid null-breaking query syntax
...
processing command has altered the column set. In practice this means they must be placed in the `WHERE` immediately
after `FROM` (or after another `WHERE`). They may also appear in per-aggregation `STATS ... WHERE` filters.
...
Full-text search functions automatically apply the field's configured analyzer at both index and query time. **These
analyzer features apply to `text` fields only — they do not apply to `semantic_text` fields**, which use vector-based
similarity rather than token analysis.
...
**Contrast with `LIKE`/`RLIKE`:** These operators match against exact stored values and bypass all analyzer processing.
