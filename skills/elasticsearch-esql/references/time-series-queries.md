...
Retrieve metrics data from Elasticsearch using the `TS` source command and time series aggregation functions. Requires
Elasticsearch 9.2+.
...
`TS` takes the place of `FROM` when querying
[time series data streams (TSDS)](https://www.elastic.co/docs/manage-data/data-store/data-streams/time-series-data-stream-tsds).
It unlocks time series aggregation functions (`RATE`, `AVG_OVER_TIME`, etc.) within `STATS`.
...
- Operates exclusively on TSDS indices (`index.mode: time_series`)
- Activates the inner/outer aggregation paradigm within `STATS`
- Cannot be combined with `FORK` before `STATS` is applied
- Optimized for time series data processing; using `FROM` on TSDS indices may produce unexpected results
- When **no** `STATS` command is present, `TS` returns rows sorted by `@timestamp` descending by default —
  convenient for listing the most recent values across many time series.

**Best practices:**

- Always use `TS` (not `FROM`) for aggregations on time series indices
- Include a time range filter with `TRANGE` to keep scan volume manageable
- Avoid combining multiple metrics with different dimensional cardinalities within a single query

---

## Inner/Outer Aggregation Paradigm

The first `STATS` following a `TS` command employs a two-level aggregation model:

1. **Inner function** (time series aggregation) -- computed per individual time series (e.g. `RATE`, `AVG_OVER_TIME`)
...
A single host can correspond to multiple underlying time series. The outer `SUM` merges the per-time-series rates into one
value per host per bucket. Use `SUM` as the outer function for counters (rates are additive). Use `AVG` or `MAX` for
gauges based on intent.

When the inner function is omitted, `LAST_OVER_TIME()` is implicitly assumed:
...
Since 9.3 (preview), a time series function can be used directly without an outer aggregation to obtain one value per time series
per bucket. The result is implicitly grouped by all dimensions of each time series and includes a `_timeseries` column
carrying the dimension key/value pairs — see [WITHOUT Grouping Function](#without-grouping-function) for how to narrow this
...
All functions below are usable under `TS ... | STATS`. Each takes a required field argument and an optional sliding
window (`time_duration`, 9.3+). The window must be a multiple of the `TBUCKET` interval. When omitted, the bucket interval
...
Indicate whether a field contains data within a given time window. Return `boolean`.
...
Provide a `time_duration` as the second argument to any time series function to apply a sliding window for the
per-time-series aggregation. The window is independent of the output time bucketing (`TBUCKET`). When no window is
given, the `TBUCKET` interval is used implicitly.
...
Groups `@timestamp` values into time buckets. Use in the `BY` clause of `STATS` for time-based grouping.
...
The interval may be a time duration (`1 hour`, `5 minute`, `30s`) or a date period (`1 month`). A string representation
(`"1 hour"`) is also accepted.

`TBUCKET` is the recommended bucketing function for `TS` queries. Its signature is simpler than
`DATE_TRUNC(interval, @timestamp)` and it understands time series semantics.
...
When the first `STATS` following `TS` uses a **bare** time series aggregation function (one not wrapped in an outer
aggregation such as `AVG()` or `SUM()`), rows are implicitly grouped by **all** dimensions of each time series. The
output includes a `_timeseries` `keyword` column that holds a JSON-encoded object with the dimension key/value pairs
identifying each group. Only dimensions that are actually present for a given time series appear in `_timeseries` — not
every dimension declared in the index mappings — so different result rows may carry different dimension keys.

`WITHOUT(...)` allows you to make this grouping explicit or restrict it to a subset of dimensions:

- `BY WITHOUT(dim1, dim2, ...)` groups by **all** dimensions **except** those listed.
- `BY WITHOUT()` (no arguments) explicitly groups by every dimension; this is equivalent to the implicit "group by all"
  behavior.

When combining a bare time series function with additional groupings, **only grouping functions** (`TBUCKET`, `WITHOUT`) are
permitted in the `BY` clause — bare dimension columns are rejected. For example:
...
Use `BY TBUCKET(...)` and/or `BY WITHOUT(...)`, or wrap the time series function in an outer aggregation.

**Availability:** **GA since 9.4**. Can only appear in the **first** `STATS` command under a `TS` source — using it in
...
Two processing commands introduced in 9.4 expose the metric catalogue of a TSDS, enabling discovery of what to query
...
Produces one row per distinct metric in the targeted TSDS, including applicable dimensions and metadata. Useful for answering "what
...
Yields one row per (metric, time series) combination, including the dimension key/value pairs that identify each
series. Useful for answering "which time series report this metric?" and "what label combinations exist?".

`TS_INFO` includes **all `METRICS_INFO` columns** plus a `dimensions` column — a single-valued JSON-encoded object such as
`{"job":"elasticsearch","instance":"instance_1"}`.
...
- **Use these for TSDS schema discovery** before writing `RATE`/`AVG_OVER_TIME` queries — they supersede the older
  workflow of inspecting `_settings`, `_mapping`, or field capabilities for time series indices.
- **Start with `METRICS_INFO`** to enumerate available metrics; use `TS_INFO` only when you need the exact dimension
  combinations (label sets) for individual series.
- **Apply filters before discovery** using `WHERE` on dimension fields to scope the catalogue to a relevant subset of series.
- **The output replaces the original table.** Any subsequent `STATS` / `SORT` / `LIMIT` operates on metadata
  rows rather than raw documents — the original data points cannot be re-attached after `METRICS_INFO` or `TS_INFO`.
...
Filters data by time range using `@timestamp`. Prefer `TRANGE` over manual `WHERE @timestamp > NOW() - ...` filters.
...
Constrains metric values to a specified range. Useful for capping outliers or enforcing value limits in time series analysis.
...
When authoring ES|QL queries for Kibana (Discover, dashboards, alerting), **do not include manual time range filters**. Kibana
applies `@timestamp` filtering automatically based on the date picker selection.

Write Kibana queries without time range filters:
...
Reserve explicit time filters (`TRANGE` or `WHERE @timestamp`) for:
...
In 9.2–9.3 the window must be a multiple of the `TBUCKET` interval. In 9.4+ any window value is accepted; however, mixing windows
smaller than the bucket interval for one metric with windows larger than the bucket interval for another within the same query is not supported.

---

## Guidelines

- **Use `TS` for all aggregations on TSDS indices.** `FROM` remains available for listing raw documents, but always use `TS`
  for metrics aggregations.
- **Use `SUM` as the outer function for counters.** Rates and increases are additive across time series sharing a
  common dimension (e.g. host). Use `AVG` or `MAX` for gauges.
- **Always include a time range filter** with `TRANGE` (or `WHERE @timestamp`) to limit scan volume, except in Kibana where
  the date picker handles this automatically. Omit the range filter if the user explicitly requests it.
- **Version requirements:**
  - `TS`, `TBUCKET`, `WITHOUT`, `METRICS_INFO`, `TS_INFO`, and **all** time series aggregation functions (the
    ...
    `STDDEV_OVER_TIME`, `VARIANCE_OVER_TIME`): **GA since 9.4** (preview in 9.2–9.3 for the 9.2/9.3 functions; new in
...
- **Do not nest time series functions.** `AVG_OVER_TIME(RATE(field))` is invalid. Wrap with a standard aggregation as the
  outer function.
- **Avoid combining metrics with different dimensions** in one query. If `foo` and `bar` have different dimension values,
  `SUM(RATE(foo)) + SUM(RATE(bar))` may return nulls for dimension mismatches.
