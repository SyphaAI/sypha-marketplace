...
description: >-
  Apply this skill when architecting distributed storage for HDFS, S3, ADLS, GCS,
  MinIO, NFS, or any distributed file system supporting data workloads. Covers:
  storage backend selection, data durability and replication, file format choice,
  partitioning and compression, lifecycle policies, storage tiering, and cost
  optimization. Do NOT use for: database storage engines, local filesystem tuning,
...
## Purpose
Architect distributed storage systems for data workloads. Choose the appropriate storage backend (HDFS, S3, ADLS, GCS, MinIO), establish durability and replication settings, tune file formats and compression, and put lifecycle policies in place.
...
### Output Artifact
A distributed storage architecture document covering backend selection, tiering strategy, lifecycle policy, and cost model.
...
#### Tiering Strategy
Hot: most recent 30-90 days of data. Warm: 90 days through 1 year. Cold: 1-3 years. Archive: 3-7+ years (governed by retention policy). S3 Intelligent Tiering automatically shifts objects between hot and warm based on observed access patterns.
...
S3, ADLS, and GCS all deliver 11 9s of durability through erasure coding spread across multiple devices and facilities. The cloud provider manages replication internally. For extra protection, use cross-region replication (CRR) for disaster recovery and same-region replication (SRR) for compliance purposes.

#### HDFS Replication
The default replication factor is 3 (one primary copy plus 2 replicas on different racks), resulting in 3x storage overhead. Erasure coding cuts that overhead significantly: RS-6-3 brings it to 1.33x and RS-10-4 to 1.5x. EC is the recommended approach for cold data on HDFS deployments exceeding 100 TB.

#### Backup and DR
Back up critical data to a cross-region location. Keep versioning enabled on all buckets. Replication rules should replicate Tier 1 data to the DR region, either synchronously or asynchronously. Validate restore procedures every quarter.
...
Apply compression at the file format level (Parquet/ORC) instead of at the transport layer. Avoid stacking multiple compression passes. Validate by compressing a representative dataset with each codec and measuring both speed and compression ratio.

### Step 6: Data Layout

#### Partitioning by File Structure
Arrange storage hierarchically by source/system, data domain, date partition, and file. Example: `s3://data-lake/raw/salesforce/orders/year=2026/month=05/day=01/load_id=abc123/orders_001.parquet`. This structure provides: partition pruning, straightforward lifecycle management, and well-defined ownership boundaries.

#### File Size Targets
Very small files degrade query engine performance. Aim for files in the 128 MB–1 GB range. Run compaction jobs to consolidate small files. Track average file size per dataset as an operational metric.

### Step 7: HDFS Architecture

#### NameNode and DataNode
The Active NameNode maintains all filesystem metadata — namespace, blocks, and locations. A Standby NameNode provides HA through the QJM (Quorum Journal Manager). DataNodes hold actual block data and send status reports to the NameNode. Default block size is 128 MB or 256 MB. Rack awareness governs where replicas are placed.
...
#### Encryption at Rest
Server-side options: SSE-S3 (AES-256 with S3-managed keys), SSE-KMS (keys managed via AWS KMS), or SSE-C (customer-supplied keys). For client-side encryption, encrypt data before uploading and decrypt after downloading. When compliance is required, use SSE-KMS paired with audit logging.

#### Encryption in Transit
Require TLS 1.2 or higher for all S3, ADLS, and GCS API calls. Enforce HTTPS-only bucket policies. On HDFS, enable SSL for both RPC communication and data transfer.
...
#### Strategies
Enable intelligent tiering to automatically cut costs on variable-access data. Use S3 Reduced Redundancy Storage for non-critical data where lower durability is acceptable in exchange for lower cost. Reserve capacity for predictable usage patterns. Set up monitoring and alerting for cost anomalies.

#### Cost Monitoring
Monitor: storage cost per bucket or container, data transfer (egress) costs, request costs (PUT/GET/LIST), and lifecycle transition costs. Assign costs to teams and domains using tag-based cost allocation.
...
#### Multi-Node Setup
MinIO operates as a distributed system spanning multiple nodes. A minimum of 4 nodes is required for erasure coding protection. Each node should have 4 or more drives, with SSD/NVMe preferred for performance. Deployment options include Docker Compose (dev or small-scale), the Kubernetes Operator (production), and bare metal (HPC environments).
...
#### Erasure Coding
MinIO applies Reed-Solomon erasure coding. The default parity is N/2 drives for maximum protection, but this is configurable: use EC:4 for an 8-drive setup or EC:2 to prioritize performance. Both read and write operations can tolerate the loss of up to N/2 drives. Storage overhead is 2x at N/2 parity and 1.33x at N/4 parity.
...
- [Distributed storage fundamentals](references/distributed-storage-fundamentals.md)
- [Advanced distributed storage](references/distributed-storage-advanced.md)
- [HDFS architecture](references/hdfs-architecture.md)
- [S3-compatible storage](references/s3-compatible.md)
- [S3-compatible configurations](references/s3-compatible-configs.md)
- [Storage deployment guide](references/storage-deployment-guide.md)
