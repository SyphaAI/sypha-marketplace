...
Activate the following core AWS Config rules (a configuration recorder must exist in the region):
...
Optional (activate if compliance mandates it):
...
Note: CloudTrail data event configuration is addressed in Workflow A step 4 (logging choice). If the user selected S3 server access logging in Workflow A and subsequently wants to add CloudTrail, use the command below:
