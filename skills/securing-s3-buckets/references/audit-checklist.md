...
**IAM Access Analyzer**: An active analyzer is required for this checklist. Before running `list-findings`, confirm one exists:

```bash
aws accessanalyzer list-analyzers --region <region>
```

If the result is empty, record it as a finding: "No IAM Access Analyzer configured in `<region>`". Creating the analyzer is a remediation action (Workflow C) — it is not part of the audit itself.
