...
You MUST apply a least-privilege key policy — do NOT create a key without specifying `--policy file://key-policy.json`. See [encryption.md § Least-Privilege KMS Key Policy Template](encryption.md#least-privilege-kms-key-policy-template) for the complete template.
...
Activate either S3 server access logging or CloudTrail data events — one is sufficient.
...
**Option B — CloudTrail Data Events** (per-event charge applies; delivers full IAM principal attribution, captures anonymous requests and AccessDenied failures, and enables real-time alerting):
...
1. Run IAM Access Analyzer to surface all external access paths
2. Restrict `Action` to the minimum set of permissions required
3. Add `Condition` blocks to limit access by IP, VPC, or MFA
...
5. SCPs/RCPs (if using AWS Organizations)
