...
TRACE writes a message to the script execution log. Use it for milestone tracking, variable inspection, and row count logging. TRACE output appears in the reload dialog and in the script execution log file.

**Critical syntax rule: `;` terminates a TRACE statement unless the entire text is quoted.** Qlik treats `;` as the statement terminator outside any quoted string, and TRACE accepts an unquoted argument by default — so a bare `;` in the message ends the statement prematurely. Everything that follows that `;` is parsed as a separate (and usually invalid) statement, producing a reload error. Two safe approaches: (a) use commas, periods, or dashes as in-text separators; (b) enclose the entire trace text in single quotes so the `;` is inside a string literal.
...
Treat TRACE text like any other Qlik string argument — when uncertain, quote it.
...
Record row counts after every significant LOAD into a persistent log table. This makes it possible to compare expected versus actual row counts after the reload completes.

**If using `error-handling.qvs`:** Use the `LogRowCount` subroutine from that framework rather than this standalone pattern. The error-handling framework logs to `_ReloadLog` with severity levels and integrates with CheckError. Do NOT include both, as the duplicate SUB name will cause a collision.
...
Validate by comparing actual row counts against expected counts from the source profile:
...
Note: `%%` is required in TRACE to emit a literal `%` character.

---

## Post-Load Validation Queries

These queries generate diagnostic tables that can be reviewed in the data model viewer or exported for offline inspection. They execute AFTER all data has been loaded and before the final model is stored.
...
Note: The aliased EXISTS pattern (`_custkey_lookup`) is used to prevent symbol space contamination.
...
ScriptErrorCount is **cumulative** across the entire reload. Take a pre-operation snapshot to isolate errors to the most recent operation. For the complete error-handling framework including the CheckError subroutine, see `script-templates/error-handling.qvs`.
...
For a reusable, logged connection test, see TestConnection in `script-templates/error-handling.qvs`.

---

## Complete Diagnostic Script Section

A self-contained diagnostic section to append at the end of a reload script:
