...
`SET` preserves the right side as a literal string template. The `$1` placeholder remains unevaluated until the variable is expanded with `$(vCleanNull(field))`. `LET` would attempt to evaluate `$1` immediately at definition time and fail.
...
**`vDualBool` overview:** Converts a boolean-like field to `Dual()` with text display and numeric value. `Dual('Active', 1)` for true-like values, `Dual('Inactive', 0)` for false, `Dual('Unknown', -1)` for NULL. This gives text in filter panes, numeric values for `Sum()`, and correct sort order. The function matches common true values (`'true'`, `'yes'`, `'y'`, `'1'`, `'active'`, `'enabled'`). For domain-specific true values like `'approved'` or `'confirmed'`, write a custom IF instead.
...
Sparse dimension fields where many records have NULL values. In Qlik filter panes, NULL values appear as `-` and cannot be selected alongside non-null values within the same selection set. Users want to see `'No Entry'` or `'Unknown'` as a selectable value.
...
Then escalate to the data source owner. A NULL key is an upstream data quality problem — substituting it in Qlik conceals the issue and creates phantom associations.
...
Per Qlik's null-value-handling documentation, NULL propagates through arithmetic operators: `Today() - NULL` returns NULL, and `Floor(NULL / 365.25)` also returns NULL. A genuinely NULL date therefore does not produce a nonsense age — it produces a correctly NULL age. The real problem is **non-NULL garbage dates** that upstream systems substitute for missing values:
...
Guard date arithmetic against **both** NULL and known sentinel/out-of-range values. The NULL check is inexpensive defensive insurance; the range check is what actually catches the sentinel-date bug:
...
In a typical extraction script, all three strategies may apply to different fields within the same LOAD:
