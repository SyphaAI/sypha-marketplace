...
**The rule:** Before any JOIN, enumerate the fields in both tables and alias every non-key field that shares a name. Never rely on Qlik to infer the intended key.
...
JOIN merges tables into one (matched fields combined). KEEP filters both tables to matching rows but retains them as separate tables in the data model. Use KEEP when you want association filtering without merging.
