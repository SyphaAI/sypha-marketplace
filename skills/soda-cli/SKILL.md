...
name: soda-cli
description: >-
  How to operate the Soda CLI for data quality management — covering authentication,
...
The Soda CLI (`sodacli`) handles data quality management from the command line. Every command follows the pattern `sodacli <resource> <action>`.

## Important: always use `--output json` when running commands

When invoking `sodacli` from an agent, **always include `--output json`** to receive structured, parseable output. Without this flag, the CLI produces human-readable tables that are more difficult to parse programmatically.

## Quick Reference — What Works

**Fully working (verified against live API):**
...
- `secret` — list, get, create, update, delete (encrypted client-side with AES-256-GCM + RSA-OAEP)

**Not yet available (API blocked):**
- `incident` — list, get, update (present in OpenAPI spec but still returns HTML in dev)
- `dataset attributes` — (present in OpenAPI spec but still returns HTML in dev)
- `notification` — rules and integrations
- `job list`, `job cancel`
- `contract proposal` — list, pull, push, close
- `monitor add --type dataset` (dataset monitors exist by default; no write endpoint available)
...
`dataset onboard` works only after `datasource onboard` has been executed at least once. If only one table is needed, `datasource onboard` must still be run first (which onboards all discovered datasets), after which individual datasets can be managed separately.
...
- `skeleton` — produces schema structure plus not-null checks only (fast and deterministic)
- `copilot` — AI examines your data and generates format, range, uniqueness, and business-logic checks (takes longer; may require manual corrections — see Troubleshooting below)

When `--contracts skeleton` or `--contracts copilot` is specified, the onboard flow automatically verifies the generated contracts against the data and displays pass/fail results.
...
When `contract verify` returns exit code 2, the output may display only "0 checks passed" with no further detail. Always inspect the scan logs for the underlying error:
```bash
sodacli job logs <scan-id>
```
...
Copilot may produce `valid_format` regex checks on UUID columns (e.g., `'^[0-9a-fA-F]{8}-...'`). On Postgres, the `~` regex operator does not function on `uuid` type columns, which causes a SQL error. **Fix:** Remove `invalid` / `valid_format` checks on UUID columns — the database data type already enforces the UUID format. Retain `missing` and `duplicate` checks, which operate correctly.

### Discovery scan not complete
After `datasource create`, the discovery scan executes asynchronously. If `dataset list` returns empty results or `dataset onboard` responds with "not found", the scan may still be in progress. Verify with:
```bash
