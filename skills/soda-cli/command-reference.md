...
Status key: **Working** = verified against a live API. **Blocked** = API endpoint absent or returning errors.
...
Omitting all flags launches an interactive wizard. Supplying `--no-interactive` requires both `--api-key-id` and `--api-key-secret`.
...
Registers a datasource from a YAML connection config. Requires a self-hosted Soda Runner. Initiates an async discovery scan — datasets do not appear in `dataset list` until the scan finishes (~10-15 seconds). The scan ID is printed in the command output; run `sodacli job logs <scan-id>` to confirm completion.
...
Providing all action flags together runs the command fully non-interactively with no prompts. Using `--contracts skeleton` or `--contracts copilot` causes the generated contracts to be automatically verified against live data after they are created.
...
View or configure the diagnostics warehouse. Running without flags displays the current configuration. Uses a read-modify-write pattern so that updating a single flag leaves all other settings intact.
...
**Important:** `dataset onboard` requires that `datasource onboard` has been executed at least once for the parent datasource. Running it immediately after `datasource create` returns "dataset not found" even when the dataset is visible in `dataset list`.
...
Contract modes:
- `skeleton` — schema structure plus not-null checks only (fast and deterministic)
- `copilot` — AI-generated checks covering format, range, uniqueness, and business logic (slower; may require manual corrections for edge cases such as UUID columns on Postgres)

When all flags are supplied, the command runs non-interactively. Generated contracts are automatically verified against live data.
...
Running without flags displays current profiling data and per-column statistics.
...
Running without flags shows current settings. Diagnostics must be enabled on the datasource before using this command.
...
Pushes a local contract YAML to Soda Cloud (upsert). Reads the `dataset:` field from the file to locate or create the corresponding contract.
...
Validates contract YAML files against the Soda data contract JSON schema. No network access or authentication is required.

Accepts multiple files and glob patterns. Falls back to `contracts/*.yml` when no arguments are provided.
...
Executes the data quality checks defined in a contract file. By default, the contract is pushed to Soda Cloud and verification is performed through a Runner.

With `--local`, verification runs locally using soda-core (which must be available on PATH). In local mode, `--datasource` is required and no Soda Cloud authentication is needed unless `--push` is also supplied.
...
**Debugging exit code 2 (cloud mode):** The output may display only "0 checks passed" with no additional details. Run `sodacli job logs <scan-id>` (the scan ID appears in the verify output) to inspect the underlying SQL or runtime error. A frequent cause is copilot-generated regex checks targeting Postgres UUID columns — remove `invalid`/`valid_format` checks on those columns because the data type already enforces the expected format.
...
**Dataset monitors** (`--type dataset`): Not yet available through the API. They exist by default on every dataset and can be enabled from the Soda Cloud UI.
...
Generates runner credentials for Kubernetes deployment. Returns an API key ID and secret that are displayed only once. The runner becomes visible in `runner list` only after the Helm chart is deployed and the agent establishes a connection to Soda Cloud.
...
Results are reported separately for successful and failed invitations.
...
Creates a new secret. Values are encrypted on the client using AES-256-GCM + RSA-OAEP before transmission — Soda never receives or stores plaintext. Decryption occurs only at scan execution time inside the runner.
...
1. `sodacli secret create --name X` — masked interactive prompt (default behavior)
2. `sodacli secret create --name X --value "val"` — inline via flag (value will appear in shell history)
3. `echo "val" | sodacli secret create --name X` — piped through stdin

Reference in datasource configs: `${secret.NAME}`

### `sodacli secret update <id>`
Updates a secret's value. Applies the same encryption approach and input methods as the create command.
