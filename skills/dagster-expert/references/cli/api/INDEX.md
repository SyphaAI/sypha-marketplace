...
The `dg api` subcommands expose CLI access to Dagster Plus resources. They are particularly suited for scripting, debugging, and building automation workflows.

**Important**: Always read `general.md` before using any `dg api` subcommand. It documents shared flags (`--json`, `--response-schema`, `--deployment`, `--view-graphql`) and best practices.
...
- [dg api: General](./general.md) — always read before using any dg api subcommand
