...
The Pythonic approach uses the `@dbt_assets` decorator to declare dbt assets in Python code. This
gives the greatest degree of flexibility for advanced customization scenarios.

## Overview

Key classes:

- **`@dbt_assets`**: Decorator that exposes dbt models as Dagster assets from a manifest
- **`DbtCliResource`**: Resource for running dbt CLI commands
- **`DbtProject`**: Represents a dbt project and handles manifest compilation
- **`DagsterDbtTranslator`**: Controls how dbt nodes are mapped to Dagster assets
...
The dbt manifest can be produced at runtime (development) or at build time (production).

### Runtime Compilation (Development)

`DbtProject.prepare_if_dev()` compiles the manifest automatically during local development:
...
For production deployments, precompile the manifest in CI/CD to eliminate recompilation overhead. The
manifest at `DbtProject.manifest_path` (typically `target/manifest.json`) must be bundled with your
deployed package.
...
Build a custom translator by subclassing `DagsterDbtTranslator` and overriding `get_asset_spec()`:
...
The `get_asset_spec()` method is invoked once per dbt node and must return a complete `AssetSpec`
describing that node.

### dbt Meta Config

Embed custom metadata in your dbt project files for use by your custom
...
Retrieve this metadata through the manifest in your translator's `get_asset_spec()` method.

## Incremental Models and Partitioning

To apply partitioning to incremental dbt models:

1. Define a `PartitionsDefinition`
2. Supply it to the `@dbt_assets` decorator
3. Read `context.partition_time_window` inside the asset function
4. Forward partition-specific vars to the dbt CLI
...
Use these vars in your dbt SQL:
...
When working with multiple partitions definitions, create separate `@dbt_assets` definitions and use
`select`/`exclude` to scope models for each one.

## Metadata

Activate automatic metadata collection by chaining methods on the event iterator:
...
Use `get_asset_key_for_model()` to obtain asset keys for dbt models:
...
Use `build_schedule_from_dbt_selection()` for jobs that exclusively materialize dbt assets:
...
Combine `build_dbt_asset_selection()` with `AssetSelection` for jobs that include both dbt and non-dbt
assets:
...
Set up AutomationConditions through a custom `get_asset_spec()` implementation in your translator:
...
Leverage Dagster's config system to supply runtime parameters:
