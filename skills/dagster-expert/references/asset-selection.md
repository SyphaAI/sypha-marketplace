...
There are two ways to select assets:

- **String-based selection syntax** — behaves identically in the UI search bar, `dg` CLI (`--assets`), and `dg.AssetSelection.from_coercible()` in Python
- **`AssetSelection` in Python** — programmatic API offering set operations (`|`, `&`, `-`), traversals, and methods unavailable in string syntax
...
Selection strings (equivalent across UI, CLI, and Python):
...
`dg.AssetSelection.from_coercible()` converts a selection string (or other coercible types) into an `AssetSelection` object. Accepted inputs are:

- A selection string (parsed with the same grammar as the UI and CLI)
- An existing `AssetSelection` instance (returned unchanged)
- A sequence of strings (each parsed and unioned)
...
These methods are exclusively available through the Python API and have no equivalent in string syntax:
