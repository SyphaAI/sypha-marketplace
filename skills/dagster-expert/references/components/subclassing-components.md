...
When a [Dagster integration](../integrations/INDEX.md) already covers the external tool you're working with, it is almost always preferable to **subclass the existing component** rather than building one from scratch. This retains the core API interaction and metadata translation logic, while letting you customize the parts that need to differ for your use case.

## Common Override Points

### `get_asset_spec()` — Customize Asset Representation

Override to modify asset keys, groups, metadata, kinds, or dependencies.
...
Override to insert pre/post logic around the standard sync, or to filter which syncs run.
...
For `StateBackedComponent` subclasses, override to transform or augment the persisted state. This is generally only necessary for mock or demo purposes and should otherwise be avoided in most cases.
...
- Fundamentally redesigning the existing definitions entirely (rare). Note that if you only want to alter the runtime behavior of the existing definitions, override `execute()` instead; if you only want to change the metadata of the generated assets, override `get_asset_spec()` instead.
- Appending additional definitions on top of the base definitions (shown below)
