...
State-backed components are designed for integrations where Dagster definitions depend on external systems (APIs, compiled artifacts) rather than just code and config files. The framework handles fetching, storing, and loading this state so that costly operations (API calls, compilation) do not execute on every code server load.
...
Expose a `defs_state` field so users can select the management strategy in YAML. This is the OmniComponent pattern:
...
The `default_key` is used when the user does not specify a `key` in YAML. Convention: `ClassName` or `ClassName[discriminator]`.

### Hardcoded

Construct `DefsStateConfig` directly when the strategy should not be configurable by users. This is the DbtProjectComponent pattern:
...
**Key format convention:** `ClassName[discriminator]` — the discriminator differentiates multiple instances of the same component type (e.g., different dbt projects).

## State Serialization Strategies

### Dagster serdes (for API responses)

Define typed state objects with `@whitelist_for_serdes` and `@record`, then serialize using `dg.serialize_value()` / `dg.deserialize_value()`. Best suited when state originates from API responses. Used by OmniComponent.
...
Write tool artifacts directly and read them using the tool's own parser. Best when the tool already produces a well-defined artifact format. Used by DbtProjectComponent (copies the entire dbt project directory into the state path via `shutil.copytree`, including running `dbt deps` when necessary; `build_defs_from_state` then reads the manifest from within that full project copy).
...
- **Async support:** `write_state_to_path` can be declared `async`. The framework detects this automatically. Use async when calling async APIs (e.g., `await client.fetch(...)`).
- **Handling `state_path=None`:** `build_defs_from_state` receives `state_path=None` when no state has been written yet. Return an empty `dg.Definitions()` in this case.
