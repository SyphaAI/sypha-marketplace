...
Dagster component YAML files support Jinja2 expressions using `{{ expression }}` syntax. Any Resolvable field can accept template strings because the generated schema unions every non-`str` field type with `str`.

Templates are typically resolved at component load time, though some contexts resolve independently. For example, post-processing templates (`post_processing.assets[].attributes`) are resolved once per asset, with the current asset bound as a template variable.

## Built-in Scopes

### env — Environment Variables

Read environment variables directly or with a fallback default:
...
`{{ env.MY_VAR }}` returns the value or `None` when unset. `{{ env('MY_VAR', 'default') }}` raises an error if the variable is unset and no default is provided.
...
Available within `post_processing.assets[].attributes` blocks. The template is resolved independently per matching asset, with `asset` bound to the current `AssetSpec`:
...
Available attributes include `key`, `tags`, `group_name`, `metadata`, `kinds`, and all remaining `AssetSpec` fields.

## Custom Template Variables

### Module-Level Template Vars

Create a `template_vars.py` file and point to it in `defs.yaml`:
...
Template var functions support one of two signatures:
...
Define `@template_var` as static methods on a Component class. These are auto-discovered without requiring `template_vars_module`:
...
Template vars can return callables, which are subsequently invoked with arguments in templates:
