...
Fundamental schedule patterns are covered in the main SKILL.md Quick Reference. This reference addresses advanced schedule configuration and partitioned job automation.

## Basic Schedule Review

A schedule runs a job at defined times using cron expressions. See SKILL.md for the basic pattern.
...
Schedules operate in UTC by default. Use `execution_timezone` to target a different timezone:
...
**Timezone string format**: Provide IANA timezone database names such as `"America/New_York"`, `"Europe/London"`, or `"Asia/Tokyo"`.

**Daylight saving time**: Dagster automatically accommodates DST transitions for the specified timezone.

## Schedules from Partitioned Assets

For partitioned assets or jobs, use `build_schedule_from_partitioned_job` to generate a schedule that aligns with the partition cadence:
...
**How it works**: The cron expression for the schedule is inferred from the `PartitionsDefinition`:
...
Each run of the schedule materializes the partition that corresponds to the scheduled time.
...
- Execution timing is predictable and does not change
- No dependency logic is required (runs regardless of upstream status)
- A straightforward time-based trigger is all that is needed

**Prefer declarative automation when**:

- Dependency-aware execution is required
- Conditions depend on asset freshness or upstream state
- Determining when to execute involves complex logic
