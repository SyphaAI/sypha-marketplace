...
name: apache-arrow
description: >-
  Specialist guidance for Apache Arrow, the cross-language columnar memory format built
  for analytics workloads. Assists developers in using Arrow for
  high-performance data interchange across systems, zero-copy reads, and
  efficient columnar processing in Python (PyArrow) and JavaScript (Arrow JS).
...
Apache Arrow is the cross-language columnar memory format purpose-built for analytics workloads. This skill supports developers in leveraging Arrow for high-performance data interchange across systems, zero-copy reads, and efficient columnar processing in Python (PyArrow) and JavaScript (Arrow JS).
...
The agent installs the SDK, creates an API route that initializes the Apache Arrow client, configures streaming, selects an appropriate model, and wires the frontend to consume the stream. Error cases are handled and environment variable management for the API key is set up correctly.
...
The agent reviews the current implementation, identifies problems (incorrect model selection, absent caching, inefficient prompting, no batching), and applies optimizations suited to Apache Arrow's strengths — adjusting model parameters, introducing response caching, and adding retry logic with exponential backoff.


## Guidelines

1. **Parquet for storage, Arrow for compute** — Store data in Parquet on disk/S3; perform in-memory processing with Arrow
2. **Column pruning** — Always specify `columns=` when reading Parquet; reading every column wastes I/O and memory
3. **Predicate pushdown** — Use `filters=` on Parquet reads; the reader skips non-matching row groups automatically
4. **Zero-copy when possible** — For large tables, use `to_pandas(self_destruct=True)`; Arrow can transfer memory ownership directly
5. **Batch processing for large files** — Use `iter_batches()` rather than loading entire files into memory at once
6. **IPC for microservices** — Arrow IPC outperforms JSON/CSV for data exchange between services
7. **Partitioned datasets for scale** — Partition by date or category so queries read only the relevant partitions
