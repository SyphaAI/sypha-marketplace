...
Key settings in `capacity-scheduler.xml`:
```xml
...
Key settings in `fair-scheduler.xml`:
```xml
...
### Recommission Node
```bash
# 1. Remove the node from the exclude file
# 2. Refresh
yarn rmadmin -refreshNodes
```
...
Re-executes slow tasks on alternative nodes. Beneficial for stragglers, but counterproductive for expensive tasks:
```xml
...
### Preemption
Permits high-priority queues to reclaim resources from lower-priority ones:
```xml
<property>
...
### Locality Delay
Holds off on accepting rack-local slots until data-local slots have been waited for:
```xml
<property>
