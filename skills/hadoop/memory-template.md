...
- **No config keys visible** — describe using natural language instead
- **Learn from conversations** — observe rather than interrogate
- **Most users stay `ongoing`** — clusters change over time, so keep gathering context
- Update `last` with every use
