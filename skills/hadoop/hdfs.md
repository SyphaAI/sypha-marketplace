...
### Small Files Problem
A large number of small files places memory pressure on the NameNode.
Solutions:
...
### Optimal Block Size
Default 128MB (or 256MB). Match block size to the typical file size:
- Files < 128MB: consider combining
- Files >> 128MB: default is fine

### Read Locality
HDFS attempts to read data from the local DataNode. For best performance:
- Process data on the nodes where it resides
- Apply rack awareness configuration
