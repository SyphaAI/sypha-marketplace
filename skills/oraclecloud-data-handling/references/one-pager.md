...
Object Storage PAR (Pre-Authenticated Request) URLs expire silently with no indication. Multipart uploads exceeding 50GB require manual part management. Lifecycle policies can remove data unexpectedly. This covers the safe approaches for each scenario.

## The Solution

This skill delivers safe Object Storage operations: namespace discovery, bucket creation with versioning enabled, simple and multipart uploads via UploadManager, PAR creation with explicit expiry and audit/revocation workflows, and lifecycle policies with explicit warnings about auto-deletion risks.
