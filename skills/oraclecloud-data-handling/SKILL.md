...
description: >
  Work with OCI Object Storage — buckets, uploads, PARs, and lifecycle policies.

  Apply when uploading objects, generating pre-authenticated requests, or
...
Administer OCI Object Storage via the Python SDK. Object Storage is OCI's S3 equivalent, but PAR (Pre-Authenticated Request) URLs expire silently without an error — the URL simply returns 404. Multipart uploads exceeding 50GB require manual part management. Lifecycle policies can delete data without warning if misconfigured. This skill covers the safe patterns for each of these operations.

**Purpose:** Upload, download, and share objects reliably with correct PAR expiry management and lifecycle policy configuration.
...
Each OCI tenancy has a unique Object Storage namespace. Retrieve it before performing any operation.
...
Use the simple upload path for files under 50MB. For larger files, use the UploadManager, which handles multipart chunking automatically.
...
PARs are OCI's signed URLs. **Critical gotcha:** expired PARs return 404 NotFound rather than 401 or 403. Callers often conclude the object was deleted when the PAR itself has simply expired.
...
Lifecycle rules can automatically archive or delete objects. **Warning:** A rule with `time_amount=30` and `action=DELETE` permanently removes objects after 30 days with no recovery path unless bucket versioning is enabled.
...
- A versioned Object Storage bucket with public access disabled
- Simple upload for small files and UploadManager for large files (automatic multipart handling)
- PAR URLs with explicit expiry together with an audit and revocation workflow
- Lifecycle policies that archive aging data and purge temporary files
...
Once Object Storage is configured, refer to `oraclecloud-query-transform` to monitor storage metrics through MQL, or see `oraclecloud-schema-migration` if you need to export database data into Object Storage buckets.
