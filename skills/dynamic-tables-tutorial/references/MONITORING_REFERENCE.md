...
This guide describes how to observe Dynamic Table activity using Snowflake's built-in monitoring functions and views.
...
The primary function for tracking refresh activity.
...
Returns the dependency graph along with configuration history for Dynamic Tables.
...
Find refreshes that completed after their scheduled completion target:
...
Configure an alert for any failed refresh:
...
When you have a `query_id` from refresh history, Query Profile enables thorough analysis:

1. **In Snowsight**: Go to Activity → Query History → Search by query_id
2. **Click on the query** to open the Query Profile
3. **Look for**:
   - The costliest operators
...
Gives a concise overview of all Dynamic Tables:
...
1. **Establish regular monitoring** - Run health checks on a daily or weekly schedule
2. **Track failure trends** - Act quickly when failure rates start to rise
3. **Watch duration trends** - Catch performance degradation before it becomes a problem
4. **Alert on critical failures** - Do not rely on users to report issues
5. **Review compute costs** - Verify that refresh spending is proportional to the value it provides
6. **Check lag compliance** - Confirm that data freshness consistently satisfies requirements
