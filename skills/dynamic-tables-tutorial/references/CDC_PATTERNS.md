...
This guide contrasts traditional CDC using Streams and Tasks with the modern Dynamic Tables approach. Use it to understand when to choose each pattern and how to migrate from one to the other.
...
2. **Recording those changes** in a structured format
3. **Delivering changes** to downstream systems
...
An UPDATE is represented as a DELETE + INSERT pair where `METADATA$ISUPDATE = TRUE`.
...
That is all. No stream, no task, no MERGE logic.
...
1. Monitor changes in source tables
2. Schedule refreshes according to TARGET_LAG
3. Perform incremental processing whenever the query supports it
4. Propagate inserts, updates, and deletes
...
1. **Simplicity is the priority** - One object instead of three
2. **Standard CDC patterns** - INSERT/UPDATE/DELETE propagation
3. **Freshness-based requirements** - "Data within X time"
4. **Pipeline chaining** - Multiple sequential transformation stages
5. **Automatic dependency management** - Delegate scheduling to Snowflake

### Use Streams + Tasks When:

1. **Exact scheduling is required** - "Run at 2 AM daily"
2. **Complex procedural logic** - Loops, conditionals, multi-step workflows
3. **Multiple target systems** - One stream feeding multiple consumers
4. **Audit trail requirements** - Change history must be preserved
5. **Fine-grained control** - Custom error handling and retry behavior
...
Strong candidates are pipelines with:
...
Occasionally both patterns are needed together:
...
- **Predictable scheduling**: Executes precisely on schedule
- **Control over batch size**: Process a specific number of records per run
- **Resource isolation**: Task execution is fully dedicated

### Dynamic Tables

- **Automatic optimization**: Snowflake selects the best refresh strategy
- **Incremental by default**: Applied whenever the query supports it
- **No scheduling overhead**: No task management required
...
Dynamic Tables take care of this automatically.
...
When your MERGE contains many WHEN clauses and intricate conditions, a Dynamic Table is likely the simpler choice:
