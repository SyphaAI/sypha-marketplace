...
name: dashboarding
description: >-
  Build, update, and organise Grafana dashboards covering panels, variables,
  transformations, and alerting. Invoke when the user wants to create a Grafana
  dashboard, add a panel, set up a time series or stat panel, define template
  variables, configure dashboard linking, apply transformations, set thresholds,
  build a service dashboard, or export dashboard JSON. Activates on phrases
  such as "create dashboard", "add panel", "time series panel",
...
Dashboards in Grafana are stored as JSON documents. Each dashboard contains panels, variables, a time range, and refresh settings. Knowing the JSON schema enables you to create and update dashboards programmatically through the API or Grafana Assistant tools.
...
**`gridPos`:** Grafana lays out panels on a 24-column grid. Typical widths: full-width=24, half=12, third=8, quarter=6. Height is expressed in grid units (1 unit ≈ 30px).
...
Complete list available via the **Panel > Field > Unit** dropdown in the Grafana UI, or in the [units reference](https://grafana.com/docs/grafana/latest/panels-visualizations/configure-standard-options/#unit).

---

## Template variables

Variables allow a single dashboard to be reused across different environments and services.
...
Transformations execute on the client after data has been retrieved, reshaping the results without altering the underlying queries.
...
Display events directly on top of time series panels — useful for deployments, incidents, and similar occurrences.
...
When embedding dashboards inside app plugins, use `@grafana/scenes` rather than raw JSON.
Refer to the `grafana-o11y:grafana-scenes` skill for documentation on the React-based scenes API.
