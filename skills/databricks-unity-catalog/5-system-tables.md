...
Complete reference for Unity Catalog system tables: lineage, audit, billing, compute, jobs, and metadata.

## Overview

System tables are read-only tables within the `system` catalog that expose operational data about your Databricks account.
...
You must enable system schemas before they can be queried.
...
Audit records covering all Unity Catalog operations.
...
Tracks the flow of data between tables.
...
Tracks lineage at the column level.
...
Granular usage records for cost analysis.
...
Published list prices for SKUs.
...
Cluster configurations and metadata (historical definitions only — not live state).
...
State change and scaling events for SQL Warehouses.
...
Job definitions and their configurations.
...
Historical job run records and performance data.
...
Execution events for DLT/SDP pipelines.
...
Query execution history along with performance metrics.
...
Metadata describing Unity Catalog objects.
...
Records lineage relationships to external systems.
...
1. **Always filter by date partitions** - System tables use date-based partitioning
```sql
WHERE event_date >= current_date() - 30  -- Good
WHERE event_time >= '2024-01-01'         -- Slower (scans all partitions)
```

2. **Use LIMIT for exploration** - System tables may contain a very large number of rows
```sql
LIMIT 100  -- Always add for exploratory queries
```

3. **Create views for common queries** - Encapsulate complex logic to avoid repetition
```sql
CREATE VIEW analytics.governance.daily_audit_summary AS
SELECT ...
```

4. **Schedule aggregation jobs** - Pre-compute aggregates to feed dashboards efficiently
```sql
...
1. **Enable system tables early** during your UC setup
2. **Use column lineage** to track sensitive data flows
3. **Register external sources** for end-to-end visibility
4. **Retain audit logs** for compliance purposes (typically 1-7 years)
5. **Monitor failed access** to detect security threats
6. **Automate alerts** when sensitive operations occur
