...
Complete reference for Data Profiling: set up quality monitors on Unity Catalog tables to track data profiles, detect drift, and observe ML model performance.

## Overview

Data profiling automatically calculates statistical profiles and drift metrics for tables over time. When a monitor is created, Databricks produces two output Delta tables (profile metrics + drift metrics) along with an optional dashboard.
...
- Required privileges: `USE CATALOG`, `USE SCHEMA`, `SELECT`, and `MANAGE` on the table
...
Use the `manage_uc_monitors` tool for every monitor operation:
...
> **Note:** The MCP tool currently supports only **snapshot** monitors. To create TimeSeries or InferenceLog monitors, use the Python SDK directly (see below).
...
The new SDK gives full control over all profile types through `w.data_quality`.
...
Anomaly detection is configured at the **schema level**, not per individual table. After enabling it, Databricks automatically scans all tables in the schema at the same cadence as their updates.
...
> **Note:** Anomaly detection requires `MANAGE SCHEMA` or `MANAGE CATALOG` privileges and serverless compute must be enabled on the workspace.

---

## Output Tables

Creating a monitor generates two metric tables in the designated output schema:
...
> **Note:** Data profiling was previously called Lakehouse Monitoring. The legacy SDK accessor
> `w.lakehouse_monitors` and the MCP tool `manage_uc_monitors` continue to use the older API.
