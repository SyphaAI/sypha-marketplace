...
name: angular-forms
description: >-
  Construct signal-based forms in Angular v21+ with the Signal Forms API.
  Covers automatic two-way binding, schema-driven validation, field state
  management, and dynamic forms. Apply when implementing forms, adding
  validation, building multi-step forms, or managing conditional fields.
  Although experimental, Signal Forms are the recommended approach for new
  Angular projects. Does not address template-driven forms without signals or
  third-party libraries such as Formly or ngx-formly.
metadata:
...
Create type-safe, reactive forms with Angular's Signal Forms API. Signal Forms deliver automatic two-way binding, schema-driven validation, and reactive field state.

**Note:** Signal Forms are experimental in Angular v21. For production applications that demand stability guarantees, refer to [references/form-patterns.md](references/form-patterns.md) for Reactive Forms patterns.
...
The form model is a writable signal acting as the single source of truth:
...
Every field exposes reactive signals covering validation, interaction, and availability:
...
The form itself is also treated as a field and surfaces aggregated state:
...
For production-stable Reactive Forms patterns, see [references/form-patterns.md](references/form-patterns.md).
