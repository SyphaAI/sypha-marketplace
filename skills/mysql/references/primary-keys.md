...
InnoDB stores rows in primary key order (clustered index). This has the following implications:
- **Sequential keys = optimal inserts**: new rows are appended, minimizing page splits and fragmentation.
...
Guideline: default to **BIGINT UNSIGNED** unless you are certain the table will never approach the INT limit. The extra 4 bytes is almost always cheaper than the risk of exhausting INT.
...
- If external identifiers are needed, store UUID as `BINARY(16)` in a secondary unique column:
...
InnoDB secondary indexes store the primary key value alongside each index entry. Implications:
- **Larger secondary indexes**: each secondary index entry includes (indexed columns + PK bytes).
...
- **Hot spot**: inserts target the end of the clustered index (usually fine; can become a bottleneck at extreme insert rates).
...
When globally unique IDs must be generated outside the database:
...
- Random-key insert patterns (UUIDv4) can amplify page splits and I/O on replicas as well, increasing replication lag.
- Time-ordered IDs reduce fragmentation and tend to replicate more smoothly under heavy insert workloads.

## Composite Primary Keys

Use for join/many-to-many tables. Place the most-queried column first:
