...
Aim for `ref` or better. `ALL` on >1000 rows almost always signals the need for an index.
...
- A low `filtered` value often indicates that additional (non-indexed) predicates are eliminating many rows

## Join Order
The row order in EXPLAIN output reflects execution order: the first row is typically the first table read, with subsequent rows joined in sequence. Use this to identify suboptimal join ordering (for example, starting with a large table when a more selective table could drive the join).
...
Compare estimated vs actual to locate optimizer misestimates. Large discrepancies often improve after refreshing statistics:
...
- Cost units (arbitrary) and time (ms) are different; do not compare them directly
- Results reflect real execution, including buffer pool/cache effects (a warm cache can conceal I/O problems)
