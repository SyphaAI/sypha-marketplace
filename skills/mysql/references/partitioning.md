...
## Partition Pruning
The optimizer can eliminate partitions that cannot contain matching rows based on the WHERE clause ("partition pruning"). Partitioning is most beneficial when queries frequently filter by the partition key or expression:
...
- A partitioned table cannot define foreign key constraints referencing other tables.
- Other tables cannot reference a partitioned table with a foreign key.

If foreign keys are required, partitioning may not be an option.
...
- Queries that filter by the partition key or expression (enables pruning)
- Maintenance on subsets of data (operate on partitions vs the whole table)

**Hurts:**
- Small tables (overhead without benefit)
- Queries that do not filter by the partition key (no pruning)
- Workloads that require foreign keys
- Complex UNIQUE key requirements (partition key columns must be included in every unique/primary key)
