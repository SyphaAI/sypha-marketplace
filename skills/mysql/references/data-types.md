...
Always select the smallest correct type — more rows fit per page, caching improves, and queries run faster.
...
## TEXT/BLOB Indexing
- Full indexing of `TEXT`/`BLOB` is generally not possible; use prefix indexes instead: `INDEX(text_col(255))`.
...
## JSON
Reserve for truly dynamic data only. Index JSON values via generated columns:
...
Opt for simpler types like integers and strings over JSON whenever possible.
...
Choose **VIRTUAL** for simple expressions when storage space matters. Choose **STORED** when indexing is required or the expression is costly to recompute.
