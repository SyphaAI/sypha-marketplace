...
name: angular-di
description: >-
  Set up dependency injection in Angular v20+ with inject(), injection tokens,
  and provider configuration. Applies to service architecture, supplying
  dependencies at different levels, defining injectable tokens, and handling
  singleton vs scoped services. Triggered by service creation, provider
  configuration, injection token usage, or questions about the DI hierarchy.
metadata:
...
Set up and work with dependency injection in Angular v20+ using `inject()` and providers.

## Basic Injection

### Using inject()

Choose `inject()` rather than constructor injection:
...
Gather several values under one token:
...
Execute async code ahead of app startup via `provideAppInitializer`:
...
Execute code inside an injection context:
...
See [references/di-patterns.md](references/di-patterns.md) for advanced patterns.
