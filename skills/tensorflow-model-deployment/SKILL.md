...
Deploy TensorFlow models to production environments using the SavedModel format, TensorFlow Lite for mobile and edge devices, quantization techniques, and serving infrastructure. This skill addresses model export, optimization, conversion, and deployment strategies.
...
Apply the tensorflow-model-deployment skill when you need to:

- Export trained models for production serving
- Deploy models to mobile devices (iOS, Android)
- Optimize models for edge devices and IoT targets
- Convert models to the TensorFlow Lite format
- Apply post-training quantization to compress model size
- Stand up TensorFlow Serving infrastructure
- Deploy models inside Docker containers
- Build model serving APIs over REST or gRPC
- Reduce inference latency and improve throughput
- Shrink model size for bandwidth-constrained deployment environments
- Convert JAX or PyTorch models to TensorFlow format
- Run A/B testing across multiple model versions
- Deploy models to cloud platforms (GCP, AWS, Azure)
- Build on-device ML applications
- Tune models for specific hardware accelerators

## Best Practices

1. **Always validate converted models** - Check TFLite predictions against the original model to confirm accuracy is preserved
2. **Use SavedModel format** - This is the standard format for production deployment and serving
3. **Apply appropriate quantization** - Float16 offers a balanced speed/accuracy tradeoff; INT8 achieves maximum compression
4. **Include preprocessing in model** - Embed preprocessing logic in the SavedModel to guarantee consistent inference behavior
5. **Version your models** - Incorporate version numbers in export paths to support model management and rollback
6. **Test on target devices** - Measure actual performance on the intended deployment hardware
7. **Monitor model size** - Record model size before and after optimization to track compression gains
8. **Use representative datasets** - Supply calibration data to ensure quantization produces accurate results
9. **Enable GPU delegation** - Take advantage of GPU/TPU acceleration on devices that support it
10. **Optimize batch sizes** - Adjust batch size to balance throughput against latency requirements
11. **Cache frequently used models** - Load the model once and reuse the instance across multiple prediction calls
12. **Use TensorFlow Serving** - Rely on its built-in infrastructure for scalable and robust model serving
13. **Implement model warmup** - Execute a few dummy predictions at startup to prime serving systems
14. **Monitor inference metrics** - Track latency, throughput, and error rates continuously in production
15. **Use metadata in TFLite** - Embed labels and preprocessing details in the model metadata for portability

## Common Pitfalls

1. **Not validating converted models** - TFLite conversion can silently degrade model accuracy
2. **Over-aggressive quantization** - Applying INT8 quantization without calibration results in accuracy loss
3. **Missing representative dataset** - Quantization performed without calibration data produces unreliable results
4. **Ignoring model size** - Oversized models cannot be deployed on memory-constrained devices
5. **Not testing on target hardware** - Inference performance can differ substantially across device types
6. **Hardcoded preprocessing** - Performing preprocessing on the client side leads to inconsistencies across environments
7. **Wrong input/output types** - Type mismatches between the model and inference code cause failures at runtime
8. **Not using batch inference** - Processing one sample at a time is highly inefficient at high throughput
9. **Missing error handling** - Production serving systems require robust error handling to remain reliable
10. **Not monitoring model drift** - Without continuous monitoring, model performance degrades unnoticed over time
11. **Incorrect tensor shapes** - Shape mismatches produce runtime errors that are difficult to diagnose
12. **Not optimizing for target device** - Generic optimizations fail to exploit device-specific hardware features
13. **Forgetting model versioning** - Lacking version history makes rollback and A/B testing infeasible
14. **Not using GPU acceleration** - Restricting inference to the CPU is significantly slower on GPU-capable devices
15. **Deploying untested models** - Models must always be validated before being pushed to production
