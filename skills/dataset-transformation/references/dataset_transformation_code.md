...
- `scripts/transformation_tools.py` — contains `execute_transformation_job()` for executing the generated script as a SageMaker Processing Job. Use this when the user prefers remote execution over local.
...
- The script must function correctly in two execution contexts:
...
Save the transformation function to its own file at `<project-dir>/scripts/transform_fn.py` so the user can view and edit it directly. The `<project-dir>` is the project directory created by the directory-management skill (e.g., `dpo-to-rlvr-conversion`).
...
The execution script imports `transform_dataset` from `transform_fn.py` rather than embedding it inline. Both files must be located in the same directory (`<project-dir>/scripts/`).
...
Use `execute_transformation_job` from `scripts/transformation_tools.py` to run the script as a SageMaker Processing Job. This function handles container setup, S3 input/output mounting, and job orchestration. Do not build Processing Job logic manually — always delegate to this tool.
...
After submitting, verify status with:
...
Poll `describe_transformation_job` at regular intervals (approximately every 30 seconds) until `status` reaches `Completed`, `Failed`, or `Stopped`.
