...
description: >-
  Toolkit for producing animated GIFs optimized for Slack, with validators for
  size constraints and composable animation primitives. This skill applies when
  users request animated GIFs or emoji animations for Slack, phrased as
...
A toolkit for producing animated GIFs optimized for Slack. Supplies validators for Slack's constraints, composable animation primitives, and optional helper utilities. **Apply these tools in whatever way is needed to realize the creative vision.**
...
**Emoji GIFs are demanding** - the 64KB limit is strictly enforced. Helpful strategies include:
- Cap the total frame count at 10-15
- Use no more than 32-48 colors
- Keep designs simple
- Steer clear of gradients
- Check file size often

## Toolkit Structure

This skill offers three categories of tools:

1. **Validators** - Confirm whether a GIF satisfies Slack's requirements
...
**Full creative freedom exists in how these tools are combined and applied.**

## Core Validators

To verify that a GIF satisfies Slack's constraints, use these validators:
...
These are composable building blocks for animation. Apply them to any object in any combination:
...
**To combine primitives freely, follow these patterns:**
```python
...
These are optional helpers addressing common needs. **Use, modify, or replace them with custom implementations whenever appropriate.**
...
In small GIFs such as emojis, text legibility is difficult. A common remedy is to add outlines:
...
For custom text rendering, PIL's `ImageDraw.text()` works well for larger GIFs.

### Color Management

Polished GIFs typically rely on cohesive color palettes:
...
To handle colors directly, use RGB tuples — whatever suits the use case.
...
Smooth motion relies on easing rather than linear interpolation:
...
When a GIF exceeds size limits:

**For Message GIFs (>2MB):**
1. Lower the frame count (reduce FPS or shorten duration)
2. Cut colors (128 → 64 colors)
3. Decrease dimensions (480x480 → 320x320)
4. Enable duplicate frame removal

**For Emoji GIFs (>64KB) — be aggressive:**
1. Cap total frames at 10-12
2. Use no more than 32-40 colors
3. Avoid gradients (solid colors compress more efficiently)
4. Simplify the design (fewer elements)
5. Pass `optimize_for_emoji=True` to the save method
...
This toolkit supplies building blocks, not rigid recipes. To approach a GIF request:

1. **Understand the creative vision** - What should happen? What is the intended mood?
2. **Design the animation** - Break it into phases (anticipation, action, reaction)
3. **Apply primitives as needed** - Shake, bounce, move, effects — mix them freely
4. **Validate constraints** - Verify file size, especially for emoji GIFs
5. **Iterate if needed** - Cut frames or colors if the size limit is exceeded

**The objective is creative freedom operating within Slack's technical constraints.**

## Dependencies

To use this toolkit, install the following dependencies if they are not already present:
