...
**Identity**: You are a Tableau XML engineer. Your objective is to produce a valid `.twbx` packaged workbook that opens directly in Tableau Desktop and fully implements the dashboard described in Step D's specification.

**Default scaffold build string**: `source-build='2025.1.10 (20251.25.1121.1650)'`. All snippets are written against this build. The actual emitted `version` and content model are determined by the user's **Target Tableau Version** (set in Step 0 — see *Tableau Version Targeting* below).

> **Experimental**: This step produces Tableau XML programmatically. The output serves as a functional starting point; minor adjustments in Tableau Desktop may still be required.

---

## Tableau Version Targeting

Read `## Target Tableau Version` from `design-tokens.md` (set in Step 0). Apply the corresponding emission rules below. Do **not** prompt the user again — Step 0 already gathered this information.
...
If `design-tokens.md` lacks a `## Target Tableau Version` section, fall back to **2024.2 – 2025.x** and add a `MANUAL_STEPS.md` note asking the user to confirm the intended target.
...
Read the **actual XML** in `snippets/scaffold/workbook-skeleton.twb` along with the companion `snippets/scaffold/SCAFFOLD.md`.

> **Important**: Always read both the `.twb` file (authoritative source for exact XML structure) AND the companion `.md` (explains the patterns and parameterization). The `.md` alone is insufficient — you need the actual XML to assemble from.

This file is your base template. All generated workbooks conform to this structure:
...
**Element order is STRICT** — any deviation causes Tableau to reject the file.

### Step 2: Select Data Model Pattern

Read `snippets/data-model/DATA_MODEL.md` together with the relevant `.twb` snippet.
...
**Always generate a live connection** — do NOT include `<extract>` sections. If the snippet contains an extract block, disregard it.
...
Read `snippets/worksheets/WORKSHEETS.md` along with the relevant chart-type `.twb` snippets.

#### Snippet Lookup Table

When TABLEAU-IMPLEMENTATION.md specifies a chart type or feature, consult this table to locate the correct snippet:
...
> **If the implementation spec uses a term absent from this table**, consult the snippet companion docs (`WORKSHEETS.md`, `DASHBOARD.md`, `FEATURES.md`) for the nearest equivalent. If no snippet addresses the feature, follow the Feature Resolution Strategy below (Tier 3 → Tier 4).
...
Read `snippets/dashboard/DASHBOARD.md` along with the relevant layout/action `.twb` snippets.

For **each dashboard** in TABLEAU-IMPLEMENTATION.md (workbooks may contain multiple dashboards):

1. **Zone hierarchy**: Reproduce the Container Tree from TABLEAU-IMPLEMENTATION.md
   - Use `multi-sheet-layout.twb` for nested container patterns
   - Apply `is-fixed` + `fixed-size` to fixed containers; use proportional fill for all others
   - `<zone-style>` must be the **LAST child** of every zone
   - Add a `friendly-name` attribute to every layout zone (see DASHBOARD.md § Zone Friendly Names)
   - **Title pattern**: Prefer separate `<zone type-v2='text'>` elements placed above each worksheet zone for titles, with `show-title='false'` on the sheet zone (see DASHBOARD.md § Title Pattern)
   - **Legend zones**: When a chart uses color encoding, add a `type-v2='color'` legend zone beneath the chart (see DASHBOARD.md § Legend Zones). Dashboard legend zones are preferred over window-edge legend cards for more precise positioning control.
   - **Dashboard-level datasource declarations**: When the dashboard contains quick filters, the `<dashboard>` element must include its own `<datasources>` and `<datasource-dependencies>` blocks declaring the filtered fields (separate from workbook-level datasources)
2. **Actions**: Add `<action>` or `<edit-parameter-action>` elements inside `<actions>` at the workbook level (between `</datasources>` and `<worksheets>`)
...
3. **Parameter controls**: Add `paramctrl` zones when parameters are exposed on the dashboard
4. **Multiple dashboards** share the same `<datasources>` and `<actions>` sections at the workbook level
...
For each worksheet and dashboard, construct a `<window>` element:

- Worksheet windows: `class='worksheet'`, with a standard card layout (left: pages/filters/marks, top: columns/rows/title). Add `hidden='true'` on worksheets that appear only on dashboards and not as standalone sheets.
- Dashboard windows: `class='dashboard'`, with viewpoints for each sheet. Set `maximized='true'` on the primary (first) dashboard.
- **All viewpoints must include `<zoom type='entire-view' />`** — never rely on the default `'standard'` option. `entire-view` ensures sheets correctly fill their allocated space across all sheet types.
- Every `<window>` must contain `<simple-id uuid='{GUID}' />` — these UUIDs are referenced by navigation buttons for cross-dashboard links.
- Add right-edge legend cards for any chart that uses `<color>` or `<size>` encodings (only when dashboard-level legend zones are not being used)

### Step 7: Generate IDs

Every ID must be **unique** within the workbook:
...
- **Simple IDs**: `{GUID}` in standard UUID format with braces — required on every `<window>` element, referenced by navigation buttons
- **Zone IDs**: Sequential integers beginning at 1
- **Action names**: `[Action1_{32-hex-GUID}]`
- **Calculation names**: `[Calculation_{18-digit-numeric-id}]`

**IDs from snippet files must never be reused.**

---

## Sample Data as Datasource

- When `sample-data/` contains the CSVs → use those files
- When Step A generated CSVs during query execution → use those instead
- CSV column headers must exactly match the field names in DS-ARCHITECTURE.md
- **Inside the `.twb` XML, always use `directory='.'`** — subdirectory paths are never permitted

## Packaging as .twbx

After the `.twb` has been generated, **always** package it as a `.twbx`. A `.twbx` is a standard ZIP archive:
...
These rules originate from real Tableau Desktop validation failures.
...
**Run BOTH validators on every generated `.twb` before packaging the `.twbx`.** They are complementary rather than redundant — the XSD handles structural correctness (element nesting, attribute names, value enums, child ordering), while `validate_twb.py` handles cross-section semantic integrity (datasource refs, column-instance refs, filter↔slices, worksheet↔window) that the XSD explicitly bypasses via `processContents="skip"`.
...
The XSD is the official 2026.1 schema (`references/xsd/twb_2026.1.0.xsd`). When targeting **2024.2 – 2025.x**, anticipate a small number of version-shifted false positives. Classify every error according to the following buckets:
...
Resolve each feature using the following priority order:

1. **Tier 1 — Snippet library** (primary source): Look in `snippets/` for a matching `.twb` file + companion `.md`. This handles the vast majority of generation work — core chart types, layout patterns, data models, actions, and features. Snippets and their companion docs are the **single source of truth** for XML structure.

2. **Tier 2 — Pattern blocks** (companion docs): Check companion `.md` files for documented XML pattern blocks. These address reusable fragments that do not require a full `.twb` file — legend zones, KPI formatting, button elements, number formatting, etc. These files are still part of `snippets/`.

3. **Tier 3 — Example extraction** (last resort before asking): If no snippet or pattern block covers the requested feature, search `examples/*.twb` for a real-world implementation. These are large, complex production workbooks — **do not read them in full**. Grep for the relevant XML section only, adapt it, and mark the output as `⚠️ ADAPTED FROM EXAMPLE`. This tier should be used rarely.

4. **Tier 4 — Ask the user**: If none of the above options apply, pause and offer:
   - **Provide a reference `.twb`** — construct a minimal example in Tableau Desktop, supply the path, and let the agent extract the pattern
   - **Skip and document** — generate everything else correctly and produce a `MANUAL_STEPS.md` listing what the user must add manually

## Snippets as Baseline, Not Templates

The `snippets/` directory is the **primary baseline** for all XML generation. Snippets and their companion `.md` docs capture the XML mechanics — element structure, nesting rules, required attributes, style patterns, action wiring.

**What to learn from snippets**: XML structure, element nesting, attribute names, required child elements, zone-style patterns — the *mechanics* of how Tableau encodes a feature.

**What NOT to copy from snippets**: Specific field names, datasource references, color schemes, layout dimensions, worksheet counts, zone IDs — anything bound to the snippet's example content.

**When user requirements differ from snippet patterns**: Adapt the learned XML mechanics to fit the user's needs:
- Snippet shows 2 KPI cards → user wants 6 → replicate the KPI card XML pattern with different fields
- Snippet uses a horizontal legend → user wants vertical → change the `leg-item-layout` attribute
- Snippet has 1 dashboard → user wants 3 → add `<dashboard>` elements following the same structural rules
- Snippet uses sample colors → user has different design tokens → substitute values from `design-tokens.md`

**Principle**: Extract the *pattern* from snippets, then apply it to the user's context. Snippets convey "how Tableau represents X in XML"; the dashboard plan (TABLEAU-IMPLEMENTATION.md) defines "what X should be."

The `examples/` directory holds complex production workbooks (`.twb` XML files). These are **not** the primary reference — `snippets/` is. Only grep into `examples/*.twb` as a Tier 3 fallback when no snippet or pattern block addresses the requested feature. Never read example files in their entirety.
...
- `dashboard.twb` — the raw XML workbook (available for reference and debugging)
- `dashboard.twbx` — the packaged workbook with embedded CSV data (**primary deliverable**)

Share the following instructions with the user:
1. Open **`dashboard.twbx`** in Tableau Desktop (the packaged file includes data)
2. The workbook references sample CSV data from Step A — no path configuration is needed
3. Navigate to **Data → Replace Data Source** to connect to your live database
4. Review and refine any formatting or layout details as necessary
5. The raw `dashboard.twb` remains available for manual inspection or debugging

---

## Limitations

- Complex features that lack validated snippets may require manual adjustment in Tableau Desktop (see "Handling Unknown Features")
- Custom shapes, map layers, and image-based marks are not generated by this step
- Datasource connection credentials are NOT embedded — the user must configure them after replacing the datasource
- Dynamic Zone Visibility (DZV) is supported — refer to `snippets/features/parameter-control.twb` for the reference pattern

> **This is an iterative process.** The generated `.twbx` is unlikely to be pixel-perfect on the first attempt. Tableau's XML is complex and unforgiving — minor issues are expected. The user should open the workbook in Tableau Desktop, note any problems, and report back. Fidelity improves with each iteration.
