...
These are the default Tableau design tokens applied as a **fallback** when the user supplies neither a `template.twb` nor a `branding/` directory. Step 0 produces a project-specific `design-tokens.md` that overrides these values and must explicitly disclose every decision driven by a fallback.

For a structural TWB reference, consult `examples/top-level-workbook-example.twb`.
...
### With Main KPI Row
The same chart layouts, each preceded by a KPI row containing 4 evenly distributed KPI cards:
...
- **No rounded corners** unless the user explicitly agrees to a non-Tableau-faithful mock
- **border-style: none** on all containers
- Charts use a white background; the dashboard uses a light gray background
- Apply the distribute-evenly layout strategy to KPI rows and multi-chart rows
- The hidden filters panel uses a collapsible toggle button (DZV pattern)
- Apply `fixed-size` to structural elements (title bars, KPI rows, filter bars, accent/separator bars, icons, logo) — only chart areas and main content areas should flex
- Every `layout-flow` container must include at least one Spacer (Blank, flex) to prevent layout collapse
- When these fallback tokens are used in Step 0, the generated `design-tokens.md` must list them under `## Fallback Decisions`
