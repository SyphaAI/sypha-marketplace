...
A complete REPL session illustrating the recommended three-tier workflow: authenticate, survey the full site, trace lineage on selected assets, examine field schemas, and run VDS queries — retaining data as Python variables throughout.

> **Formalizing a workflow?** After completing REPL exploration, see [SDK.md](sdk/SDK.md) to produce a reusable script.
...
All methods are available on the `Session` object. Select and combine them based on your task — you are not required to call every tier.
...
For interactive exploration where you accumulate context across multiple cells, call `__enter__()` once and keep the session open:
...
To export data to disk (CSV, JSON), use `data.py`:
