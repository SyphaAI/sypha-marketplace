...
Override datasource parameter values at query time. Parameters must already be defined in the published data source.
...
> _Note_: For ad-hoc calculations that reference parameters, define a custom `calculation` field that uses the parameter and include the parameter override in the `parameters` array.
