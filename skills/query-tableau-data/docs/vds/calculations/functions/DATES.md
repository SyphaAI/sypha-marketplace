...
Dates appear in many data sources. When a field contains recognizable date values, it is assigned a **date** data type. Working with dates in calculations typically requires date-specific functions.
...
Type conversion function that converts string and number expressions into dates, provided they are in a recognizable format.
...
This function behaves like date rounding. It takes a specific date and returns a version of that date truncated to the desired precision. Because every date must have a value for day, month, quarter, and year, `DATETRUNC` resets those values to the lowest possible for each date part below the one specified. See the example for clarification.
...
Many date functions in Tableau accept the `date_part` argument, which is a string constant specifying which portion of a date the function should consider, such as day, week, quarter, etc. The valid `date_part` values are:
...
Some functions include the optional `[start_of_week]` parameter. It lets you specify which day is treated as the first day of the week, such as "Sunday" or "Monday". When omitted, the start of week is determined by the data source. See Date Properties for a Data Source.

In the examples below, September 22 is a Sunday and September 24 is a Tuesday. The DATEDIFF function calculates the number of weeks between these two dates.
