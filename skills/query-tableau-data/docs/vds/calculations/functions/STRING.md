...
String functions let you work with and transform string data (that is, text-based data). Tableau relies on the current International Components for Unicode (ICU) library for string comparisons. Sort and comparison behavior depends on both language and locale, so visualizations may shift as the ICU library is updated to broaden language support.

As an example, imagine a field containing full customer names such as Jane Johnson. A string function can extract just the last names from all records into a separate field.
...
> _Tip_: Refer to Format Text and related topics when working with text fields. The string functions documented here modify the content of fields rather than controlling how that content is displayed.
...
Some data sources impose restrictions on how strings can be split. The table below indicates which data sources support negative token numbers (right-side splitting) and whether a maximum number of splits is enforced.

When a `SPLIT` function uses a negative token number that would otherwise be valid, these data sources return the error: *"Splitting from right is not supported by the data source."*
