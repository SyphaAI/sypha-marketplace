...
Logical calculations let you evaluate whether a given condition is true or false (Boolean logic). A common use case is classifying values into categories based on defined thresholds.

An example of a logical calculation:
...
CASE is generally simpler to use than IF or IIF. In many situations, all three can be used to express the same logic interchangeably. Any CASE statement can be rewritten as an IF statement, though CASE tends to be more concise and readable. Not every IF statement can be converted to a CASE statement, however, since each ELSEIF clause may reference a different field.

**Tip**: The following example shows the same logic written using each of the three functions:
...
**Note**: In many cases, a group can produce the same results as a complex CASE function. Test both approaches to determine which performs better in your scenario.

### Examples

CASE functions are well suited to scenarios such as re-aliasing:
...
or enabling users to choose which measure to display in a chart when combined with a parameter(Link opens in a new window):
