...
Number functions enable you to carry out mathematical operations on the values in your fields. These functions apply exclusively to fields that hold numeric data.

As an example, suppose you have a field called Budget Variance that holds budget deviation values, one of which is -7. The ABS function can be applied to return the absolute value of that number as well as every other value in the field.

The calculation would look like this:

`ABS[Budget Variance]`

For the -7 value, this produces an output of 7.
...
| Example | ``` ZN(Grade) = 0 ``` |
| Notes | This function is particularly helpful when a calculation involves fields that may contain null values. Enclosing a field in `ZN` prevents errors that would otherwise arise from operating on nulls. |
