...
Aggregate functions let you summarize your data or adjust its granularity.

For instance, you might need to know the exact number of orders your store received in a given year. You can use the COUNTD function to count the distinct number of orders and then break the visualization down by year.
...
The following rules govern aggregate calculations:

* Within any aggregate calculation, you cannot mix an aggregated value with a disaggregated one. For example, `SUM(Price)*[Items]` is invalid because SUM(Price) is aggregated and Items is not. However, `SUM(Price*Items)` and `SUM(Price)*SUM(Items)` are both valid.
* Constant terms in an expression function as either aggregated or disaggregated values depending on context. For example: `SUM(Price*7)` and `SUM(Price)*7` are both valid expressions.
* All functions can operate on aggregated values. However, all arguments to a given function must be either all aggregated or all disaggregated. For example: `MAX(SUM(Sales),Profit)` is invalid because Sales is aggregated and Profit is not. By contrast, `MAX(SUM(Sales),SUM(Profit))` is valid.
...
* Like predefined aggregations, aggregate calculations produce correct grand totals. Refer to Grand Totals for more information.
