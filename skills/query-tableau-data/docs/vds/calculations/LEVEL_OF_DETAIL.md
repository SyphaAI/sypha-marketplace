...
Level of Detail expressions (also known as LOD expressions) let you compute values at both the data source level and the visualization level. Unlike standard aggregations, LOD expressions give you direct control over the granularity at which a value is computed. Computation can target a more granular level (INCLUDE), a less granular level (EXCLUDE), or an entirely independent level (FIXED).

This article covers the types of LOD expressions available in Tableau, when each is appropriate, and how to write them correctly.
...
INCLUDE is useful when you need to compute at a finer level of detail in the database, then re-aggregate to a coarser level in your view. Fields based on INCLUDE level of detail expressions change as you add or remove dimensions from the view.
...
You can define a level of detail expression at the table level without specifying any scoping keyword. For example, the expression below returns the minimum (earliest) order date across the entire table:
