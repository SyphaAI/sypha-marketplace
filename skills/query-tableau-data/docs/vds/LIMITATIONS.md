...
VDS is available across all license models. Each **Creator license** assigned to a site increases the site-wide query cap by 100 queries per hour.

> _Note_: Exceeding the query cap causes the server to return HTTP `429` with error code `429000`.
...
VDS supports table calculations through the dedicated `tableCalculation` object — NOT by embedding table calc functions directly in a `calculation` string. For the complete schema, partitioning semantics, and worked examples, see [TABLE_CALCULATIONS.md](./calculations/TABLE_CALCULATIONS.md).
...
- **Do not** place table calculation functions (`RUNNING_SUM`, `WINDOW_AVG`, `RANK`, `LOOKUP`, `FIRST`, `LAST`, `INDEX`, `SIZE`, `TOTAL`, `PREVIOUS_VALUE`, etc.) inside a regular `calculation` field. This will fail. You **must** use the `tableCalculation` object wrapper.
- Raw table calc function syntax (e.g., `RUNNING_SUM(SUM([Sales]))`) is only usable via `tableCalcType: "CUSTOM"` with both a `calculation` string AND a `tableCalculation` wrapper.
- Table calculations **cannot** appear inside LOD aggregate expressions.
