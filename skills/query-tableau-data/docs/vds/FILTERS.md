...
Filters restrict the result set. All filter types support an optional `context` property (boolean).


### The `FilterField` Type

The `field` property in each filter accepts one of three shapes:
...
**When to use:**
- Set `context: true` on dimension filters (`SET`, `DATE`, `QUANTITATIVE` on dimensions) that establish the analysis scope.
- Set `context: false` on `TOP` filters to rank or limit results within that established context.
- Omit `context` for straightforward queries with only a single filter.
...
- A maximum of one filter per field is permitted.
- `SET`, `MATCH`, and `DATE` filters cannot apply functions or calculations to the field (pre-2026.1 restriction; see [Version Gating](#version-gating)).
- `MATCH` filters must supply at least one of `startsWith`, `endsWith`, or `contains`.
- Quantitative `MIN`/`MAX` operators are **inclusive**. To express strictly greater-than or less-than logic, apply a small offset (for example, `min: 10.01` for `> 10`).
- `TOP` filters must specify `fieldToMeasure`.
- The `SET` filter `values` array must not be empty.
...
VDS accepts dates in **RFC 3339** format exclusively.

- **Valid**: `2025-03-14`
- **Invalid**: `2025-03-14T00:00:00Z` (datetime), `03/14/2025` (locale-dependent)

> _Note_: When supplying dates in custom calculations or filters, always use the `#YYYY-MM-DD#` literal format for Tableau calculation syntax (e.g., `#2021-05-05#`).
