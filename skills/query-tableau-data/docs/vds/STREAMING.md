...
Beginning with Tableau 2026.1, VDS supports streaming results to enhance performance and eliminate the previous 1 GB response size limit.
...
An `ERROR` event may arrive at any point during streaming — including after some `DATA` events have already been received. This differs from pre-streaming HTTP errors, which return non-200 status codes.
...
The SDK's `query()` method reads the entire response body into memory before returning. For very large result sets (hundreds of thousands of rows), you may prefer to consume the SSE stream line-by-line to avoid buffering the full payload. This section demonstrates how to do that directly with Python's `http.client` stdlib — no additional packages required.
