...
After a data source has been identified via the [Data Catalog](./CATALOG.md), the agent must introspect it to obtain comprehensive field metadata so it can construct valid VDS queries. This document covers **Phase 2: Datasource Introspection** in the agent's three-phase pipeline.

1. **Discovery** — Locate published data sources on the Tableau site and determine which are relevant.
2. **Introspection** — Retrieve field metadata and the datasource model.
...
> **IMPORTANT:** The agent must call `readMetadata` before querying. VDS `readMetadata` is the primary source of truth for which fields are queryable. The Metadata API (GraphQL) provides secondary enrichment.
...
Given a datasource LUID, retrieve comprehensive metadata. This phase calls **up to three APIs** and merges their results using a strict fallback chain.
...
**Critical rule:** If GraphQL fails or is disabled, the agent still receives usable VDS-only metadata. Queries are never blocked by Metadata API downtime.


### Field Names vs. Captions

When building VDS queries, it is important to understand the relationship between introspection output and query request fields:
...
A brief summary is printed to stdout (first 10 datasource names) so the agent can reason in context without opening the file.
