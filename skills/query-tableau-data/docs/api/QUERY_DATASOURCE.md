...
1. **Discovery** — Locate data sources via the Data Catalog. See [CATALOG.md](./CATALOG.md).
...
Before executing queries, verify that VDS is reachable.
...
Returns the Tableau functions supported for a specific data source. These are the functions available when creating calculated fields.
