...
The **Tableau Data Catalog** describes data sources, workbooks, and views — the complete content hierarchy of a Tableau site. This lets agents navigate from user-facing views through workbooks down to queryable published datasources.
...
> **IMPORTANT**: Tableau environments can contain large volumes of datasources with metadata, so it is best practice to store API responses in the `temp/` folder for parsing and exploration as described in [TEMP_DATA.md](../sdk/TEMP_DATA.md). You must delete this temporary data when your work is complete.
...
Catalog discovery is organized into three tiers, each with distinct responsibilities, transport choices, and cost profiles. Use the tier that matches how much you already know — you do not need to run all three.
...
Answers "what exists on this site?" using flat, paginated REST calls. No GraphQL. No nested relationship data. Always completes regardless of site size.
...
**When to use:** First step on any task — get a flat list of all published datasources with certification and tag signals. Use `filter_name` or `filter_project` to narrow results on large sites before iterating.
...
**When to use:** Retrieve a flat workbook list for initial orientation. Does not include sheets, dashboards, or datasource links — use `workbook_lineage()` for those.
...
> **Note:** `owner_id` is the owner's LUID, not their display name. Resolving the name requires a separate users call.
...
Answers "how does this specific asset connect to others?" using targeted, per-asset GraphQL queries. Each function fetches one node by `luid` — no pagination, one HTTP request. Applies the corrected socket timeout ordering from POSTMORTEM_2 §8.1.
...
**When to use:** After `inventory_datasources()` — to understand which consumers depend on a datasource and which physical data it reads from, before committing to a full `introspect()` call.
...
View discovery relies on two separate GraphQL queries — one for sheets and one for dashboards — and then merges the results.
