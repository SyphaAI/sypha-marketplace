...
> **Prefer REPL exploration for interactive workflows.** Keeping catalog, schema, and query results as Python variables prevents data from entering the LLM's context window and eliminates cleanup overhead. Use `temp/` persistence for specific use cases: user-requested CSV export, multi-session continuity, or cross-tool handoff. See [SDK.md — REPL Exploration](SDK.md) for the recommended default pattern.

When persistence is required, the best practice is to **store API responses and query results in `temp/`**. The skill's `.gitignore` excludes `temp/*` (keeping only `temp/.gitkeep`), so nothing inside is ever committed to version control. This allows you to explore large result sets locally with standard Unix tools, then remove the contents when finished.
