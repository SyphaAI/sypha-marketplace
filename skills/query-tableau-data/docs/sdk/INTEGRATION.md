...
The simplest and recommended approach. Offloads the entire sync Session lifecycle to a thread, leaving your event loop unblocked.
...
For services that require explicit control over the thread pool — e.g., capping concurrency to prevent overwhelming Tableau with parallel connections.
...
Use this when:
- You need to cap concurrency (rate-limit protection)
- Multiple async tasks query Tableau simultaneously
...
**Why the wrong pattern passes tests:** `AsyncMock` and `MagicMock` will implement whatever protocol you configure without complaint. The mock object has no awareness that the real `Session` class only supports `with`. Your tests validate mock behaviour, not production behaviour.
...
This compiles and runs — but `__aenter__` calls `__enter__`, which performs synchronous HTTP (sign-in). Your event loop remains blocked for the entire network round-trip. The `async` keyword provides no benefit here; it merely conceals the blocking call from linters and reviewers.
