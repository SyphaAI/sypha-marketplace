...
This document covers advanced usage patterns for agents who require precise control over the auth lifecycle, HTTP transport, or query payload construction. Most agents should use `Session` (see [SDK.md — Default Workflow](SDK.md#default-workflow)).
...
For complete control over the auth lifecycle. You are responsible for sign-out — always use `try/finally`.
...
The same raw functional API applies to workbook and view workflows:
...
When you need a custom SSL context, proxy, or explicit connection reuse, create the `http.client.HTTPSConnection` yourself and supply it as `conn`:
...
**Do not use it** for standard agent workflows — `Session` ([Default Workflow](SDK.md#default-workflow)) is cleaner and safer.

---

## Raw VDS Payload: `query_raw()`

When the typed `QueryRequest` model does not yet expose a VDS field, build the payload manually and call `session.query_raw()`:
...
Access `Session` internals to invoke functional APIs directly while still benefiting from Session-managed auth and sign-out:
...
`query_stream()` is not part of the SDK's public API. The buffered `session.query()` handles all standard REPL exploration and script use cases.
...
Run validation against the datasource schema before sending a query to catch errors on the client side:
...
Tokens remain valid for approximately 240 minutes. The `Session` context manager handles token expiry transparently:
