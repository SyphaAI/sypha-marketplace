...
**New to this skill? Start with [REPL.md](../REPL.md)** — it walks through a complete REPL exploration session before you need this reference.
...
Use this path when persistence is explicitly required — not as the default exploration workflow. See [TEMP_DATA.md](./TEMP_DATA.md) for full documentation on file formats, naming conventions, and `jq` exploration commands.
...
> **NOTE**: The `Session` class manages the authentication lifecycle automatically — transparent 401 retry and sign-out on exit. Import it from `session.py` directly.
