...
This file establishes the **ubiquitous language** for the `query-tableau-data` skill in accordance with Domain-Driven Design (DDD) principles. It is a **C1 foundational document** — every agent working with this skill should read it before writing or modifying any code to ensure consistent terminology and correct placement of domain concepts.
...
This glossary defines the authoritative terms for the skill domain. **All agents must use these exact terms** when writing code, documentation, issues, and task descriptions. Inconsistent terminology introduces ambiguity that compounds across documents.

When adding a term, include: the term itself, a concise definition, the bounded context(s) in which it applies, and an example when the term is easily confused with another.


## Bounded Context: Tableau Platform

Terms drawn from the **Tableau** platform — the external system this skill integrates with. These terms describe Tableau's native concepts, not the skill's own workflow.
...
Terms describing the **skill's multi-phase workflow** for autonomously discovering, understanding, and querying Tableau data sources. These are not Tableau-native concepts; they constitute the skill's own domain language.
...
Terms describing the **local SDK design** — the architectural decisions and patterns that differentiate this implementation from the official SDK.
