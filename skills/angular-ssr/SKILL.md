...
name: angular-ssr
description: >-
  Configure server-side rendering and hydration in Angular v20+ with
  @angular/ssr. Covers SSR setup, hydration strategies, prerendering static
  pages, and handling browser-only APIs. Triggers when setting up SSR,
  resolving hydration mismatches, prerendering routes, or adapting code to be
  SSR-compatible.
metadata:
...
Enable server-side rendering, hydration, and prerendering in Angular v20+.
...
|------|-------------|----------|
| `RenderMode.Prerender` | Static HTML generated at build time | Marketing pages, blogs |
| `RenderMode.Server` | SSR performed dynamically per request | User-specific content |
| `RenderMode.Client` | Rendered only on the client (SPA) | Authenticated dashboards |

## Hydration

### Default Hydration

Hydration is enabled by default through `provideClientHydration()`:
...
Postpone hydration for individual components:
...
| `hydrate on interaction` | Upon click, focus, or input |
| `hydrate on idle` | When the browser is idle |
| `hydrate on immediate` | Right after load |
| `hydrate on timer(ms)` | Once the specified delay passes |
| `hydrate when condition` | When the expression is true |
| `hydrate never` | No hydration ever (static) |

### Event Replay

Capture user events that happen before hydration completes:
...
Run code after rendering, exclusively in the browser:
...
|----------|-------------|
| `PrerenderFallback.Server` | SSR for routes that are not prerendered |
| `PrerenderFallback.Client` | Rendering on the client side |
| `PrerenderFallback.None` | 404 for routes that are not prerendered |

## HTTP Caching

### TransferState

Automatically transfer HTTP responses from the server to the client:
...
For advanced usage, see [references/ssr-patterns.md](references/ssr-patterns.md).
