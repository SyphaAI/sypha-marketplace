...
What to track, what to run, and what to alert on. Covers the metrics every Redis deployment should monitor and the built-in commands available for ad-hoc diagnosis.

## When to apply

- Setting up monitoring or alerts for a Redis instance.
- Diagnosing a Redis performance regression (elevated latency, memory pressure, connection storms).
- Profiling a slow `FT.SEARCH` query or pipeline.
...
Turn to these commands when something appears wrong.
...
The two most useful commands for incident triage:

- **`SLOWLOG GET`** to identify queries that exceeded the `slowlog-log-slower-than` threshold (10ms by default). The output includes the exact command and its duration in microseconds.
- **`MEMORY DOCTOR`** for memory pressure — it returns a one-paragraph summary describing what is unusual about the current memory usage.
...
For interactive use — running queries, browsing keys, profiling indexes — [Redis Insight](https://redis.io/insight/) is the official GUI. It surfaces the same `SLOWLOG` / `INFO` / `FT.PROFILE` data visually and includes Redis Copilot for natural-language queries. It is useful during development and incident response, but is not a substitute for exporting metrics to your monitoring system.
