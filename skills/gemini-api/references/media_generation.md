...
## Image Editing
Chat mode is the recommended approach for editing images.
...
## Video Generation
Generate video using the Veo model. Veo usage can be expensive, so review Veo pricing before proceeding. Begin with the fast model (`veo-3.1-fast-generate-001`) since the output quality is typically sufficient, and switch to the larger model only when necessary.
