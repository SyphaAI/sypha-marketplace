...
## Function Calling
Allow the model to emit function calls that your code can execute.
...
## Search Grounding
Anchor the model's responses in Google Search or your own data with Agent Search (formerly known as Vertex AI Search).
...
## Code Execution
Enable the model to execute Python code so it can compute precise answers.
...
## Url Context
The URL context tool lets you supply Gemini with URLs as supplementary context for your prompt. The model retrieves content from those URLs and uses it to inform and shape its response.
