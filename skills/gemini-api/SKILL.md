...
name: gemini-api
description: >-
  Use when the user asks about Gemini in an enterprise context or explicitly
  references Vertex AI, Google Cloud, or Agent Platform. Guides usage of the
  Gemini API on Agent Platform with the Google Gen AI SDK. Covers SDK usage
  (Python, JS/TS, Go, Java, C#) and capabilities including multimodal inputs,
  tools, media generation, caching, batch prediction, and the Live API.
...
IMPORTANT: Agent Platform (full name Gemini Enterprise Agent Platform) was previously known as "Vertex AI" and many online resources still use that legacy branding.

# Gemini API in Agent Platform

Access Google's most capable AI models for enterprise use cases through the Gemini API in Agent Platform.

The following key capabilities are available:
...
- **Legacy SDKs**: DO NOT use `google-cloud-aiplatform`, `@google-cloud/vertexai`, or `google-generativeai` — these are deprecated.
...
Prefer environment variables over hard-coded parameters when instantiating the client. Initializing the client without arguments will automatically read these environment values.

### Application Default Credentials (ADC)
Configure these variables for standard [Google Cloud authentication](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/start/gcp-auth.md.txt):
```bash
...
- By default, use `location="global"` to reach the global endpoint, which automatically routes requests to regions with available capacity.
- When a user explicitly requests a specific region (e.g., `us-central1`, `europe-west4`), set that region in the `GOOGLE_CLOUD_LOCATION` parameter instead. Consult the [supported regions documentation](https://docs.cloud.google.com/gemini-enterprise-agent-platform/resources/locations.md.txt) if needed.

### Agent Platform in Express Mode
Configure these variables when using [Express Mode](https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/start/api-keys.md.txt) with an API key:
```bash
...
### Initialization
Construct the client without arguments to have it read the environment variables automatically:
```python
...
Alternatively, you can supply parameters explicitly when creating the client.
...
Use the models below only when explicitly requested:
...
When implementing or troubleshooting API integration for Agent Platform, consult the official Agent Platform documentation:
...
The Gen AI SDK on Agent Platform targets the `v1beta1` or `v1` REST API endpoints (e.g., `https://{LOCATION}-aiplatform.googleapis.com/v1beta1/projects/{PROJECT}/locations/{LOCATION}/publishers/google/models/{MODEL}:generateContent`).
...
See the [Python Docs Samples repository](https://github.com/GoogleCloudPlatform/python-docs-samples/tree/main/genai) for additional code samples and specific usage scenarios.

Based on the specific user request, consult the following reference files for detailed code samples and usage patterns (Python examples):
