...
DataStore is a **lazy, ClickHouse-backed pandas replacement**. Existing pandas code runs without modification — operations are compiled to optimized SQL and executed only when results are actually needed (e.g., `print()`, `len()`, iteration).
...
All 16+ sources and URI schemes are documented in [connectors.md](references/connectors.md)
...
209 DataFrame methods are supported. Full API → [api-reference.md](references/api-reference.md)
...
Additional join examples → [examples.md](examples/examples.md)
...
> Note: This skill covers how to *use* chdb DataStore.
> For raw SQL queries, use the `chdb-sql` skill. Source-contribution workflows are out of scope for this package.
