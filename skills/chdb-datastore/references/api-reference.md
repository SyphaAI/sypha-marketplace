...
> Full method signatures for the DataStore class.
> DataStore exposes a pandas-compatible API powered by a ClickHouse backend.
...
**Cross-source joins work transparently** — you can join a MySQL table directly with a Parquet file:
...
Accessed via `ds['column'].str.*`. 56 methods are available, including:
...
Accessed via `ds['column'].dt.*`. 42+ methods are available, including:
...
These properties/methods **trigger execution** of the deferred query:
...
These methods **do not trigger execution** of the query:
...
Use the `insert_into` / `select_from` chaining pattern:
