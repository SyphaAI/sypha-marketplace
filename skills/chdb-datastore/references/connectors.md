...
> A quick reference for connecting DataStore to any data source.
> Once connected, all sources are accessed through the same pandas API.
...
The format is auto-detected from the file extension: `.parquet`, `.csv`, `.tsv`, `.json`, `.jsonl`, `.arrow`, `.orc`, `.avro`, `.xml`.
...
**Notes:**
- Glob patterns (`*`, `**`) support querying multiple files in a single call
- Compression formats (`.gz`, `.zst`, `.bz2`, `.xz`, `.lz4`) are auto-detected from the extension
- Use the `structure` parameter to declare column types explicitly: `structure="id UInt64, name String"`
...
**Note:** The port must be included in the `host` string (e.g., `"db:3306"`) or supplied via the `port` parameter.
...
A universal one-liner that infers the source type from the URI scheme:
