...
description: >-
  ...
  project initialization (uv workspaces, devcontainers, Makefile), Python
  packaging with explicit dependencies, local and cloud execution, experiment
  reproducibility, and extensibility patterns (pipelines, datasets, linting).
  Use this skill whenever the user asks to create, modify, run, test, or deploy
  an AzureML-based ML project — or when they need guidance on project layout,
  dependency management, or cloud job submission with Azure Machine Learning.
metadata:
  category: data
  source:
    repository: 'https://github.com/bepuca/azureml-scaffolding'
    path: azureml-scaffolding
    license_path: LICENSE
    commit: f7a95c6dbe77dcc15ee545bf25e87cc5bb802cde
---

# AzureML Project Scaffolding

A proven structure for AI projects that need reproducible experimentation and leverage
AzureML for cloud execution. It guarantees reproducibility from day one without sacrificing the path
to production — and without undermining the ability to keep experimenting once you're there. Code,
environments, specs, and dependencies are wired so that whatever runs locally also runs on [AzureML][aml],
with no surprises.
...
- **One mental model** — Everything is a *package*: a [uv workspace][uv-workspace] member with its
  ...
  The same structure applies to every package — no special cases, nothing to restructure later. A
  package exists for *computation* — read from paths, do work, write to paths. If a task does not
  compute (registering assets, deploying models, downloading data via platform tools), it belongs in
  orchestration, not a package.
...
- **Colocation** — Everything needed to understand and run a piece of work lives together in one
  folder. Easy to locate, easy to reason about.
...
- **Run anywhere the same** — Same command, same lockfile, same result — whether on your laptop, a
  ...
  runner. Python deps are installed at runtime by uv, not baked in.

- **Complexity must be earned** — Start with the simplest correct thing. Add structure only when a
  specific need demands it. But respect what already exists: if the project has grown beyond the
  basics, that complexity was earned and must not be regressed without understanding why it was
  introduced.
...
1. **Verify local.** Run the verification loop below. Do not proceed until every command passes.
...
- **One Dockerfile, two roles** — devcontainer and cloud runner. The devcontainer is optional — you
  ...
  CLI tools, native builds) can still conflict across projects. The devcontainer resolves that, and
...
migrate the rest. If conflicts exist (e.g., an existing AGENTS.md), merge carefully rather than
overwriting.

## Cloud execution (after local works)

Treat cloud execution as a separate step: first verify `make run`, then use `make aml` to submit to AzureML.
...
1. **Fill YAML placeholders.** Ask the human to supply values for any remaining placeholders in the
...
This skill covers what runs inside a job and how to submit it. What comes after — registering
outputs as versioned data or model assets, deploying models to endpoints, scheduling recurring runs
— is orchestration that belongs outside the job, typically in CI pipelines or operational scripts.
The same layer rule applies: those concerns must never leak into Python code or job YAML. How they
are implemented varies by project; where they live does not — always the outermost layer.
...
Keep the core scaffold minimal. Add the following only when the project requires them. Each reference
file includes an **AGENTS.md section** — merge it into the project's `AGENTS.md` when applying the
extension so future agent sessions discover the added capabilities.
