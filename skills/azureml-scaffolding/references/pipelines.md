...
Apply this pattern when a single package is no longer sufficient and work needs to be composed
across multiple steps.

## What this adds

- Step packages/components with well-defined input/output contracts.
- A pipeline spec that wires steps and data flow explicitly.
- The ability to run steps independently while keeping orchestration declarative.

## Practical structure

- Keep each step as an independent package/component with colocated code, environment, and spec.
- Retain `aml-job.yaml` in packages that are runnable standalone.
- You may add `aml-component.yaml` ([docs][component-docs], [schema][component-schema]) to packages
  that participate in more than one pipeline, to reuse the step definition.
...
Extend `run` and `aml` to also accept `pipe=`. The YAML / command path is resolved by
...
Cloud: ask the human to confirm the pipeline job completed in Azure ML Studio and that step outputs
are visible.
...
- Introduce pipelines only when users request multi-step orchestration, reusable components, or
  explicit step-level lineage.
...
- **No double-runs.** If a package is a pipeline step, do not also run it standalone in the same
