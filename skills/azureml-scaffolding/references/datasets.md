...
Bring data to the developer's machine for exploration and testing. Cloud jobs receive data through
YAML inputs (Blob URIs or registered [Data Assets][data-assets]) — never through a download
pre-step.

Both targets rely on [`azcopy`][azcopy] — it parallelises transfers and resumes on failure, making
it considerably faster than `az storage blob download` or SDK equivalents for large datasets.

## What this adds

Two Makefile targets for pulling data into `./data/` (gitignored):

- `get-dataset` — download a registered Data Asset by name and version.
- `get-data` — download directly from blob storage using a regex filter.
...
- An active [Azure CLI][az-cli] session and `TENANT_ID` set in `.env.local`.
- `get-dataset` additionally requires `AZUREML_WORKSPACE` / `AZUREML_RESOURCE_GROUP` and the `ml` CLI
  extension.
- [`azcopy`][azcopy] must be installed. Add the [Microsoft package registry][ms-pkg] to the Dockerfile so
...
Then add `azcopy` to the existing `apt-get install` line already present in the Dockerfile
...
With Azure access: run the download, then `ls data/` to confirm the files arrived. Without access:
run targets with no arguments — they should print usage and exit non-zero.
...
- Suggest this extension when users need data on their local machine for exploration or testing.
- Prefer `get-dataset` when the team uses registered Data Assets.
- Use `get-data` when data is not registered or the user needs regex-based file selection from blob
  storage.
- Confirm that `data/` is listed in `.gitignore`.
