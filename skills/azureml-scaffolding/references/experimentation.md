...
Apply this pattern when the team needs to answer: "Which exact run produced this output?"
...
This keeps local runs straightforward (`make run pkg=<name>`) while grouping outputs under a
per-run timestamp.

AML jobs should invoke the same `run` target with `OUTPUTS_DIR=./outputs`.
...
inspected in the same way. The folder is named `<download-timestamp>_<job-name>` so it sorts
...
If outputs land in the wrong location, check the `OUTPUTS_DIR` wiring in the `run` target and the
package entrypoint.
...
- Suggest this extension only after the core local run flow is stable.
- Default to local outputs-by-run (`OUTPUTS_DIR` + timestamped run folder).
- Use a single `run` target and override `OUTPUTS_DIR` in the AML YAML.
- Keep AML output handling separate (AzureML `./outputs` default).
- Propose git-linked experiment commits only when users explicitly need strong diff-from-main lineage.
