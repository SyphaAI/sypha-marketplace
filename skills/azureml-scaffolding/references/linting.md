...
This is an extension pattern. The base scaffold remains minimal. Apply the additions below only
when enabling linting.

Keep linting deliberately simple:
...
Add a single lint command to the root `Makefile` and use it for both local and CI runs:
...
This keeps `make lint` as the authoritative source of truth for lint behavior.
...
All three must exit 0. Re-run `make lint` after fixes — ruff auto-corrects most issues on the first pass.
