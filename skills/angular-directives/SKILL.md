...
description: >-
  Build custom directives in Angular v20+ for DOM manipulation and extending
  behavior. Applies to attribute directives that change element
  behavior/appearance, structural directives for portals/overlays, and host
  directives used for composition. Triggered by building reusable DOM
  behaviors, adding functionality to elements, or composing behaviors across
  components. Note - for control flow, rely on native @if/@for/@switch rather
  than custom structural directives.
metadata:
...
Build custom directives in Angular v20+ to package reusable DOM manipulation and behavior.
...
Choose `host` instead of `@HostBinding`/`@HostListener`:
...
Reserve structural directives for DOM manipulation that goes beyond control flow (portals, overlays, dynamic insertion points). Conditionals and loops should use native `@if`, `@for`, `@switch`.

### Portal Directive

Place content at a different spot in the DOM:
...
Hold off rendering until a condition becomes true (once only):
...
Attach directives compositionally to components or to other directives:
...
Bring several behaviors together:
...
See [references/directive-patterns.md](references/directive-patterns.md) for advanced patterns.
