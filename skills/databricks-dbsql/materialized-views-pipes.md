...
Materialized views (MVs) are Unity Catalog-managed tables that physically store precomputed query results. Unlike standard views that recompute on every query, MVs cache results and refresh automatically -- either on a schedule, when upstream data changes, or on-demand.
...
- **Incremental refresh**: Can process only changed data from source tables under certain conditions
...
By default, Databricks applies a cost model to choose between incremental and full refresh. Use `EXPLAIN CREATE MATERIALIZED VIEW` to verify the chosen refresh type.
...
Both approaches ultimately rely on similar underlying mechanisms (serverless pipelines) and support incremental refresh. The primary distinction is in management: DBSQL MVs are self-contained with auto-managed pipelines, while pipeline MVs are part of a broader orchestrated data flow.
...
Temporary tables are session-scoped, physical Delta tables used for storing intermediate data. They exist only within the session that created them.

#### Key Characteristics

- **Session-scoped**: Visible only to the creating session; isolated from other users
...
Temporary views are session-scoped, logical views that store a query definition rather than data. They are recomputed on each access.
...
Pipe syntax (introduced February 2025) enables composing SQL queries as a top-down, left-to-right chain of operations using the `|>` operator. It eliminates deeply nested subqueries and makes SQL read like a DataFrame pipeline.
...
- Multi-step aggregations would otherwise require nested subqueries
- You want DataFrame-like readability in SQL
- Building exploratory or iterative queries (easy to add/remove steps)
- Handling complex transformations with many joins, filters, and projections

**Use traditional SQL when:**
- Simple queries that are already readable
- The team is more comfortable with standard SQL
