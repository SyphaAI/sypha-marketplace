...
A complete reference for advanced Databricks SQL functions, covering built-in AI functions, HTTP requests, Lakehouse Federation remote queries, and file ingestion.
...
Databricks AI Functions are native SQL functions that call state-of-the-art generative AI models from within SQL. They execute via Databricks Foundation Model APIs and are accessible from Databricks SQL, notebooks, Lakeflow Spark Declarative Pipelines, and Workflows.

**Common Requirements for All AI Functions:**
- The workspace must reside in a region that supports AI Functions optimized for batch inference
- Not supported on Databricks SQL Classic (a Serverless SQL Warehouse is required)
- Databricks Runtime 15.1+ for notebooks; 15.4 ML LTS is the recommended runtime for batch workloads
- Models are licensed under Apache 2.0 or the LLAMA 3.3 Community License
- Currently optimized for English (the underlying models handle multiple languages)
- Public Preview, HIPAA compliant

**Rate Limits and Billing:**
- AI Functions are governed by Foundation Model API rate limits
- Charged as Databricks SQL compute plus token consumption on Foundation Model APIs
- Apply `LIMIT` to queries during development to avoid unexpected costs

---

## ai_query -- General-Purpose AI Function

The most capable and versatile AI function. It sends requests to any serving endpoint—Foundation Models, external models, or custom ML models—for real-time or batch inference.
...
> **Note:** The top-level `responseFormat` STRUCT must have exactly one field. If multiple fields are needed, nest them inside a single outer field.
...
These functions offer streamlined, single-purpose interfaces with no need to specify an endpoint or model.

### ai_gen

Produce text output from a given prompt.
...
Assign text to one of the supplied classification labels.
...
**Returns:** STRING equal to one of the provided labels, or NULL when classification is unsuccessful.
...
Pull named entities out of text.
...
**Returns:** STRUCT with one field per label, each holding the extracted entity as a STRING. Returns NULL when content is NULL.
...
Run sentiment analysis against a text string.
...
**Returns:** STRING -- one of `'positive'`, `'negative'`, `'neutral'`, or `'mixed'`. Returns NULL when the sentiment cannot be determined.
...
Calculate the semantic similarity score between two text strings.
...
**Returns:** FLOAT -- A semantic similarity score, where 1.0 indicates identical strings. This score is relative and is intended for ranking purposes only.
...
Produce a condensed summary of a text input.
...
**Returns:** STRING. Returns NULL when content is NULL.
...
Render text into a specified target language.
...
Fix grammatical mistakes found in a text string.
...
**Returns:** STRING with grammar corrected. Returns NULL when content is NULL.
...
Redact specified entity types from text (PII masking).
...
**Returns:** STRING with the specified entities substituted by `[MASKED]`. Returns NULL when content is NULL.
...
Parse and extract structured content from unstructured document formats (PDF, DOCX, PPTX, images).
...
Generate forecasts from time series data using a built-in prophet-style model. This function is a table-valued function (TVF).
...
Search a Mosaic AI Vector Search index from SQL. This function is a table-valued function.
...
Issue HTTP requests to external services directly from SQL via Unity Catalog HTTP connections.
...
Execute SQL against external databases in their native dialect, returning results as a table inside Databricks SQL. This is a table-valued function.

### Overview

Lakehouse Federation lets you query external databases in place, with no data migration required. Two access modes are supported:
- **Query Federation** -- SQL is pushed down to the external database over JDBC
- **Catalog Federation** -- Queries read foreign tables directly from object storage
...
Query files in cloud storage or Unity Catalog volumes from SQL directly, with built-in format detection and automatic schema inference.
