...
> Databricks SQL procedural extensions built on the SQL/PSM standard. Encompasses SQL scripting (compound statements, control flow, exception handling), stored procedures, recursive CTEs, and multi-statement transactions.
...
SQL scripting introduces procedural logic grounded in the SQL/PSM standard. Every SQL script begins with a compound statement block (`BEGIN...END`).

### Compound Statements (BEGIN...END)

A compound statement is the primary building block that holds variable declarations, condition/handler declarations, and executable statements.
...
- Variable declarations come before condition declarations, which in turn come before handler declarations
- Top-level compound statements may not carry labels
- `NOT ATOMIC` is the default and only supported behavior (no automatic rollback on failure)
- In notebooks, the compound statement must be the only statement present in the cell
...
- Variables default to `NULL` when no `DEFAULT` is specified
- The data type may be omitted when `DEFAULT` is provided (the type is inferred from the expression)
- Declaring multiple variable names in a single `DECLARE` is supported from Runtime 17.2+
- Variable scope is limited to the enclosing compound statement
- Name resolution proceeds from the innermost scope outward; use labels to disambiguate when needed
...
Use `SET VAR` to unambiguously target a local variable when a session variable of the same name exists.
...
Runs the statements associated with the first condition that evaluates to `TRUE`.
...
Available in two forms: **simple CASE** (compares an expression) and **searched CASE** (evaluates boolean conditions independently).
...
Only the statements in the first matching branch are executed.
...
Steps through the rows returned by a query.
...
- For Delta tables, changes made to the source table during iteration do not influence cursor results
- The loop may not consume all query rows if it is terminated early via `LEAVE` or an error
...
An unconditional loop that runs indefinitely until a `LEAVE` statement exits it.
...
Runs the body at least once, then continues repeating until the condition becomes `TRUE`.
...
Both statements require a labeled loop as their target.

### Exception Handling

#### Condition Declaration

Assign names to specific SQLSTATE codes for clearer error handling.
...
- Must not begin with `'00'`, `'01'`, or `'XX'`
- Defaults to `'45000'` when not specified
...
Intercept and handle exceptions that arise within compound statements.
...
- `SQLEXCEPTION` catches all error states (SQLSTATE class other than `'00'` or `'01'`)
- `NOT FOUND` catches `'02xxx'` states (no data returned)
- A handler cannot be invoked by statements within its own body
...
Emit or re-emit exceptions from within SQL scripts.
...
- Inside handlers, prefer `RESIGNAL` over `SIGNAL` — `RESIGNAL` retains the diagnostic stack whereas `SIGNAL` clears it
- `MESSAGE_ARGUMENTS` accepts a `MAP<STRING, STRING>` literal
...
Run SQL statements that are assembled as strings at runtime.
...
- `sql_string`: a constant expression that produces a well-formed SQL statement
- `INTO`: stores a single-row result into variables (yields `NULL` for zero rows; raises an error for multiple rows)
- `USING`: binds values to positional (`?`) or named (`:param`) parameter markers (styles cannot be mixed)
...
Stored procedures store SQL scripts durably in Unity Catalog and are called using `CALL`.
...
- Parameter names must be unique within a procedure
- `DEFAULT` is not allowed for `OUT` parameters
- After a parameter is given a `DEFAULT`, all following parameters must also carry defaults
- Default expressions cannot reference other parameters or include subqueries
- The body undergoes syntactic validation at creation time; semantic validation occurs only at invocation
...
- Up to 64 levels of nesting are supported
- For `IN` parameters: any expression that is castable to the parameter type, or `DEFAULT`
- For `OUT`/`INOUT` parameters: the argument must be a session variable or a local variable
- Arguments must conform to the parameter's data type (use typed literals, e.g., `DATE'2025-01-01'`)
- Fewer arguments than parameters are permitted when the remaining parameters carry `DEFAULT` values
...
- Without `IF EXISTS`, attempting to drop a procedure that does not exist raises `ROUTINE_NOT_FOUND`
- Requires the `MANAGE` privilege, ownership of the procedure, or ownership of its containing schema, catalog, or metastore
...
- Basic form: returns the procedure name and its parameter list
- `EXTENDED`: also returns the owner, creation time, body, language, security type, determinism setting, data access class, and configuration
...
Recursive CTEs make self-referential queries possible, enabling hierarchical data processing, graph traversal, and series generation.
...
The recursive member consumes rows generated in the prior iteration and produces new rows. Recursion stops when the recursive member returns zero rows.
...
- Recursive CTEs cannot be used inside UPDATE, DELETE, or MERGE statements
- Recursive step queries may not contain correlated column references to the CTE name
- Random number generators may yield the same values across different iterations
- The default row limit is 1,000,000 rows (supersede with `LIMIT ALL` in Runtime 17.2+)
- The default recursion depth is 100 (increase using `MAX RECURSION LEVEL`)
...
Multi-statement transactions (MST) let you group several SQL statements into atomic units that either commit in full or roll back entirely.
...
If any statement fails, the entire set of changes is rolled back.

> **Note:** Tables used in `BEGIN ATOMIC` blocks must have the `catalogManaged` table feature enabled. Create tables with `TBLPROPERTIES ('delta.feature.catalogManaged' = 'supported')`. Existing tables cannot be upgraded in place — they must be recreated with this property set.

### Python Connector Transaction API

The Databricks SQL Connector for Python exposes explicit transaction control:
...
- `sql.TransactionError` is raised if you attempt to commit without an active transaction
- Changing `autocommit` while a transaction is in progress is not permitted
- Calling `rollback()` when no transaction is active is a safe no-op

### Isolation Levels

Databricks employs **Snapshot Isolation** (surfaced as `REPEATABLE_READ` in standard SQL terminology).
...
- **Repeatable reads**: Data read inside a transaction stays consistent for its duration
- **Atomic commits**: Uncommitted changes are not visible to other connections
- **Write conflicts**: Simultaneous writes to the same table result in conflicts
- **Cross-table writes**: Concurrent writes targeting different tables can proceed without conflict

### Write Conflicts and Concurrency

**Row-level concurrency** (Runtime 14.2+) lowers conflict rates for tables that use deletion vectors or liquid clustering:
...
1. **Keep transactions short** to shrink conflict windows
2. **Always wrap in try/except/finally** and roll back on errors
3. **Restore autocommit** inside the `finally` block
4. **Apply partition pruning** in MERGE conditions to narrow the conflict scope
5. **Enable row-level concurrency** (deletion vectors + liquid clustering) for high-concurrency workloads
6. **Prefer a single-statement MERGE** over a multi-statement transaction when only one table is involved
7. **Commit and start a new transaction** to observe changes committed by other connections
