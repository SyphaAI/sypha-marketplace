...
Complete reference for data modeling patterns, DBSQL performance tuning, and operational guidance on the Databricks Lakehouse Platform.
...
The Databricks Lakehouse provides full support for dimensional modeling. Star schemas map cleanly to Delta tables and frequently outperform fully denormalized alternatives.

**Star Schema (Dimensional Modeling):**
- Central fact table connected to multiple denormalized dimension tables
- Well-suited for complex analytics and multi-dimensional aggregations
- Offers clear business process mapping and scales effectively with SCDs
- Accommodates up to ~10 filtering dimensions (5 tables x 2 clustering keys each)
- Separation of concerns enables fine-grained governance

**One Big Table (OBT):**
- Single wide table with all attributes pre-joined
- Eliminates joins and simplifies governance (one table to manage)
- Liquid Clustering limited to 1-4 keys, restricting effective filtering to 1-3 dimensions
- Full table scans become performance bottlenecks as data grows
- Lacks structured business process mapping
- Complicates fine-grained access controls and data quality enforcement
...
**Key principle:** "The better you model your data upfront, the more easily you can leverage AI on top of it out of the box." Well-designed schemas unlock downstream AI/BI capabilities without additional work.
...
- In-place updates with no history tracking
- Use MERGE INTO with matched UPDATE
- Appropriate for corrections or attributes where historical values are unnecessary

**SCD Type 2 (History Tracking):**
- Version records using surrogate keys and metadata columns
- Include `effective_start_date`, `effective_end_date`, and `is_current` columns
- Use MERGE INTO to implement SCD Type 2 logic in DBSQL
...
**Delta Lake Time Travel** provides access to historical data within configured log retention periods and serves as a complementary capability to SCD.
...
**Sizing strategy:**
- Begin with a single larger warehouse and rely on serverless features to handle concurrency
- Scale down if needed rather than starting small and scaling up
...
**Best practice:** Design dashboards and reports to use parameterized queries that follow consistent underlying patterns, maximizing cache reuse.
...
- **Primary/Foreign Key constraints:** Communicate table relationships to the query optimizer
...
Unity Catalog generates entity relationship diagrams when primary and foreign key constraints are defined, offering visual documentation of the dimensional model.
...
1. **Skipping dimensional modeling in Gold layer:** OBTs are acceptable for Silver, but Gold should use star schemas for multi-dimensional analysis
...
4. **Missing constraints:** Omitting PK/FK constraints deprives the optimizer of relationship information
