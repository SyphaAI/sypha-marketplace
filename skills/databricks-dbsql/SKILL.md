...
description: >-
  Advanced features of Databricks SQL (DBSQL) and SQL warehouse capabilities.
  Invoke this skill whenever the user references: "DBSQL", "Databricks SQL",
  ...
  "pipe operator". SHOULD also invoke when the user inquires about SQL best
...
Consult these files for detailed syntax, complete parameter references, and advanced usage patterns:
...
- **Always use Serverless SQL warehouses** when working with AI functions, MVs, and http_request
- **Use `LIMIT` during development** alongside AI functions to keep token costs in check
- **Prefer Liquid Clustering over partitioning** on new tables (1-4 keys max)
- **Use `CLUSTER BY AUTO`** whenever the right clustering keys are unclear
- **Star schema in Gold layer** for BI workloads; OBT is acceptable in Silver
- **Define PK/FK constraints** on dimensional models to enable query optimization
- **Use `COLLATE UTF8_LCASE`** on user-facing string columns that require case-insensitive search
- **Use MCP tools** (`execute_sql`, `execute_sql_multi`) to test and validate all SQL prior to deployment
