...
description: >-
  Implement signal-based reactive state management in Angular v20+. Covers
  reactive state via signal(), derived state via computed(), dependent state via
  linkedSignal(), and side effects via effect(). Triggers for state management
  questions, converting BehaviorSubject/Observable patterns to signals, or
  constructing reactive data flows.
metadata:
...
Signals are Angular's reactive primitive for state management, providing synchronous, fine-grained reactivity.
...
**Effect rules:**
- Must execute inside an injection context (constructor or via `runInInjectionContext`)
