---
name: csv-query
description: Execute SQL queries against CSV/TSV/Excel files using the Polars SQL engine
allowed-tools:
  - mcp__qsv__qsv_sniff
  - mcp__qsv__qsv_count
...
Query tabular data files with SQL via the Polars-powered `sqlp` command.

> **Cowork note:** If relative paths don't resolve, call `mcp__qsv__qsv_get_working_dir` and `mcp__qsv__qsv_set_working_dir` to synchronize the working directory.

## Decision Tree

**Is the query simple (single column filter, basic select)?**
- Yes -> Consider `select` + `search` for straightforward operations
...
1. **Prepare the file**: Run `mcp__qsv__qsv_index` and `mcp__qsv__qsv_stats` with `cardinality: true, stats_jsonl: true` to build an index and stats cache.

2. **Read the stats cache**: Read `<FILESTEM>.stats.csv` (e.g., `data.stats.csv` for `data.csv`) to understand column metadata before writing SQL. This is the most critical step for writing efficient queries.

3. **Run frequency on key columns**: For columns you plan to GROUP BY, filter on, or join on, run `mcp__qsv__qsv_frequency` to examine actual value distributions. This reveals the best filter values and whether a GROUP BY will yield a manageable result set.

4. **Write and run SQL**: Use `mcp__qsv__qsv_sqlp` with a SQL query informed by stats and frequency data. The table name in SQL is the filename stem (e.g., `data.csv` -> `SELECT * FROM data`). For Parquet files, use `read_parquet('data.parquet')` as the table source instead.

5. **Refine if needed**: Examine the results and adjust the query accordingly.

## Using Stats to Write Better SQL

After reading the `.stats.csv` cache, consult these columns to guide your SQL:
...
- **Pick selective filters**: If `frequency` shows "active" accounts for 90% of rows, filtering on `WHERE status = 'active'` is wasteful — target the rare values instead
- **Validate expected values**: If you plan `WHERE category IN ('A','B','C')`, check frequency first to confirm those values exist and to catch any that may be missing
- **Avoid GROUP BY on high-cardinality columns**: If frequency reveals thousands of unique values, GROUP BY will produce an enormous result — add LIMIT or aggregate differently
...
- For very complex queries that fail, recommend DuckDB as an alternative
- The stats cache helps Polars select optimal data types for columns
- Results are written to stdout by default; use `--output file.csv` for large result sets
- Column names are case-sensitive in SQL queries
- Use `LIMIT` to preview large result sets before running the full query
- `sqlp` can query multiple CSV files within a single SQL statement (useful for joins)
