...
name: vault-api
description: >-
  Apply when interacting with the HashiCorp Vault REST API — health checks,
  init/unseal, auth login, KV read/write, policy management, token operations.
  Covers endpoints, auth methods, and curl examples.
metadata:
...
Vault provides a RESTful JSON API on port 8200. Authenticated endpoints require the `X-Vault-Token` header on every request. Unauthenticated endpoints (health, init) do not require a token.
...
- **KV v2 path includes `data/` prefix.** For a KV v2 engine mounted at `secret`, the correct read path is `/v1/secret/data/myapp`, not `/v1/secret/myapp`. The latter returns a 404.
- **Health endpoint returns non-200 for sealed/standby.** A 503 (sealed) is NOT an error — it is expected behavior after a restart. Inspect the `initialized` and `sealed` fields in the response body rather than relying on the HTTP status code alone.
- **Token in URL is stripped by proxies.** Use the `X-Vault-Token` header instead of the `?token=` query parameter. Proxies and load balancers may log or strip query parameters.
- **Kubernetes auth needs SA token with right audience.** The `vault` audience must be set on the service account, otherwise the default token may be rejected. Use `kubectl create token` with `--audience=vault` to specify the audience explicitly.
- **`list` capabilities for metadata listing.** Listing keys at `/v1/secret/metadata/` requires the `list` capability at that path, not `read`. Without it, the response will be empty.
- **Raft join after unseal.** A sealed node is unable to join the Raft cluster. Always unseal a node before running `raft join`. The joining node will synchronize data from the leader.
- **Snapshot restore requires same cluster size.** Raft snapshots can only be restored onto a cluster with the same number of peers. Adding or removing nodes after a restore may cause failures.
