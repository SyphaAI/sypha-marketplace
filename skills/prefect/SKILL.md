...
name: prefect
description: >
  Prefect is a contemporary workflow orchestration framework for Python data
  pipelines.

  Learn to declare flows and tasks using decorators, manage retries and
  scheduling,

  build deployments, and observe runs through the Prefect UI.
...
Prefect transforms Python functions into observable, schedulable workflows with minimal boilerplate. Attach `@flow` and `@task` decorators to gain retries, logging, caching, and a monitoring UI.
