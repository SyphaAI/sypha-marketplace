...
description: "Troubleshoot crawler schema detection problems including incorrect data types and missing columns"
status: active
severity: MEDIUM
triggers:
  - "wrong schema"
...
SHOULD:
- Compare the detected schema against the expected schema
- Confirm whether custom classifiers are configured and matching as intended
- Inspect crawler logs for classification decisions
- Ensure data files follow a consistent format across the entire crawl target

MAY:
- Run a test against a file subset to narrow down the problematic data
- Determine whether mixed file formats exist under the same S3 prefix
...
- Correct source data format inconsistencies wherever feasible

SHOULD:
- Build custom classifiers for non-standard data formats
- Configure SchemaChangePolicy to LOG so automatic overwrites are prevented
- Apply exclude patterns to bypass files that confuse the classifier

MAY:
- Define tables manually rather than relying on crawlers for schemas that must be exact
- Adopt Glue Schema Registry for enforcement of schema contracts

## Common Issues

- symptoms: "Crawler detects all columns as string type"
  diagnosis: "CSV files without headers or with inconsistent quoting cause the crawler to fall back to string for all columns."
...
- symptoms: "Crawler creates multiple tables for the same dataset"
  diagnosis: "Mixed file formats (CSV and Parquet) or schema inconsistencies exist within the same prefix."
...
- symptoms: "Columns disappear after crawler re-run"
  diagnosis: "SchemaChangePolicy DeleteBehavior is set to DELETE_FROM_DATABASE and the source files have changed."
