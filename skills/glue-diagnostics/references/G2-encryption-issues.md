...
context: "Glue supports encryption at rest for the Data Catalog, job bookmarks, S3 targets, and CloudWatch logs through KMS. Encryption behavior is defined by Glue security configurations. Typical problems include KMS key policies that do not grant the Glue role access, an incorrect KMS key being specified, cross-account KMS access requirements, security configuration mismatches, and SSE-KMS on S3 buckets that demand explicit decrypt permissions."
---
...
SHOULD:
- Confirm the KMS key is in the same region as the Glue job
- Verify the KMS key is enabled and not in a pending-deletion state
- Check that S3 bucket encryption settings align with the Glue security configuration
- Identify any cross-account KMS key access requirements
...
- Confirm connection password encryption settings

## Phase 2 — Remediate

MUST:
- Add the Glue IAM role to the KMS key policy with kms:Decrypt and kms:GenerateDataKey permissions
- Make sure the security configuration matches the encryption type of the S3 bucket
- Confirm the KMS key ARN in the security configuration is correct

SHOULD:
- Use AWS managed keys (aws/glue, aws/s3) to simplify permission management
- Create a dedicated KMS key for Glue and configure its key policy appropriately
- Enable Data Catalog encryption for metadata that is sensitive

MAY:
- Set up KMS key rotation
- Apply separate KMS keys for different data classification levels
- Turn on CloudTrail logging to monitor KMS key usage

## Common Issues

- symptoms: "Job fails with 'AccessDeniedException' on KMS Decrypt"
  diagnosis: "The Glue IAM role is not included as a key user in the KMS key policy."
  resolution: "Add the Glue role ARN to the KMS key policy with kms:Decrypt, kms:DescribeKey, and kms:GenerateDataKey permissions."

- symptoms: "Job fails writing to SSE-KMS encrypted S3 bucket"
  diagnosis: "The Glue role lacks the kms:GenerateDataKey permission for the bucket's KMS key."
...
- symptoms: "Data Catalog encryption error when creating tables"
  diagnosis: "Data Catalog encryption is enabled but the crawler role does not have the necessary KMS permissions."
  resolution: "Add KMS permissions to the crawler role. Verify the KMS key policy grants access to the crawler role."
