...
description: "Troubleshoot Glue JDBC connection test failures and job connection errors"
status: active
severity: HIGH
triggers:
...
SHOULD:
- Validate database connectivity from an EC2 instance in the same subnet
- Confirm that database credentials are current and have not expired
- Determine whether the database requires SSL/TLS and that the connection is configured accordingly
- Confirm DNS resolution for the database hostname

MAY:
- Review VPC flow logs for rejected traffic
- Confirm the database is accepting new connections (not at the max connections limit)
- Check whether the RDS/Aurora security group permits traffic from the Glue security group

## Phase 2 — Remediate

MUST:
- Correct the JDBC URL, credentials, or security group rules as needed
- Confirm the Glue security group includes a self-referencing inbound rule (all TCP from itself)
- Add an inbound rule to the database security group that allows the Glue security group

SHOULD:
- Adopt Secrets Manager for credential storage rather than plaintext in connection properties
- Enable SSL/TLS to encrypt database connections
- Record the required networking configuration for team reference

MAY:
- Provision VPC endpoints to eliminate the NAT gateway dependency
- Add connection pooling to the Glue script to improve throughput

## Common Issues

- symptoms: "Connection test fails with timeout"
  diagnosis: "There is no network path between the Glue ENI subnet and the database, or the security group is blocking traffic."
...
- symptoms: "Connection test fails with 'Authentication failed'"
  diagnosis: "The username or password is incorrect, or the database user is missing required privileges."
...
- symptoms: "Connection works in test but fails in job"
  diagnosis: "The job references a different connection name, or the number of concurrent connections is exceeding the database limit."
