...
description: "Determine why a Glue crawler is failing to run or complete successfully"
status: active
severity: HIGH
triggers:
  - "crawler failed"
...
SHOULD:
- Confirm that S3 paths in the crawler targets are reachable
- Investigate JDBC connection problems when crawling a database
- Review the total S3 object count in the target path (extremely high counts slow crawlers significantly)

MAY:
- Review Glue service quotas for crawler limits
- Confirm no other crawler is already running against the same targets

## Phase 2 — Remediate

MUST:
- Correct IAM permissions or S3 access problems
- Resolve any JDBC connection issues where applicable
- Confirm the crawler completes successfully after applying the fix

SHOULD:
- Limit crawler targets to specific prefixes to shorten crawl time
- Schedule the crawler to run outside peak data ingestion windows
- Configure the recrawl policy so only new folders are processed

MAY:
- Distribute large crawl targets across multiple separate crawlers
- Leverage S3 event notifications with Lambda to initiate focused crawls
...
- symptoms: "Crawler runs for hours and times out"
  diagnosis: "Too many S3 objects or a deeply nested folder structure are slowing the crawl."
...
- symptoms: "Crawler fails with JDBC connection error"
  diagnosis: "The database is unreachable due to a VPC or security group misconfiguration, or the credentials have expired."
