...
description: "Troubleshoot VPC and subnet configuration problems affecting Glue jobs and connections"
status: active
severity: HIGH
triggers:
  - "VPC configuration"
...
SHOULD:
- Confirm a NAT gateway is present in the route table when services route over the internet
- Confirm VPC endpoints exist for S3 and Glue: `aws ec2 describe-vpc-endpoints --filters Name=vpc-id,Values=<vpc-id>`
- Ensure subnet NACLs are not dropping traffic
- Confirm that DNS resolution is active for the VPC

MAY:
- Inspect VPC flow logs for rejected packets
- Validate VPC peering or transit gateway routes when accessing cross-VPC resources
- Look for overlapping CIDR ranges in peered VPCs

## Phase 2 — Remediate

MUST:
- Add a self-referencing inbound rule to the Glue security group (all TCP, source: same security group)
- Confirm the subnet has available IP addresses (Glue requires IPs for ENIs)
- Add routes to the target resources (NAT gateway, VPC peering, or transit gateway)

SHOULD:
- Provision VPC endpoints for S3 (gateway endpoint, free) and Glue (interface endpoint)
- Prefer private subnets with VPC endpoints over NAT gateways to lower cost
- Ensure the subnet resides in an Availability Zone that Glue supports

MAY:
- Provision dedicated subnets for Glue with an adequate IP address range
- Set up VPC flow log monitoring to track Glue ENI traffic

## Common Issues

- symptoms: "ENI creation failed: insufficient IP addresses"
  diagnosis: "The subnet CIDR is too small or too many ENIs are already allocated in it."
...
- symptoms: "Job fails with 'Unable to reach Glue service'"
  diagnosis: "The job is running in a private subnet that has no NAT gateway or Glue VPC endpoint."
...
- symptoms: "Job can reach database but not S3"
  diagnosis: "An S3 VPC endpoint is missing and there is no NAT gateway route for S3 traffic."
