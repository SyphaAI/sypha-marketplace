...
description: "Identify Spark shuffle bottlenecks causing slow Glue job performance"
status: active
severity: MEDIUM
triggers:
  - "shuffle"
...
- Pinpoint which transformations are triggering shuffle (joins, groupBy, distinct, repartition)

SHOULD:
- Inspect Spark UI for shuffle read/write byte counts per stage
- Watch for shuffle spill (memory) and shuffle spill (disk) metrics
- Confirm partition counts before and after shuffle operations
- Evaluate whether broadcast joins could substitute for shuffle joins when tables are small

MAY:
- Capture Spark event logs for an in-depth shuffle analysis
- Monitor network throughput between executors

## Phase 2 — Remediate

MUST:
- Lower shuffle data volume by pushing filters earlier in the pipeline
- Apply broadcast joins for small dimension tables (< 100 MB)
- Tune partition count: too few causes oversized shuffles; too many introduces scheduling overhead

SHOULD:
- Configure spark.sql.shuffle.partitions to an appropriate value (default 200; adjust to data volume)
- Use Glue 3.0+ to take advantage of optimized push-based shuffle
- Pre-sort or pre-partition data in recurring jobs to reduce runtime shuffle
- Substitute coalesce() for repartition() whenever the goal is to reduce partition count

MAY:
- Activate adaptive query execution (AQE) in Glue 3.0+ for automatic shuffle tuning
- Introduce bucketing for tables that are joined frequently
- Use map-side aggregation (reduceByKey instead of groupByKey) where applicable

## Common Issues

- symptoms: "Join operation takes 90% of total job time"
  diagnosis: "A shuffle join between two large tables is occurring without partition alignment."
...
- symptoms: "Shuffle spill to disk causing slow performance"
  diagnosis: "Shuffle data exceeds available executor memory and is spilling to local disk."
...
- symptoms: "Job slow after repartition() call"
  diagnosis: "repartition() forces a complete shuffle of all data across the cluster."
