...
context: "Glue uses DynamicFrames (Glue-native) and DataFrames (Spark-native) for transformations. DynamicFrames manage schema flexibility through resolveChoice() when types are ambiguous. Frequent errors include ApplyMapping with incorrect column names, type casting failures, null handling gaps, nested struct flattening problems, and incompatible operations across the DynamicFrame and DataFrame APIs."
---
...
SHOULD:
- Insert printSchema() before the failing transformation to examine the actual schema
- Check for null values in columns referenced by the transformation
- Confirm column name case sensitivity (DynamicFrame is case-sensitive)
- Look for nested structs or arrays that require flattening

MAY:
- Test the transformation against a small data sample
- Compare DynamicFrame vs DataFrame behavior for the particular operation

## Phase 2 — Remediate

MUST:
- Update the transformation to align with the actual input schema
- Resolve type ambiguities using resolveChoice() prior to running transformations
- Confirm the fix produces the correct output

SHOULD:
- Use apply_mapping() to define explicit column names and type mappings
- Add null handling (coalesce, fillna) before operations that break on null values
- Use Relationalize to flatten nested data structures
- Introduce schema validation between transformation stages

MAY:
- Implement custom error handling with try-except blocks around transformations
- Apply Glue's stageThreshold and totalThreshold settings for error tolerance
- Record transformation metrics for ongoing monitoring

## Common Issues

- symptoms: "ApplyMapping fails with 'column not found'"
  diagnosis: "The column name in the mapping does not match the actual schema due to case sensitivity or a schema change in the source data."
  resolution: "Print the schema before mapping. Use exact column names including case. Update the mapping to reflect schema changes."

- symptoms: "resolveChoice fails with 'no choice to resolve'"
  diagnosis: "The column does not contain ambiguous types, making resolveChoice unnecessary for that column."
  resolution: "Apply resolveChoice only to columns with ChoiceType. Inspect the schema to confirm which columns actually have choice types."

- symptoms: "Type cast error converting string to int"
  diagnosis: "The source data includes non-numeric values in a column that is being cast to integer."
  resolution: "Use resolveChoice with cast:int to manage the type conversion. Filter or clean invalid values before casting."
