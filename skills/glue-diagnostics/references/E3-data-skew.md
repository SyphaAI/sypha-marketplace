---
title: "E3 — Data Skew"
description: "Identify data skew causing uneven task distribution and straggler tasks"
status: active
severity: MEDIUM
triggers:
  - "data skew"
...
- Isolate the skewed key by analyzing data distribution
- Check CloudWatch metrics for uneven executor utilization
- Examine the transformation responsible for the skew (join key, groupBy key, partition key)

SHOULD:
- Sample the data to surface hot keys by counting records per key value
- Look for null values in join keys (nulls all route to the same partition)
- Assess partition count and size distribution
- Determine whether the skew originates in the source data or is introduced by a transformation

MAY:
- Run a dedicated analysis job to profile key distribution
- Review historical data to determine whether the skew pattern has worsened over time

## Phase 2 — Remediate

MUST:
- Tackle the root skew pattern (null keys, hot keys, or uneven partition distribution)
- Confirm the fix reduces variance in task duration

SHOULD:
- Apply salting to skewed join keys: attach a random prefix to the hot key and replicate the other table to match
- Exclude null values before joins and process them as a separate path
- Activate Spark AQE skew join optimization (Glue 3.0+): `--conf spark.sql.adaptive.skewJoin.enabled=true`
- Re-partition data using a key with a more even distribution

MAY:
- Write custom partitioning logic tailored to well-understood skew patterns
- Pre-aggregate data before joins to decrease skew at join time
- Use two-phase aggregation: a partial per-partition aggregate followed by a final aggregate on the original key

## Common Issues

- symptoms: "One task takes 10x longer than others in a join stage"
  diagnosis: "The join key contains a hot value (e.g., 'unknown', null, or a highly frequent category) with millions of associated records."
...
- symptoms: "GroupBy operation has straggler tasks"
  diagnosis: "The groupBy key has low cardinality and an uneven value distribution."
...
- symptoms: "Null values causing skew in join"
  diagnosis: "Null join keys all hash to the same partition, producing an extremely large skewed partition."
