...
description: >
  Use this skill to investigate and troubleshoot AWS Glue problems by examining
  ETL jobs, crawlers, connections, Data Catalog, DPU utilization, Spark
  execution, and job bookmarks through structured runbooks. Activate when: job
  ...
  generation failures, or the user indicates something is wrong with Glue without
  specifying particular symptoms.
compatibility: >
...
Any AWS Glue investigation where the console alone is not sufficient — job failures, OOM errors, Spark crashes, crawler schema misdetection, connection timeouts, Data Catalog drift, DPU under/over-provisioning, data skew, bookmark corruption, or Glue Studio generation errors.
...
Read `references/glue-guardrails.md` before drawing conclusions on any Glue issue.
...
2. Never assume OOM is always executor-side. Verify whether the error originates on the driver or executor — the remediation differs.
