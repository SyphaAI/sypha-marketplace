...
These patterns apply to apps deployed **outside** Databricks Apps (e.g., on Vercel, AWS, or standalone Node.js servers) that integrate directly with Databricks AI Gateway via AI SDK v6. For AppKit-based apps, use the built-in serving plugin provided by the **`databricks-apps`** skill instead.

## AI SDK v6 Streaming Pattern

Apply this pattern to stream AI chat responses through Databricks AI Gateway with Vercel AI SDK v6 in off-platform apps.

**Dependencies:** `ai@6`, `@ai-sdk/react@3`, `@ai-sdk/openai`, `@databricks/sdk-experimental`

**Auth helper** — supports both local development (CLI profile) and deployed apps (service principal token):
...
- `DATABRICKS_WORKSPACE_ID` — to retrieve it explicitly: `databricks api get /api/2.1/unity-catalog/current-metastore-assignment --profile <PROFILE>` → read the `workspace_id` field
- `DATABRICKS_ENDPOINT` — the model endpoint name (e.g. `databricks-meta-llama-3-3-70b-instruct`). Run `databricks serving-endpoints list --profile <PROFILE>` to view available models.
...
Notable differences from AI SDK v5: call `sendMessage({ text })` (NOT `append`), render the `m.parts` array (NOT `m.content`), and the `status` values are `ready`, `submitted`, and `streaming`.

## Embeddings Pattern

Generate text embeddings by calling a Databricks AI Gateway endpoint.
...
Commonly used embedding endpoints: `databricks-gte-large-en` (1024d) and `databricks-bge-large-en` (1024d). Configure `DATABRICKS_EMBEDDING_ENDPOINT` in both `.env` and `app.yaml`.

To perform vector similarity search with these embeddings, refer to the **`databricks-lakebase`** skill.
