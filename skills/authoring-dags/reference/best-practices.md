...
For compatibility, the examples that follow use Airflow 2 imports. These continue to work on Airflow 3 but are deprecated there (AIR31x warnings). Prefer `airflow.sdk` imports when starting new Airflow 3 projects.
...
DAG files get re-parsed roughly every 30 seconds, and anything outside a task executes on each parse.
...
**Airflow 3 context injection**: With the Airflow 3 Task SDK, context variables are injected automatically into function parameters matched by name. A plain type annotation is enough — a `= None` default is not needed:
...
Handle a variable number of items in parallel rather than looping:
...
When data is large, use the **claim-check pattern**: persist it to external storage (S3, GCS, ADLS) and hand a URI/path reference through XCom.
...
**Airflow 3 XCom serialization**: The Task SDK in Airflow 3 can serialize common Python types, DataFrames included, out of the box. In Airflow 2, non-primitive types needed a custom XCom backend or manual serialization.

To offload automatically, use the Object Storage XCom backend (provider `common-io`).
```bash
...
Use `deferrable=True` whenever it is supported. If not, choose `mode='reschedule'` for any wait beyond a few minutes, and keep `mode='poke'` (the default) strictly for checks under a minute.
...
Scheduling DAGs off each other's data:
...
**Outlets without inlets are valid.** A task may declare `outlets` even when no other DAG yet consumes that asset as an inlet/schedule trigger. Assets that exist only as outlets are encouraged for lineage tracking.
