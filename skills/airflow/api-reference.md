...
Call Airflow REST API endpoints directly when the high-level commands don't cover them.
...
|------|----------|----------|
| `-F key=value` | Converts automatically: `true`/`false` → bool, numbers → int/float, `null` → null | The typical case |
| `-f key=value` | Leaves the value as a raw string | Values that resemble numbers but must stay strings |
| `--body '{}'` | Raw JSON body | Nested objects with structure |
| `-F key=@file` | Loads the value from a file | Large values, configs |
...
| Task | Use |
|------|-----|
| Listing/fetching DAGs, runs, tasks | `af dags`, `af runs`, `af tasks` |
| Triggering runs and watching them | `af runs trigger-wait` |
| Deleting or clearing runs | `af runs delete`, `af runs clear` |
| Diagnosing failures | `af runs diagnose` |
| XCom, event logs, backfills | `af api` |
| Creating/updating variables, connections | `af api` |
| Endpoints the high-level CLI lacks | `af api` |
