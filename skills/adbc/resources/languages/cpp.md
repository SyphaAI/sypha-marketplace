...
Conda is the only source of pre-compiled ADBC packages. Use these conda packages:
...
The ADBC packages ship CMake targets, so a CMakeLists.txt along these lines will work:
...
As an alternative, when using pkg-config, pass `-larrow -lparquet -ladbc_driver_manager`.
...
Before calling `AdbcDatabaseInit`, assign a `profile://` URI to the `"uri"` or `"driver"` option. The driver manager loads the profile and applies its options on its own — when using `profile://`, do not set a `"driver"` option as well:
...
Another option is to supply a profile name through the `"profile"` option (leaving off the `profile://` prefix):
...
If needed, more detailed documentation is available at https://arrow.apache.org/adbc/current/cpp/index.html.
