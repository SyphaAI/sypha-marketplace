...
Be sure that the latest version of each of these packages is used.
...
Important: github.com/apache/arrow/go and github.com/apache/arrow-go must not be mixed together.
...
Because the Go `drivermgr` package wraps the C++ driver manager, it inherits its support for connection profiles. Supply a `profile://` URI as the value of the `"uri"` option — when the profile names the driver, no `"driver"` key is required.
...
Alternatively, through the `"driver"` key:
...
If needed, more detailed documentation is available at https://pkg.go.dev/github.com/apache/arrow-adbc/go/adbc.
