...
This file provides detailed patterns, checklists, and code samples that the skill references.
...
### Do's
- **Test early** - Validate source data prior to any transformations
- **Test incrementally** - Introduce tests as new issues are discovered
- **Document expectations** - Write clear descriptions for every test
- **Alert on failures** - Wire checks into your monitoring system
- **Version contracts** - Maintain a history of schema changes

### Don'ts
- **Don't test everything** - Concentrate effort on critical columns
- **Don't ignore warnings** - Warnings frequently signal upcoming failures
- **Don't skip freshness** - Stale data undermines analytical accuracy
- **Don't hardcode thresholds** - Rely on dynamic baselines instead
- **Don't test in isolation** - Validate cross-table relationships as well
