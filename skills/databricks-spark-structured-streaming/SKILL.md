...
description: >-
  End-to-end reference for Spark Structured Streaming in production.
  Use when creating streaming pipelines, ingesting from Kafka,
  enabling Real-Time Mode (RTM), setting triggers (processingTime,
  availableNow), managing stateful operations with watermarks, tuning
  checkpoints, running stream-stream or stream-static joins, writing to
  multiple sinks, or optimizing streaming cost and performance.
...
Build production-ready streaming pipelines with Spark Structured Streaming. This skill provides navigation to detailed patterns and best practices.
...
- [ ] Checkpoint location is durable (UC volumes, not DBFS)
- [ ] Each stream has a unique checkpoint path
- [ ] Cluster is fixed-size (autoscaling disabled for streaming)
- [ ] Monitoring in place (input rate, lag, batch duration)
- [ ] Exactly-once delivery verified (txnVersion/txnAppId)
- [ ] Watermark set for stateful operations
- [ ] Stream-static joins use left join, not inner
