...
description: Set up and manage checkpoint locations for dependable Spark Structured Streaming. Apply when configuring new streaming jobs, diagnosing checkpoint problems, relocating checkpoints, or guaranteeing exactly-once semantics through proper checkpoint storage and organization.
---

# Checkpoint Best Practices

Set up checkpoint locations to support reliable streaming with exactly-once semantics. Checkpoints record progress and provide fault tolerance.
...
- [ ] Checkpoint location uses persistent storage (S3/ADLS, not DBFS)
- [ ] Each stream has its own dedicated checkpoint
- [ ] Checkpoints are organized by target rather than source
- [ ] A backup strategy has been established
- [ ] Monitoring is in place for checkpoint size and access failures
- [ ] State store growth is tracked for stateful workloads
- [ ] Recovery procedure has been documented
- [ ] Migration procedure has been documented
