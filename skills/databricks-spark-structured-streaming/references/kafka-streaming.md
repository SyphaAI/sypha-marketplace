...
description: Complete reference for Kafka streaming patterns covering Kafka-to-Delta ingestion, Kafka-to-Kafka pipelines, and Real-Time Mode for sub-second latency. Apply when building Kafka ingestion pipelines, implementing event enrichment, performing format transformation, or handling low-latency streaming workloads.
---

# Kafka Streaming Patterns

Complete reference for Kafka streaming with Spark Structured Streaming: ingesting into Delta, building Kafka-to-Kafka pipelines, and leveraging Real-Time Mode for sub-second latency.
...
Apply minimal transformations and retain original columns:
...
Execute on a schedule rather than running continuously:
...
Use RTM when latency requirements fall below 800ms (as low as 5ms). Requires DBR 16.4 LTS+:
...
Augment events with data from dimension tables:
...
Distribute events across multiple Kafka topics based on content:
...
Validate incoming records against a schema and redirect invalid ones:
...
- [ ] Each pipeline uses a unique checkpoint location
- [ ] Fixed-size cluster configured (no autoscaling for streaming/RTM)
- [ ] RTM enabled only when latency requirements fall below 800ms
- [ ] Consumer lag is monitored with alerts in place
- [ ] Producer acks=all set for durability
- [ ] Schema validation with DLQ is in place
- [ ] SASL/SSL security configured for production environments
- [ ] Exactly-once semantics confirmed and verified
