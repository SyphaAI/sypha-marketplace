...
description: Choose and tune triggers for Spark Structured Streaming to strike the right balance between latency and cost. Apply when deciding between processingTime, availableNow, and Real-Time Mode (RTM), deriving optimal trigger intervals, reducing costs via cluster right-sizing, scheduled streaming, multi-stream clusters, or navigating latency vs cost trade-offs.
---

# Trigger and Cost Optimization

Choose and tune triggers to meet latency requirements while keeping costs in check. Reduce streaming job costs through trigger adjustments, cluster right-sizing, multi-stream consolidation, storage optimization, and scheduled execution.
...
Execute processing on a fixed schedule:
...
Consume all pending data, then shut down:
...
Achieve sub-second latency using Photon:
...
Find the right equilibrium between latency and cost:
...
Pick the execution pattern that fits your SLA:
...
Size clusters appropriately to the actual workload:
...
Consolidate multiple streams onto a single cluster:
...
Switch from continuous execution to a scheduled approach:
...
Maximize cluster utilization by co-locating streams:
...
Apply RTM when true real-time latency is required:
...
Batch duration must remain below the trigger interval:
...
Label jobs and monitor DBU consumption to attribute costs per stream:
...
Track CPU and memory usage to keep the cluster properly sized:
...
1. **Switch from continuous to a 15-minute schedule** - Delivers meaningful cost savings
2. **Consolidate multiple streams on one cluster** - Improves cluster utilization
3. **Enable auto-optimize** - Lowers storage overhead
4. **Use Spot instances** - Suitable for non-critical streams (use with caution)
5. **Archive old data** - Shift to less expensive storage tiers
...
- [ ] Trigger type chosen to meet latency requirements
- [ ] Trigger interval derived from SLA (SLA / 3)
- [ ] Batch duration is tracked and stays below the trigger interval
- [ ] Cluster is right-sized to 60-80% utilization
- [ ] Multiple streams consolidated per cluster where applicable
- [ ] Scheduled execution is in use when the SLA permits
- [ ] RTM is enabled for any pipeline requiring sub-second latency
- [ ] Auto-optimize has been turned on
- [ ] Storage costs are under active monitoring
- [ ] Cost per stream is tracked over time
