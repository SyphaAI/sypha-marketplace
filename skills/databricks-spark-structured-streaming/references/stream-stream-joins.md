...
Correlate events that arrive at different times and rates by joining two streaming sources in real-time. Stream-stream joins depend on watermarks to manage state and handle data that arrives late.
...
Stream-stream joins are stateful: both sides must hold events in a buffer until matches are found or state expires. Watermarks determine when state can be safely discarded.
...
Route late-arriving events to a dedicated table:
...
When the state store exceeds available memory, switch to RocksDB:
...
Strike a balance between latency and completeness:
...
When the streams being joined have different latencies:
...
Chain joins with care — each additional join increases state overhead:
...
Always rely on event time for stream-stream joins:
...
Select the appropriate state store backend for your workload:
...
Always specify explicit time bounds in join conditions:
