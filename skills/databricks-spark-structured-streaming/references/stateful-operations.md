...
description: Set up watermarks and manage state stores for Spark Structured Streaming stateful operations. Use when configuring stateful operations, adjusting watermark duration, dealing with late-arriving data, enabling RocksDB for large state, tracking state store size, or tuning state performance.
---

# Stateful Operations: Watermarks and State Stores

Use watermarks to accommodate late-arriving data and govern state stores for stateful streaming operations. Watermarks drive state cleanup, while state stores are responsible for storing and retrieving stateful data.
...
**Rule of thumb**: Begin at 2-3× your p95 latency. Track the late data rate and tune accordingly.
...
Switch to RocksDB when state stores grow beyond available memory capacity:
...
Assign separate watermarks to streams that have different latency characteristics:
...
Inspect the state store for partition skew:
...
Watermarks handle the automatic removal of expired state entries:
...
- [ ] Watermark set for every stateful operation
- [ ] Watermark duration aligned with latency requirements (2-3× p95)
- [ ] RocksDB activated for large state stores
- [ ] State size is being monitored with alerts in place
- [ ] State partition balance reviewed on a regular basis
- [ ] State growth measured and trended over time
- [ ] Late data monitoring is in place
- [ ] Recovery procedure written and accessible
