...
description: Direct a single Spark stream to multiple Delta tables or Kafka topics via ForEachBatch. Use when fanning out streaming data to multiple sinks, implementing medallion architecture (bronze/silver/gold), conditional routing, CDC patterns, or building materialized views from a single stream.
---

# Multi-Sink Writes

Direct a single streaming source to multiple Delta tables or Kafka topics efficiently using ForEachBatch. Read the source once, write to many destinations — eliminating redundant reprocessing of the same source data.
...
Maintain a single checkpoint for the complete multi-sink stream:
...
One stream populating all three medallion layers simultaneously:
...
Dispatch events to distinct tables based on their type:
...
Distribute writes across multiple independent sinks concurrently:
...
Derive multiple independent views from the same stream:
...
Redirect invalid records to a DLQ:
...
Cache the batch DataFrame to prevent redundant recomputation:
...
Apply ThreadPoolExecutor when writing to independent destinations:
...
Always pair txnVersion with batch_id:
