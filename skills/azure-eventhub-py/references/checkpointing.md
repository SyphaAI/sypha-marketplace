...
Strategies for dependable event processing using checkpoint stores.

## Why Checkpointing?

Checkpointing records which events have already been processed, making the following possible:
- **Resume after failure** — Continue from the last recorded position
- **Scalable consumers** — Multiple consumers divide work without overlap
- **At-least-once delivery** — Guarantee that no events are dropped
...
Blob checkpoint stores persist data in the following format:
...
1. **Use a dedicated container** — Keep checkpoint data isolated from application data
2. **Align with consumer groups** — Each consumer group requires its own set of checkpoints
3. **Strike the right frequency** — Too often creates overhead; too infrequently causes excess reprocessing on failure
4. **Account for idempotency** — Architect for at-least-once delivery, since events may be replayed
5. **Track checkpoint lag** — Measure the elapsed time between event enqueue and checkpoint completion
6. **Validate failure recovery** — Confirm that resume behavior is correct after crashes
7. **Prefer the async store** — The async BlobCheckpointStore delivers better performance
8. **Only checkpoint on success** — Never checkpoint events that failed to process
