...
Techniques for handling partitions, load balancing, and event ordering.

## Understanding Partitions

Partitions make the following possible:
- **Parallel processing** — Different consumers handle distinct partitions concurrently
- **Ordered delivery** — Events within a single partition are delivered in sequence
- **Scalability** — Adding more partitions increases overall throughput
...
When multiple consumers within the same consumer group share a checkpoint store, partitions are distributed among them automatically.
...
1. **Apply partition keys** for correlated events that require a defined order
2. **Avoid hard-coding partition IDs** unless there is a deliberate reason to do so
3. **Align consumer count with partitions** — target a maximum of 1 consumer per partition
4. **Watch for lag** — track the gap between enqueued and processed event sequence numbers
5. **Anticipate ownership changes** — build the system to handle partitions shifting between consumers
6. **Use checkpoint stores** to enable automatic load balancing
7. **Resume from checkpoint** — let the checkpoint store determine the starting position
8. **Partition count is immutable** — size capacity correctly before creation, since it cannot be changed afterward
