...
**CRITICAL:** Managed Identity REQUIRES the `accountKind` property. Accepted values: `StorageV2`, `BlobStorage`, `BlockBlobStorage`
...
**Required Permission:**
The ADF managed identity must hold the `Get` permission on secrets.
...
**Azure ML SDK v1 support ends June 2026.** Move any existing `AzureMLExecutePipeline` usage over to batch endpoints via WebActivity. Migration patterns are covered in the skill `adf-master:adf-ml-analytics`.
