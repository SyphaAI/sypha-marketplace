...
The Copy Activity transfers data from one supported data store to another.
...
Loops through a collection, running the contained activities for every item.
...
- Access the item for the current iteration with `@item()`

**Limitations:**
- A ForEach cannot be nested inside another ForEach
- An Until cannot be nested inside a ForEach
- Max 100,000 items
...
Branches conditionally based on the result of evaluating an expression.
...
**Limitations:**
- ForEach, If, Switch, Until, and Validation activities cannot be nested inside the branches
- For complex nested logic, use Execute Pipeline instead

---

## Switch Activity

Branches into multiple paths depending on the value of an expression.
...
Repeats until the condition becomes true or the timeout expires.
...
Fetches data from a source so it can be referenced in expressions.
...
Invokes another pipeline, which allows for modular design.
...
Invokes REST endpoints for integration purposes.
...
Coordinates the execution of Databricks Workflow Jobs.
...
Assigns values to pipeline variables.
...
**Note:** SetVariable is not usable inside a parallel ForEach. Rely on AppendVariable or switch to sequential mode.

---

## Append Variable Activity

Adds a value to the end of an array variable.
...
Halts execution for a defined amount of time.
...
Deliberately fails the pipeline and raises a custom error.
...
Returns metadata for datasets or files.
...
Runs a published Azure Machine Learning pipeline. **SDK v1 support ends June 2026.** Move to batch endpoints via WebActivity.
...
Executes a Mapping Data Flow to perform Spark-based transformations.
