...
Executes pipelines on a repeating schedule.
...
Handles data in fixed-size time windows, with retry and dependency support built in.
...
**Key Properties:**
- `delay`: How long to wait after the window closes before starting (accommodates late-arriving data)
- `maxConcurrency`: How many windows run in parallel (1-50)
- The window times can be read via `@trigger().outputs.windowStartTime/windowEndTime`
...
Time zone values that are supported:
- `UTC`
...
| State | Description |
|-------|-------------|
| Started | The trigger is active and firing |
| Stopped | The trigger is disabled |
...
- Schedule trigger: At most 5 executions per minute
- Tumbling window: maxConcurrency 1-50
- Event trigger: Subject limited to 1024 characters
- Blob trigger: Does not fire when a folder is created
- Rerun trigger: Must stay within the original trigger's window range
