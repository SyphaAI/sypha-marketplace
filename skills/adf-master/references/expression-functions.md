...
ADF expressions start with the `@` prefix and may be used in:
- Parameter values
...
For string interpolation, embed `@{expression}` inside strings
```json
...
### substring
Pulls out a portion of a string.
```text
...
### replace
Substitutes occurrences within a string.
```text
...
### coalesce
Yields the first value that is not null.
```text
...
### createArray
Builds an array out of multiple values.
```text
...
### rand
A random number (32-bit signed integer).
```text
...
### ticks
Converts a value to ticks (100-nanosecond intervals since 1/1/0001).
```text
