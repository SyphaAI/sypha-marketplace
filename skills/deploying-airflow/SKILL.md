...
description: >-
  Deploy Airflow DAGs and projects. Invoke when the user wants to deploy code, push
...
This skill addresses deploying Airflow DAGs and projects to production, whether using Astro (Astronomer's managed platform) or open-source Airflow on Docker Compose or Kubernetes.

**Choosing a path:** Astro suits managed operations and faster CI/CD. For open-source, use Docker Compose for development and the Helm chart for production.
...
Use this when `requirements.txt`, `Dockerfile`, `packages.txt`, plugins, or any non-DAG file has been changed.
...
This is considerably faster than a full deploy because it skips the image build. Use this when only DAG files have changed and dependencies or configuration remain unmodified.
...
This is suited to multi-repo setups where DAGs are deployed separately from the image, or to CI/CD pipelines that manage image and DAG deploys as independent processes.
...
When multiple deploys are triggered in rapid succession, Astro processes them sequentially in a deploy queue. Each deploy must complete before the next one begins.
...
Deploy Airflow using the official Docker Compose setup. This approach is recommended for learning and exploration — for production workloads, use Kubernetes with the Helm chart (see below).
...
Add packages to `requirements.txt` and then rebuild:
...
1. **Git-sync** (recommended): DAGs are automatically synchronized from a Git repository
2. **Persistent Volume**: Mount a shared PV that contains the DAGs
3. **Baked into image**: Embed DAGs directly in a custom Docker image
