...
name: azure-storage
description: >-
  Covers Azure Storage Services including Blob Storage, File Shares, Queue
  Storage, Table Storage, and Data Lake. Addresses questions about storage
  access tiers (hot, cool, cold, archive), when to use each tier, and how they
  compare. Supports object storage, SMB file shares, async messaging, NoSQL
  key-value, and big data analytics. Covers lifecycle management. USE FOR: blob
  storage, file shares, queue storage, table storage, data lake, upload files,
  download blobs, storage accounts, access tiers, storage tiers, hot cool cold
  archive, storage tier comparison, when to use storage tiers, lifecycle
  management, Azure Storage concepts. DO NOT USE FOR: SQL databases, Cosmos DB
  (use azure-prepare), messaging with Event Hubs or Service Bus (use
  azure-messaging).
metadata:
  author: Microsoft
  version: 1.1.2
...
**If Azure MCP is not enabled:** Run `/azure:setup` or activate it via `/mcp`.
...
For in-depth documentation on specific services:
...
To build applications using Azure Storage SDKs, consult the condensed guides:
...
For a complete package listing across all languages, see [SDK Usage Guide](references/sdk-usage.md).

## Azure SDKs

To build applications that interact with Azure Storage programmatically, Azure offers SDK packages in multiple languages (.NET, Java, JavaScript, Python, Go, Rust). See [SDK Usage Guide](references/sdk-usage.md) for package names, installation commands, and quick start examples.
