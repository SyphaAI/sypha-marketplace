...
All examples authenticate using `DefaultAzureCredential`, which is suitable for **local development only**. For production workloads, use `ManagedIdentityCredential` instead — see [auth-best-practices.md](auth-best-practices.md). Rust uses `DeveloperToolsCredential` because it has no `DefaultAzureCredential` equivalent.
