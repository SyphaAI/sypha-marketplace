---
name: dd-logs
description: 'Log management - searching, archiving, metrics generation, and cost control.'
metadata:
  version: 1.0.1
  author: datadog-labs
...
Search, process, and archive logs with an eye on cost.

## Prerequisites

Datadog Pup must already be installed. See [Setup Pup](https://github.com/datadog-labs/agent-skills/tree/main?tab=readme-ov-file#setup-pup) if it is not.

## Command Execution Order (Token-Efficient)

For scoped commands, follow this order:

1. Check context first (prior outputs, conversation history, saved values).
2. If a required value is absent, run a discovery command first.
3. If still ambiguous, ask the user to confirm.
4. Then execute the target command.
5. Avoid speculative commands that are likely to fail.
...
Retain logs at low cost for compliance purposes:
...
Generate metrics from logs (more economical than indexing):
