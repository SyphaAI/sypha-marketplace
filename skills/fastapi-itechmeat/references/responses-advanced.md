...
Crafting custom responses with specific status codes, cookies, and headers in FastAPI.
...
- JSON Lines differs from a JSON array; clients need to parse the response one line at a time.
- When the return type annotation is omitted, FastAPI falls back to `jsonable_encoder` for serialization.

### Stream bytes / binary data

For raw binary content such as audio, video, or arbitrary chunks, use `StreamingResponse` and specify `media_type`. When the streaming logic involves blocking operations (disk or network reads), use a synchronous `def` function so FastAPI can offload it to a thread pool.
...
SSE relies on the `text/event-stream` content type and is natively supported by browsers through the `EventSource` API. In FastAPI, configure `response_class=EventSourceResponse` and `yield` event objects.
...
- `ServerSentEvent.data` is JSON-encoded automatically; use `raw_data` when you need to send a raw text payload.
- Clients can reconnect after a disconnect by sending the `Last-Event-ID` header.
- When operating behind a proxy, configure keep-alive pings, set `Cache-Control: no-cache`, and disable buffering (for example, `X-Accel-Buffering: no` in Nginx).
- From `0.136.2+`, SSE field validation is stricter. Build events via `ServerSentEvent(...)` rather than assembling raw payload dicts by hand, and verify any optional `event`, `id`, or `retry` fields your code generates dynamically.
...
Note: Prefix custom headers with `X-`. To make custom headers accessible to browsers via CORS, list them in `CORSMiddleware(expose_headers=[...])`.
