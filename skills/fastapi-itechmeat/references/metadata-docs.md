...
Customizing OpenAPI metadata and the documentation UI.
...
Tags appear in the docs in the same order as the list, not alphabetically.
...
Mounting attaches a fully self-contained application at a designated path. Mounted apps are:

- Entirely self-contained
- Excluded from the main app's OpenAPI schema
- Responsible for handling all of their own sub-paths
...
- Executes the block only when the file is run directly: `python myapp.py`
- Does NOT execute when the module is imported: `from myapp import app`
