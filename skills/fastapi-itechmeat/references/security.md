...
Authentication and authorization approaches available in FastAPI.
...
The example below illustrates a minimal OAuth2 password flow:
...
Extract and validate the current user from the token using a dependency:
...
Important: Always use `secrets.compare_digest()` for credential comparison — a plain `==` check is susceptible to timing attacks.

## OAuth2 Scopes

Scopes enable fine-grained permission control:
