...
Working with callbacks, webhooks, custom operation IDs, and SDK generation.

## OpenAPI Callbacks

Use this to document external API callbacks that your API will invoke:
...
In the callback path expression, `{$callback_url}` is resolved from the query parameter and `{$request.body.id}` is resolved from the request body.

## OpenAPI Webhooks

Use this to document webhooks that your API emits (OpenAPI 3.1):
...
Appears in `/docs` under the Webhooks section.

## Custom Operation IDs

Generate clean method names suitable for SDK generation:
...
Attach arbitrary additional OpenAPI fields:
...
### Preprocess OpenAPI for Cleaner Operation Names
...
Self-contained applications each with their own OpenAPI schema:
