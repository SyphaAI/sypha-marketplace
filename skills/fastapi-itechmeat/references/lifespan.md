...
Managing startup and shutdown logic in FastAPI applications.
...
- The lifespan context manager is the current recommended approach
- `yield` acts as the boundary between startup and shutdown logic
- Store shared resources on `app.state` for access across handlers
- Running cleanup on shutdown prevents resource leaks
- `TestClient` triggers lifespan events automatically
