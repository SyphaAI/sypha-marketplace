...
How to handle form data and file uploads in FastAPI.
...
- ❌ You cannot combine `Body` (JSON) with `Form`/`File` in the same endpoint
- ❌ Avoid using `bytes` for large files due to memory consumption
