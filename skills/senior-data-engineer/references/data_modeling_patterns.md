...
A thorough reference for data modeling in analytics and data warehousing contexts.
...
The most prevalent pattern for analytical data models. A single fact table is surrounded by dimension tables.
...
Dimensions are normalized to reduce storage consumption and eliminate update anomalies.
...
A fully denormalized single table — increasingly popular with modern columnar warehouses.
...
Updates are not permitted — the original value is retained indefinitely.
...
The old value is replaced in place with the new one. No historical record is kept.
...
A new row is inserted with the updated values. The complete history is maintained.
...
A column is added to store the prior value. History is limited, typically to only the immediately preceding value.
...
Attributes that change frequently are extracted into a separate mini-dimension table.
...
Merges Types 1, 2, and 3 to achieve maximum modeling flexibility.
...
Data Vault delivers:
- Complete historization of all data
...
Stores only business keys and surrogate keys.
...
Holds descriptive attributes along with their complete history.
...
Captures relationships between hub entities.
