...
A thorough reference for designing and implementing production-ready data pipelines.
...
The Lambda architecture unifies batch and stream processing to enable end-to-end data handling.
...
1. **Batch Layer**
   - Processes the full historical dataset
   - Produces precomputed batch views
   - Handles complex transformations and ML training workloads
   - Can be reprocessed from raw data at any time

2. **Speed Layer**
   - Ingests and processes the real-time data stream
   - Generates real-time views covering recent data
   - Operates at low latency with simpler transformations
   - Fills the gap left by the batch layer's processing delay

3. **Serving Layer**
   - Combines batch and real-time views
   - Fulfills incoming queries
   - Exposes a unified interface to consumers
...
Kappa streamlines the Lambda approach by relying solely on stream processing combined with replay capability.
...
1. **Single Processing Path**: Every dataset is handled as a stream
2. **Immutable Log**: Kafka/Kinesis serves as the authoritative source of truth with extended retention
3. **Reprocessing via Replay**: Rerun the stream processor from the start whenever reprocessing is required
...
Widely used in data lakehouse platforms such as Databricks and Delta Lake.
