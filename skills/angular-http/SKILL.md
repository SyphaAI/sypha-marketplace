...
name: angular-http
description: >-
  Retrieve HTTP data in Angular v20+ via resource(), httpResource(), and
  HttpClient. Covers API calls, signal-driven data loading, request/response
  handling, and interceptors. Apply for data fetching, API integration, loading
  states, error handling, or when converting Observable-based HTTP code to
...
Load data in Angular through the signal-based `resource()` and `httpResource()` APIs, as well as the traditional `HttpClient`.

## httpResource() - Signal-Based HTTP

`httpResource()` wraps HttpClient and layers signal-based state management on top:
...
Prefer this for async operations outside of HTTP or whenever custom fetch logic is required:
...
Turn to this in complex scenarios or when Observable operators are required:
