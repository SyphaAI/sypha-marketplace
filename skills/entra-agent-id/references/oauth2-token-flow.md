...
Agent Identities perform authentication at runtime via credentials set on the **Blueprint** (not on the Agent Identity — Agent Identities are unable to hold credentials).
...
Both options supply the two-step `fmi_path` exchange described in [runtime-token-exchange.md](runtime-token-exchange.md).
...
Use the typed path — FICs belong on the Blueprint, not on the Agent Identity SP:
...
- **FICs belong on the Blueprint** via the typed path (`.../microsoft.graph.agentIdentityBlueprint/federatedIdentityCredentials`).
...
- **Save `secretText` right away** — it cannot be retrieved later.
- **Secrets are valid only on the Blueprint** — Agent Identities do not support password credentials (`PropertyNotCompatibleWithAgentIdentity`).
- **Not suitable for production** — use MI + WIF instead.
- **Observe the org credential-lifetime policy** when setting `endDateTime`.
- Avoid using `DefaultAzureCredential` to obtain Blueprint tokens — Azure CLI tokens include `Directory.AccessAsUser.All` and will be rejected. Use `ClientSecretCredential` or the raw HTTP exchange.
