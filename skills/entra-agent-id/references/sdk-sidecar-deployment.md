...
Companion reference for [sdk-sidecar.md](sdk-sidecar.md). This file documents calling-side code patterns in Python and TypeScript, container deployment manifests, security hardening guidance, and troubleshooting steps.
...
> ⚠️ The SDK API must NOT be publicly reachable. Restrict access to pod-local or same-Docker-network only.

1. **Bind to localhost** — `Kestrel__Endpoints__Http__Url=http://127.0.0.1:5000`
2. **Never expose via LoadBalancer or Ingress**
3. **Use Workload Identity in AKS** — prefer `SignedAssertionFilePath` over client secrets
4. **Use Key Vault for certificates** — set `SourceType=KeyVault` in production
5. **Keep ConfigMap and Secrets separate** in Kubernetes
