...
description: >-
  JSON processing with jq: querying, filtering, and transforming JSON. Use for
  parsing JSON files, filtering arrays/objects, reshaping structures, or
  extracting fields out of JSON.
...
Specialized knowledge for querying, processing, and transforming JSON data with jq, the flexible and lightweight command-line JSON processor.

## When to Use This Skill

| Use this skill when... | Use yq-yaml-processing instead when... |
|---|---|
| Filtering or extracting fields from JSON input | Handling YAML files (Kubernetes, GH Actions, Helm) |
| Processing responses from `gh`, `curl`, or `kubectl -o json` | Editing YAML in place with comments preserved |
| CI pipelines running on minimal images (jq is installed everywhere) | Converting YAML → JSON ahead of processing |

| Use this skill when... | Use nushell-data-processing instead when... |
|---|---|
| One-off pipeline transforms that a single expression can express | Transforms with multiple steps spanning JSON, YAML, CSV, and TOML |
| JSON piped via stdin from another command | Aggregations, group-by, or exploring data in visual tables |

## Core Expertise

**JSON Operations**
- Use path expressions to query and filter JSON
- Reshape and restructure JSON
- Merge, combine, and split JSON data
- Check JSON syntax and structure for validity

**Data Extraction**
- Pull specific fields out of JSON objects
- Apply conditions to filter arrays
- Traverse nested JSON structures
- Deal with null values and absent keys
...
**Query Construction**
- Begin with a simple query and add complexity step by step
- Try filters against small datasets first
- In scripts, use the `-c` flag for compact output
- Use the `-r` flag to get raw strings (no quotes)

**Performance**
- Apply `select()` early in the pipeline to cut down the data
- For large files, use targeted queries with specific paths
- Use the `--stream` flag to stream large JSON
- For faster processing, consider `jq -c`

**Error Handling**
- For optional access, use the `?` operator: `.field?`
- Filter out nulls/errors with `// empty`
- Handle errors gracefully with `try-catch`
- Verify a field exists with `has("field")`

**Readability**
- Split complex queries into multiple steps
- For clarity, use variables via `as $var`
- Include comments in shell scripts
- Lay out multi-line jq programs so they read well
...
- `|` - Pipe (chains operations)
- `,` - Emit multiple outputs
- `?` - Optional (suppresses errors)
- `//` - Alternative operator (supplies a default value)

### Functions
- `keys`, `values` - Keys/values of an object
- `length` - Length of array/object/string
- `map()`, `select()` - Operations on arrays
- `sort`, `sort_by()` - Sort data
- `group_by()` - Group data
- `unique` - Drop duplicates
- `add` - Sum numbers or concatenate
- `has()` - Test whether a field exists
- `type` - Return a value's type

### Type Conversions
- `tostring`, `tonumber` - Type conversion
- `@csv`, `@tsv`, `@json` - Output formatting
