...
To support two-way binding via the `[(value)]` syntax:
...
Look up elements and components declared in the template:
...
Look up content projected into the component:
...
- `on viewport` - Once the element scrolls into the viewport
- `on idle` - Once the browser goes idle
- `on interaction` - When the user interacts (click, focus)
- `on hover` - When the mouse hovers over it
- `on immediate` - Right after the non-deferred content
- `on timer(500ms)` - Once the given delay elapses
- `when condition` - Once the expression evaluates to true
