...
- **Thin mode** (default): pure Python, no Oracle Client libraries needed. Covers most features.
- **Thick mode**: requires Oracle Client (Instant Client or full client). Needed for advanced capabilities such as Advanced Queuing, Sharding, and certain proxy authentication scenarios.
...
Always use bind variables — never format user input directly into SQL strings.
...
with conn.cursor() as cur:
    ...
    # fetchall — all remaining (use with care on large result sets)
    rows = cur.fetchall()
...
- **Use a connection pool** in web and multi-threaded apps; do not share connections across threads.
- **Use context managers** (`with`) for connections and cursors to guarantee proper cleanup.
- **Use `executemany`** for bulk inserts and updates rather than looping over `execute`.
...
- **Prefer thin mode** unless you specifically need features only available in thick mode.
...
- Oracle 23ai JSON Relational Duality Views are accessible through normal SELECT queries.
