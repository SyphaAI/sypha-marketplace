...
Oracle SQL Property Graph enables you to model and query graph data — vertices (nodes) and edges (relationships) — directly on top of existing relational tables, views, materialized views, or external tables. No data is duplicated; the graph definition holds only metadata, and queries run against live table data.

The core components are:
- **`CREATE PROPERTY GRAPH`** — defines which tables act as vertices and edges, their keys, labels, and properties
- **`GRAPH_TABLE` operator** — queries the graph using a pattern-matching syntax (`MATCH`) inside ordinary SQL `SELECT` statements

This guide covers SQL Property Graph only. For PGX (in-memory graph analytics using PGQL), refer to the Oracle Graph Server documentation.
...
A property graph is constructed on top of ordinary database objects (tables, views, materialized views, external tables, and supported synonyms). In practice, keys should uniquely identify vertices and edges; in `ENFORCED MODE`, Oracle validates this strictly via key/constraint rules.
...
The graph engine automatically derives keys from constraints:
...
`ENFORCED MODE` guarantees unique `VERTEX_ID`/`EDGE_ID` values and surfaces errors early when constraints are missing. It is **not supported** when graph element tables are views.

---

## Querying with GRAPH_TABLE

`GRAPH_TABLE` is a table operator used in the `FROM` clause of any SQL `SELECT`. It returns a relational result set derived from a graph pattern match.
...
Quantifiers match repeated edge hops without requiring each hop to be spelled out individually.
...
Controls the granularity of output rows when iterating over matched paths.
...
`VERTEX_ID`/`EDGE_ID` return `NULL` when the variable is not bound. In `TRUSTED MODE`, duplicate identifiers are possible if key columns lack a unique constraint.
...
JSON columns are fully supported as properties. Use Oracle dot-notation or `JSON_VALUE` to surface nested JSON fields.
...
`GRAPH_TABLE` queries are converted internally to SQL, so standard Oracle query tuning techniques apply:
...
**Recommendation:** Create indexes on key and frequently filtered columns in the underlying tables. Because `GRAPH_TABLE` translates to SQL joins, standard B-tree indexes on foreign key and primary key columns typically deliver the greatest performance benefit.
...
- **Build on constrained tables.** Define `PRIMARY KEY` on vertex tables and `FOREIGN KEY` on edge tables; the graph engine auto-infers keys, eliminating the need for explicit `KEY` declarations and enabling `ENFORCED MODE`.
- **Use `ENFORCED MODE` for new graphs on tables.** It validates constraints at creation time and guarantees unique `VERTEX_ID`/`EDGE_ID` values, preventing silent data correctness issues.
- **Use `OR REPLACE` when iterating on the graph schema.** Property graph definitions are immutable after creation; `OR REPLACE` is the only way to modify them without dropping first, and it preserves previously granted privileges.
...
- **Use `TRUSTED MODE` with views** since `ENFORCED MODE` is not available for views; independently validate uniqueness of key columns in the view output.
...
A property graph stores only metadata. It reads from the live underlying tables at query time. Dropping or truncating an underlying table affects all graph queries that reference it.
...
SQL graph query support has evolved across 23ai and 26ai release updates. Verify against the release-specific "Supported Features and Limitations for Querying a SQL Property Graph" page before relying on advanced clauses.

---

## Oracle Version Notes (19c vs 26ai)

- **Oracle 19c:** SQL Property Graph (`CREATE PROPERTY GRAPH`, `GRAPH_TABLE`) is **not available**. Graph processing in 19c requires Oracle Graph Server (PGX) as a separate component using the PGQL query language.
- **Oracle 23ai (23c):** First release to support SQL Property Graph DDL and the `GRAPH_TABLE` operator natively in the database engine. Core features: `CREATE PROPERTY GRAPH`, `DROP`, `ALTER PROPERTY GRAPH COMPILE`, `RENAME`, `GRAPH_TABLE` with `MATCH`, label expressions, quantified paths (`{n}`, `{n,m}`, `{,m}`), `VERTEX_ID`/`EDGE_ID`, `ONE ROW PER` clause.
