...
`godror` is the recommended Go driver for Oracle Database. It conforms to the standard `database/sql` interface and relies on Oracle's ODPI-C library internally (Oracle Instant Client is required).

```bash
go get github.com/godror/godror
```

**Oracle Instant Client** must be installed and present on the library search path (`LD_LIBRARY_PATH` on Linux, `DYLD_LIBRARY_PATH` on macOS, `PATH` on Windows).
...
`database/sql` manages the connection pool. Set it up at application startup:
...
`godror` supports named binds using `:name` syntax. Always rely on bind variables — never interpolate user data directly into SQL strings.
...
- **Always use named binds** (`sql.Named`) — eliminates positional ordering mistakes.
- **Set pool sizes** at startup using `SetMaxOpenConns` and `SetMaxIdleConns`.
- **Always call `rows.Close()`** in a `defer` right after opening rows.
- **Check `rows.Err()`** after finishing the loop — iteration errors are captured there.
- **Use transactions** for multi-statement DML; `database/sql` does not auto-commit.
- **Reuse prepared statements** via `db.PrepareContext` for queries executed repeatedly in tight loops.
...
- `godror` supports Oracle Database 11.2 and above through ODPI-C.
- The Oracle 23ai `VECTOR` type requires godror 0.44+ and Oracle Instant Client 23.
