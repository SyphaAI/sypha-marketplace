...
Oracle Spatial and Graph (formerly Oracle Spatial) delivers a native spatial data type, geometry operators, and spatial indexing capabilities directly within Oracle Database. It integrates seamlessly with SQL, allowing spatial queries to participate in joins, aggregations, and query optimizations alongside conventional relational data.

Oracle Spatial is built on the `MDSYS` schema and requires the Oracle Spatial option (included with Enterprise Edition; also available in Standard Edition 2 in recent versions as a subset). Oracle's spatial capabilities conform to the **OGC (Open Geospatial Consortium) Simple Features for SQL** standard.

---

## The SDO_GEOMETRY Type

All spatial data in Oracle is stored using the `SDO_GEOMETRY` object type, which is defined in the `MDSYS` schema.
...
**Every spatial table must have an entry in `USER_SDO_GEOM_METADATA`** before a spatial index can be created. This metadata specifies the valid coordinate range.
...
Oracle spatial uses **operators** (not functions) for primary spatial predicates. The optimizer relies on these operators to take advantage of the spatial index.

### SDO_RELATE: General Topological Relationship

`SDO_RELATE` evaluates the topological relationship between two geometries using the 9-intersection model (DE-9IM).
...
The **SRID (Spatial Reference Identifier)** specifies the coordinate system in use. Oracle stores these definitions in `MDSYS.SDO_COORD_REF_SYSTEM`.
...
- **Always register `USER_SDO_GEOM_METADATA`** before creating a spatial index. The metadata establishes the valid coordinate extent and tolerance.
- **Use WGS84 (SRID=4326)** for general-purpose geographic data (GPS coordinates). Switch to projected coordinate systems (UTM, State Plane) when precise metric distances are required.
- **Set tolerance appropriately**: ~0.00001 degrees (≈1 meter) for geographic data, 0.001 for projected data in meters. Tolerance that is too tight causes false "not equal" results; too loose conflates nearby features.
- **Use spatial operators (`SDO_RELATE`, `SDO_NN`)** in WHERE clauses — not spatial functions (`SDO_GEOM.*`) — to take advantage of the spatial index.
...
Oracle's SDO_GEOMETRY for WGS84 (SRID=4326) uses **(longitude, latitude)** order — not (lat, lon). This follows the mathematical (x, y) convention and the OGC/GeoJSON standard, but is opposite to how many people verbally describe coordinates.
