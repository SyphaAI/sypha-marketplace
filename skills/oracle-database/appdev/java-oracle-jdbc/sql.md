...
Use this skill when running SQL or PL/SQL from Java via Oracle JDBC. It covers bind variables, named binds, DML, batching, stored procedures, functions, OUT parameters, and REF CURSOR results.

For connection URL configuration, begin with [JDBC Connections](connections.md).

## Bind Variables

Always use `PreparedStatement` with `?` placeholders for user-supplied values. Never concatenate user input directly into SQL strings.
...
Oracle JDBC supports named binds via `OraclePreparedStatement`.
...
For DML-heavy code, manage transaction boundaries explicitly.
...
Use `addBatch` and `executeBatch` to perform bulk inserts or updates.
...
For large result sets, tune the fetch size to reduce database round trips.
...
Bind variables protect values, not SQL identifiers. When user input controls table names, column names, sort directions, or other structural SQL elements, validate it against an allowlist before assembling the SQL string.
...
- Use `PreparedStatement` whenever SQL includes values.
- Use try-with-resources for connections, statements, and result sets.
- Disable `autoCommit` for DML-heavy workflows and commit explicitly.
...
- Set `fetchSize` appropriately for large query result sets.
...
- Oracle 23ai and 26ai JSON Relational Duality Views are accessible through standard JDBC.
- The `VECTOR` data type introduced in 23ai and available in 26ai is supported by the 23.x/26ai JDBC driver line.
- Ensure SQL examples remain compatible with the target database version, particularly when using newer data types or JSON features.
