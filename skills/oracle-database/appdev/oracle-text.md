...
Oracle Text (formerly ConText and interMedia Text) is Oracle's full-text search engine, built directly into the database kernel. Unlike application-level search libraries such as Lucene or Elasticsearch, Oracle Text indexes reside inside the database alongside the data, allowing full-text search to participate in SQL joins, transactions, and access control without any external infrastructure.

Oracle Text is well suited for:
...
Oracle Text offers four primary index types. Selecting the appropriate one is the most critical decision.
...
CONTEXT indexes are not refreshed in real-time by default. New and modified rows must be synchronized into the index.
...
CTXCAT is designed for catalog-style searches (short text, combined text and structured filters). Unlike CONTEXT, it refreshes automatically with DML (no manual sync required).
...
`CONTAINS` is the primary search operator for CONTEXT indexes. It returns a relevance score from 0 to 100, where 100 indicates the highest relevance.
...
Fuzzy search locates words that are similar based on edit distance, making it useful for catching misspellings.
...
Stemming finds morphological variants of a word (for example, searching "run" also matches "running", "ran", and "runs").
...
These functions produce context-aware result display, similar to excerpt highlighting in search engines.
...
`SNIPPET` extracts the most relevant portions of a document — the "hits in context" — as short excerpts with highlighted terms.
...
Over time, as documents are updated and deleted, the CONTEXT index becomes fragmented. Optimization consolidates fragmented posting lists.
...
Section groups allow searching within specific parts of HTML or XML documents.
...
- **Choose `CTXCAT` for short text that changes frequently** (product names, titles, tags). It self-maintains automatically. Use `CONTEXT` for large documents.
- **Schedule `SYNC_INDEX` according to acceptable staleness.** `SYNC ON COMMIT` carries overhead; `SYNC EVERY n MINUTES` via the scheduler is typically preferable.
- **Run `OPTIMIZE_INDEX` regularly** (weekly or nightly for active systems). Fragmented indexes produce degraded relevance scores.
- **Use `SCORE()` to rank results** and filter with `CONTAINS > threshold` rather than `> 0` to exclude marginally relevant documents.
- **Index only columns that genuinely require full-text search.** Oracle Text indexes consume substantial storage (often 20–40% of the original data size).
- **Use section groups** for structured documents to enable section-scoped searches instead of whole-document searches.
- **Test fuzzy and stem parameters** against your actual data before going to production. Overly aggressive fuzzy matching returns too many irrelevant results.
- **Use `MULTI_COLUMN_DATASTORE`** instead of creating separate indexes per column. A single index across all text columns is faster to query than `CONTAINS(col1, q) > 0 OR CONTAINS(col2, q) > 0`.
...
When you delete or update many documents without running `OPTIMIZE_INDEX`, the index accumulates stale "garbage" entries. This bloats the index and degrades query performance. After any bulk DML, run `CTX_DDL.OPTIMIZE_INDEX` with `FAST` or `FULL`.

### Mistake 4: Wrong Filter for Binary Document Types

If you index a column containing Word documents, PDFs, or HTML stored as BLOBs, you must configure the `FILTER` preference to `INSO_FILTER` or `AUTO_FILTER`. Without this, Oracle Text indexes the raw binary content, which is meaningless.
...
- Baseline guidance in this file is valid for Oracle Database 19c unless a newer minimum version is explicitly stated.
- Features marked as 21c, 23c, or 23ai should be treated as Oracle Database 26ai-capable features; retain 19c-compatible alternatives for mixed-version environments.
- For dual-support environments, validate syntax and package behavior in both 19c and 26ai because defaults and deprecations can differ between release updates.
