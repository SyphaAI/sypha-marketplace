...
A transaction is a logical unit of work comprising one or more SQL statements. Oracle's transaction model ranks among the most robust in the relational database world, delivering full ACID guarantees while supporting high concurrency through its multi-version concurrency control (MVCC) implementation.

Understanding how Oracle manages transactions is critical for writing correct, performant applications. Subtle errors — uncommitted transactions held too long, improper use of savepoints, or misunderstanding autonomous transactions — are among the most common causes of data corruption, lock contention, and deadlocks.
...
Every statement in a transaction either completes entirely or fails entirely. When a statement fails mid-execution (e.g., a unique constraint violation), Oracle automatically rolls back that single statement's changes via **statement-level rollback** — the transaction itself remains open with all prior changes intact.
...
One benefit of PL/SQL is that a PL/SQL block also operates as a statement-level transaction. If any of the three DML statements above were to fail, all three would be rolled back automatically because the entire PL/SQL block is treated as a single statement-level transaction.

---

## Savepoints

Savepoints enable partial rollbacks within a transaction. A `ROLLBACK TO savepoint_name` undoes all changes made after the savepoint was established, but does **not** end the transaction — earlier changes and the savepoint itself remain.
...
An **autonomous transaction** is an independent transaction spawned from within a calling transaction. It operates with its own commit/rollback scope — entirely separate from the parent transaction. Changes made by an autonomous transaction become visible to other sessions immediately after its commit, even when the calling transaction has not yet committed.
...
Oracle supports the X/Open XA protocol for distributed transactions that span multiple databases or resource managers (databases + message queues).
...
Long-running transactions are the most frequent source of performance problems in Oracle applications. They:

- Hold row locks that block other sessions
- Generate undo data that must be retained until the transaction completes
- Can trigger `ORA-01555: snapshot too old` for other long-running queries
...
For example, a single large UPDATE may hold locks for too long a period:
...
To avoid this, commit every N rows and reopen the cursor each time to prevent ORA-1555 errors.
...
- **Keep transactions as short as possible.** Perform all computation before the transaction, execute DML, then commit immediately.
- **Never wait for user input inside a transaction.** The user might step away, leaving locks held indefinitely.
- **Always handle exceptions. Call ROLLBACK if necessary.** An unhandled exception in application code that disconnects without rolling back may leave the transaction open until the session is killed.
- **Use `COMMIT WRITE BATCH NOWAIT` only for bulk loads** where you fully understand the durability trade-off.
...
- **Never use `PRAGMA AUTONOMOUS_TRANSACTION` to work around locking issues.** It creates invisible data dependencies that are difficult to debug.
...
In Oracle, any DDL statement (`CREATE`, `ALTER`, `DROP`, `TRUNCATE`) issues an implicit COMMIT both before and after it executes. This silently commits any pending DML.
...
JDBC connections have `autoCommit=true` by default. Every individual statement is immediately committed. This is almost never the desired behavior for OLTP.
...
When a DML statement fails with an exception, only that statement is rolled back. The transaction remains open. If you catch the exception and do nothing, the earlier statements in the transaction are still uncommitted and their locks are still held.
