...
`node-oracledb` is the official Oracle driver for Node.js. Like `python-oracledb`, it offers two modes:

- **Thin mode** (default): pure JavaScript, no Oracle Client required. Covers most features.
- **Thick mode**: requires Oracle Instant Client. Necessary for Advanced Queuing, Sharding, and certain proxy auth scenarios.

Unless a snippet includes its own wrapper, JavaScript examples below assume execution inside an `async` function where `conn` is an open connection acquired beforehand.
...
- **Always use bind variables** — never use template literals with user data in SQL.
- **Use a connection pool** in Express/Fastify/Koa apps — create once at startup, close at shutdown.
- **Set `oracledb.outFormat = oracledb.OUT_FORMAT_OBJECT`** globally for more readable code.
- **Use `executeMany`** for bulk DML instead of looping over `execute`.
- **Use result sets or streams** for large queries rather than `maxRows`.
- **Return connections to the pool** in a `finally` block — always call `conn.close()`.
- **Use `autoCommit: true`** only for single-statement transactions; otherwise manage commits explicitly.
