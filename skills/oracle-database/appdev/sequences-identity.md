...
Producing unique, monotonically increasing identifiers is a foundational requirement in relational database design. Oracle offers two mechanisms:

- **Sequences** — standalone schema objects that generate numbers independently of any table
- **Identity Columns** — column-level syntax that couples sequence generation with table DDL (introduced in Oracle 12c)

Both rely on the same internal sequence engine, but identity columns provide a cleaner, more declarative approach to defining surrogate keys.
...
When the database restarts, cached but unused sequence values are **discarded**. If you cached 1000 values and consumed 50, the next session starts at value 1051 — a gap of 950. This behavior is normal and expected. If your application cannot tolerate gaps, sequences are not suitable (and you will pay the performance cost accordingly).
...
Sequence values are **not gap-free** by design. Gaps arise because:
...
**If gap-free numbering is required:** Use an alternative mechanism such as a dedicated table with a row lock or `DBMS_LOCK` for serialization. This is almost always a design mistake — gap-free sequences require serial execution and become a performance bottleneck. Business requirements for consecutive numbering (for example, invoice numbers for tax compliance) should use a separate post-commit numbering step.

---

## ORDER vs. NOORDER (RAC Environments)

In Oracle RAC (Real Application Clusters), multiple instances can generate sequence values independently from their own cache. With `NOORDER` (default), values are generated with no guaranteed ordering across nodes — node 1 might generate 100–199, node 2 generates 200–299, and requests could receive 150, 250, 160, and 210 in interleaved order.

With `ORDER`, Oracle guarantees strictly ordered generation across all RAC nodes by serializing through the cluster interconnect. This carries significant performance implications.
...
Oracle generates the value when the column is omitted. You can supply an explicit value to override it.
...
Functions like `GENERATED BY DEFAULT`, but also generates a value when `NULL` is explicitly inserted:
...
If RFC 4122 UUID strings are required (for example, for REST APIs that expose IDs):
...
- **Sequences / Identity columns**: The default choice for primary keys in any application. Fast, compact, and index-friendly.
- **SYS_GUID() / UUID**: Use when records will be merged across multiple databases, when IDs are exposed in APIs (avoids enumeration attacks because values are non-sequential), or when generating IDs outside the database prior to insertion.

**Important**: Random UUIDs as primary keys cause **index fragmentation**. Each new UUID inserts into a random position in the B-tree index, triggering frequent page splits. For high-insert tables, this can significantly degrade performance compared to monotonic sequence values. Consider using a sequence for the database key and a UUID for the external-facing API key.

---

## Best Practices

- **Use `CACHE 100` or higher** for OLTP sequences. The default of 20 is insufficient for high-insert systems.
- **Prefer identity columns over manual sequence-in-insert** for new tables. The syntax is cleaner and communicates intent more clearly.
- **Never assume gap-free sequences**. Design your application to tolerate gaps. If business rules require consecutive numbering (invoices, receipts), assign that number in a separate post-commit step.
- **Use `GENERATED ALWAYS`** unless you have a specific reason to allow overrides. It prevents accidental manual inserts.
- **For bulk data loads**, raise `CACHE` significantly (for example, `CACHE 10000`) before the load and restore it to normal afterward to reduce redo contention.
- **In RAC environments**, use `NOORDER` with adequate CACHE. The performance cost of `ORDER` is rarely justified.
...
Some teams use `NOCACHE` to obtain "no gaps" in audit sequences. This approach combines the worst of both worlds: it is still not truly gap-free (gaps occur on rollback), and it is vastly slower because every `NEXTVAL` writes to the data dictionary.
...
`CURRVAL` returns the last value produced by `NEXTVAL` in **the current session**. It has no meaning in another session or after reconnecting.

### Mistake 5: Forgetting Identity Columns in Data Exports/Imports

When using `GENERATED ALWAYS`, Data Pump exports include the sequence definition. During import, if the target table already contains rows, the sequence may generate duplicate values. Always check `LAST_NUMBER` after import and reset the sequence.
