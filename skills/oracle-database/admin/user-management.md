...
Oracle user management covers creating database accounts, controlling what those accounts can access through privileges and roles, enforcing security requirements through profiles, and handling account lifecycle events such as locking, expiring, and unlocking. In a Multitenant (CDB/PDB) environment, the additional concepts of common users and local users also come into play.

Sound user management is foundational to database security. Poorly maintained accounts — stale service accounts, default passwords, overprivileged schemas — rank among the most common entry points for data breaches.
...
**`QUOTA`** — how much space the user may consume in a tablespace. Without a quota, the user can create objects but cannot allocate any extents. `UNLIMITED` removes the cap.

**`PROFILE`** — assigns a resource and password policy profile (see below).

**`PASSWORD EXPIRE`** — immediately expires the password, requiring the user to change it upon first login. A security best practice for new accounts.

**`ACCOUNT LOCK / UNLOCK`** — creates the account in a locked state (prevents login until explicitly unlocked).

**`NOAUTHENTICATION`** — creates the account without any mechanism for establishing a session under this account. Best practice for accounts that hold schema objects where direct login should not be permitted.
...
`CASCADE` removes all objects owned by the user, including tables, indexes, views, procedures, sequences, and more. Use with extreme caution — this operation cannot be undone.

---

## Profile Management

A **profile** is a named collection of limits that governs password policy and resource consumption. Every user is assigned exactly one profile. When no profile is specified at creation time, the `DEFAULT` profile applies.
...
Oracle includes built-in password complexity functions:
...
You can also write a custom PL/SQL function that conforms to the documented interface:
...
Resource limits (CPU, SESSIONS, IDLE_TIME, etc.) are enforced only when the `RESOURCE_LIMIT` parameter is set to `TRUE`. Password limits are always enforced regardless.
...
Oracle maintains a database-level default tablespace and a default temporary tablespace. Users created without explicit tablespace assignments inherit these values.
...
Several temporary tablespaces can be grouped together so Oracle assigns users to the least-loaded member:
...
Oracle automatically locks an account after `FAILED_LOGIN_ATTEMPTS` consecutive failed login attempts (as defined in the user's profile). The lock duration is controlled by `PASSWORD_LOCK_TIME` (which can be set to `UNLIMITED`, requiring manual intervention to unlock).
...
When a password reaches its `PASSWORD_LIFE_TIME`, the account enters `EXPIRED` or `EXPIRED (GRACE)` status. Users can still authenticate during the grace period but are required to change their password immediately.
...
Proxy authentication allows one database user (the **proxy user**) to connect on behalf of another user (the **client user**). The connection is visible as the client user in `V$SESSION`, while the proxy user's credentials handle authentication.
...
In Oracle Multitenant, a **Container Database (CDB)** contains the root container (CDB$ROOT) along with one or more **Pluggable Databases (PDBs)**. Users fall into one of two categories:

- **Common users** — created in the CDB root; present in CDB$ROOT and optionally in all PDBs
- **Local users** — created within a specific PDB; exist only within that PDB

### Common Users

Common users are conventionally prefixed with `C##` (enforced by default since Oracle 12c). They authenticate against the CDB root but can be granted privileges in individual PDBs.
...
By default, Oracle enforces the `C##` prefix for common users. This behavior can be modified (not recommended for security purposes, but sometimes required for legacy applications):
...
- **Apply the principle of least privilege.** Provision application accounts with only the specific object privileges they require. Avoid granting `DBA` or `RESOURCE` to application users.

- **Never use SYS or SYSTEM as the application schema owner.** Create dedicated schemas for each application.

- **Assign a profile to every production user** — do not leave users on the `DEFAULT` profile. The `DEFAULT` profile's limits may not align with your security requirements.

- **Use `PASSWORD EXPIRE`** when creating new accounts so users are forced to set their own password.

- **Lock all default Oracle accounts** that are not actively used. Many default accounts (e.g., `SCOTT`, `HR`, `OUTLN`) are present in new databases and should be locked or expired.
  ```sql
...
- **Review stale accounts monthly.** Lock accounts for users who have departed the organization or whose projects have concluded.

- **Use proxy authentication for connection pools** to retain end-user identity in audit trails.
...
The `DBA` role includes the ability to drop tables, read any table, create users, and much more. Application accounts should have only `CREATE SESSION` and the specific object privileges they need. Audit `DBA` role grants regularly:
```sql
SELECT grantee, granted_role FROM dba_role_privs WHERE granted_role = 'DBA';
```

**Not setting a quota — and being surprised when INSERTs fail**
A user without a quota may hold `RESOURCE` or `CREATE TABLE` but cannot actually store any rows. Always assign explicit quotas or `UNLIMITED` on the user's default tablespace.

**Leaving the DEFAULT profile unchanged**
Many organizations leave `DEFAULT` with `PASSWORD_LIFE_TIME = UNLIMITED` and no `FAILED_LOGIN_ATTEMPTS` limit. This means no lockout policy and no password rotation. Harden the `DEFAULT` profile or create custom profiles and assign them.
```sql
...
There is no valid reason to modify `USER$` or related base tables directly. Always use `ALTER USER`. Direct manipulation can leave the data dictionary in an inconsistent state.

**Forgetting that DROP USER CASCADE is permanent**
`DROP USER foo CASCADE` drops all objects owned by `foo` immediately and without a confirmation prompt. Always inspect the schema contents before running this command in production.
```sql
...
Local users (without `C##`) created in CDB$ROOT behave as common users without the prefix enforcement. They introduce confusion and management complexity. Always create application users as local users inside the appropriate PDB.
