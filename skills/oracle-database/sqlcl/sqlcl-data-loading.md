...
SQLcl includes a built-in `LOAD` command that enables direct ingestion of CSV and JSON data into Oracle tables without requiring SQL*Loader, external tables, or any additional tooling. The `LOAD` command is designed for interactive and scripted data imports, developer workflows, and moderate-volume data loading scenarios.
...
For very high-volume production loads (hundreds of millions of rows), SQL*Loader's direct-path load mode or Oracle Data Pump remain the appropriate tools. The `LOAD` command is a general-purpose, high-convenience option for the vast majority of development and operational data loading needs.
...
External tables are preferable when:
...
- In production load scripts, set `ERRORS 0` to abort immediately on the first error rather than silently skipping bad rows. Use a non-zero value only when partial loads with rejection logging are acceptable.
- Always specify `DATEFORMAT` explicitly when loading date columns. Do not rely on the session NLS_DATE_FORMAT, which can vary across environments and users.
- Use `BATCHSIZE 500` or `BATCHSIZE 1000` for large files to reduce individual commit sizes and improve recoverability. If the load fails midway, you lose at most one batch rather than all progress.
- Before running the full file, test the load against a small representative sample (50–100 rows) to catch column mapping errors, date format mismatches, and data type issues early.
- For production reference data loads, verify the row count after loading and fail the pipeline if the count does not match the expected value.
...
SQLcl `LOAD` matches column names case-insensitively. A CSV header `employee_id` will match the Oracle column `EMPLOYEE_ID`. However, Oracle columns created with double quotes that have mixed case will not match unless the case is exact.

**Mistake: Loading dates without specifying DATEFORMAT**
Oracle's default date format is locale-dependent. A CSV containing `2025-01-15` will fail to load without `DATEFORMAT YYYY-MM-DD`. Always specify the format explicitly.

**Mistake: Silent truncation of numeric values**
If a CSV contains a value such as `999999999999` for a `NUMBER(6)` column, the LOAD will generate an error for that row (not silently truncate the value). Review your ERROR_LOG to identify precision mismatches.

**Mistake: Encoding issues with non-ASCII characters**
SQLcl reads CSV files using the JVM's default file encoding. If your CSV contains UTF-8 characters and the JVM is configured for a different charset, characters will be corrupted. Set `JAVA_TOOL_OPTIONS="-Dfile.encoding=UTF-8"` before starting SQLcl when loading international data.

**Mistake: Loading into a table with active foreign key constraints**
...
The default behavior is APPEND. Running a load script twice will produce duplicate rows. For idempotent loads (safe to run multiple times), use `TRUNCATE ON` to clear the table first, or use a staging-plus-merge pattern with a unique key.
