...
Combine `/nolog` connect with inline commands:
...
SQLcl exits with code `0` on success and a non-zero code on failure. However, to ensure that failures in SQL scripts produce non-zero exits, you must use `WHENEVER SQLERROR` at the top of your scripts.
...
With SEPS the heredoc can stay quoted, requiring no shell-side escapes:
...
Pass credentials through `/nolog` + stdin or a SEPS wallet so they never appear as process arguments. With SEPS the password is absent entirely; with `/nolog` it travels on fd 0 rather than argv.
...
Do not enable shell tracing (`set -x`, `set -v`, `bash -x`) in steps that handle the stdin connect — tracing will print the expanded `CONNECT` line to the job log.
...
- Always place `WHENEVER SQLERROR EXIT SQL.SQLCODE ROLLBACK` at the top of every CI/CD SQL script. Without it, SQLcl continues executing after a SQL error and exits with code 0 even when statements have failed.
- Use the `-S` (silent) flag for all CI/CD invocations. Without it, the SQLcl banner and connection messages appear in your pipeline log and may confuse log parsers.
...
- Use `lb tag` before every deployment to create a rollback target. Always include the commit SHA or pipeline ID in the tag name so you can identify the exact database state at any point.
...
- Capture SPOOL output and upload it as a CI artifact on failure. Raw SQLcl output alone may not be enough to diagnose what went wrong.
...
If `TNS_ADMIN` is not set, SQLcl cannot locate the wallet and the connection fails. Set `TNS_ADMIN` as an environment variable in the pipeline step or export it in the CI step's `before_script`. Confirm the value with `echo $TNS_ADMIN` before running SQLcl.

**Mistake: Wallet files have incorrect permissions**
On Linux, Oracle requires wallet files to be readable only by the owning user (chmod 600). CI runners may unzip files with permissions that are too permissive, causing SSL handshake failures. Always run `chmod 600 /tmp/wallet/*` after unzipping.

**Mistake: Heredoc with variable expansion in quotes kills substitution**
Using `<<'EOF'` (with quotes around EOF) prevents shell variable expansion inside the heredoc. Use `<<EOF` (without quotes) when you need shell variables expanded, and `<<'EOF'` when you want `&variable` substitutions passed through to SQLcl literally.

**Mistake: Pipeline uses DBA account for all operations**
Using a DBA or ADMIN account for routine deployments is a security risk and makes it difficult to audit which changes originated from the pipeline versus manual intervention. Use a dedicated deployment account with only the minimum required privileges.
