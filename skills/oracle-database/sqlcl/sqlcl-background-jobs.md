...
SQLcl provides `BACKGROUND` (alias `bg`) and `JOBS` (alias `jb`) commands for running SQLcl commands asynchronously and managing the resulting background tasks. This lets long-running operations execute without blocking the interactive prompt, and allows multiple tasks to run concurrently within the same SQLcl session.

The `WAIT4` (alias `w4`) command complements these by blocking the session until specified tasks finish — enabling straightforward dependency chains.

Use background jobs for:
- Running multiple export or reporting commands in parallel
- Dispatching a long-running operation while continuing interactive work
...
| `-wait4 <names>` | `-w4` | Comma-separated list of task names this job must wait for before starting |
...
Displays all background tasks with their IDs, names, status, and result summary.
...
Shows the complete output produced by the background task.
...
When you run `jobs`, each task displays one of these statuses:
...
- Always assign task names with `-taskname` when running more than one background job. Auto-assigned numeric IDs are difficult to track.
- Use `wait4` before accessing the results of background tasks — reading logs while a task is still running returns partial output.
- For long-running tasks, use `-delay` with `wait4` to reduce CPU polling overhead: `wait4 -delay 1000 mytask`.
- At the end of a session, clear completed jobs with `jobs delete -finished` to keep the jobs list manageable.
- Background jobs share the database connection within the same SQLcl session. Avoid running background jobs that issue conflicting DDL or DML against the same objects at the same time.
- For scheduled recurring background tasks rather than ad-hoc parallelism, use the SQLcl Scheduler Daemon instead.
...
`jobs logs` returns whatever output exists at that moment. If the task is still running, the output is partial. Use `wait4` first.

**Mistake: `-wait4` referencing a non-existent task name**
If the task named in `-wait4` does not exist or has already been deleted, the dependent job may start immediately or hang. Verify the dependency task is listed by running `jobs`.

**Mistake: Running conflicting DML in parallel tasks**
Background tasks share the session connection. Two tasks updating the same rows concurrently will cause lock contention. Design parallel tasks to operate on separate data sets.

**Mistake: Forgetting to clean up**
The jobs list accumulates throughout the session. Use `jobs delete -all` or `jobs delete -finished` to keep it manageable.
