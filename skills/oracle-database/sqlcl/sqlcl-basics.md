...
SQLcl (SQL Command Line) is Oracle's modern successor to SQL*Plus. It is a free, Java-based command-line interface for Oracle Database that ships with Oracle Database installations and is also available as a standalone download. SQLcl offers substantial improvements over SQL*Plus, including tab completion, command history, in-line editing, built-in Liquibase support, a JavaScript scripting engine, and richer output formatting options.

SQLcl is distributed as a single ZIP file (no installer required) and runs on any platform with JDK 17 or newer (JDK 21 recommended). It is also available via Homebrew on macOS. The executable is named `sql` (not `sqlcl`) to simplify migration from SQL*Plus.
...
After installation, the command is available as `sql`. Homebrew handles updates through `brew upgrade sqlcl`.
...
SQLcl 25.2 and later requires Java 17 or 21. If multiple JDKs are installed, ensure the correct one is on your PATH:
...
Omit the password from the connect string. SQLcl will prompt for it without echoing input:
...
Alternatively, point `TNS_ADMIN` at the wallet directory and use the pre-defined TNS aliases it contains:
...
For local SYSDBA work, prefer OS authentication — no password is required:

```shell
sql / as sysdba
```

For a remote SYSDBA connection, omit the password to receive a prompt:
...
Embedding the password in the connect string (`sql user/password@service`) is **discouraged** and must not appear in scripts, documentation, training material, or shared commands. The credential is exposed to:
...
If a workflow genuinely cannot be reorganized to use a prompt, a wallet, or SEPS, treat the inline-password form as a temporary, isolated exception and rotate the credential afterwards.
...
Use the `CONNECT` command to switch connections without restarting SQLcl. Omit the password so SQLcl prompts:
...
SQLcl maintains persistent command history across sessions (stored in `~/.sqlcl/history.log`):
...
Re-run a command from history by its number:
...
SQLcl supports multi-line SQL editing. Press Enter to continue a SQL statement on the next line. The prompt changes from `SQL>` to the line number. Use `/` on a blank line or a semicolon to execute:
...
The `/` command re-executes the SQL buffer. Use `L` (LIST) to display the current buffer:
...
Press Tab once to complete, or twice to display all options when multiple matches exist.
...
- Always use the `CONNECT` command with a password prompt (omit the password from the command line) in shared or logged environments to prevent credentials from appearing in shell history or process lists.
- Set `TNS_ADMIN` in your shell profile rather than embedding wallet paths directly in connection strings, so scripts remain portable across environments.
- Store common `SET` commands and `ALIAS` definitions in `login.sql` so every session starts with a consistent configuration.
- Use `SET FEEDBACK ON` and `SET TIMING ON` during interactive work; switch them `OFF` in scripts where only output data matters.
- Prefer `SET SERVEROUTPUT ON SIZE UNLIMITED` over a fixed size to avoid truncated DBMS_OUTPUT.
- Wrap batch operations in `SPOOL /path/to/output.log` and `SPOOL OFF` to capture all output for later review.
- When connecting to Autonomous Database, store the wallet path in `TNS_ADMIN` and commit the `tnsnames.ora` and `sqlnet.ora` (without the wallet files themselves) to version control so connection strings are reproducible.
...
Omit the password from the connection string. SQLcl will prompt for it securely. Alternatively, use a wallet or use the `DEFINE` command to prompt at script startup.

**Mistake: Scripts failing due to SQL*Plus compatibility**
Most SQL*Plus scripts run in SQLcl without changes. If you encounter issues, look for use of `COLUMN ... NOPRINT` with complex formatting or very old `SET` options. SQLcl supports the vast majority of SQL*Plus directives.
...
Row count messages such as `25 rows selected.` will appear in your output file. Always set `SET FEEDBACK OFF` and `SET HEADING OFF` when spooling data for downstream consumption.

**Mistake: Tab completion not working**
Tab completion requires the terminal to be in a mode that passes control characters. Ensure SQLcl is running in an interactive terminal, not through a pipe or non-interactive shell substitution.
