...
SQLcl includes a built-in `DDL` command that produces the `CREATE` statement for any database object in the current schema or a named schema. It serves as a simplified interface to Oracle's `DBMS_METADATA` package, with sensible defaults that remove storage clauses, physical attributes, and other environment-specific details that make raw `DBMS_METADATA` output hard to use across different environments.

The `DDL` command is particularly useful for:
- Recording object definitions for version control
- Comparing object definitions across environments
- Producing migration scripts from an existing database
- Documenting schema structure
- Quickly reviewing the complete definition of a table, package, or trigger without querying multiple catalog views
...
By default, the `DDL` command generates clean, portable DDL with minimal storage and physical attribute clauses. You can adjust its behavior using the options below.

### SHOW Options

DDL command output is managed primarily through `SET DDL` session-level settings (see below). This guide deliberately uses session-level commands because they represent the documented, version-stable method for controlling storage, segment attributes, tablespace clauses, and schema qualification.

The recommended way to control DDL output is through session settings:
...
SQLcl offers `SET DDL` options that govern DDL output throughout the entire session:
...
Output includes the complete `CREATE OR REPLACE VIEW` statement along with the query body.

### Packages

Package spec and body are distinct objects in Oracle:
...
Always extract both. The spec defines the public interface; the body holds the implementation.
...
Triggers belong to their parent table. The DDL output includes `CREATE OR REPLACE TRIGGER ... ON table_name`.
...
Note: The generated DDL reflects the current `LAST_NUMBER` from the sequence. For version control purposes, you will typically want to normalize the `START WITH` value to a baseline (such as 1 or the next anticipated value) rather than the live sequence value.
...
Indexes that enforce constraints (primary key, unique) are usually included automatically when generating table DDL. Use explicit index DDL for standalone functional or composite indexes.
...
A more practical approach uses a wrapper script:
...
For DDL extraction that works well with version control, generate one file per object:
...
Alternatively, use DBMS_METADATA directly:
...
When using `DBMS_METADATA` in SQL scripts, apply these transforms to get clean output:
...
A well-organized directory structure for Git:
...
- Always configure `SET DDL STORAGE OFF`, `SET DDL SEGMENT_ATTRIBUTES OFF`, and `SET DDL TABLESPACE OFF` before generating DDL for version control. Schema DDL that includes storage clauses is environment-specific and will not deploy cleanly to other environments.
- Extract package spec and package body as separate files. These are distinct Oracle object types and can be deployed independently — the body can be updated without modifying the spec.
- For sequences stored in version control, normalize the `START WITH` value to 1 or a known baseline. Capturing the live `LAST_NUMBER` produces a sequence that cannot be deployed reliably to a fresh environment.
- Extract DDL on a recurring schedule (for example, nightly in CI) and commit the output to a dedicated `schema-snapshot` branch. This gives you a searchable, diffable history of schema changes even when developers are not consistently writing migration scripts.
- Use SQLcl's `lb generate-schema` for a more structured approach. When raw DDL files are needed for direct deployment or documentation, the `DDL` command combined with SPOOL is the simpler option.
...
Raw `DBMS_METADATA` and even the `DDL` command with default settings may include `STORAGE`, `PCTFREE`, `TABLESPACE`, and other clauses that differ between environments. Always set `SET DDL STORAGE OFF` and related options before capturing DDL for version control.

**Mistake: `SET LONG` not set high enough**
The default `LONG` value (80 characters) silently truncates CLOB output from `DBMS_METADATA`. Set `SET LONG 1000000` before performing any DDL extraction that queries `DBMS_METADATA` in a SQL statement.

**Mistake: Package body DDL not captured separately**
When querying `user_objects`, `PACKAGE BODY` has `object_type = 'PACKAGE BODY'` (with a space). A query for `'PACKAGE'` retrieves only the spec. Always query `'PACKAGE'` and `'PACKAGE BODY'` as separate queries.

**Mistake: System-generated constraint names in DDL**
Oracle automatically generates constraint names like `SYS_C00789123` for unnamed constraints. These names vary between environments, leading to DDL conflicts during re-deployment. Make sure all constraints in source code are explicitly named before capturing schema DDL.

**Mistake: Capturing DDL for objects with compile errors**
`DBMS_METADATA.GET_DDL` returns DDL even for invalid objects, but deploying invalid PL/SQL DDL results in objects with `INVALID` status. Run `SELECT object_name, status FROM user_objects WHERE status = 'INVALID'` first and resolve all invalid objects before capturing a baseline DDL snapshot.
