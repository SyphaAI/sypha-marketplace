...
SQLcl comes with Liquibase bundled. No separate Liquibase installation, Java classpath configuration, or Oracle JDBC driver setup is needed — SQLcl already includes the Oracle driver and Liquibase as integrated components. The `lb` command (also available as `liquibase`) works at the SQLcl prompt and from the command line.

SQLcl's Liquibase integration is designed specifically for Oracle and provides Oracle-specific extensions that the standalone Liquibase CLI lacks by default, most notably the ability to **generate changelogs from an existing Oracle schema** — pulling in tables, views, packages, triggers, sequences, indexes, synonyms, and other object types directly from the database catalog.

This makes SQLcl Liquibase well suited for:
- Establishing a baseline of an existing database schema in version control
- Tracking schema changes incrementally over time
- Propagating changes across environments (dev → test → prod)
- Incorporating database changes into CI/CD pipelines
...
A changelog is a file (XML, YAML, JSON, or SQL) that holds an ordered list of **changesets**. Liquibase tracks which changesets have been applied to a database in a tracking table named `DATABASECHANGELOG`.

### Changeset

A changeset is an atomic unit of change uniquely identified by the combination of `id`, `author`, and `filename`. Once applied, a changeset is not executed again (unless explicitly rolled back). Changesets may contain DDL, DML, or stored procedure definitions.

### DATABASECHANGELOG Table

Liquibase automatically creates this table in the schema where changes are deployed. It stores:
- `ID` — The changeset ID
...
A companion lock table that prevents concurrent Liquibase operations from interfering with one another.

---

## Generating Changelogs from Existing Schema

This is SQLcl's most powerful Liquibase capability — the ability to reverse-engineer an existing Oracle schema into a Liquibase changelog.
...
The `-split` flag generates one file per object (recommended for version control). Without `-split`, all DDL is written to a single file.
...
Current SQLcl documentation identifies XML as the default output for schema generation, with SQL output available through the `-sql` flag:
...
Liquibase reads the changelog, consults `DATABASECHANGELOG` to determine which changesets have already been applied, and executes only the pending ones.

### Preview Changes (updateSQL)

Before applying changes, generate the SQL that would be executed without actually running it:

```sql
lb update-sql -changelog-file controller.xml
```

This sends the SQL to the screen. Redirect it to a file using SPOOL:
...
Liquibase reverts all changesets applied after the specified tag. For automatic rollback to work, each changeset must include a `<rollback>` element (for DDL such as `ALTER TABLE`, Liquibase can usually auto-generate the rollback).
...
For stored procedures, packages, and triggers, use `<sql>` with `splitStatements="false"` and an appropriate `endDelimiter`:
...
- Use `-split` when generating schema changelogs. Single-file changelogs become unwieldy and produce large merge conflicts in version control.
- Apply `runOnChange="true"` to changesets for PL/SQL objects (packages, procedures, functions, triggers, views). This lets Liquibase re-apply the changeset when the source is modified without requiring a new changeset ID.
- Always add a `<rollback>` element to changesets. Liquibase can auto-generate rollback SQL for straightforward DDL like `createTable` and `addColumn`, but not for arbitrary SQL or PL/SQL.
- Tag the database state before applying production changes with `lb tag`. This creates a reliable rollback point in case something goes wrong.
- Keep changelogs in the same repository as application code so database changes are versioned alongside the code that depends on them.
- Never edit the `DATABASECHANGELOG` table by hand. Use `lb changelog-sync` to mark changesets as applied without executing them (for example, for objects created manually before Liquibase was adopted).
- Run `lb status` before `lb update` in CI/CD pipelines to confirm the expected number of changesets will be applied. Fail the pipeline if the count is zero when changes are expected.
...
Liquibase records a checksum for each changeset. Modifying an applied changeset causes the next run to fail with a checksum mismatch. For PL/SQL object changes, use `runOnChange="true"`. For data corrections, write a new changeset instead.

**Mistake: Using the same changeset ID in different files**
Changeset uniqueness depends on the combination of `id + author + filename`. Two changesets may share the same `id` if they reside in different files, but this creates confusion. Adopt a consistent ID naming scheme such as sequential numbers or timestamps.

**Mistake: Not running `lb generate-schema` against a clean schema**
If the target schema contains manual changes that are not captured in the changelog, `lb update` will fail on objects that already exist. Run `lb changelog-sync` after the initial baseline generation to record all existing objects as applied.

**Mistake: Inline PL/SQL without `splitStatements="false"`**
Liquibase splits SQL on semicolons by default. PL/SQL body code contains semicolons internally. Always set `splitStatements="false"` in any changeset that includes PL/SQL.

**Mistake: Attempting to commit the `DATABASECHANGELOG` to version control**
The `DATABASECHANGELOG` resides in the database, not in version control. Do not attempt to commit it. What belongs in version control are the changelog files. The database table is Liquibase's runtime tracking state.
