...
**Recovery Service**
The OCI managed service that stores protected database backups and recovery catalog metadata. It consolidates backup protection across supported Oracle Database deployments.
...
**Protection Policy**
The policy governing backup retention, retention lock, and, for Oracle Multicloud databases, preferred backup storage location. Each protected database must be assigned exactly one protection policy.
...
**Recovery Window**
The maximum time range, counted backward from the current time, within which the database can be recovered.

**Real-Time Data Protection**
An optional extra-cost feature that continuously transmits redo from a protected database to Recovery Service to reduce RPO toward the last sub-second.
...
Operational backups sent to two destinations can introduce data loss scenarios. Before enabling automatic backups to Recovery Service, disable any manual backup scripts or processes targeting other operational backup destinations.

### Limit Checks

Review service limits in the OCI Console before onboarding:
...
For Oracle Multicloud databases, select the correct multicloud subscription before requesting a limit increase. Otherwise the increase will apply to OCI resources.
...
- Use `/27` only as a lower bound when available IP addresses are constrained.
...
- Multiple protected databases may share the same Recovery Service subnet.

For OCI databases, Recovery Service can automatically register the backup subnet if it has sufficient free IP addresses. You may use the automatically registered subnet or register your own.

### Security Rules

Recovery Service requires stateful network access on two destination ports:
...
If both security lists and NSGs are present, validate NSG rules carefully because NSG rules take precedence for attached VNICs.
...
If a Recovery Service subnet exhausts its available private IP addresses:

- Add another subnet to the Recovery Service subnet resource, or
- Use a different Recovery Service subnet.

When replacing a subnet, add the new subnet first, then remove the old subnet. A Recovery Service subnet must always have at least one subnet associated with the database VCN.
...
Oracle-defined policies cannot be modified. Silver is the default policy when automatic backups to Recovery Service are enabled.

### Custom Policies

Create a custom policy when the Oracle-defined policies do not satisfy your retention, lock, or multicloud backup-location requirements.

Custom policy rules:

- Retention must be between `14` and `95` days.
- Retention lock is optional.
- For Oracle Database@Azure, Oracle Database@Google Cloud, and Oracle Database@AWS, you can choose whether backups remain in Oracle Cloud or in the same cloud provider as the database.

Caution: The "Store backups in the same cloud provider as the database" option cannot be undone after the policy is created.
...
- During the delay period, you can adjust retention or disable the lock.
- Once the lock takes effect, it cannot be disabled.
- After the lock takes effect, retention can only be increased, up to 95 days.
- Backups cannot be modified or deleted until the retention period ends.
- A protected database cannot be reassigned to a different policy while its current policy is permanently locked.
...
Do not rely on separate manual operational backup destinations once Recovery Service automatic backups are active.
...
|---|---|---|
| `Protected` | Recovery Service can recover to any point in the full recovery window. Data loss exposure is within the expected threshold. | Continue normal monitoring. |
| `Warning` | Recovery is available within the current recovery window, but data loss exposure exceeds the expected threshold. | Check backups, redo shipping, network, metrics, and work requests. |
...
For active protected databases, the details page refreshes Health and Data loss exposure approximately once per minute.

---

## Skill 6: Enable Real-Time Data Protection

Use this skill when the database requires lower RPO than scheduled backups can provide.
...
After enabling, verify the protected database details page shows real-time protection enabled and that Data loss exposure remains within the expected threshold.

---

## Skill 7: Create Long-Term Retention Backups

Use this skill for compliance, regulatory, or business-retention backup needs that extend beyond the automatic backup recovery window.
...
Recovery Service deletes the LTR backup once its retention period ends.
...
|---|---|---|
| `SpaceUsedForRecoveryWindow` | GB | Track backup storage used to satisfy the recovery window. |
...
Run a dry run before creation to detect:
...
Use this skill when creating event rules or investigating who modified Recovery Service resources.
...
Use this skill when a protected database backup fails, health changes to Warning or Alert, or data loss exposure increases.
...
If `TNS-12535: TNS:operation timed out` or a similar timeout appears, verify:
...
- Whether custom DNS correctly resolves Recovery Service hostnames
...
- Re-run the failed workflow after capacity is restored.
...
Use this checklist when setting up protection for a new OCI or Oracle Multicloud database.
...
6. Confirm the Recovery Service subnet is IPv4-only and appropriately sized.
...
11. Decide whether multicloud backups should remain in Oracle Cloud or the source cloud provider.
