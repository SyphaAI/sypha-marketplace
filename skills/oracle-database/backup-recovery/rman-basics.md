...
RMAN (Recovery Manager) is Oracle's native backup and recovery utility. Unlike OS-level file copies, RMAN is aware of Oracle's internal block structure and can validate blocks for corruption, skip unused space, perform incremental backups, and interface with Oracle's media management layer for tape.

This guide covers the day-to-day RMAN commands a DBA requires to operate, configure, and report on backups. For broader coverage of architecture, retention policies, and recovery scenarios, consult `rman-overview.md`.

---

## Connecting to RMAN

RMAN establishes a privileged connection to the target database and can optionally connect to a recovery catalog, which is a separate Oracle schema that stores backup metadata.
...
Channels are server processes responsible for backup and restore I/O. They can be configured persistently (saved in the control file or catalog) or allocated manually within a `RUN` block for a single session.
...
Manual allocation overrides automatic channel settings for the duration of that `RUN` block only:
...
The `BACKUP` command produces backup sets or image copies of the database, tablespaces, datafiles, archived logs, control files, and SPFILEs.
...
Tags provide logical names for backup sets, making them easier to identify at restore time:
...
`RESTORE` copies backup pieces or image copies back to their original or alternate locations. It does not apply redo — that step is handled by `RECOVER`.
...
`RECOVER` applies archived redo logs and incremental backups to advance restored datafiles to a consistent state.
...
`CROSSCHECK` confirms that backup pieces and image copies recorded in the RMAN repository are present at their expected locations. Any that are missing are marked `EXPIRED`.
...
`DELETE` removes backup pieces or image copies from storage and from the RMAN repository.
...
`LIST` displays information about backups and copies stored in the repository.
...
`REPORT` delivers higher-level analysis than LIST.
...
RMAN supports several compression algorithms. Higher compression levels conserve more space but consume more CPU resources.
...
RMAN can encrypt backup sets so backup pieces are unreadable without the correct key. Transparent (keystore-based) encryption of backups to disk requires the **Oracle Advanced Security** option, while password-based encryption does not.
...
For transparent (keystore-based) encryption, the Oracle Wallet (TDE keystore) must be open prior to running backups:
...
Use `ADMINISTER KEY MANAGEMENT SET KEYSTORE OPEN` on current Oracle releases instead of relying on older wallet-opening syntax.

---

## Parallelism

Parallelism enables multiple channels to operate concurrently, substantially improving backup and restore throughput.
...
RMAN spreads datafiles across available channels, with each channel writing to a distinct backup piece.
...
- **Always run `CROSSCHECK BACKUP` before `DELETE OBSOLETE`** to make sure expired pieces are cleared from the repository before obsolete ones are removed.

- **Use `BACKUP DATABASE PLUS ARCHIVELOG DELETE INPUT`** as a single command to prevent archived logs from building up on disk.

- **Set `CONTROLFILE AUTOBACKUP ON`** without exception. The downside is negligible.

- **Use tags** on all backup operations to simplify identification at restore time.

- **Schedule `REPORT NEED BACKUP`** as part of a monitoring job to detect datafiles that missed a backup cycle.

- **Limit channel parallelism** to what the storage I/O can support. More channels than storage can handle creates contention and reduces overall throughput.

- **Run `RESTORE DATABASE VALIDATE`** periodically in a maintenance window to confirm backup recoverability without performing an actual restore.
...
RMAN requires `SYSDBA` or `SYSBACKUP` privilege on the target. Connecting without these privileges produces cryptic errors.

**Forgetting `DELETE INPUT` when backing up archived logs**
Archived logs accumulate rapidly. Always pair archivelog backups with `DELETE INPUT` (backed-up logs) or `DELETE ALL INPUT` (all archived logs regardless of backup status — use with caution).

**Not using `PLUS ARCHIVELOG`**
A database backup taken without archived logs cannot be recovered to a consistent state. Always incorporate archived logs in the backup strategy.

**Misunderstanding `DELETE ARCHIVELOG ALL`**
`DELETE ARCHIVELOG ALL` removes all archived logs regardless of backup status. Use `DELETE ARCHIVELOG ALL BACKED UP 2 TIMES` to safely remove only logs that have at least two backup copies.

**Confusing VALIDATE with RESTORE VALIDATE**
- `VALIDATE BACKUPSET` checks the integrity of backup pieces (can RMAN read them?)
- `RESTORE DATABASE VALIDATE` simulates a complete restore to confirm all required pieces are present and readable
- Only `RESTORE DATABASE VALIDATE` indicates whether recovery is actually feasible

**Manually deleting backup files without notifying RMAN**
Deleting backup pieces using OS commands causes RMAN's repository to fall out of sync. Always use `CROSSCHECK BACKUP` followed by `DELETE EXPIRED BACKUP` to clean up.

**Not adjusting `CONTROL_FILE_RECORD_KEEP_TIME`**
Without a catalog, backup records in the control file can be overwritten. The default is 7 days. Raise this value to align with your retention policy.
```sql
