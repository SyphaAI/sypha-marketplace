...
Encryption in Oracle protects data from unauthorized access at the storage layer — meaning that even if an attacker obtains your datafiles, backup tapes, or tablespace exports, they cannot read the data without the encryption keys. This is a separate protection layer from access control (which prevents unauthorized queries) and auditing (which detects unauthorized access). Together, these layers form a defense-in-depth strategy.

Oracle offers two primary encryption mechanisms:

- **Transparent Data Encryption (TDE)**: Encrypts datafiles, tablespaces, and individual columns on disk. Encryption and decryption are transparent to applications — queries, DML, and DDL behave exactly as before.
- **DBMS_CRYPTO**: A PL/SQL API for encrypting specific values within application logic, suitable for cases where the application needs direct control over encryption beyond what TDE provides.

TDE requires the **Oracle Advanced Security Option**, which is a separately licensed add-on for Oracle Database Enterprise Edition in on-premises deployments (including 19c). It is included at no extra cost in certain Oracle Cloud database service tiers (BaseDB EE-HP, BaseDB EE-EP, ExaDB). Always confirm current licensing with Oracle's official Licensing Information User Manual for your specific version and deployment type.
...
A password-protected wallet must be opened manually after every database restart. An auto-login wallet opens automatically:
...
Encrypting entire tablespaces is the most widely used and recommended TDE deployment. All objects created within an encrypted tablespace (tables, indexes, LOBs, undo data) are encrypted automatically.
...
Column-level TDE encrypts individual columns rather than whole tablespaces. It offers finer granularity but introduces more overhead and constraints (encrypted columns cannot use standard indexes, and column values remain stored encrypted in the buffer cache until decrypted for processing).
...
Periodic key rotation is a security best practice and a compliance requirement under many frameworks. Oracle TDE supports re-keying without requiring the database to go offline.
...
When an application needs direct control over encryption — for example, encrypting specific values before storing them, or encrypting data destined for an external system — use `DBMS_CRYPTO`:
...
TDE-encrypted tablespaces are automatically backed up in encrypted form by RMAN. For non-TDE databases or additional backup encryption:
...
1. **Encrypt tablespaces, not just columns**: Tablespace encryption is simpler to administer, carries less performance overhead than column-level encryption, and protects all data including indexes, undo, and temp data.

2. **Use AES256**: It is the strongest algorithm Oracle supports for TDE, is FIPS 140-2 compliant, and the performance difference compared to AES128 is negligible on modern hardware.

3. **Use a local auto-login wallet in production only when the server itself is physically secured**: An auto-login wallet file (cwallet.sso) opens without a password. If the file is stolen together with the datafiles, the data can be decrypted.

4. **Back up the wallet in a location separate from the database**: If you lose your wallet and your datafiles are encrypted, your data is permanently inaccessible. Store wallet backups in a separate, secured location.

5. **Enable wallet backups before rotating keys**: Always use `WITH BACKUP` when rotating keys. The backup preserves the old key so you can decrypt older data if required.

6. **Audit wallet operations**: All `ADMINISTER KEY MANAGEMENT` commands should be audited. A DBA who rotates keys without authorization can render data inaccessible.

7. **Test decryption after enabling encryption**: After applying TDE to a tablespace, always run a test query to confirm data is readable. Verify the wallet is the expected version.

8. **Document your key management procedures**: In a disaster recovery scenario, the steps to restore and open the wallet are time-critical. Written runbooks must be tested and stored securely.
...
Column-level TDE with SALT prevents standard index usage (because every encryption of the same value produces a different result). With NO SALT, equality queries work but frequency analysis attacks become feasible.
...
Data Pump exports (`expdp`) decrypt data before export. The resulting dump file is unencrypted unless you explicitly encrypt it:
