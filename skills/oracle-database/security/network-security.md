...
Oracle's network security stack encompasses the Oracle Listener, Oracle Net Services (formerly SQL*Net), `sqlnet.ora` configuration, the Oracle Wallet, and Access Control Lists (ACLs) for outbound network calls initiated from within the database.

---

## The Oracle Listener

The listener is the entry point to the database. It accepts incoming connection requests and routes them to Oracle server processes. A misconfigured listener is among the most prevalent Oracle security vulnerabilities.
...
Configuring SSL/TLS for Oracle connections requires storing the server certificate in the Oracle Wallet and updating `sqlnet.ora` on both the server and client sides.
...
Oracle Advanced Security also provides native **network data encryption** through `sqlnet.ora` without requiring certificates or a wallet. This approach is simpler to deploy than TLS but offers less assurance (no server identity verification).
...
Oracle PL/SQL packages such as `UTL_HTTP`, `UTL_TCP`, `UTL_SMTP`, and `UTL_FILE` allow the database to make outbound network connections. Without ACLs, these packages can be exploited to exfiltrate data, launch internal network attacks from the database server, or reach internal services.
...
To make HTTPS calls from UTL_HTTP with remote certificate verification, the CA certificate of the remote server must be present in the Oracle Wallet:
...
Oracle Net provides a native IP allowlisting capability via the `valid_node_checking` parameter in `sqlnet.ora`:
...
Valid node checking is evaluated by the listener before the connection reaches the database, making it an efficient first line of defense.
...
2. **Use TLS 1.2 or 1.3 minimum**: Disable SSL 3.0, TLS 1.0, and TLS 1.1. These versions have well-known vulnerabilities (POODLE, BEAST, etc.).

3. **Restrict connecting hosts via `TCP.VALIDNODE_CHECKING`**: Allowlisting application server IPs at the listener level blocks unauthorized scans and connection attempts before they reach the Oracle authentication layer.

4. **Never expose the listener directly on the public internet**: The database listener should be reachable only from your application tier servers. Use a connection proxy or equivalent if external access is required.

5. **Remove the extproc listener entry unless it is explicitly needed**: The external procedure listener is a common attack vector that can enable OS-level code execution from within the database.

6. **Grant `UTL_HTTP`, `UTL_TCP`, and `UTL_SMTP` only to specific users**: These packages can be used for data exfiltration by issuing outbound HTTP calls containing sensitive data. Tightly control who can use them and to which destinations.

7. **Set `ADMIN_RESTRICTIONS_LISTENER = ON`**: This prevents remote `LSNRCTL` commands from reconfiguring the listener (e.g., changing log file paths to overwrite system files).

8. **Rotate SSL certificates before they expire**: Expired certificates cause outages. Set calendar reminders 90 days before expiry for all Oracle wallet certificates.
...
Without `SSL_SERVER_CERT_DN`, clients will accept any valid certificate, leaving the connection open to man-in-the-middle attacks.
...
- Network encryption combined with checksumming (`SQLNET.CRYPTO_CHECKSUM_SERVER = REQUIRED`) together satisfy this requirement
