...
Data masking and redaction are complementary security controls that protect sensitive data from unauthorized disclosure. They serve distinct purposes:

- **Oracle Data Redaction** (`DBMS_REDACT`) masks data dynamically at query time, in memory, before results leave the database engine. Data on disk remains unchanged. This guards against unauthorized SQL access and application-layer exposure.
- **Oracle Data Safe / Advanced Masking** permanently transforms data in non-production copies. Once masked, the original values are irretrievable. This protects sensitive data in development, test, and UAT environments.

Both features address a fundamental challenge: organizations routinely need to expose sensitive data (PII, PCI, PHI) to developers, testers, analysts, and third-party applications — yet those audiences should never encounter real values.

Oracle Data Redaction requires **Oracle Database Enterprise Edition 12c+** (bundled with Advanced Security Option in 12c, included in EE in 19c+). Oracle Data Safe is a cloud service (free tier available in OCI).

---

## Oracle Data Redaction (DBMS_REDACT)

Data Redaction transparently alters query results for users who lack the `EXEMPT REDACTION POLICY` privilege. The modification occurs after the query executes but before results are returned to the client — so indexes, queries, and DML are unaffected.
...
Full redaction substitutes the entire value with a default: `0` for numbers, a single space `' '` for character types, and `01-JAN-01` for dates.
...
Partial redaction retains the format and some characters of the original value, masking only the sensitive portion. This is the right approach for credit card numbers, SSNs, and phone numbers.
...
Regex redaction is the most adaptable option, applying `REGEXP_REPLACE`-style pattern matching to selectively mask data.
...
Random redaction returns a randomly generated value on each query, preserving the data type. This is appropriate when realistic-looking but fabricated data is acceptable to the consuming application.
...
For non-production copies, data must be permanently transformed before it is delivered to developers, testers, or third parties. Oracle provides several approaches.
...
- Sensitive data discovery (automatically identifies PII, PCI, PHI)
...
Before masking, you must identify which columns hold sensitive data. Oracle Data Safe automates this process, but a pattern-based search can be performed manually:
...
1. **Apply redaction at the base table level**: Attach policies to base tables rather than views. Views built on top of redacted tables inherit the redaction automatically.

2. **Use conditional expressions for role-based redaction**: A policy with `expression => '1=1'` always redacts. Use `SYS_CONTEXT` expressions to permit privileged roles to view real data.

3. **Do not treat redaction as your sole control**: Redaction is a defense-in-depth measure. Users with direct export tools (Data Pump, SQL*Plus SPOOL) or backup access can bypass it. Pair redaction with privilege management and auditing.

4. **Never use actual production data in non-production environments**: Even when production has redaction in place, non-production copies should contain permanently masked data. Sensitive information can leak through logs, error messages, and debugging tools in too many ways.

5. **Audit access to redacted columns**: Enable fine-grained auditing (see `auditing.md`) on tables with redaction policies so that unusual access patterns can be detected.

6. **Document your masking strategy**: For compliance purposes, you must be able to demonstrate which columns in which tables are masked and describe the masking technique applied.

7. **Test redaction using a non-privileged account**: Always verify that policies behave correctly by connecting as a user who should see masked data and confirming the output.
...
Data Pump (`expdp`), SQL*Plus `COPY`, database links, and GoldenGate can all bypass Data Redaction because they run with DBA-level privileges or use direct path access.
...
If a column is fully redacted but an index on it allows binary search, a determined attacker can use bisection to deduce the real value. Combine redaction with access controls.

### Mistake 3: Not Testing the expression Parameter

An incorrectly written expression can cause the policy to never apply (everyone sees real data) or always apply (privileged users also see masked data):
...
### HIPAA
- The Safe Harbor de-identification standard mandates removal or masking of 18 specific identifiers (name, address, dates, phone, SSN, medical record numbers, etc.).
- Data Redaction satisfies this requirement for query-time access control.
- For data shared externally, permanent masking is required.

### GDPR
- Personal data must not be more accessible than necessary for its stated purpose.
- Data Redaction helps enforce purpose limitation at the database layer.
- For the right to erasure, permanent deletion or permanent masking is required — dynamic redaction alone is not sufficient.
