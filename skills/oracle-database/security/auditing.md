...
Database auditing produces an immutable record of who performed which actions, when, and from where. It is a foundational control for regulatory compliance, forensic investigation, and insider threat detection. Oracle offers several auditing mechanisms, but the primary and recommended approach since Oracle 12c is **Unified Auditing**.

Unified Auditing supersedes the older fragmented auditing system — which consisted of standard auditing, fine-grained auditing, OS auditing, and SYS auditing — with a single, coherent framework. All audit records from every source are written to the `UNIFIED_AUDIT_TRAIL` view and are managed through a consistent set of commands.
...
By default, Oracle 12c databases operate in "mixed mode" where both traditional and unified auditing are active. Switching to Pure Unified Auditing requires relinking the Oracle executable:
...
Audit policies specify what to capture. They are defined once and can be activated for all users or for specific users only.
...
The `WHEN` clause restricts the policy so that it fires only when a specific condition holds. This can substantially reduce audit volume by narrowing capture to exactly the cases of interest.
...
Privileged users such as `SYS` can bypass standard security controls. Oracle provides mechanisms to audit even these accounts.
...
Fine-Grained Auditing (FGA) enables auditing of SELECT statements that access specific columns and optionally captures the actual query text. In unified auditing mode, `DBMS_FGA` policies are still supported and their records appear in `UNIFIED_AUDIT_TRAIL`.
...
The unified audit trail resides in the `AUDSYS` schema within the `SYSAUX` tablespace by default. For databases with high audit volumes, consider relocating it to a dedicated tablespace.
...
1. **Always audit authentication events**: Logon failures are the primary indicator of brute-force attacks and unauthorized access attempts. Logon successes establish the baseline for anomaly detection.

2. **Audit all DDL**: Schema changes can destroy audit trails, drop tables, or introduce backdoors. Every `CREATE`, `ALTER`, `DROP`, and `TRUNCATE` in your application schemas must be audited.

3. **Audit all privilege and role grants**: These are change events in your access control model. Under SOX and PCI, they must be reviewed.

4. **Use `DBMS_FGA` for sensitive column access**: When you need to know exactly which query accessed salary data, FGA is the appropriate tool. SQL text is captured in `UNIFIED_AUDIT_TRAIL.SQL_TEXT`. Note: the `audit_trail` parameter (DBMS_FGA.DB + DBMS_FGA.EXTENDED) is desupported in Oracle 23ai — all FGA records automatically land in the unified audit trail.

5. **Store audit records separately from the audited database**: Ideally, ship audit records to a SIEM or a separate audit database in real time. A DBA who can delete audit records can conceal their own activity.

6. **Do not over-audit SELECT on high-traffic tables**: Auditing every SELECT on a table with millions of reads per day will exhaust your audit tablespace quickly and may degrade performance. Use FGA with conditions to focus on what matters.

7. **Review audit policies on a quarterly basis**: Policies that were relevant six months ago may no longer apply. Retire unused policies and introduce new ones as the application evolves.

8. **Verify that policies fire**: After creating a policy, confirm it generates records by executing the audited action and querying `UNIFIED_AUDIT_TRAIL`.
