...
Privilege management is the cornerstone of Oracle database security. Oracle uses a discretionary access control (DAC) model in which users are granted explicit rights to perform operations or access objects. Getting privilege management right is the single most impactful security control in any Oracle deployment — overly permissive privilege grants are the root cause of the vast majority of database breaches and insider threats.

Oracle privileges fall into two major categories: **system privileges** (the right to perform a class of operation anywhere in the database) and **object privileges** (the right to perform a specific operation on a specific object). Both types can be granted directly to users or collected into **roles** for easier administration.

---

## System Privileges

System privileges grant the ability to perform actions that affect the database as a whole or objects in any schema. Oracle has over 200 system privileges. The most dangerous are those containing the `ANY` keyword.
...
Oracle supports UPDATE and REFERENCES grants at the column level, providing a powerful tool for least-privilege enforcement:
...
Roles are named collections of privileges that simplify privilege administration. Rather than granting individual privileges to each user, you grant them a role. Roles can contain both system and object privileges, and roles can be granted to other roles.
...
The principle of least privilege holds that any user, application, or process should have only the minimum access rights necessary to perform its function and nothing beyond that.
...
Oracle 12c+ includes the `DBMS_PRIVILEGE_CAPTURE` package, which enables you to capture which privileges a user actually exercises during a period of activity. This is the definitive tool for right-sizing privileges.
...
Granting privileges to `PUBLIC` makes them available to every user in the database, including any users created in the future. This is extremely risky for sensitive objects.
...
1. **Use schema separation**: The account that owns objects should be locked and never used for application connections. Application service accounts should hold only grants on the objects they require.

2. **Never grant DBA to application accounts**: The DBA role confers almost every privilege in the database. Application service accounts must never hold it.

3. **Prefer roles over direct grants**: Roles make privilege management more auditable and easier to revoke. Grant privileges to roles, then grant roles to users.

4. **Avoid `WITH ADMIN OPTION` and `WITH GRANT OPTION`**: These allow privilege propagation outside your control. Grant them only to DBA accounts and only when strictly required.

5. **Never grant to PUBLIC**: Anything granted to PUBLIC is accessible to everyone. Even apparently harmless packages can be leveraged for SQL injection or data exfiltration.

6. **Run DBMS_PRIVILEGE_CAPTURE before major releases**: Capture the privileges used in a staging environment and ensure the production account has exactly those privileges — no more.

7. **Audit DBA_SYS_PRIVS regularly**: Schedule a weekly or monthly job to report on any new system privilege grants and have them reviewed by a responsible party.

8. **Apply the principle of separation of duties**: The user who creates data should not be the same user who can delete or export it.
...
The `RESOURCE` role grants `CREATE TABLE`, `CREATE PROCEDURE`, `CREATE SEQUENCE`, and other object-creation privileges. Application service accounts should never have the ability to create their own objects.
...
Without unified auditing enabled for privilege grants, there is no record of who granted what. Always audit `GRANT` and `REVOKE` statements (see `auditing.md`).

### Mistake 5: Granting Privileges Without Time Limits

Oracle does not natively support privilege expiry, but you can implement it:
...
- Users accessing cardholder data must have the minimum privileges necessary
