...
SQL injection is a code injection technique in which an attacker embeds malicious SQL fragments into a query by manipulating user-supplied input. In Oracle environments, injection can occur in both static application code and dynamic PL/SQL. The outcomes range from unauthorized data access and data manipulation to privilege escalation and complete database compromise.
...
Defense in depth means applying validation at multiple layers. Bind variables protect data values; these techniques address everything else.

### Application Tier Validation

- Validate data types before sending them to the database (confirm that numeric inputs are actually numeric).
...
- **Validate dynamic identifiers** (table/column names) with `DBMS_ASSERT` functions or explicit whitelists before interpolating them into SQL strings.
- **Never build SQL from unvalidated input** of any kind, even from internal sources — insider threat and compromised middleware are real concerns.
- **Apply least privilege** to database accounts used by applications. An account with only `SELECT` on specific tables cannot drop tables even if injected SQL executes.
- **Log and alert on `DBMS_ASSERT` exceptions.** Repeated assertion failures are a strong indicator of active probing.
- **Audit sensitive queries** using Fine-Grained Auditing (FGA) so that injection attempts leave a trace.
...
- **Virtual Private Database (VPD)** automatically appends predicates to queries based on application context, restricting data exposure even if a query is injected.
...
- The `CURSOR_SHARING = FORCE` parameter can enforce bind variable use system-wide at the cost of some optimizer accuracy — it is a last resort, not a replacement for proper bind variable use in application code.
- Oracle **SQL Firewall** (21c+) can whitelist exactly which SQL statements are permitted from each database account, blocking any SQL that does not match the learned baseline.
