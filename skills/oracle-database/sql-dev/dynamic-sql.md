...
Dynamic SQL describes SQL statements that are assembled and run at runtime instead of being compiled at parse time. It is required whenever the SQL structure itself — table names, column names, the number of bind variables, or entire clauses — cannot be determined until execution. Oracle offers two mechanisms: `EXECUTE IMMEDIATE` handles the majority of use cases, while `DBMS_SQL` covers advanced scenarios that require a truly variable number of bind variables or cursor sharing across procedure boundaries.

Dynamic SQL carries risks — chiefly SQL injection and performance overhead from hard parsing — that must be handled with care. This guide explains how to use both mechanisms safely and efficiently.

---

## When to Use Dynamic SQL

Use dynamic SQL when a static SQL statement cannot express what you need:

- A table name or column name is resolved at runtime
- The number or names of bind variables vary between executions
- DDL must be run from PL/SQL (DDL statements cannot appear in static PL/SQL)
- The WHERE clause is assembled conditionally depending on which filter parameters are supplied
- A SQL string passed in as a parameter needs to be executed

Do **not** use dynamic SQL:
- When static SQL is capable of the same task (static SQL is always faster and safer)
- To bypass privilege constraints by executing SQL under a different schema
- When the "dynamic" portion is actually a small, predictable set of values (use CASE instead)

---

## EXECUTE IMMEDIATE

`EXECUTE IMMEDIATE` is the recommended dynamic SQL mechanism for the overwhelming majority of situations. It parses and executes a string in one step.
...
Static PL/SQL has no ability to execute DDL directly. `EXECUTE IMMEDIATE` is the standard approach for working around this restriction:
...
Ref cursors (SYS_REFCURSOR) enable you to return a dynamic result set to the caller. This is the established pattern for delivering dynamic query results to a calling application or to another PL/SQL block.
...
`DBMS_SQL` is the lower-level dynamic SQL API. It demands more code but provides support for:

- A variable number of bind variables (no branching logic needed)
- Fetching into a variable number of output columns
- Reusing a parsed cursor across several executions (lowering parse overhead)
- Converting between a `DBMS_SQL` cursor and a `REF CURSOR`
...
Oracle permits conversion of a `DBMS_SQL` cursor to a `SYS_REFCURSOR` after binding and execution, which is convenient for handing results back to a calling application:
...
**Recommendation:** Start with `EXECUTE IMMEDIATE` as the default. Move to `DBMS_SQL` only when:
- The number of bind variables or output columns is unknown at compile time and branching logic would become unworkable
- A parsed cursor must be reused across multiple calls (the parse-once, execute-many pattern)
- `TO_REFCURSOR` conversion is required

---

## Avoiding SQL Injection in Dynamic SQL

Dynamic SQL is the main attack surface for SQL injection in PL/SQL. The rules are:
...
Any element that is not an identifier — sort direction, SQL keywords, operator symbols — must be validated against an explicit whitelist:
...
Internal systems, APIs, and middleware are all potential injection vectors. Validate input regardless of its origin:
...
Every distinct SQL string triggers a hard parse (parse, optimize, generate execution plan). Hard parses are costly and create contention on the shared pool latch.
...
When running the same dynamic statement multiple times in succession (for example, inside a bulk load loop), parse it once with `DBMS_SQL` and execute it N times:
...
`EXECUTE IMMEDIATE` works with `BULK COLLECT` and `FORALL` for dynamic DML operations:
...
- **Start with static SQL.** Introduce dynamic SQL only when static SQL is genuinely insufficient.
- **Bind all data values.** Never concatenate user input or any runtime data value directly into a SQL string.
- **Validate all structural elements** (table names, column names) using `DBMS_ASSERT` functions or explicit whitelists prior to concatenation.
- **Always close cursors.** Use `DBMS_SQL.IS_OPEN` checks in exception handlers to guard against cursor leaks.
- **Log the constructed SQL during development and testing.** Output the final SQL string via a debug logging call during development to verify its correctness before promoting to production.
- **Default to `EXECUTE IMMEDIATE` for simplicity;** reach for `DBMS_SQL` only when its additional features are actually required.
- **Use `CLOB` for lengthy dynamic SQL.** `VARCHAR2` is capped at 32,767 bytes in PL/SQL; complex queries with many columns or subqueries can exceed this limit.
- **Keep DDL out of transactional code.** DDL auto-commits; a DDL statement issued in the middle of a transaction will commit all preceding changes.
...
`EXECUTE IMMEDIATE` accommodates the `RETURNING INTO` clause for DML statements that need to capture column values from the row that was affected:
...
The `RETURNING INTO` bind positions are OUT binds and are placed after the `USING` clause, not within it.

---

## Column Whitelist Pattern Using the Data Dictionary

`DBMS_ASSERT.SIMPLE_SQL_NAME` verifies that a string is a syntactically valid SQL identifier but does not confirm that the column actually exists in the target table. For the highest level of security when column names originate from user input, validate them against the data dictionary:
...
Call this function before concatenating a column name into a dynamic ORDER BY, SELECT list, or WHERE clause.
...
Each unique SQL string triggers a hard parse. Track the ratio of hard parses to total parses; in a well-tuned OLTP system this figure should remain below 1%:
...
Dynamic SQL that generates many distinct SQL strings fragments the shared pool. Use the following query to locate cursors with a high parse-to-execute ratio, which indicates missing bind variables:
...
- The baseline guidance in this file applies to Oracle Database 19c unless a higher minimum version is explicitly stated.
- Features labeled 21c, 23c, or 23ai should be considered Oracle Database 26ai-capable features; retain 19c-compatible alternatives for mixed-version deployments.
- In environments that must support both versions, verify syntax and package behavior against both 19c and 26ai, as defaults and deprecations can differ between release updates.
- **Oracle 8i+**: `EXECUTE IMMEDIATE` and `OPEN cursor FOR dynamic_sql USING` were introduced; they replaced `DBMS_SQL` for most use cases.
- **Oracle 11gR2+**: `DBMS_SQL.TO_REFCURSOR` and `DBMS_SQL.TO_CURSOR_NUMBER` enable conversion between `DBMS_SQL` and `REF CURSOR` types.
- **Oracle 12cR1+**: `DBMS_SQL.RETURN_RESULT` enables implicit result sets from stored procedures.
- **Oracle 21c+**: Enhanced JSON support in dynamic SQL construction patterns.
- **All versions**: `DBMS_ASSERT` has been available since 10.2 — apply it consistently for all dynamic identifier validation.
