...
Oracle's Development Guide recommends performing as much work as possible within the database and using built-in set-based processing rather than transferring large data sets to the client for processing there. Oracle identifies this as a way to eliminate network overhead and take advantage of the database's set-oriented operations.

Treat this as the default principle for SQL design:

- Favor a single SQL statement over many small client round trips.
- Favor set-based SQL over procedural row-by-row processing when the operation can be expressed that way.
- Keep data-intensive work inside SQL when the database can accomplish it with set-oriented operations.
...
Oracle identifies bind variables as particularly important for SQL statements that are executed repeatedly, and names OLTP systems as the primary scenario where they matter. The same documentation also notes that the impact is negligible when SQL is not reused, and that bind variables are frequently undesirable in data warehouse systems because statements tend to be ad hoc and literal values can assist the optimizer in choosing plans.

Apply this rule selectively:

- Use bind variables for values that differ between executions of the same SQL statement.
- Expect the greatest benefit in OLTP workloads where the same statement text is executed repeatedly.
- Do not assume this guidance applies unchanged to ad hoc data warehouse queries.
...
Oracle's SQL Language Reference states that when two tables in a join share any column name, you must qualify all references to those columns throughout the query to prevent ambiguity. The same reference notes that if a join condition is omitted, Oracle returns a Cartesian product. For outer joins, Oracle recommends the `FROM` clause `OUTER JOIN` syntax rather than the Oracle join operator `(+)`.

Oracle also documents that the `join_clause` lets you specify join conditions independently of search or filter conditions in the `WHERE` clause. Maintain that separation explicitly:

- Place row-matching logic in `JOIN ... ON`.
...
Oracle also warns that if two tables in a join query have no join condition, Oracle returns their Cartesian product. The join documentation gives the concrete example that two 100-row tables produce 10,000 rows.
...
Oracle Database 12c Release 1 (12.1) introduced native SQL support for query row limits and row offsets through the `FETCH FIRST` and `OFFSET` clauses. Oracle's 19c `ROWNUM` documentation also states that the `row_limiting_clause` of the `SELECT` statement offers superior support for restricting the number of rows a query returns.

Oracle's `SELECT` documentation for 19c and 26ai adds one further important point: to obtain consistent results, specify the `order_by_clause` when using the `row_limiting_clause`.
...
Treat this as the primary row-limiting pattern:

- When row order matters, specify `ORDER BY`.
...
Oracle still documents `ROWNUM`, but the same 19c page states that the `row_limiting_clause` offers superior support for limiting rows. Reserve `ROWNUM` primarily for existing SQL that already uses it, or when you specifically need the pseudocolumn behavior Oracle documents.
...
Oracle addresses null handling in two relevant locations. The SQL Language Reference states to use `IS NULL` when testing for nulls and `IS NOT NULL` when testing for non-null values. It also notes that Oracle currently treats a character value with zero length as null, but this behavior may not persist in future releases.
...
Apply this guidance as written:
...
Oracle's Development Guide advises using the most specific type available, including the most precise length or precision. Oracle provides concrete examples such as `NUMBER(38)` instead of `NUMBER`, `CHAR(16)` instead of `CHAR(2000)`, and `VARCHAR2(30)` instead of `VARCHAR2(4000)`.

Apply that guidance directly when designing tables and query interfaces:

- Use `VARCHAR2` for variable-length character data. Oracle says `VARCHAR2` values are not blank padded, are more space efficient than `CHAR`, and typically perform better.
- Use `CHAR` when ANSI compatibility is required and trailing blanks are unimportant. Oracle says `VARCHAR2` is more space efficient and usually performs better.
- Use `NCHAR`, `NVARCHAR2`, and `NCLOB` only when national-character-set storage is needed. Oracle says that if the database character set is Unicode, define Unicode data with `CHAR`, `VARCHAR2`, and `CLOB` instead.
...
- Remember that `NUMBER` without precision and scale implies Oracle's maximum range and precision for that data type.
- Use `DATE` when second precision is sufficient and time zone information is not needed. Oracle documents that `DATE` stores hour, minute, and second, but not fractional seconds or time zone.
...
Oracle's SQL Language Reference recommends explicit conversions over implicit ones. The documented reasons include improved SQL statement readability, reduced dependence on context, reduced vulnerability to changes in conversion algorithms and defaults, and better performance. Oracle also states that if implicit data type conversion occurs in an index expression, Oracle Database might not use the index because the index is defined for the pre-conversion data type.
...
Oracle's SQL Tuning Guide provides a concrete index-related example with the predicate `WHERE char_col=1`. Oracle says that unless the index is a function-based index, the database indexes the values of the column — not the values of the column with the function applied — and this prevents index use.

Apply this guidance:

- Compare values using matching data types rather than relying on implicit conversion.
- Be especially careful with predicates on indexed columns.
- If the workload requires a function or expression in the predicate, Oracle documents function-based indexes as the appropriate index type.
...
- Oracle also says that if implicit conversion occurs in an index expression, Oracle Database might not use the index.
- Oracle's SQL Tuning Guide cites `WHERE char_col=1` as a typical application mistake that prevents use of a normal index unless the index is function-based.
...
Oracle's SQL Language Reference states that a virtual column is not stored on disk and its value is derived on demand from the defining expression. Oracle also says virtual columns can be used in queries, DML, and DDL statements, can be indexed, and can have statistics collected on them.

Apply this guidance when a derived value should be exposed at table level:

- Use a virtual column when the derived value should be referenced as a column in SQL, not only as an ad hoc expression within a single query.
...
- Keep the virtual-column expression scalar and based on columns in the same table. Oracle says the expression can reference only columns in the same table and the result must be a scalar value.
- Do not attempt to update a virtual column directly. Oracle documents that virtual columns cannot be updated directly.
...
- The expression cannot reference another virtual column by name.
- The expression cannot use a PL/SQL function that is not declared `DETERMINISTIC`.
- If the expression uses a deterministic user-defined function, the virtual column cannot be used as a partitioning key.
- If the virtual column expression refers to a deterministic user-defined function and that function is replaced, Oracle does not automatically update dependent statistics.

### Example: Query Expression Versus a Virtual Column

If you write the derived expression inline only, it remains just an expression within that statement:
...
Oracle documents function-based indexes as a means of accelerating queries that evaluate a function or expression in a predicate or in the `ORDER BY` clause. Oracle's 26ai Development Guide says they are appropriate when the function is expensive to compute or when many queries must evaluate the same expression. The same section advises against using them when the expression is not queried often enough to justify the extra work, or when the overhead of maintaining the index during `INSERT` and `UPDATE` operations outweighs the query benefit.
...
- If the index expression causes internal character conversion, changing session-level NLS settings other than `NLS_SORT` and `NLS_COMP` can cause queries to return incorrect results.
- If a user-defined function in the index becomes invalid or is dropped, Oracle marks the index `DISABLED`.
...
Oracle's function-based index example begins by creating the index:

```sql
CREATE INDEX upper_ix ON employees (UPPER(last_name));
```

Avoid assuming Oracle will always use the index once it is created. Oracle explicitly states that without the `WHERE` clause in the next query, Oracle Database may perform a full table scan.
...
- Process data inside the database and favor set-based SQL.
- Use bind variables for reused SQL, particularly in OLTP workloads.
...
- Test nulls with `IS NULL` and `IS NOT NULL`.
- Select the most specific data type, length, and precision that matches the data you store.
- Avoid implicit data type conversion in predicates, particularly on indexed columns.
- Use a virtual column when a derived value should behave like a named column in SQL.
- Use function-based indexes only when the same expression is queried frequently enough to justify the maintenance cost.
...
- The data type examples in this file stay within the 19c-compatible set of character, numeric, datetime, interval, and LOB types.
...
- The current Oracle behavior that treats zero-length character values as null is documented in both releases, but Oracle also notes this may not continue to be true in future releases.
