...
Sequelize is a well-established Node.js ORM with an official Oracle dialect backed by `node-oracledb`. It offers model definitions, associations, migrations, and a rich query interface.
...
Oracle does not support auto-increment in the same way as MySQL or PostgreSQL. Use a `beforeCreate` hook to fetch the next sequence value:
...
- **Always use `replacements`** in raw queries — never build SQL by interpolating values into strings.
- **Set `timestamps: false`** unless your Oracle table actually has `createdAt`/`updatedAt` columns.
- **Always declare `field`** on each column to map camelCase model attributes to the UPPER_CASE Oracle column names.
- **Use `beforeCreate` hooks** for Oracle sequence-based PKs.
- **Use the managed `sequelize.transaction(async t => {})`** form to avoid accidentally omitting commit or rollback.
...
- The Oracle 26ai `BOOLEAN` and `VECTOR` types must be accessed through raw SQL queries.
