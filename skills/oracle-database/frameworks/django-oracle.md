...
Django supports Oracle Database natively through its `django.db.backends.oracle` backend, which uses `python-oracledb`. The legacy `cx_Oracle` driver is deprecated in modern Django releases. Most Django ORM features function correctly with Oracle, but a number of Oracle-specific quirks require attention.
...
- On currently supported Oracle versions, Django relies on Oracle identity columns for `AutoField` / `BigAutoField`.
- **Table names** are uppercased by Oracle; Django quotes them to preserve case when required.
- **`db_table`** must match the exact Oracle table name, since Oracle uppercases unquoted identifiers.
...
When `ATOMIC_REQUESTS = True`, Django wraps each request in a transaction by default.
...
Oracle persists `''` (empty string) as `NULL` in `VARCHAR2` columns. As a result:

```python
# This will NOT match rows with empty strings — Oracle treats them as NULL
...
Oracle stores identifiers in uppercase. Django quotes table and column names derived from `db_table` and `db_column`. When writing raw SQL, use uppercase identifiers:
...
Django correctly translates `.limit(n)` and `.offset(n)` to Oracle `ROWNUM` or `FETCH FIRST`:
...
- **Use `only()` and `defer()`** to skip large `CLOB`/`BLOB` columns when they are not needed.
- **Set `CONN_MAX_AGE`** in `DATABASES` to keep connections alive and reduce connection overhead.
- **Use `db_table`** to map models precisely to existing Oracle tables.
- **Validate migrations against a staging Oracle instance** — the generated SQL can differ from PostgreSQL/SQLite development databases.
...
- **Do not hardcode credentials** in Django settings:
  ```python
...
- **Integrate with Django secret management** or an external secret store:
  - Django SECRET_KEY or custom settings modules
  - HashiCorp Vault, AWS Secrets Manager, Azure Key Vault
  - Use django-environ or a comparable package for secure configuration
...
- **Use TCPS (TLS)** for all production connections:
  ```python
  DATABASES = {
      "default": {
          "ENGINE":   "django.db.backends.oracle",
...
- **Validate SSL certificates** to guard against man-in-the-middle attacks:
  - Confirm proper certificate chain validation in the Oracle client configuration
  - Consider certificate pinning for high-security environments

- **Consider Oracle Cloud IAM authentication** for cloud deployments:
  ```python
...
- **The Django ORM provides protection**, but developers must remain vigilant:

- **Safe: QuerySet methods** (the Django ORM uses parameterized queries internally):
  ```python
...
- **Exercise caution with the extra() method** (it can introduce SQL injection if misused):
  ```python
...
- **Grant the database user only the privileges it actually needs:**
  ```sql
...
- **Encrypt sensitive fields** at the application level before storing them:
  ```python
  from django.db import models
  import hashlib
...
- **Use Oracle Transparent Data Encryption (TDE)** to protect data at rest:
  - Configure it at the tablespace or column level in Oracle
  - Django operates transparently with TDE-encrypted tables

- **Consider Oracle Data Redaction** to mask sensitive data in query results:
  ```sql
...
- **Enable database logging** for monitoring purposes (development/test environments only):
  ```python
  LOGGING = {
      'version': 1,
...
- **Set client information** to enable traceability in database logs:
  ```python
  from django.db import connection
...
- **Handle LOB data carefully**, as it may contain sensitive information:

- **Validate and sanitize LOB content** before storing it:
  ```python
...
- **Encrypt sensitive LOB data** before writing it to storage:
  ```python
  from django.db import models
...
- **Keep transactions brief** to limit lock contention and exposure:

- **Select appropriate isolation levels** for your use case:
  ```python
  from django.db import transaction
...
- **Avoid long-running transactions** that hold locks or open snapshots:
  ```python
...
- **SAFE: Process records in batches using iterators or pagination:**
  ```python
  batch_size = 100
  offset = 0
...
- **Keep Django, python-oracledb, and all dependencies up to date:**
  - Routinely check for CVEs affecting Django, python-oracledb, and related packages
  - Integrate dependency scanning tools (OWASP Dependency-Check, Snyk, pip-audit) into CI/CD

- **Monitor for vulnerabilities specific to python-oracledb:**
  - Track Oracle's security patches for their Python drivers
  - Choose thick mode or thin mode based on your security requirements

### Compliance Considerations

- **PCI-DSS**:
  - Encrypt cardholder data (application-level or via TDE)
  - Limit access to cardholder data environments
  - Apply strong access control measures
  - Periodically test security controls and processes

- **HIPAA**:
  - Establish access controls and audit trails for PHI
  - Encrypt ePHI at rest and in transit where applicable
  - Enforce minimum-necessary access to protected health information
  - Implement audit controls and integrity verification

- **GDPR**:
  - Apply data protection by design and by default
  - Incorporate data minimization principles into model design
  - Build mechanisms for the right to erasure and data portability
  - Apply pseudonymization techniques where appropriate
...
- Django 4.2+ supports Oracle 19c and later versions.
- Oracle 26ai introduces a native `BOOLEAN` SQL type; however, Django's `BooleanField` continues to use `NUMBER(1)` for backwards compatibility.
