...
Dapper is a lightweight .NET micro-ORM that augments `IDbConnection` with convenient query and execute methods. It works directly with ODP.NET's `OracleConnection`, providing full raw SQL control alongside automatic result mapping.
...
### Dynamic Results (no class required)
...
Oracle returns column names in UPPERCASE. Dapper maps column aliases to property names in a case-insensitive manner. Always alias Oracle columns to align with C# property names:
...
- **Always alias columns** to match C# property names — this prevents silent mapping failures.
- **Use `DynamicParameters`** for OUT parameters and PL/SQL procedure calls.
- **Prefer async overloads** (`QueryAsync`, `ExecuteAsync`) in ASP.NET Core.
- **Reuse `OracleConnection`** — Dapper does not manage connection pooling itself; depend on ODP.NET's built-in pool.
- **Pass `transaction:`** to every Dapper call when executing within a transaction.
...
- The Oracle 26ai native `BOOLEAN` type maps to C# `bool` through standard ODP.NET binding.
