...
Pandas connects to Oracle through SQLAlchemy (`read_sql`, `to_sql`) or directly via `python-oracledb` cursors. It is a common choice for data extraction, transformation, reporting, and bulk loading workflows.
...
For tables that exceed available memory, read the data in chunks:
...
Oracle returns column names in UPPERCASE by default. Normalize them to lowercase:
...
By default, `to_sql` issues one INSERT per row. For bulk loads, use `chunksize` together with `method="multi"`:
...
For peak throughput, skip `to_sql` and use `python-oracledb` directly:
...
Always provide `dtype=` in `to_sql` when writing to Oracle to prevent short strings from being mapped to `CLOB`:
...
- **Perform aggregation in Oracle** — avoid pulling millions of rows into pandas just to group or filter. Write SQL that aggregates the data first.
- **Use `chunksize`** when reading large tables to prevent out-of-memory errors.
- **Always specify `dtype=`** in `to_sql` for Oracle — the default string mapping produces `CLOB`.
...
- **Lowercase column names** immediately after reading — Oracle uppercase names add noise to your code.
- **Set `index=False`** in `to_sql` unless you specifically want the DataFrame index written as a column.
...
- Oracle 21c+ native JSON: use `pd.read_sql` with `JSON_VALUE`/`JSON_TABLE` in the query to flatten JSON columns into DataFrame columns.
