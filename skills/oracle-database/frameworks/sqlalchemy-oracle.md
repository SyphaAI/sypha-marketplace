...
SQLAlchemy is the leading Python SQL toolkit and ORM. It connects to Oracle through the `oracle+oracledb` dialect (backed by `python-oracledb`) or the legacy `oracle+cx_oracle` dialect. SQLAlchemy exposes two distinct APIs:

- **Core** — SQL expression language, operating close to raw SQL
- **ORM** — complete object-relational mapping with sessions and models
