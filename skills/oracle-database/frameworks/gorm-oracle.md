...
GORM is the most widely used Go ORM. Oracle support is community-maintained rather than provided first-party. A current community dialect is `github.com/godoes/gorm-oracle`, whose README advises against production use. Teams requiring production-grade Oracle support often reach for `godror` with `database/sql` directly instead of a GORM dialect.
...
Community GORM Oracle dialects differ in implementation details and support level. Confirm behavior against your exact version before depending on features such as schema migration or auto-generated keys in production.
...
GORM's `autoIncrement` does not map to Oracle sequences. Use a `BeforeCreate` hook instead:
...
// Use with caution — does not drop or rename columns
db.AutoMigrate(&Employee{}, &Department{})

// Preferred: use SQL migration files for production
```
...
- **Use `BeforeCreate` hooks** for Oracle sequence-based PKs — never rely on `autoIncrement`.
- **Use `db.Transaction(func(tx) error {...})`** in preference to manual `Begin/Commit/Rollback`.
- **Pass `map[string]interface{}`** in `Updates` to prevent GORM from silently skipping zero-value fields.
- **Implement `TableName()`** on every model to enforce explicit Oracle schema-qualified names.
- **Use `Preload`** for associations rather than `Joins` for cleaner N+1 avoidance.
- **Do not use `AutoMigrate` in production** — control schema changes through explicit DDL scripts.
...
- All patterns shown work on Oracle 19c+.
- Oracle 26ai adds native `BOOLEAN` and modern `IDENTITY` column support at the database level, but actual GORM Oracle dialect behavior depends on the specific community driver version in use.
