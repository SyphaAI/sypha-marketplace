...
MyBatis is a SQL mapping framework that keeps developers in full control of their SQL while taking care of result mapping, parameter binding, and connection management. It is widely used in Oracle-heavy applications where precise SQL control outweighs the convenience of abstraction.
...
- **Always use `#{param}` (not `${param}`)** — `${}` performs string substitution and opens the door to SQL injection.
- **Use `<resultMap>`** for all non-trivial mappings — `map-underscore-to-camel-case` covers simple cases, but explicit maps are more reliable.
- **Use `<selectKey>` with Oracle sequences** to generate insert keys.
- **Apply `statementType="CALLABLE"`** to every PL/SQL call.
- **Separate SQL concerns** — keep complex SQL in XML mapper files rather than Java annotations.
- **Enable `log-impl`** in development to inspect generated SQL; turn it off in production.
...
- Oracle 26ai: The native `BOOLEAN` type can be mapped directly to Java `boolean` in result maps.
