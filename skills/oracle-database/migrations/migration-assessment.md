...
A thorough pre-migration assessment is the most critical phase of any database migration project. Cutting short or rushing the assessment is the single most common driver of budget overruns, missed deadlines, and production incidents. The assessment answers four fundamental questions: What needs to move? How complex is it? What will not translate automatically? And what is the risk?

This guide provides a comprehensive assessment checklist, a schema complexity scoring methodology, a framework for identifying incompatible features, effort estimation guidance, a risk matrix, and guidance on using Oracle's Database Migration Assessment tools.
...
Complete the following inventory before writing a single line of migration code:
...
Use this scoring model to estimate migration effort per object. Multiply the unit cost by the count to obtain total estimated days.
...
Run these queries against the source database to classify objects by complexity:
...
Use this formula as a starting point, then adjust for project-specific factors:
...
Score each risk factor on a 1–5 scale for Likelihood and Impact. Multiply the two scores to obtain a Risk Score.
...
When the target is OCI (Oracle Autonomous Database or DBCS), use the Oracle Cloud Migration Advisor:
...
1. **What is the source database version?** Older versions may carry deprecated features that add migration complexity.
...
5. **Are there any tables with no primary key?** These create problems for replication-based migration tools.

6. **What is the maximum acceptable downtime?** This drives the choice between offline (Data Pump / SQL*Loader) and online (GoldenGate / ZDM) migration approaches.

7. **Is the database schema stable or actively changing?** A schema in flux during migration requires change management procedures to capture DDL changes.

### Organizational Questions

8. **Who owns the application code?** Stored procedure changes may involve the application team, not just DBA work.

9. **What is the test coverage?** A well-tested application makes regression detection feasible. No test coverage means manual regression testing is required.
...
11. **What is the rollback plan?** If migration fails at cutover, how quickly can you revert? Is a point-in-time restore available? Is the source system kept running during parallel operation?

12. **What is the go-live criteria?** Define specific, measurable go/no-go criteria before work begins (see `migration-cutover-strategy.md`).

13. **Are there regulatory or contractual freeze periods?** Many organizations observe change freeze windows (year-end, quarter-end) that must be avoided for major migrations.

14. **What are the performance SLAs?** Identify the 10–20 most performance-critical queries. These must be benchmarked on Oracle before cutover.
...
2. **Complexity Assessment Report:** Per-object complexity scores with justification and the recommended approach for each.

3. **Incompatible Features List:** Specific features in the source database that have no Oracle equivalent or require substantial redesign.
...
7. **Go/No-Go Criteria:** Specific, measurable criteria that must be satisfied before production cutover.
