...
Cutover is the moment when production workload shifts from the source database to Oracle. It is the highest-risk phase of any migration — execution errors at this point can result in data loss, prolonged downtime, and emergency rollbacks. A well-designed cutover transforms this moment into a routine, predictable operational procedure rather than a crisis.

This guide covers the complete arc of cutover planning: phases, parallel run strategy, dual-write patterns, go/no-go criteria, rollback planning, minimizing downtime with Oracle GoldenGate, and stakeholder communication.
...
The cutover plan should be finalized and rehearsed before the migration project begins. During this phase:
...
The rehearsal should surface:
- Steps that require more time than estimated
...
- DBAs on standby to address performance issues (missing indexes, bad plans, lock contention)
- Application teams available to resolve any SQL compatibility issues found under production traffic
- Run drift detection queries every hour (see `migration-data-validation.md`)
- Decommission the source database only after this stabilization period, not immediately
...
The dual-write pattern sends every data change to both the source and Oracle simultaneously. This keeps Oracle fully synchronized without relying on batch replication, and enables instant cutover of reads.
...
For production migrations, Oracle GoldenGate delivers real-time, low-latency replication without requiring application changes:
...
During the parallel run period, GoldenGate continuously applies changes from the source to Oracle. At cutover, the only downtime required is the time needed for lag to drain to zero and the application to reconnect.
...
A rollback plan must be defined, documented, and rehearsed before cutover begins. Establish a clear rollback deadline — a specific time after cutover starts at which you will automatically initiate rollback if Oracle is not stable.
...
- Data corruption is detected (validation query returns unexpected results)
...
Oracle GoldenGate is the industry-standard tool for near-zero-downtime database migration. The following describes the high-level architecture and cutover procedure:
...
- Send a detailed impact summary: expected downtime, affected services
...
- Ensure everyone knows the rollback decision deadline

**Cutover day:**
- Send "maintenance starting" notification at window open
- Provide status updates every 30 minutes during window
- Send "migration complete" notification when service is restored
...
1. **Never skip the dry run.** Executing the cutover runbook in staging uncovers timing problems, missing steps, and team coordination gaps that would cause extended downtime in production.

2. **Define the rollback deadline in writing.** "We will roll back if Oracle is not stable by T+2 hours" must be agreed upon in advance by all stakeholders. Without a pre-committed deadline, pressure during the cutover can lead to continuing with an unstable system for too long.

3. **Make connection string changes atomic.** Use a load balancer, service mesh, or DNS-based redirect for Oracle connection switching. This enables instantaneous rollback of connectivity without restarting applications one by one.

4. **Do not decommission the source database immediately.** Keep the source database read-only and available for at least 72 hours after cutover as a safety net for unexpected issues.

5. **Plan for the unexpected.** Every migration encounters at least one issue not anticipated during planning. The cutover window must include buffer time (30–50% of estimated active work time) for unexpected debugging.

6. **Treat cutover as a product launch.** All the communication, monitoring, and stakeholder coordination disciplines of a major product launch apply here. The technical work is the easier part; coordination and communication determine the overall experience.
...
After data migration, Oracle sequences must be reset to values higher than any existing ID. If sequences are not updated before the application begins writing, INSERT statements will fail with ORA-00001 (unique constraint violated).
```sql
...
Oracle session time zone defaults to the OS time zone of the database server. If the application was connecting to a UTC source database but Oracle is configured for a local time zone, timestamp values may be offset. Set the Oracle session time zone explicitly:
```sql
...
Application users need appropriate privileges in Oracle. A common oversight is neglecting to grant access to sequences, synonyms, or procedures that were not in the original migration scope.

**Pitfall 5 — Performance regression on first production queries.**
Oracle's optimizer builds statistics over time. The first executions of complex queries may use suboptimal plans because statistics reflect the initial data load rather than the pattern of production queries. Monitor the top wait events in V$SESSION_WAIT immediately after cutover and proactively address any slow queries.
