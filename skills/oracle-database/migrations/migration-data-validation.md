...
Data validation is the most frequently underestimated phase of database migrations. The schema can be converted automatically, data can load without errors, and the migration can appear complete — only for the application to silently produce incorrect results because rows are missing, numeric values have been truncated, dates are offset by hours due to timezone differences, or NULL handling differs between the source and Oracle.

This guide provides a comprehensive, query-driven data validation framework. Apply these patterns immediately after the initial load, after each incremental sync, and as ongoing drift detection after cutover.
...
For partitioned tables, also validate row counts per partition:
...
`ORA_HASH(expr, max_bucket, seed)` returns a deterministic hash bucket number (0 to max_bucket) for any scalar expression. By concatenating all column values and hashing each row, then summing or XOR-ing the resulting row hashes, you can produce a "fingerprint" of an entire table's content.
...
The result on Oracle should match a corresponding hash computed on the source database. Because hash functions differ by platform, you need to use the same hash algorithm or compare intermediate results.
...
Full-table hash validation is not practical for billion-row tables during initial validation. Use stratified sampling to achieve statistical confidence in less time.
...
Verify that the largest or most significant records migrated correctly:
...
The most common NULL-related migration issue is the conversion of empty strings to NULL. Oracle 21c and earlier treat `''` as NULL. PostgreSQL treats `''` as an empty string distinct from NULL.
...
After cutover, if the source database remains online for fallback purposes or you are operating in parallel-write mode, continuously detect drift between source and Oracle.
...
After migration, verify that Oracle's constraints are actively enforced and that all migrated data conforms:
...
1. **Automate validation from the start.** Build validation queries into a script that can be executed with a single command. Manual validation is error-prone and tends to be skipped under time pressure.

2. **Validate incrementally, not only at the end.** Run row count checks after each table loads. Do not wait until all tables are loaded to detect a problem.

3. **Store source row counts in a staging table.** Extract source counts before migration begins and store them in Oracle. This provides a baseline for comparison even after the source database has been decommissioned.

4. **Test with edge case data explicitly.** Include rows containing NULL values, maximum-length strings, dates near Unix epoch boundaries, negative numbers, and zero values in your sample validation queries.

5. **Compare aggregate checksums for financial data.** For any table containing monetary values, compare SUM(amount), SUM(quantity), etc. between source and Oracle. A row count match does not guarantee that amounts are accurate.

6. **Document every validation failure and resolution.** Maintain a log of what failed, why it failed, and how it was resolved. This becomes the validation sign-off documentation for the go/no-go decision.
...
IEEE 754 floating-point arithmetic can yield slightly different results on different hardware. When comparing float columns across databases, apply a tolerance:
```sql
...
If the source database uses case-insensitive collation and values were normalized differently (e.g., 'ALICE' vs 'alice'), the data is present but appears different. Use `UPPER()` normalization in hash computations for string columns.

**Pitfall 4 — Row count can match but data is wrong:**
A matching row count guarantees only that the correct number of rows exists. Rows could be in the wrong order, have swapped column values, or be corrupted. Always supplement row counts with hash-based or aggregate-based validation for critical tables.

**Pitfall 5 — Partitioned table stats lag:**
Oracle's `ALL_TAB_PARTITIONS.NUM_ROWS` is populated from statistics, not a live count. Use `COUNT(*)` for validation rather than the statistics-based NUM_ROWS, until fresh statistics are gathered after the load.
