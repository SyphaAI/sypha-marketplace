...
Sybase Adaptive Server Enterprise (ASE), now SAP ASE, shares a common lineage with Microsoft SQL Server — both descended from Sybase's original database engine. Consequently, Sybase ASE and SQL Server have very similar Transact-SQL (T-SQL) dialects, procedural language syntax, and overall architecture. Much of what is covered in the SQL Server migration guide (`migrate-sqlserver-to-oracle.md`) applies equally to Sybase ASE.
...
**Important distinction:** Sybase `TIMESTAMP` is a binary(8) row version number (similar to SQL Server's ROWVERSION), NOT a date/time type. Applications that use Sybase TIMESTAMP for optimistic concurrency control must be redesigned in Oracle:
...
This is one of the most critical areas of a Sybase-to-Oracle migration.

### Sybase Transaction Modes

Sybase ASE operates in two modes:

**Unchained mode (default):** Each DML statement is its own implicit transaction. You must explicitly `BEGIN TRANSACTION` to group multiple statements.
...
**Migration implication:** Sybase applications running in unchained mode have auto-commit behavior for individual statements. When migrating to Oracle, uncommitted DML is visible within the same session but NOT to other sessions. Application connection management and transaction scoping must be reviewed carefully.
...
1. **Determine Sybase transaction mode before migrating application code.** Run `SELECT @@TRANCHAINED` in Sybase to check whether applications use chained (1) or unchained (0) mode. If unchained, explicit transaction management must be added for Oracle.

2. **Audit all @@ERROR checks.** Sybase code that checks `@@ERROR` after every statement is verbose and fragile. Oracle PL/SQL's exception handling block is cleaner — refactor error handling during migration rather than replicating the @@ERROR pattern.

3. **Test IDENTITY column migration carefully.** Sybase IDENTITY columns behave similarly to Oracle identity columns but have different starting and step options. Verify that migrated IDENTITY columns begin above the maximum existing ID after data migration.

4. **Handle TEXT and IMAGE columns specially.** Sybase TEXT/IMAGE LOBs require special handling in BCP and are stored differently from regular columns. Use the character format BCP export and validate LOB content sizes before importing into Oracle CLOB/BLOB.
...
6. **Review Sybase user-defined datatypes.** Sybase supports user-defined types that combine a base type with rules and defaults. Oracle uses domains (introduced in 23ai, available in 26ai) or simply base types with constraints.
...
The most frequent source of behavioral differences. In Sybase unchained mode, each DML statement auto-commits. In Oracle, nothing auto-commits. Applications that rely on data being committed immediately after a single UPDATE (e.g., for visibility to other sessions) will observe different behavior.

**Pitfall 2 — RAISERROR syntax:**
Sybase RAISERROR specifies the error number first, then the message. Oracle RAISE_APPLICATION_ERROR takes a negative number and a message string. Custom error numbers in Sybase can be any user-defined value; in Oracle they must fall in the range -20000 to -20999.
```sql
-- Sybase
...
As noted in the type mapping section, Sybase TIMESTAMP is NOT a datetime — it is a binary row version counter. Any Oracle code that treats this column as a date will fail. The optimistic concurrency pattern must be redesigned.

**Pitfall 4 — Subquery result comparison:**
Sybase permits some non-standard subquery comparisons. Oracle enforces strict subquery semantics. Review all `= (subquery)` patterns to ensure the subquery truly returns exactly one row, or add appropriate handling:
```sql
...
Sybase supports both page-level and row-level locking modes. Oracle uses MVCC and row-level locking exclusively. Applications designed around Sybase's locking behavior (especially those attempting to read uncommitted data) require MVCC review.

**Pitfall 6 — NULL handling in aggregates:**
Both Sybase and Oracle ignore NULL in aggregates (COUNT, SUM, etc.) following ANSI SQL — this is compatible. However, Sybase's `ISNULL()` function must be replaced with Oracle's `NVL()`:
```sql
-- Sybase
