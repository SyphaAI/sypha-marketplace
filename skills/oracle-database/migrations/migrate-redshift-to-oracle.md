...
Amazon Redshift is a columnar, massively parallel processing (MPP) data warehouse derived from a fork of PostgreSQL 8.0. Oracle Database is a traditional row-oriented RDBMS with optional in-memory columnar processing via the In-Memory Column Store (IMCS). Moving from Redshift to Oracle involves more than syntax translation — it requires a fundamental architectural shift: from distribution-key-based parallelism to shared-memory or RAC-based scale-out, and from a columnar storage model to a row-based heap with optional column store overlays.
...
Redshift is a shared-nothing MPP cluster. Every table is physically distributed across compute nodes based on a distribution key or a round-robin/ALL strategy. Queries are compiled and executed in parallel across all nodes, with each node processing its own local data slice.

Oracle has no direct equivalent to Redshift's distribution model. Oracle's parallelism works as follows:
...
- **Oracle RAC:** Multiple database instances share the same storage, with global cache coordination.
...
When transitioning from Redshift's columnar store:

1. **Row-based storage is the Oracle default.** Queries that scan millions of rows and aggregate them depend on Oracle's optimizer using indexes, partitioning, and parallel query. Add partitioning to large fact tables.

2. **Enable Oracle IMCS for analytical workloads.** Designate frequently scanned analytical tables for the in-memory column store:
...
Redshift's `COPY` command loads data from S3 (or other sources) directly into a table with MPP parallelism. Oracle provides two primary bulk loading mechanisms.
...
External tables allow Oracle to read files from the OS filesystem as though they were database tables. They are well suited for staging loads.
...
Redshift WLM allocates memory and concurrency slots across query queues. Oracle's equivalent facility is the Database Resource Manager (DBRM).
...
1. **Wide-table scans selecting few columns:** Redshift excels here because columnar storage only reads the needed columns. Oracle full-table scans read entire row blocks. Mitigate this with:
   - Oracle In-Memory Column Store (selected tables/columns)
   - Appropriate indexes on filter columns
   - Partition pruning

2. **Aggregate-heavy queries:** Redshift's vectorized columnar aggregation is highly efficient. Oracle's parallel query engine with IMCS can match this for in-memory data, but disk-based aggregations will generally be slower.
...
1. **Profile your Redshift queries before migrating.** Export STL_QUERY and STL_SCAN logs to understand which tables are scanned most frequently and with what filter predicates. This informs the Oracle partitioning and IMCS strategy.

2. **Do not over-index.** Redshift automatically uses zone maps (block-level min/max statistics). Oracle relies on B-tree indexes, which carry maintenance overhead. Add indexes only where queries demonstrably benefit.
...
**Pitfall 1 — Assuming Redshift SQL is standard SQL.** Redshift includes numerous PostgreSQL-derived extensions and AWS-specific functions. Not everything that runs in Redshift is valid standard SQL, and some of it will not translate to Oracle.

**Pitfall 2 — Distribution-key assumptions in application code.** Application code that uses distribution keys for routing (uncommon but possible in advanced ETL) has no Oracle equivalent. Redesign those patterns accordingly.

**Pitfall 3 — SUPER type migrations.** Redshift SUPER columns hold JSON-like semi-structured data. Map these to Oracle JSON columns (21c+) or CLOB with IS JSON constraints. PartiQL queries against SUPER need to be rewritten using Oracle's JSON_VALUE, JSON_TABLE, and JSON_QUERY functions.
...
**Pitfall 5 — Redshift late-binding views.** Redshift supports late-binding views that reference tables not yet present at view creation time. Oracle does not; all referenced objects must exist and be accessible when the view is compiled.
