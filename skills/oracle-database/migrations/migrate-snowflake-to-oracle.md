...
Snowflake is a cloud-native data platform built around a distinctive architecture: storage and compute are separated, virtual warehouses handle on-demand processing, and its SQL dialect blends PostgreSQL-style syntax with Snowflake-specific extensions. Oracle is a traditional shared-disk RDBMS deployable on-premises, in the cloud (Oracle Cloud Infrastructure), or as Oracle Autonomous Database. Migrations from Snowflake to Oracle typically involve analytical workloads moving into Oracle Autonomous Data Warehouse (ADW) or on-premises Oracle Data Warehouse setups.

Important architectural differences include Snowflake's semi-structured data types (VARIANT, OBJECT, ARRAY), Snowflake Time Travel versus Oracle Flashback, virtual warehouse sizing versus Oracle resource management, and SQL dialect divergence.
...
Snowflake uses colon `:` and dot `.` notation to traverse VARIANT data:
...
Snowflake Time Travel lets you query historical data up to 90 days in the past using `AT(TIMESTAMP => ...)` or `BEFORE(STATEMENT => ...)` syntax. Oracle's **Flashback** feature provides equivalent capability.
...
Snowflake Streams track DML changes on a table; Tasks are scheduled procedures that consume streams.
...
1. **Map Snowflake databases to Oracle PDBs or schemas systematically.** Document the hierarchy before migrating to prevent confusion about where objects will reside.

2. **Re-evaluate VARCHAR size constraints.** Snowflake VARCHAR can be up to 16 MB. Oracle VARCHAR2 is limited to 4,000 bytes (32,767 with extended). Measure actual column lengths and use CLOB for long text:
   ```sql
   -- Snowflake: survey max lengths
   SELECT MAX(LENGTH(description)) FROM products;
   ```

3. **Redesign VARIANT columns explicitly.** Each VARIANT column requires a concrete mapping decision: fully normalize, store as JSON, or use a Duality View. Make this choice early, based on actual query patterns.

4. **Replace Snowflake's schema-on-read with schema-on-write.** Snowflake VARIANT lets you store arbitrary data and define schema later. Oracle requires defined schemas upfront. Use the migration as an opportunity to establish proper types.

5. **Test Flashback configuration before cutover.** Confirm that UNDO_RETENTION and UNDO tablespace size support the flashback window needed to match Snowflake's Time Travel guarantees.

6. **Benchmark parallel query configuration.** Snowflake scales compute automatically for large queries. Oracle requires tuning the PARALLEL clause and PGA/SGA sizes. Run representative queries and adjust the degree of parallelism accordingly.
...
Snowflake TIMESTAMP_NTZ carries no timezone. Oracle TIMESTAMP likewise carries no timezone. The mapping is correct — however, applications that implicitly assume UTC for TIMESTAMP_NTZ must record that timezone assumption explicitly when migrating.
...
Snowflake silently coerces strings to numbers in arithmetic. Oracle does this in certain contexts but raises errors in others. Test all arithmetic expressions.

**Pitfall 4 — Case sensitivity:**
Snowflake is case-insensitive for unquoted identifiers (stored as uppercase by default, matching Oracle behavior). This is one of the more compatible areas — but verify any use of quoted identifiers in Snowflake code, as those are case-sensitive.

**Pitfall 5 — VARIANT NULL vs SQL NULL:**
In Snowflake, a VARIANT column can hold both SQL NULL (absence of value) and a JSON null literal. Oracle's JSON NULL is similarly distinct from SQL NULL. Review null handling in all JSON extraction queries.

**Pitfall 6 — Snowflake Iceberg tables:**
If the Snowflake account uses Apache Iceberg tables backed by S3/Azure, the underlying data files (Parquet) can be read directly by Oracle External Tables using Oracle's ORC/Parquet access driver, bypassing Snowflake entirely during the extraction phase.
