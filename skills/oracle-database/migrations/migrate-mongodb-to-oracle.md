...
Moving from MongoDB to Oracle represents a fundamental change in data modeling philosophy: from a document-oriented, schema-flexible NoSQL approach to a relational, schema-enforced model. That said, Oracle has invested heavily in narrowing this gap. Oracle Database 21c added native JSON storage with binary-encoded OSON format, and Oracle 23c introduced **JSON Relational Duality Views**, which expose document-style access over relational tables — substantially reducing the friction between these two paradigms.

This guide covers strategies for mapping documents to relational structures, Oracle's JSON capabilities, aggregation pipeline translation, BSON type mapping, and data extraction from MongoDB.
...
Map each MongoDB collection to a table. Break down embedded documents and arrays into child tables linked by foreign keys. This delivers maximum query flexibility and referential integrity.
...
Retain the document structure inside Oracle's native JSON column. This approach works best for documents with highly variable or deeply nested structures where relational normalization would be unwieldy.
...
JSON Relational Duality Views, introduced in Oracle 23ai, allow you to treat a relational schema as a document store. Applications can read and write JSON documents while Oracle stores the underlying data relationally. This is the most powerful option for new Oracle-target migrations.
...
MongoDB BSON Date is stored as milliseconds since Unix epoch (UTC). To convert to Oracle TIMESTAMP WITH TIME ZONE:
...
`JSON_TABLE` is Oracle's most capable tool for querying JSON data stored in JSON or CLOB columns.
...
For complex document structures that require transformation:
...
1. **Preserve MongoDB _id values during migration.** Store the original MongoDB ObjectId in a `mongo_id` column on the Oracle side. This enables reconciliation and simplifies rollback if necessary.

2. **Use a staging JSON table as an intermediate step.** Load raw MongoDB JSON documents into an Oracle JSON staging table first, then transform to the relational target schema using SQL and JSON_TABLE. This cleanly separates the extraction and transformation phases.

3. **Profile your collections before normalizing.** MongoDB's schema flexibility means documents within the same collection may have very different structures. Query your collections to understand actual field distribution:
   ```javascript
...
4. **Consider Oracle JSON Duality Views for applications that require document-style access.** If your application was designed for MongoDB's document API, Duality Views (introduced in Oracle 23ai, available in 26ai) let you preserve the JSON interface while storing data relationally.

5. **Handle missing fields gracefully.** MongoDB documents may omit fields that other documents include. In Oracle, these become NULL. Use `JSON_VALUE(doc, '$.field' DEFAULT NULL ON ERROR)` for safe extraction.

6. **Index frequently queried JSON fields.** When using the hybrid JSON storage approach, create JSON Search Indexes or function-based indexes on JSON paths that are queried often:
   ```sql
...
MongoDB collections frequently contain documents of mixed shapes (for example, an "events" collection with different fields per event type). In Oracle, use a base table with common fields plus a JSON column for variable fields, or apply a table-per-type approach with a discriminator column.

**Pitfall 2 — _id type assumptions:**
Not all MongoDB _id fields are ObjectIds. Applications sometimes use custom strings, integers, or compound values. Verify the actual _id types in each collection:
```javascript
db.orders.aggregate([{ $group: { _id: { $type: "$_id" }, count: { $sum: 1 } } }])
```

**Pitfall 3 — Embedded arrays with updates:**
MongoDB supports efficient array element updates (`$push`, `$pull`, `$set.array.0`). Oracle relational equivalents require DELETE + INSERT on child tables for the same operations. Review all array mutation patterns in your application code.

**Pitfall 4 — MongoDB transactions vs Oracle transactions:**
MongoDB only added multi-document ACID transactions in version 4.0 (replica sets) and 4.2 (sharded clusters). Oracle has always supported full ACID transactions. Applications that worked around MongoDB's historical lack of transactions may contain complex compensation logic that can be simplified on Oracle.
...
MongoDB BSON Date is always UTC. Oracle TIMESTAMP stores without a timezone; TIMESTAMP WITH TIME ZONE stores the offset. Load as `TIMESTAMP WITH TIME ZONE` to preserve the UTC intent, then let the application convert to local time as required.
