...
Teradata is a purpose-built analytical data warehouse designed for massively parallel processing (MPP) of large analytical workloads. Oracle Database — particularly Oracle Autonomous Data Warehouse (ADW) and Oracle Exadata — is the primary migration destination. Both are enterprise-grade platforms, but they differ considerably in SQL dialect, architectural design, data loading tooling, and analytical capabilities.

This guide addresses Teradata-specific SQL syntax (including BTEQ and SEL shorthand), multiset vs set table semantics, data type mapping, the QUALIFY clause, Teradata-specific aggregates, and the transition from Teradata's TPT (Teradata Parallel Transporter) to Oracle SQL*Loader or Data Pump.
...
Teradata uses `SEL` as shorthand for `SELECT`, and BTEQ (Basic Teradata Query) introduces scripting commands with a `.` prefix:
...
This is among the most essential Teradata concepts to grasp before undertaking a migration.

### Teradata Table Semantics

In Teradata:
- **SET table** (the default in older TD): duplicate rows are silently discarded at insert time based on the Primary Index (PI) or unique constraints. No error is raised and no insert occurs.
- **MULTISET table**: permits duplicate rows (standard SQL behavior).
...
Oracle tables permit duplicates by default (equivalent to MULTISET). To enforce uniqueness, add a PRIMARY KEY or UNIQUE constraint.
...
**Critical migration issue:** If your Teradata SET tables have been depending on silent duplicate rejection as a data-cleansing mechanism, that behavior will NOT carry over to Oracle. Data that functioned correctly in Teradata (duplicates silently dropped) will trigger ORA-00001 (unique constraint violated) errors in Oracle once primary key constraints are added. Profile your data for duplicates before applying any constraints.
...
**Teradata DATE storage:** Teradata stores dates internally as integers using the format (year-1900)*10000 + month*100 + day. Oracle uses a 7-byte internal format. This internal distinction is transparent when working through SQL but becomes significant during binary-level bulk data transfers.
...
Teradata's `QUALIFY` is a filter clause applied to window function results, analogous to `HAVING` for aggregates. No other major RDBMS supports `QUALIFY` natively (Snowflake also supports it, but Oracle does not).
...
Teradata provides native support for temporal (bi-temporal) tables through `PERIOD` type columns and temporal DML. Oracle lacks a native bi-temporal SQL extension, but the same patterns can be implemented manually.
...
For Oracle-to-Oracle transfers (where an intermediate Oracle staging database is used):
...
1. **Profile PI (Primary Index) usage.** Teradata's Primary Index governs data distribution across AMPs. When migrating to Oracle, the PI column is often a strong candidate for partitioning or the primary key. Understand how queries filter and join on the PI to guide Oracle index strategy.

2. **Resolve MULTISET table duplicates before migrating.** Check for duplicate rows in MULTISET tables:
   ```sql
   -- Teradata: find duplicate rows in a MULTISET table
   SEL order_id, COUNT(*) cnt FROM fact_orders GROUP BY order_id HAVING cnt > 1;
   ```
   Decide whether to deduplicate, retain all rows, or introduce a surrogate key.

3. **Convert QUALIFY to subqueries systematically.** Use a search-and-replace-aware tool or regex to locate every QUALIFY usage and rewrite it as an equivalent subquery or CTE pattern. This is mechanical work but must be completed for every occurrence.

4. **Review Teradata macro objects.** Teradata macros are parameterized SQL templates stored in the database. They map most naturally to Oracle stored procedures.

5. **Map Teradata UDFs to Oracle.** Teradata User-Defined Functions (UDFs) written in C may need to be rewritten in PL/SQL or Java (Oracle Java stored procedures). Identify all UDF dependencies before starting the migration.

6. **Account for Teradata views referencing derived tables.** Teradata permits complex derived table views. Oracle supports these as well but may require materialized views for performance in cases where Teradata's optimizer handled them more efficiently.
...
As discussed above, this is the most prevalent silent data issue. SET table semantics vanish in Oracle and must be replaced with explicit constraints. Always audit data for duplicates first.

**Pitfall 2 — BYTEINT type:**
Oracle does not have a 1-byte integer type. Use NUMBER(3) or NUMBER(5). Any applications performing bitwise operations on BYTEINT values require review.

**Pitfall 3 — Teradata NULL comparison:**
Teradata follows ANSI SQL for NULL comparisons (NULL = NULL is unknown). Oracle behaves the same way — these are compatible. However, Teradata's `SEL` allows certain non-standard comparisons that may yield different results.

**Pitfall 4 — ANSI mode vs Teradata mode:**
Teradata sessions can operate in ANSI mode or Teradata mode. In Teradata mode, SET table behavior, implicit type conversions, and transaction handling all differ. If sessions use Teradata mode, be aware that some behaviors may not align with Oracle's ANSI-compliant behavior.

**Pitfall 5 — Teradata COMPRESS keyword on columns:**
Teradata column-level compression stores NULL values and the most frequently occurring values more efficiently. This is an internal storage optimization that is invisible to SQL. In Oracle, use table-level compression instead:
```sql
...
BTEQ error handling via `.IF ERRORCODE <> 0 THEN .QUIT n` does not translate directly to SQL*Plus or SQLcl. Use `WHENEVER SQLERROR EXIT n ROLLBACK` and `WHENEVER OSERROR EXIT n` in SQL*Plus for equivalent behavior.
