...
IBM DB2 (now IBM Db2) and Oracle share a long history as enterprise RDBMS platforms. The two systems have more in common than most database pairs: both follow ANSI-standard SQL, both provide PL/SQL-style procedural extensions (DB2 uses SQL PL), both feature mature optimizer technology, and both offer advanced capabilities such as XML, partitioning, and materialized views. This shared foundation makes DB2-to-Oracle migrations more predictable than migrations from MySQL or SQL Server, but meaningful differences in SQL dialect, data types, administrative concepts, and procedural language still require careful attention.
...
DB2 (like PostgreSQL) permits `SELECT` without `FROM`:
...
DB2 and Oracle both support the ANSI standard `CURRENT_DATE` and `CURRENT_TIMESTAMP`, though DB2 allows spaces (`CURRENT DATE`) which Oracle does not accept:
...
This is a compatibility area — DB2 introduced `FETCH FIRST n ROWS ONLY` before Oracle did, and Oracle adopted the same SQL standard syntax in 12c:
...
DB2 supports inline isolation level hints. Oracle relies on MVCC and has no equivalent hints:
...
DB2's `REORG TABLE` reclaims space from deleted rows and reorganizes table data. The Oracle equivalent is `ALTER TABLE ... MOVE` (which rebuilds the segment), followed by rebuilding indexes.
...
1. **Take advantage of the DB2-to-Oracle syntax similarity.** DB2 SQL PL and Oracle PL/SQL share many constructs. Focus migration effort on the differences (assignment operator, loop syntax, condition handlers) rather than rewriting from scratch.

2. **Use GENERATED AS IDENTITY where possible.** DB2 11.1+ supports `GENERATED ALWAYS AS IDENTITY`, and Oracle 12c+ supports the same syntax — making this a migration-friendly area.

3. **Review DB2 compression settings.** DB2 table compression (ROW COMPRESSION, VALUE COMPRESSION) does not map directly to Oracle. Oracle provides its own compression options (COMPRESS FOR OLTP, COMPRESS BASIC, etc.). Re-evaluate the compression strategy for Oracle.
...
5. **Test DB2 LOCATE vs Oracle INSTR argument order** throughout application code and stored procedures. This is a frequent source of subtle bugs.
...
As noted above, DB2 DATE carries no time component; Oracle DATE does. Insert and comparison logic must account for this distinction.
...
DB2 `CONCAT(a, b)` accepts only two arguments. Oracle's `CONCAT` also takes only two arguments. Use `||` for multi-part concatenation in both databases.

**Pitfall 5 — EXCEPTION table name conflict:**
`EXCEPTION` is a reserved word in Oracle PL/SQL. DB2 applications with a table named `EXCEPTION` will need renaming (double quotes work as a workaround, but renaming is preferable to avoid long-term issues).

**Pitfall 6 — Isolation level compatibility:**
DB2's `WITH UR` (dirty read) has no Oracle equivalent because Oracle's MVCC prevents dirty reads by design. Remove all isolation hints; queries will be non-blocking by default.
