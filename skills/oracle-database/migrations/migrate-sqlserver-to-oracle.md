...
Microsoft SQL Server and Oracle are the two most widely deployed enterprise relational databases, and migrations between them are among the most common in the industry. Despite both being mature, ACID-compliant RDBMS engines with broad SQL standard support, they differ substantially in procedural language (T-SQL vs PL/SQL), transaction management, DDL behavior, data type naming, and administrative concepts. This guide covers all major translation requirements for a SQL Server to Oracle migration.
...
When Oracle's database character set is AL32UTF8, `VARCHAR2` already handles all Unicode characters, making `NVARCHAR2` redundant. In AL32UTF8 databases, prefer `VARCHAR2` for all character data.
...
SQL Server's date/time handling is richer and more granular than Oracle's. This is one of the most intricate translation areas.
...
1. **Start with the AWS SCT assessment report.** Before writing any migration scripts, run AWS SCT's assessment to obtain a conversion complexity score by object category. This establishes realistic expectations.

2. **Normalize NVARCHAR to VARCHAR2 in AL32UTF8 databases.** Unicode support is built into AL32UTF8 without requiring the national character set types.

3. **Review all SET options in T-SQL code.** SQL Server stored procedures frequently use `SET NOCOUNT ON`, `SET XACT_ABORT ON`, and similar options. Oracle PL/SQL does not use these; evaluate each one for the Oracle equivalent or remove it.

4. **Handle tempdb usage.** SQL Server code makes heavy use of temporary tables (`#temp`, `##global_temp`) and table variables (`@TableVar`). Oracle equivalents are global temporary tables (GTTs) or PL/SQL collections. GTTs require `CREATE GLOBAL TEMPORARY TABLE` DDL up front.
...
5. **Test collation-sensitive comparisons.** SQL Server supports configurable case/accent sensitivity per database or column. Oracle comparisons are always case-sensitive and accent-sensitive by default. Use `NLS_SORT` and `NLS_COMP` parameters for locale-aware sorting.

6. **Replace NOLOCK hints.** `WITH (NOLOCK)` is a widely used SQL Server hint for dirty reads. Oracle uses multi-version concurrency control (MVCC) and never requires dirty reads — remove all NOLOCK hints and queries will remain non-blocking.
...
SQL Server in default mode uses implicit transactions for DML only when `IMPLICIT_TRANSACTIONS` is ON. Oracle always requires an explicit `COMMIT` or `ROLLBACK`. Application code that depends on SQL Server autocommit behavior needs review.

**Pitfall 4 — MERGE statement differences:**
Both SQL Server and Oracle support `MERGE`, but the syntax differs. SQL Server MERGE requires a semicolon terminator and `MATCHED`/`NOT MATCHED` clauses with a slightly different structure. Review all MERGE statements individually.
