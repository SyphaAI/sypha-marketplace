...
Oracle's Health Monitor is a framework embedded in the database kernel that provides automated, on-demand, or scheduled diagnostic checks against the database's structural integrity. It is the proactive counterpart to reactive alert log monitoring—rather than waiting for errors to appear, Health Monitor actively interrogates database internals to surface problems before they cause data loss or downtime.

The Health Monitor is accessed through the `DBMS_HM` PL/SQL package and its results are stored in the ADR. Closely related are Oracle's advisory framework components—the SQL Tuning Advisor, Segment Advisor, and Memory Advisor—which deliver actionable recommendations for performance and space management rather than structural integrity.
...
- **Findings:** The output of each check run—structured records describing what was discovered (healthy, failed, or advisory).
...
Validates the consistency of the database's control file, datafile headers, and redo log headers. Verifies that all files listed in the control file exist, are accessible, and have consistent SCNs.

- **Use when:** After a crash, after restoring from backup, when suspecting control file corruption
- **Offline capable:** Yes (can run in mount mode)
- **Run time:** Fast (seconds to minutes)

#### Data Block Integrity Check

Scans data blocks for logical corruption (bad checksums, invalid block types, mismatched object IDs). Does not check every block by default—it targets blocks already identified as suspect.
...
Validates redo log files and archived redo logs for completeness and readability. Identifies gaps or unreadable redo that would prevent recovery.

- **Use when:** Before or after media recovery, when investigating redo-related ORA- errors
- **Offline capable:** Yes

#### Undo Segment Integrity Check

Verifies that all undo segments referenced in the system are accessible and consistent. Detects undo corruption that could trigger ORA-01578 or ORA-00600 errors related to transaction rollback.

- **Use when:** After unexpected shutdowns, when investigating ORA-01555 or rollback errors
- **Offline capable:** No

#### Transaction Integrity Check

Examines a specific transaction to verify its undo records are complete and consistent. Useful when a particular transaction is suspected to be corrupt.
...
Validates the internal consistency of the Oracle data dictionary—the master catalog of all database objects. A corrupt dictionary can trigger cascading failures across all database operations.
...
The Oracle advisory framework delivers workload-based recommendations that are distinct from structural Health Monitor checks. The advisors most relevant to monitoring and diagnostics are the SQL Tuning Advisor, Segment Advisor, and Memory Advisor.

### SQL Tuning Advisor

The SQL Tuning Advisor evaluates one or more SQL statements and recommends profile-based tuning, new indexes, statistics collection, or SQL restructuring.
...
The Memory Advisor provides sizing recommendations for SGA and PGA based on current workload.
...
1. **Run Health Monitor checks proactively after every maintenance window.** Patches, imports, manual datafile operations, and storage maintenance can all introduce subtle corruption. A post-maintenance Health Monitor run surfaces problems before users encounter them.

2. **Always run `DB Structure Integrity Check` after storage or hardware changes.** It validates control files, datafile headers, and redo log consistency—the structural foundation that everything else depends on.
...
4. **Use Segment Advisor before SHRINK or MOVE operations.** It pinpoints which segments have the most reclaimable space, helping you prioritize maintenance work and estimate impact.

5. **Consult the Buffer Cache and Shared Pool advisors before resizing SGA.** These advisors model actual workload—they reveal diminishing returns and help you avoid over-allocating memory that could cause swapping.

6. **Store Health Monitor run names with timestamps.** Consistent naming such as `CHECK_TYPE_YYYYMMDD_HH24MISS` makes it straightforward to query historical runs and detect whether check results have changed over time.

7. **Integrate advisor findings into your change management workflow.** SQL Tuning Advisor profiles and SQL Plan Baselines should be validated in non-production before being accepted in production—even though Oracle's recommendations are generally reliable.
...
Checks such as Dictionary Integrity can be resource-intensive, acquiring internal locks and performing extensive reads. Schedule them during maintenance windows or low-traffic periods.

**Mistake: Not reviewing recommendations after a check finds failures.**
Detecting a failure without following up on recommendations is worse than not running the check at all—it creates a false sense of security. Always address FAILURE findings promptly.

**Mistake: Accepting SQL Tuning Advisor profiles without testing.**
SQL profiles alter execution plans. Although the advisor applies comprehensive analysis, always validate in a non-production environment first, particularly for complex OLTP queries where plan stability is critical.

**Mistake: Ignoring ADVISORY-type findings.**
`ADVISORY` findings are not failures, but they frequently signal early warning signs—segments approaching threshold sizes, dictionary inconsistencies that have not yet caused errors, etc. Review them during weekly DBA checks.

**Mistake: Running Segment Advisor on SYS/SYSTEM schemas.**
These schemas contain internal Oracle objects that should not be shrunk or moved without Oracle Support guidance. Exclude them from Segment Advisor tasks.
