...
`adrci` (Automatic Diagnostic Repository Command Interpreter) is Oracle's command-line interface for managing the Automatic Diagnostic Repository (ADR). Introduced in Oracle 11g, it became the standard tool for searching diagnostic data, managing incidents, packaging diagnostic information for Oracle Support, and purging stale diagnostic content.

Every DBA should be comfortable with `adrci` because it offers structured, filterable access to the alert log, trace files, incidents, and problems—functionality that is impossible or impractical with plain file system tools like `grep` or `tail`. It is particularly critical when preparing diagnostic packages for Oracle Support (IPS packages) and when linking multiple related incidents together.

---

## ADR Repository Structure

The ADR is a file-based repository arranged in a predictable directory hierarchy. Familiarity with this structure helps when navigating the file system directly or when interpreting `adrci` output.
...
A single server may host multiple ADR homes—one per Oracle product instance (database instances, listeners, ASM, etc.). `adrci` can operate across all of them at once:
...
An **incident** is a single occurrence of a critical error. Each incident receives a unique ID and its own subdirectory under `$ADR_HOME/incident/incdir_<id>/` containing all diagnostic data tied to that error.

A **problem** is a collection of incidents sharing the same root cause, identified by a **problem key** (a string such as `ORA 600 [kcbz_check_objd_typ_3]`).
...
Correlation is the process of identifying related incidents that share the same root cause or occurred within the same time window. `adrci` handles this automatically through IPS or manually via `SHOW INCIDENT` filtering.
...
1. **Always use `adrci` to package diagnostics for Oracle Support.** Manually zipping trace files omits metadata, XML alert log excerpts, and correlated data that Oracle Support requires. The IPS workflow guarantees completeness.

2. **Set up automatic purging.** Without regular purging, the ADR can grow to tens or hundreds of gigabytes on a busy system. Schedule a weekly `PURGE` command and consider lowering trace file retention if disk space is limited.
...
4. **Always record the incident ID when an ORA-00600 or ORA-07445 occurs.** The incident ID links the alert log entry to the trace file directory, simplifying later analysis and IPS packaging.
...
6. **Correlate incidents before packaging.** Use `IPS CREATE PACKAGE INCIDENT <id> CORRELATE ALL` to ensure that related incidents from the same root cause are included in the support package—this frequently accelerates Oracle Support resolution.

7. **Monitor the ADR disk usage.** Include `$ADR_HOME` in your disk space monitoring. A flood of incidents (e.g., from a recurring ORA-00600) can fill the file system within hours.
...
If multiple homes exist and you do not call `SET HOMEPATH`, `adrci` either uses the first found home or queries all of them. Explicitly set the home at the beginning of every `adrci` session or script.

**Mistake: Manually deleting trace files from the OS.**
Removing files from `$ADR_HOME/trace/` or `$ADR_HOME/incident/` directly bypasses ADR metadata, leaving orphaned entries that cause `adrci` errors. Always use `PURGE` inside `adrci`.

**Mistake: Creating an IPS package without `CORRELATE ALL`.**
A minimal package (`CORRELATE BASIC` or no correlation) often omits critical trace files from related background processes. Always use `CORRELATE ALL` when the root cause is unclear.

**Mistake: Not specifying `IN /path` for `IPS GENERATE`.**
Without a target path, the ZIP file is created in the current directory, which may lack sufficient space or may be a temporary location that gets cleaned up. Always provide an explicit destination path with adequate free space.
...
`PURGE -AGE 30` purges data older than 30 minutes, not 30 days. Convert: 30 days = 43,200 minutes. Always verify the unit before executing purge commands in production.

**Mistake: Confusing `PURGE -AGE` units with `SET CONTROL` units.**
`PURGE -AGE` is in **minutes**; `SET CONTROL (SHORTP_POLICY/LONGP_POLICY)` is in **hours**. These are distinct units for two separate commands.
