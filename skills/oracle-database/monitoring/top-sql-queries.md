...
Identifying and tuning the most resource-intensive SQL statements is one of the highest-leverage activities available to a DBA. A single poorly-written query can consume 90% of the CPU on a busy system, and resolving it can produce performance improvements that benefit every user. Oracle offers multiple views and tools for locating these queries—from the real-time `V$SQL` and `V$SQLAREA` views to historical Automatic Workload Repository (AWR) tables such as `DBA_HIST_SQLSTAT`.

This guide covers the key views for locating top SQL across various resource dimensions, AWR-based historical analysis, and the real-time SQL monitoring feature (`V$SQL_MONITOR`) that delivers execution-level visibility into long-running queries.
...
In practice, begin with `V$SQLAREA` for top-N queries by total resource consumption, then drill into `V$SQL` for specific execution plans and bind variable details.
...
CPU-heavy queries are typically those performing full table scans, excessive sorting, or processing large result sets without adequate filtering.
...
Elapsed time captures wall-clock time inclusive of waits (I/O, locking, network). High elapsed time combined with low CPU time indicates wait-bound SQL.
...
High disk reads frequently indicate missing or unused indexes, full table scans on large tables, or an undersized buffer cache.
...
High buffer gets can point to full scans, inefficient index usage, or queries scanning more data than necessary. This serves as a useful proxy for memory pressure.
...
Some queries are expensive per execution; others are inexpensive per execution but run thousands of times. Distinguish between the two:
...
`V$SQLAREA.SQL_TEXT` is truncated to 1000 characters. Use `V$SQLTEXT` for the complete statement:
...
The Automatic Workload Repository (AWR) captures snapshots of key statistics every hour by default. This historical data resides in `DBA_HIST_*` tables and enables trending and comparison across time periods.
...
A sudden increase in `avg_elapsed_sec` paired with a change in `plan_hash_value` indicates that a plan change triggered the regression.
...
SQL Monitoring was introduced in Oracle 11g and provides real-time, per-execution visibility into long-running SQL statements. It activates automatically for any statement that:
- Runs for more than 5 seconds of CPU or I/O time, **or**
- Uses parallel execution, **or**
- Has the `MONITOR` hint applied

**Requires:** Oracle Tuning Pack. In practice, pack access is governed by `CONTROL_MANAGEMENT_PACK_ACCESS`, where Tuning Pack implies Diagnostics Pack.
...
`V$SQL_PLAN_MONITOR` supplies statistics at the **execution plan operation level**—showing precisely which step in the plan is consuming the most resources:
...
The most powerful way to review SQL Monitoring data is the HTML or text report, which presents a formatted view of the plan, statistics, and timing:
...
Save the HTML output to a `.html` file and open it in a browser to access the interactive report, which shows:
...
1. **Start with `V$SQLAREA` sorted by elapsed time, not CPU.** Elapsed time captures the complete user experience including waits. A query consuming 100 seconds of CPU within 10 minutes of wall-clock time is less urgent than one using 10 seconds of CPU while blocking others for 30 minutes.

2. **Always divide by executions to determine per-execution cost.** Total elapsed time can be misleading when a query runs a million times. Compare per-execution averages to decide whether optimization should target reducing execution count (result caching, improved application logic) or reducing per-execution cost (indexing, plan changes).

3. **Use `plan_hash_value` to detect plan changes.** A sudden shift in `plan_hash_value` within AWR history almost always corresponds to a performance change. Keep this in mind when investigating regressions.

4. **Preserve SQL Monitor reports for major incidents.** HTML SQL Monitor reports are invaluable post-incident artifacts. Save them to a shared location before the monitoring data ages out (default retention is 30 days for completed statements).

5. **Apply the `MONITOR` hint to force monitoring for critical short-running queries.** By default, SQL Monitoring activates only for long-running queries. For critical short-duration queries (sub-5-second but high-frequency), add `/*+ MONITOR */` during investigation to capture per-operation statistics.

6. **Investigate high `disk_reads/executions` ratios before buffer gets.** Physical I/O is orders of magnitude slower than logical I/O. A query reading 10,000 physical blocks per execution should take priority, even if another query reads 1,000,000 buffer blocks from cache.

7. **Use `DBA_HIST_SQLSTAT` for monthly capacity planning.** Trending SQL resource consumption over months helps predict when current hardware will approach saturation and supports the business case for infrastructure changes.

---

## Common Mistakes and How to Avoid Them

**Mistake: Tuning SQL based on V$SQLAREA without verifying it still runs.**
`V$SQLAREA` retains cursor data since the last cursor flush or instance restart. A query with the highest elapsed time may not have executed in days. Always check `LAST_ACTIVE_TIME` before investing tuning effort.

**Mistake: Using V$SQL or V$SQLAREA without retrieving the full SQL text.**
`SQL_TEXT` in `V$SQLAREA` is truncated to 1000 characters. Correctly identifying or analyzing a query is impossible without the complete text. Always use `V$SQLTEXT` or `V$SQL.SQL_FULLTEXT` when submitting queries for tuning.

**Mistake: Ignoring parse activity.**
High `LOADS` or `INVALIDATIONS` counts in `V$SQLAREA` indicate cursors are being hard-parsed or invalidated repeatedly. This does not surface prominently in elapsed time rankings but causes serious scalability problems. Monitor the `LOADS/EXECUTIONS` ratio.

**Mistake: Comparing AWR stats without accounting for different snapshot durations.**
AWR `_delta` values accumulate over the snapshot interval. A snapshot spanning 2 hours contains twice as many delta values as one spanning 1 hour. Normalize by dividing by the interval duration when comparing snapshots of different lengths.
...
AWR snapshots are typically taken every hour. During a live performance crisis, use `V$SQL`, `V$SESSION`, `V$SESSION_WAIT`, and `V$SQL_MONITOR` for real-time data. AWR is suited for trend analysis and post-incident review.
