...
Space management ranks among the most critical and ongoing responsibilities of a DBA. Running out of tablespace is one of the most frequent causes of application outages—yet it is almost entirely avoidable with proper monitoring and a proactive management strategy. Beyond preventing full tablespaces, effective space management means reclaiming wasted space left behind by deleted rows, understanding how Oracle's high water mark functions, managing temporary space, and handling the distinct space behavior of LOB columns.

This guide covers the essential views, queries, tools, and techniques for monitoring and managing space in an Oracle database.
...
For more granular analysis at the datafile level:
...
The **high water mark (HWM)** is an internal marker within a segment that records the highest block ever used by that segment. Oracle's full table scans read all blocks up to the HWM regardless of how much data currently exists below it—meaning a table that once held 10 million rows but had every row deleted will still incur the same full-scan cost as when it was fully populated.
...
- **Low HWM (ASSM only):** The point below which all blocks are guaranteed to be formatted. Space below the Low HWM is used first for new inserts.
...
`ALTER TABLE ... SHRINK SPACE` compacts table data by relocating rows below the HWM, then resets the HWM. It operates online (DML can continue) and requires the tablespace to use Automatic Segment Space Management (ASSM).
...
`ALTER TABLE ... MOVE` physically relocates the entire table into a new segment (in the same or a different tablespace), fully resetting the HWM and defragmenting storage.
...
**When to use MOVE:**
- Maximum space reclamation is needed
- A maintenance window is available (unless using the ONLINE option)
- Moving data between tablespaces (e.g., from slow disk to fast SSD)
- Table belongs to an MSSM (Manual Segment Space Management) tablespace
...
Oracle's Segment Advisor examines segments and recommends those worth shrinking. See also `health-monitor.md` for running the advisor.
...
- **Out-of-line storage:** Larger LOBs are stored in a dedicated LOB segment, with the table row holding only a pointer (locator).
...
LOBs maintain a separate HWM and do not benefit from table `SHRINK` operations. Reclaiming LOB space requires different techniques:
...
For BasicFiles LOBs, the `RETENTION` clause controls how long old LOB versions are kept. Excessive retention values can cause LOB segments to grow unexpectedly.

---

## Temp Space Monitoring

Temporary tablespace is used for sort operations, hash joins, global temporary tables, and parallel query operations. Unlike permanent tablespace, it is recycled after each operation—but it can become "stuck" when sessions are killed or internal errors occur.
...
When temp space is not released after sessions disconnect (a known issue under certain bug conditions or in RAC scenarios):
...
1. **Monitor `DBA_TABLESPACE_USAGE_METRICS` daily and alert at 80% used.** Do not wait until 95%—autoextend files may already be at their maximum size, and sudden space consumption (bulk loads, runaway processes) can fill a tablespace within minutes.

2. **Understand autoextend limits, not just current usage.** A tablespace at 50% used with a very small `MAXSIZE` on its datafiles may be closer to running out of space than one at 85% with large autoextend headroom.

3. **Gather table statistics before analyzing HWM waste.** `DBA_TABLES.NUM_ROWS` and `AVG_ROW_LEN` are only as current as the last `DBMS_STATS` run. Stale statistics produce inaccurate waste calculations.

4. **Use `SHRINK SPACE COMPACT` first during business hours, then `SHRINK SPACE` during low traffic.** The COMPACT phase handles the heavy lifting (moving rows) with minimal locking. The final HWM reset phase is brief.

5. **Always rebuild indexes after `MOVE`.** Failing to do this is a common production incident—unusable indexes cause ORA-01502 errors on any DML against the affected table.

6. **Size temp tablespace for peak usage, not average.**  Use AWR history (`DBA_HIST_TBSPC_SPACE_USAGE`) to identify historical peak temp consumption and size accordingly.  Temp space that fills up during month-end reporting causes operations to fail.

7. **Avoid `DROP TABLESPACE` as a temp space fix in production.** Always verify there are no active temp sessions first. Dropping a temp tablespace while sessions are actively using it can destabilize the instance.
...
`DBA_DATA_FILES.BYTES` reflects the allocated file size, which can far exceed actual data volume. Use `DBA_TABLESPACE_USAGE_METRICS` or join `DBA_DATA_FILES` with `DBA_FREE_SPACE` to get accurate used versus free figures.

**Mistake: Expecting `DELETE` to free space immediately.**
`DELETE` removes rows logically but does not lower the HWM or release space back to the tablespace. Use `TRUNCATE` (which resets the HWM) where appropriate, or `SHRINK/MOVE` following a mass delete.

**Mistake: Shrinking tables without considering row movement impacts.**
`ENABLE ROW MOVEMENT` permits Oracle to change a row's ROWID during a shrink operation. Any application that caches stored ROWIDs externally will break. Confirm that no application stores physical ROWIDs before enabling row movement.
...
Unlimited autoextend datafiles can fill an entire file system, bringing down both the OS and the database. Always set a `MAXSIZE` that reserves headroom for the OS and other applications. Monitor the file system, not just Oracle.

**Mistake: Ignoring undo tablespace space.**
Undo tablespace behaves differently—Oracle manages retention automatically, but a very long-running transaction can cause the undo tablespace to grow dramatically. Include the undo tablespace in space monitoring and configure a reasonable `UNDO_RETENTION`.
