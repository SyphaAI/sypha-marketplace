...
The Oracle alert log is the primary diagnostic record of a database instance. It captures startup and shutdown events, administrative operations, structural changes, and—critically—errors that arise during normal operation. Every Oracle DBA must be able to navigate, read, and monitor the alert log as the first step in any troubleshooting workflow.

Working effectively with the alert log means knowing where it resides in the Automatic Diagnostic Repository (ADR), what distinguishes the XML and text formats, how to interpret common ORA- errors, and how to automate monitoring so issues surface before users report them.
...
Introduced in Oracle 11g, the **Automatic Diagnostic Repository (ADR)** is a unified file-based repository that stores diagnostic data for all Oracle products. Each database instance maintains its own ADR home beneath a root directory governed by the `DIAGNOSTIC_DEST` initialization parameter.
...
2. **Text format** — Located in `$ADR_HOME/trace/`. File is named `alert_<SID>.log`. This is the human-readable format most DBAs access directly. It is generated alongside the XML format and contains the same information in a simpler layout.
...
Prior to ADR, the alert log was written to `$ORACLE_BASE/admin/<DB_NAME>/bdump/alert_<SID>.log` or the directory specified by `BACKGROUND_DUMP_DEST`. If you work with older databases, check that parameter:
...
The most severe Oracle error. It signals an internal inconsistency detected by Oracle. Always open a Support Request with the complete arguments list and the associated trace file.
- Arguments appear in square brackets: `ORA-00600: [kcbz_check_objd_typ_3], [0], [1], ...`
- Each argument combination maps to a specific bug or problem
- Look for the trace file in `$ADR_HOME/trace/` with a matching process ID and timestamp
...
Indicates a corrupt data block. Check for hardware issues immediately and run RMAN `VALIDATE DATABASE`.
...
The Shared Pool or another SGA component has been exhausted. Indicates memory pressure or a possible memory leak.
...
The archiver process (ARCn) is unable to write archive logs. Verify disk space on the archive log destination.
...
Undo data was overwritten before a long-running query completed reading it. Typically caused by an undersized undo tablespace or very long transactions.
...
The temp tablespace is full. This points to a large sort or hash operation, or a temp space leak from a killed session.

**ORA-00060: deadlock detected**
Two sessions are waiting on each other's locks. Oracle resolves this by killing one statement. The trace file contains the deadlock graph—always examine it.

**ORA-03113 / ORA-03114: end-of-file on communication channel**
The client lost its connection to the server. Typically means a crashed server process; look for matching trace files.
...
Frequent log switches (more than once every 15-20 minutes) suggest the redo logs are undersized, resulting in performance overhead and increased I/O:
...
The message `checkpoint not complete` in the alert log means Oracle cannot reuse a redo log group because the checkpoint has not finished writing dirty buffers to disk. This causes the log writer to stall, blocking all DML:
...
Any occurrence should be investigated. Repeated occurrences of the same error code arguments indicate a specific bug—search My Oracle Support (MOS) using the argument signature.
...
`adrci` (ADR Command Interpreter) is the command-line tool for working with ADR content, including the alert log. See the companion `adrci-usage.md` guide for complete details.
...
A common and effective pattern is to track the last-read position of the text alert log and scan only new content on each run:
...
Using `V$DIAG_ALERT_EXT`, you can build a SQL-based monitoring query executable by any scheduler (DBMS_SCHEDULER, Enterprise Manager, Grafana with JDBC):
...
1. **Never delete or truncate the alert log directly.** Oracle rotates it automatically when needed. If file size is a concern, use `adrci purge` to remove old incidents and XML log segments rather than manually deleting the text log.

2. **Monitor the alert log continuously in production.** A 5-10 minute polling interval catches problems before users escalate. Target near-real-time alerting for critical errors (ORA-00600, ORA-07445, ORA-01578).

3. **Always examine the associated trace file.** When the alert log references a trace file (it usually names it explicitly), review that file before searching MOS. The trace file contains the full context, stack trace, and often the root cause.

4. **Correlate with system events.** Alert log timestamps are the starting point—cross-reference them with OS metrics (CPU, I/O, memory) from the same time window to distinguish Oracle bugs from resource pressure.

5. **Establish a baseline.** Know what "normal" looks like in your alert log. How frequently do log switches occur? Are checkpoint warnings routine? A deviation from baseline is often the earliest sign of trouble.

6. **Use `adrci` for production alerting workflows.** Its filtering against the XML format is more precise than grep on the text file, particularly when correlating incidents with trace files.
...
These internal errors always indicate something unexpected occurred inside Oracle. Even if the database appears healthy afterward, the error may recur under higher load. Always open a Support Request and capture the trace file.

**Mistake: Grepping only for "ORA-" and missing other critical messages.**
Not all serious alert log entries contain "ORA-". Messages such as `checkpoint not complete`, `db_block_checking detected an error`, and `WARNING: Heavy swapping observed` are equally significant. Extend your monitoring patterns to include these strings.

**Mistake: Monitoring the text alert log file on a different server than the DB host.**
If the alert log is on a shared NFS mount and the mount becomes stale, your monitoring silently stops receiving new entries. Always verify the monitoring tool can actually read the file.

**Mistake: Not recording the state of the alert log before and after maintenance.**
Before any maintenance window, note the last line or timestamp in the alert log. Afterward, review everything written during the window so you do not overlook errors that occurred but did not surface as visible problems.

**Mistake: Treating all ORA-01555 errors as a tuning problem.**
ORA-01555 can also stem from application bugs (uncommitted transactions held open), queries inside scheduled jobs competing with undo purging, or incorrectly sized undo. Review the full context before simply increasing `UNDO_RETENTION`.
