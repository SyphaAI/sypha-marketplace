...
When an AI agent generates and executes DML (INSERT, UPDATE, DELETE, MERGE), it must follow safety patterns to prevent accidental data loss, runaway operations, or unrecoverable changes. These patterns apply whether the agent operates autonomously or under human confirmation.

## Always-WHERE Clause Guards

Never generate an UPDATE or DELETE without a WHERE clause. Validate before executing.
...
**Agent rule**: If a user says "update all employees" or "delete old records" without indicating which rows — ask for clarification before generating SQL. See `intent-disambiguation.md`.

## Count Before Delete/Update

Always run a `SELECT COUNT(*)` first to verify the scope of the operation.
...
Use SAVEPOINT to simulate a DML operation and review the results before committing.
...
Before executing DML that could affect millions of rows, review the execution plan.
...
Record intent before executing a destructive operation to establish an audit trail.
...
CLOB, BLOB, and NCLOB columns require special care in agent-generated DML. Selecting a full LOB column can transfer gigabytes of data, cause out-of-memory errors, or silently truncate values.
...
**Agent rule**: When an agent detects a CLOB/BLOB/NCLOB column in a query, warn the user that a full-table SELECT of LOB columns is costly and can move very large volumes of data. Always recommend DBMS_LOB.GETLENGTH for size checks and DBMS_LOB.SUBSTR for previewing content.

## Deferred Constraint Handling

Some constraints are defined as DEFERRABLE, meaning validation is postponed until COMMIT rather than enforced after each DML statement. This is useful for bulk loads but requires explicit constraint management.
...
**Agent rule**: When deferred validation fails, the error surfaces at `SET CONSTRAINTS IMMEDIATE` or at `COMMIT` time — not at the DML statement responsible. The offending row may not be apparent from the error message alone; query the staging data to locate unmatched keys.

## UPDATE RETURNING INTO

`RETURNING INTO` captures values from updated rows in the same round-trip as the DML. Use it for audit logging, LOB locator retrieval, or feeding the result into a subsequent operation — without issuing a separate SELECT.
...
MERGE does not raise `DUP_VAL_ON_INDEX`. If the source dataset contains duplicate keys, Oracle may update the same target row multiple times within a single MERGE, producing unpredictable results. Deduplicate the source using `DISTINCT` or aggregation before merging.
...
MERGE into a view is supported only when the view is key-preserving (each row in the view maps to exactly one row in the underlying base table). Merging into a non-key-preserving view raises `ORA-38106`.
...
`SQL%ROWCOUNT` after a MERGE reflects the total rows processed (inserts + updates + deletes combined). There is no built-in mechanism to separate counts per operation without adding logging.
...
- Use SAVEPOINT + ROLLBACK for interactive dry runs; always confirm with the user before COMMIT on destructive operations
...
- Never use `TRUNCATE` from an agent — use `DELETE` so the operation can be rolled back
- Refuse to generate `DELETE FROM <table>` without a WHERE clause; require an explicit `DELETE FROM <table> WHERE 1=1` if the user truly intends to remove all rows

## Common Mistakes

**Generating DELETE without WHERE** — even if a user says "clear the table", always confirm and use DELETE with an explicit `WHERE 1=1` so the agent generates an intentional, auditable statement.

**Committing inside a loop without error handling** — if the loop fails midway, partial data is permanently committed. Use a SAVEPOINT before each batch and ROLLBACK TO SAVEPOINT on error.

**Not accounting for cascading deletes** — before deleting from a parent table, check `ALL_CONSTRAINTS` for child foreign keys. An unguarded delete can violate constraints or cascade in unexpected ways.

## Oracle Version Notes (19c vs 26ai)

- **19c and later**: All patterns in this skill apply; SAVEPOINT, autonomous transactions, and FETCH FIRST are all available in 19c+
- **26ai**: No new DML safety primitives; the patterns remain the same; agent integration via MCP server adds a natural confirmation layer
