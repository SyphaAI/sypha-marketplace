...
When an AI agent runs SQL against Oracle, those queries appear in V$SESSION, V$SQL, ASH, and AWR alongside all other workloads. Without identification metadata, agent queries are indistinguishable from regular application queries, making debugging, auditing, and performance analysis extremely difficult.

Configure client identification at session startup so that every query the agent executes is traceable.

## DBMS_APPLICATION_INFO

The primary package for registering who is running what.

```sql
-- Set at the start of every agent session
BEGIN
  DBMS_APPLICATION_INFO.SET_MODULE(
    module_name => 'sypha-agent',   -- max 48 chars; identifies the agent system
...
```
MODULE:  '<agent-name>'                  e.g. 'sypha-agent', 'my-chatbot'
...
CLIENT_IDENTIFIER: '<agent>:<user>:<id>' e.g. 'sypha-agent:klrice:session-42'
```

Keep MODULE and ACTION consistent across sessions so that AWR/ASH history remains queryable by these values.
...
EXEC init_agent_session('sypha-agent', 'rag-ingest', 'klrice', 'req-1042');
```

## CLIENT_IDENTIFIER Truncation and Encoding

Oracle silently truncates any values that exceed field length limits. Structure your values so the most identifying information fits within the limit even when truncation occurs.
...
Always truncate explicitly before passing values to these calls so that truncation is deliberate and predictable:
...
Truncated to 64 bytes, with the most important segment (AGENT_TYPE) placed first so it survives truncation. Example: `sypha-agent:rag-ingest:task-042:step-3`.

## Connection Pool Identification (UCP / DRCP)

When agents connect through Universal Connection Pool (UCP) or Database Resident Connection Pooling (DRCP), the underlying physical session is shared across multiple logical connections. `CLIENT_IDENTIFIER` and `DBMS_APPLICATION_INFO` values persist on the physical session and are not automatically cleared when a connection is returned to the pool.
...
Failing to clear at return means the next logical connection borrowing that physical session will inherit the previous agent's identity, causing misattribution in V$SESSION, ASH, and audit trails.
...
When an agent connects through the Oracle MCP server (Oracle 26ai), the MCP server can automatically populate `MODULE` and `CLIENT_INFO` before the agent's first SQL statement executes. In this scenario, the agent should not overwrite `MODULE` or `ACTION` — only set `ACTION` to reflect the current task.

Verify whether the MCP layer has already configured identification:
...
Only call `DBMS_APPLICATION_INFO.SET_ACTION` to update the current step — leave `MODULE`, `CLIENT_INFO`, and `CLIENT_IDENTIFIER` as set by the MCP layer. This preserves end-to-end traceability from the MCP connection through to AWR and audit records.

## Best Practices

- Set MODULE, ACTION, and CLIENT_IDENTIFIER at session start — do it before the first query
- Update ACTION as the agent transitions between tasks within a session so AWR reports reflect per-task workload
- Use a consistent MODULE name across all sessions from the same agent — this enables AWR top module reports
- Keep MODULE under 48 chars and ACTION under 32 chars (Oracle silently truncates)
- Clear CLIENT_IDENTIFIER at session end so it does not carry over to connection pool reuse
- Include a correlation ID (request ID, task ID) in CLIENT_INFO or CLIENT_IDENTIFIER for cross-system tracing

## Common Mistakes

**Not setting any identification** — agent queries show up as anonymous application queries; impossible to distinguish from user queries in AWR/ASH.

**Setting MODULE to the schema/username** — that information is already present in V$SESSION. Use MODULE for the agent system name and ACTION for the specific task.

**Setting identification once and forgetting to update ACTION** — long-running agents that switch tasks without updating ACTION render ASH analysis useless.

**Exceeding field length limits** — MODULE (48), ACTION (32), CLIENT_INFO (64), CLIENT_IDENTIFIER (64). Excess is silently truncated; structure your values to remain meaningful within these limits.

## Oracle Version Notes (19c vs 26ai)

- **19c and later**: DBMS_APPLICATION_INFO and DBMS_SESSION.SET_IDENTIFIER have been available since Oracle 8i; all patterns in this skill apply from 19c+
