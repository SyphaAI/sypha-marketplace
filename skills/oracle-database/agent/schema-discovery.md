...
Before generating SQL or making schema changes, an agent should introspect the database to understand what already exists. This skill provides the recommended query sequence for agent startup and schema exploration.

## Agent Startup Sequence

Execute these queries at the start of any session to establish context:
...
Run this to get a high-level overview of a schema before beginning work:
...
Oracle 26ai (and 23ai) introduce several object types that agents should recognize when introspecting a schema.
...
- Use `ALL_*` views (not `DBA_*`) to respect the current user's access level; fall back to `DBA_*` only when the user holds DBA privileges
- Cache schema metadata within a session to avoid repeated data dictionary queries
...
- Check `num_rows` and `last_analyzed` before generating queries on large tables — stale statistics mislead the optimizer
