...
DROP, TRUNCATE, and unguarded DELETE are irreversible or difficult to reverse. Before an agent executes any of these, it should run pre-flight checks to verify that the operation is safe, intentional, and recoverable.
...
**Agent rule**: Never generate TRUNCATE without explicit user confirmation showing the row count and acknowledging irreversibility. Prefer DELETE with a SAVEPOINT when rollback capability is required.
...
Before dropping a table, check whether remote databases reference it through DB links. Oracle cannot automatically query remote databases for this information, but agents should check for local synonyms that expose the table remotely and review ALL_DB_LINKS for awareness.
...
Note: Oracle cannot automatically detect whether a remote database queries this table via DB link. Alert the user that cross-database impacts cannot be automatically verified.

## Checking for Grants on the Object

If a table has been granted to other users or roles, dropping it automatically revokes those grants, which may break access for other users.
...
Agent rule: if any grants exist, include them in the pre-flight report. The user should know which users or roles will lose access.
...
Synonyms become broken (INVALID) after the table is dropped and are not removed automatically.

## Checking for Scheduler Jobs Referencing the Object

Jobs that query or modify the table will fail at their next run if the table has been dropped.
...
Note: This is a text search over job_action and may produce false positives. Always review results manually.
...
- Always capture DDL via `DBMS_METADATA.GET_DDL` before any DROP — store it in a log table or present it to the user
...
- Run dependent object checks before DROP — a table with 5 dependent views will silently invalidate all of them
...
**Not checking for enabled foreign keys** — if child records exist, `DROP TABLE` with `CASCADE CONSTRAINTS` silently removes the FK; without it, the command fails. Either way, the agent should flag this.
