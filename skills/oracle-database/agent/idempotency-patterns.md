...
An idempotent operation produces the same outcome whether it runs once or many times. Agents must generate idempotent SQL because they may retry after failures, be invoked multiple times by orchestration systems, or encounter partial failures partway through execution.
...
Oracle does not support `CREATE TABLE IF NOT EXISTS`. Use exception handling or a data dictionary check instead.
...
Sequences are always unique; identity columns are inherently idempotent for inserts (each insert receives a new ID). The risk lies in generating orphaned records when a retry occurs.
...
`GRANT` is idempotent — Oracle silently ignores a grant that is already in place. `REVOKE` is not — it raises `ORA-01927` if the privilege was never granted. Wrap `REVOKE` in an exception handler or verify before executing.
...
`CREATE OR REPLACE SYNONYM` is idempotent. When that form is unavailable (for example, inside a conditional block that must not replace an existing synonym), use an existence check.
...
For multi-step migrations, record each completed step in a tracking table so that retries skip work that already succeeded. This mirrors the approach used by Liquibase (`DATABASECHANGELOG`) and Flyway (`flyway_schema_history`).
...
Maintaining your own tracking table provides the same idempotency guarantee as Liquibase or Flyway without requiring any external tooling.
...
**INSERT in a loop without duplicate handling** — if the agent retries the loop after a failure at row 500, rows 1–500 generate ORA-00001. Use MERGE or exception handling.

**CREATE TABLE without existence check** — ORA-00955 on retry halts the migration. Always check first.

**Using DELETE + INSERT instead of MERGE** — delete-then-insert is not idempotent if the delete succeeds but the insert fails (the row disappears permanently). Use MERGE.

**Relying on SEQUENCE.NEXTVAL in idempotent code** — each retry consumes a sequence number and generates a new ID. Use a natural key in the MERGE ON clause to prevent duplicate logical records.
