...
When users or AI agents describe what they want in natural language, the intent must be translated into Oracle SQL. This skill provides common NL phrasings, their SQL equivalents, and disambiguation notes for cases where the intent is ambiguous.
...
- When multiple SQL patterns fit, choose the one that leverages an index: `WHERE date_col >= SYSDATE - 30` rather than `WHERE TRUNC(date_col) >= TRUNC(SYSDATE - 30)` (the latter prevents index use)
- Use bind variables (`:param`) instead of literal values whenever possible
- For aggregation queries, always include `ORDER BY` to produce deterministic results
- For schema introspection, always apply `UPPER()` to user-supplied names — Oracle stores identifiers in uppercase by default
