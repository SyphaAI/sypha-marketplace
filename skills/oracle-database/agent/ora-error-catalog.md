...
When an agent encounters an Oracle error, it should diagnose the underlying cause and apply corrective action without escalating to a human. This catalog covers the 25 most common ORA- errors agents run into, with a root cause and remediation step for each.
...
**Root cause**: An INSERT attempted to write a duplicate value into a unique or primary key column.
...
**Root cause**: A DDL or `SELECT FOR UPDATE NOWAIT` statement tried to lock an object that is already held by another session.
...
**Root cause**: Two sessions are each blocked waiting for a lock the other holds, creating a circular dependency. Oracle automatically resolves the deadlock by rolling back one of the statements (not the full transaction) and raising ORA-00060 in that session.
...
**Root cause**: The archive log destination (local disk or FRA) has run out of space. Oracle cannot write new archive logs, so all non-SYSDBA connections are blocked to prevent data loss.
...
**Root cause**: The table name is incorrect, the schema prefix is missing, or the current user lacks SELECT privilege.
...
**Root cause**: Oracle attempted to convert a string to a number, but the string is non-numeric. This often stems from VARCHAR2 columns being compared against numbers.
...
**Root cause**: The date string format does not match the NLS_DATE_FORMAT or the supplied format mask.
...
**Root cause**: An INSERT into a child table references a value that does not yet exist in the parent table.
...
**Root cause**: A DELETE or UPDATE of a parent row that still has dependent child rows.
...
**Root cause**: A `CONNECT BY` hierarchical query hit a cycle in the data — a row appears as its own ancestor, triggering an infinite loop. This is a data quality problem (for example, an employee recorded as their own manager, or a circular parent-child link).
...
**Root cause**: A long-running query began reading data that was subsequently overwritten before the query completed — Oracle can no longer reconstruct the consistent read image from undo.
...
**Root cause**: The current user is missing a required system or object privilege.
...
**Root cause**: Excessive failed login attempts (28000) or a password that has passed its profile expiry date (28001). Both conditions require DBA intervention.
...
**Root cause**: A DDL statement (for example, `ALTER TABLE` or `CREATE OR REPLACE PROCEDURE`) could not acquire a DDL lock on the object within the allowed time. Another session is holding a DML or parse lock on the object. This is the DDL counterpart to ORA-00054.
...
**Root cause**: A trigger on the accessed table has compilation errors. Oracle attempted to auto-recompile the trigger at runtime and failed. This commonly occurs after a DDL change to a table, package, or type that the trigger depends on.
...
**Root cause**: A PL/SQL block passed to `EXECUTE IMMEDIATE` or `DBMS_SQL` contains a syntax or semantic error Oracle could not compile. ORA-06550 is always accompanied by one or more `PLS-` codes identifying the actual issue. Common causes include misspelled identifiers, incorrect argument counts, type mismatches, and references to nonexistent objects.
...
**Root cause**: The data being inserted or updated exceeds the column's declared maximum length. The error includes both the actual byte/char length of the value and the column maximum. This frequently occurs when loading data from external systems with different length assumptions, or when a VARCHAR2 column uses byte semantics but the data contains multibyte characters.
...
- Always parse the full ORA- error code and constraint name from the error message before taking action
- For ORA-00942: check `ALL_TABLES` and `ALL_SYNONYMS` before concluding the table is missing
- For ORA-00001 and ORA-02291: inspect the data first, then adjust the SQL — not the reverse
- For ORA-01555: do not retry immediately — the issue persists until undo is tuned or the query is restructured
- For ORA-28000/28001: escalate to a human DBA — never attempt to change passwords autonomously
