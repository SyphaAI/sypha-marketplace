...
Not every user request is specific enough for an agent to act on safely. This skill defines when agents should request clarification before generating or executing SQL, and provides question templates along with safe default rules.
...
When clarification is needed, ask one focused question per turn. Do not front-load multiple clarifying questions at once. Identify the single most important unknown first, get the answer, then ask the next question only if still needed.
...
Certain operations must be escalated to a human DBA rather than attempted autonomously by an agent. When asked to perform any of the following, the agent should prepare the SQL for review but not execute it.
...
3. **Show before execute** — for any multi-row UPDATE or DELETE, display the affected rows first
...
- Execute `DELETE` or `UPDATE` without displaying the affected row count first
- Execute `DROP TABLE`, `DROP USER`, or `TRUNCATE TABLE` without a full impact analysis
...
- When seeking clarification, be specific about what information you need and why — vague questions frustrate users
...
- For schema introspection requests, always proceed without asking — it is always safe

## Oracle Version Notes (19c vs 26ai)

All patterns in this skill are behavioral and process-oriented guidelines independent of Oracle version.
