...
`database/rac_ru` is the Oracle Container Registry repository for Oracle Real Application Cluster Release Update Container Images. Oracle places this repository in the OCR Database business area and exposes both a latest pull command and a tags table for choosing specific image versions.
...
- **Image selection:** Resolve an exact reviewed digest for `container-registry.oracle.com/database/rac_ru` from Oracle Container Registry and pull `container-registry.oracle.com/database/rac_ru@sha256:<reviewed-digest>`. Do not use a moving tag.
...
- The OCR readme identifies `rac_ru` as the Release Update container-image line for Oracle RAC in Linux containers.
- The page indicates that RAC on Podman is supported beginning with 19c (19.16) and 21c (21.7) for this Release Update stream.
- OCR references the RAC installation guide for Podman on Oracle Linux and addresses preparation, network planning, storage, and password management.

## Oracle Version Notes (19c vs 26ai)

The OCR detail page explicitly positions `rac_ru` as a Release Update image stream and documents Podman support starting with 19c (19.16) and 21c (21.7). The current latest OCR pull command targets `latest-19`.
