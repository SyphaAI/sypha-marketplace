...
`database/microtx-ee-console` is the Oracle Container Registry repository for Oracle Transaction Manager for Microservices(MicroTx) Console. Oracle places this repository in the OCR Database business area and exposes both a latest pull command and a tags table for choosing specific image versions.
...
- **Image selection:** Resolve an exact reviewed digest for `container-registry.oracle.com/database/microtx-ee-console` from Oracle Container Registry and pull `container-registry.oracle.com/database/microtx-ee-console@sha256:<reviewed-digest>`. Do not use a moving tag.
...
- The OCR readme identifies this image as the graphical web console for Oracle Transaction Manager for Microservices (MicroTx).
- The page indicates that the console is intended for managing and monitoring transactions.
- OCR further notes that the console is available exclusively with MicroTx Enterprise Edition.

## Oracle Version Notes (19c vs 26ai)

The OCR detail page is organized around MicroTx release tags rather than a 19c-versus-26ai database matrix. Use the repository tags table to choose the console version you require.
