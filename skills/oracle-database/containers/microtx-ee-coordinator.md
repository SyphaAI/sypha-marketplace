...
- **Image selection:** Resolve an exact reviewed digest for `container-registry.oracle.com/database/microtx-ee-coordinator` from Oracle Container Registry and pull `container-registry.oracle.com/database/microtx-ee-coordinator@sha256:<reviewed-digest>`. Do not use a moving tag.
...
- The OCR readme indicates that MicroTx Enterprise Edition is designed to preserve transaction consistency across distributed microservices applications.
- The page explicitly enumerates XA, Saga based on Eclipse MicroProfile Long Running Action (LRA), and Try-Confirm/Cancel (TCC) as the supported distributed-transaction protocols.
- OCR further states that the Enterprise Edition introduces capabilities such as transaction promotion, metrics visualization, and integration with the MicroTx console.

## Oracle Version Notes (19c vs 26ai)

The OCR detail page is organized around MicroTx image releases rather than a 19c-versus-26ai database matrix. Use the repository tags table to select the coordinator version that fits your needs.
