...
Use this guide to quickly identify the appropriate Oracle Container Registry (OCR) database-category image before consulting a repo-specific skill file.
...
1. Sign in and accept the repository terms on OCR for the target image.
2. Pull the image using an explicit tag whenever possible.
3. Begin with the OCR README sample command for that repository.
...
For database server images, supply the required environment variables, persistent volumes, and published ports exactly as documented on the repository page before production use.

## Oracle Version Notes (19c vs 26ai)

OCR container repositories are organized by product line rather than a single unified 19c-vs-26ai matrix:

- 26ai-oriented images are found in repositories such as `enterprise`, `free`, and `graph-quickstart`.
- 19c/RU-oriented streams appear in repositories such as `enterprise_ru` and `rac_ru`.
- Tooling and runtime images (`sqlcl`, `ords`, `operator`, `observability-exporter`, `instantclient`) maintain their own release tags and should be pinned by image tag.
