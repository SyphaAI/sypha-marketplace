...
`database/otmm` is the Oracle Container Registry repository for Oracle Transaction Manager for Microservices (MicroTx) Free. Oracle places this repository in the OCR Database business area and exposes both a latest pull command and a tags table for choosing specific image versions.
...
- The OCR readme identifies `otmm` as the free MicroTx image intended to help preserve consistency across distributed microservices applications.
- The page documents that MicroTx supports XA, Saga based on Eclipse MicroProfile Long Running Action (LRA), and Try-Confirm/Cancel (TCC) style workflows.
- OCR provides a link to the install-and-use workflow and publishes both `latest` and versioned MicroTx tags for this repository.

## Oracle Version Notes (19c vs 26ai)

The OCR detail page is organized around MicroTx image releases rather than a 19c-versus-26ai database matrix. Use the repository tags table to select the MicroTx release that fits your requirements.
