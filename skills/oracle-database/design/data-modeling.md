...
Data modeling defines the way data is structured, stored, and interconnected within a database. It spans three layers of abstraction: **conceptual** (business concepts), **logical** (platform-independent relational structure), and **physical** (Oracle-specific DDL with storage parameters). This guide addresses logical and physical modeling techniques, data warehouse schema patterns, Operational Data Store design, and Oracle-specific physical model considerations such as storage clauses, compression, and partitioning.
...
The conceptual model captures business entities and their high-level relationships with no technical detail. It is tool-agnostic and designed for communication with business stakeholders.
...
The logical model is platform-independent yet technically complete. It specifies:
...
The physical model is Oracle-specific DDL. It introduces:
...
Online Transaction Processing (OLTP) systems place the highest priority on **write performance**, **data integrity**, and **concurrency**. The logical model for OLTP adheres closely to normalization rules (3NF minimum).
...
Data warehouses optimize **read performance** for analytical queries across large data volumes. **Dimensional modeling** (Ralph Kimball methodology) organizes data into **fact tables** and **dimension tables**.

### Fact Tables

Fact tables record measurable business events (sales, transactions, events). They hold:
...
Dimension tables hold descriptive context for facts. They include:
...
The **star schema** is the simplest and most query-efficient dimensional model. The fact table sits at the center with dimension tables radiating outward. Dimension tables are not normalized against each other — all descriptive attributes are flattened into a single wide dimension table.
...
**Note:** Bitmap indexes are well-suited to data warehouse FK columns (low cardinality, read-heavy, infrequent DML). Do not use bitmap indexes on OLTP tables with high concurrent writes — they produce severe lock contention.

> **Important:** `COMPRESS FOR QUERY HIGH` relies on Oracle Hybrid Columnar Compression (HCC). HCC is **not** a standard Advanced Compression feature — it requires Exadata, Oracle ZFS Storage Appliance, Oracle Database Appliance (ODA), or another HCC-compatible engineered system. On conventional server storage this clause will not deliver columnar compression. For non-Exadata environments, use `ROW STORE COMPRESS ADVANCED` (Advanced Row Compression, requires the Advanced Compression option) or `COMPRESS BASIC` (direct-path inserts only, all editions).

---

## 5. Snowflake Schema

The **snowflake schema** normalizes dimension tables by splitting sub-hierarchies into separate tables. This approach reduces storage consumed by dimension data at the cost of additional joins.
...
An **Operational Data Store** acts as a bridge between OLTP source systems and the data warehouse. It delivers:

- Near-real-time integrated operational reporting
- A staging and cleansing layer prior to DW loading
- A unified integrated view across multiple source systems

### ODS Design Principles

- **Subject-oriented**: Structured around business subjects rather than individual source systems
- **Integrated**: Data from multiple sources is reconciled into a unified model
- **Current**: Reflects the near-real-time operational state (unlike the DW, which is historical)
- **Volatile**: ODS data is updated in-place (unlike the DW, which is append-only)
...
SCDs handle changes to dimension data as they occur over time. Oracle supports all three common SCD types.

### SCD Type 1 — Overwrite

No history is retained. This is the simplest approach and is used when historical tracking is unnecessary.
...
A new row is inserted for each change. The previous row is closed out with an end date.
...
Retains both the current value and the single most recent prior value. History coverage is limited, but queries remain simple.
...
Select data types carefully to maximize storage efficiency and ensure correctness:
...
Oracle storage parameters govern how physical space is allocated within a segment:
...
Oracle offers several compression tiers:
...
For data warehouse tables, enable parallel DML and query execution:
...
- **Separate OLTP and DW schemas** into distinct tablespaces (and ideally separate databases or PDBs) to isolate I/O profiles and backup strategies.
- **Use surrogate keys in dimension tables**, never business or natural keys as dimension primary keys. Natural keys change over time; surrogate keys do not.
- **Pre-aggregate selectively.** Oracle materialized views can act as pre-aggregated summaries that the query optimizer exploits automatically via query rewrite.
- **Apply compression to data warehouse tables.** On Exadata (HCC), `COMPRESS FOR QUERY HIGH` achieves 10x or greater compression on fact tables. In non-Exadata environments, use `ROW STORE COMPRESS ADVANCED` (Advanced Compression option required), which typically yields 2:1 to 4:1 ratios for DW bulk loads.
- **Design for ETL patterns.** Include audit columns (`LOAD_DATE`, `SOURCE_SYSTEM`, `BATCH_ID`) in every DW table from the outset — adding them after the fact is costly.
- **Avoid triggers on DW fact tables.** Row-level triggers during high-volume bulk loads severely degrade performance. Incorporate that logic into the ETL load process instead.
- **Document the grain** of every fact table explicitly — the grain is the finest level of detail recorded by the fact table (e.g., "one row per order line item per day").
...
Executing analytical queries against a normalized OLTP schema results in large join trees and full table scans. Keep a separate dimensional model (star schema) for analytics workloads.

### Mistake 2: Using Operational Primary Keys as Dimension Surrogate Keys

Business keys (order numbers, product SKUs, customer email addresses) are subject to change over time. Always generate a dedicated surrogate key for dimension tables.

### Mistake 3: Missing SCD Strategy

Not deciding on an SCD type before go-live means historical changes are silently discarded. Define and implement the SCD type for each dimension table before the initial data load.

### Mistake 4: Not Partitioning Large Fact Tables

Fact tables that exceed 50–100 million rows without partitioning will be subject to full table scans and extremely slow maintenance operations (archiving, deletes, statistics gathering).
...
Bitmap indexes acquire locks at the bitmap segment level during DML — a single insert or update can block dozens of concurrent transactions. Restrict bitmap indexes exclusively to data warehouse tables that are loaded in bulk during maintenance windows.

### Mistake 6: Storing Calculated Fields Without Documentation

Pre-storing calculated values (gross profit, discounts) can be justified for performance reasons, but without documenting the underlying formula, discrepancies between source data and derived values become inevitable. Use virtual columns wherever feasible, or record the formula in a column comment.
...
- Design schemas granting **only the privileges application users actually need**:
  ```sql
  ...
  ```
- Keep the schema owner account separate from the application user account:
  ```sql
...
- **Identify and isolate sensitive data** (PII, PCI, PHI) into dedicated tables or schemas:
  ```sql
  ...
  ```
- Use **Virtual Private Database (VPD)** to enforce row-level security at the schema layer:
  ```sql
...
- **Encrypt sensitive columns** directly at the storage layer:
  ```sql
  ...
  ```
- **Never store CVV/CVC** — doing so violates PCI-DSS requirement 3.2
- Use **tokenization** for payment card data rather than storing the actual PAN wherever possible

### Auditing and Monitoring Considerations
- Design tables to include **audit columns** that support forensic analysis:
  ```sql
  ...
  ```
- Enable **Fine-Grained Auditing (FGA)** on tables that contain sensitive data:
  ```sql
  BEGIN
    DBMS_FGA.ADD_POLICY(
...
- **Enforce validation at the schema level** using database constraints:
  ```sql
  CREATE TABLE users (
  ...
  ```
- Use **check constraints** to enforce business rules and block invalid data
- Consider **virtual columns** for derived values that should not be persisted directly:
  ```sql
  CREATE TABLE orders (
...
- **Use edition-based redefinition** to perform zero-downtime schema upgrades:
  ```sql
  ...
  ```
- Apply **backward-compatible changes** whenever practical:
  ```sql
...
- **Design for data minimization** (GDPR Article 5(1)(c)):
  ...
  - Implement automated purging and archive strategies:
    ```sql
    ...
    ```
- **Support the right to erasure** (GDPR Article 17):
  ```sql
...
- **Design for data portability** (GDPR Article 20):
  - Structure data so it can be exported readily in standard formats (JSON, XML, CSV)
  - Consider Oracle's JSON relational duality views for flexible, format-agnostic export
...
- Features labeled 21c, 23c, or 23ai should be considered Oracle Database 26ai-capable; retain 19c-compatible alternatives for estates running mixed versions.
- In dual-support environments, verify syntax and package behavior in both 19c and 26ai, as defaults and deprecations may vary across release updates.
