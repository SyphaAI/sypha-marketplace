...
Solid error handling is among the most important concerns in PL/SQL development. Oracle's structured exception model, applied correctly, yields code that is reliable, diagnosable, and easy to maintain. This guide addresses the full exception hierarchy, techniques for raising exceptions, diagnostic functions, logging patterns, and propagation rules.

---

## Oracle Exception Hierarchy

All Oracle exceptions share a common internal structure and fall into three distinct categories:

### 1. Predefined Exceptions

Named exceptions covering frequently encountered Oracle errors. No declaration is required — reference them directly in exception handlers.
...
Declared within a package spec or a procedure/function to represent business logic errors:
...
Assign a meaningful name to any Oracle error number that does not already have a predefined exception name:
...
**Best practice**: Centralize frequently used PRAGMA exceptions in a shared exceptions package so they do not have to be redeclared across the codebase.
...
Used to re-raise the currently active exception or to raise a previously declared exception:
...
Raises an application-defined error with a custom message string. Error numbers must fall within the reserved range of `-20000` to `-20999`.
...
**Third parameter — preserve the existing error stack:**
...
Passing `TRUE` keeps the original Oracle error in the stack, which makes subsequent diagnosis considerably easier.
...
Returns the error message associated with the currently active exception or with a specific error code:
...
**Limitation**: `SQLERRM` output is truncated at 512 bytes. Use `DBMS_UTILITY.FORMAT_ERROR_STACK` to retrieve the complete message.

### DBMS_UTILITY.FORMAT_ERROR_STACK

Returns the complete error message (up to 2000 bytes), including any chained errors:
...
**Critical for debugging**: Returns the line number at which the exception was originally raised, not merely the location where it was caught.
...
**Best practice**: Capture both `FORMAT_ERROR_STACK` and `FORMAT_ERROR_BACKTRACE` in every error log entry.

---

## Robust Error Logging with Autonomous Transactions

The core challenge: when a main transaction rolls back because of an error, any ordinary `INSERT INTO error_log` rolls back along with it. Declaring `PRAGMA AUTONOMOUS_TRANSACTION` on the logging procedure ensures the log entry is committed independently and survives the rollback.
...
Propagates the original exception type and error code unchanged:
...
Converts the original exception into a new application-level error while retaining the original stack information:
...
1. When an exception is raised and the current block has **no matching handler**, it propagates to the **enclosing block**.
2. If it reaches the **outermost block** without being handled, it propagates to the **caller**.
3. If no caller handles it, the exception is returned to the client as an error message.
4. Handlers are evaluated in declaration order — the **first matching** handler takes effect.
5. `WHEN OTHERS` catches every exception and must always be the **last** handler in the block.
6. An exception raised **inside a handler** propagates immediately to the enclosing block because the current block's handlers are already consumed.
...
This is one of the most harmful anti-patterns in PL/SQL. It silently discards all exceptions, rendering bugs effectively invisible and impossible to diagnose.
...
**The only acceptable use** of `WHEN OTHERS` without re-raising is in a genuinely "best effort" scenario where failure is truly tolerable and explicitly documented — the most common example being the logging procedure itself.

---

## Exception Handling Best Practices

1. **Always capture `FORMAT_ERROR_BACKTRACE`** in error logs — line numbers are indispensable for diagnosis.
2. **Use autonomous transactions for error logging** so that log entries survive a transaction rollback.
3. **Never use `WHEN OTHERS THEN NULL`** — always log the exception and re-raise it.
4. **Centralize error logging** within a single package to guarantee consistent behaviour across the application.
5. **Declare shared exceptions in a common package** — avoid repeating PRAGMA exception declarations throughout individual package bodies.
6. **Handle specific exceptions before the catch-all** — `WHEN NO_DATA_FOUND` should appear before `WHEN OTHERS`.
7. **Avoid empty exception sections** — if an exception is caught, handle it in a meaningful way.
8. **Retain the original error context** when wrapping with `RAISE_APPLICATION_ERROR` by supplying `TRUE` as the third argument.
9. **Do not exploit exceptions for flow control** — exceptions signal unexpected conditions and should not be used as ordinary branching logic.
...
- **Oracle 12c+**: The `UTL_CALL_STACK` package provides a structured programmatic API for inspecting the call stack, error stack, and backtrace — a more capable alternative to the string-based `DBMS_UTILITY` functions. Functions confirmed in 19c documentation: `ERROR_DEPTH`, `ERROR_NUMBER`, `ERROR_MSG`, `BACKTRACE_DEPTH`, `BACKTRACE_UNIT`, `BACKTRACE_LINE`, `DYNAMIC_DEPTH`, `SUBPROGRAM`, `OWNER`, `UNIT_LINE`.
- **All versions**: `DBMS_UTILITY.FORMAT_ERROR_BACKTRACE` must be called while the exception is still active (i.e., from within the exception handler); otherwise the backtrace is cleared.
- **Oracle 10g+**: `DBMS_UTILITY.FORMAT_ERROR_BACKTRACE` was introduced. Prior to 10g, line number information was unavailable without the PL/SQL Debugger.
