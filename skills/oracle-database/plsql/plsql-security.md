...
PL/SQL security covers how stored code exercises database privileges, how it resists SQL injection, and how it avoids unintended privilege escalation. Oracle provides a sophisticated rights model and a set of validation utilities (`DBMS_ASSERT`) that, when applied correctly, make PL/SQL code resilient against common attack vectors.

---

## AUTHID: Definer Rights vs Invoker Rights

Every PL/SQL unit executes under either **definer rights** (the default) or **invoker rights** (`AUTHID CURRENT_USER`). This setting determines which schema's privileges are active when the code runs.

### Definer Rights (Default: AUTHID DEFINER)

The code runs with the privileges of the schema that owns it, regardless of who invokes it.
...
- The procedure accesses a specific schema's tables and should abstract privilege details away from callers
- You want to grant execute on the procedure instead of granting direct table access
- Implementing a controlled API layer where callers cannot directly reach the underlying tables

**Security implication**: The calling user can perform actions for which they have no direct privileges. This is intentional for controlled APIs, but becomes a risk when the procedure has SQL injection vulnerabilities — an attacker can exploit the elevated privilege context.
...
- Multi-tenant systems where each tenant has an identical schema structure
- Preventing unintended privilege escalation
...
SQL injection occurs when attacker-controlled data is concatenated into a SQL or PL/SQL statement. In PL/SQL, this vulnerability most commonly appears in dynamic SQL.
...
Table names, column names, and schema names cannot be supplied as bind variables — they are structural SQL components. For these cases, use `DBMS_ASSERT` to validate input before it is concatenated into the statement.

---

## DBMS_ASSERT Functions

`DBMS_ASSERT` provides validation routines that raise exceptions for invalid or potentially malicious input before it is incorporated into dynamic SQL.
...
A definer rights procedure that contains a SQL injection vulnerability grants the attacker the owner's privileges rather than the caller's. This is the most dangerous form of SQL injection in Oracle.
...
4. Avoid assigning `SELECT ANY TABLE` or `DBA` roles to schemas that own accessible PL/SQL
...
Oracle Database Vault (an optional licensed feature) adds realm-based access control on top of standard Oracle privileges. Even accounts holding the DBA role can be blocked from reaching application data when Database Vault realms are properly configured.
...
- **Oracle 18c+**: `DEFAULT COLLATION` and `COLLATION` on individual columns can affect string comparison in dynamic SQL — confirm that DBMS_ASSERT behavior is consistent with the collation settings in use.
- **All versions**: `DBMS_ASSERT` has been available since Oracle 10.2; apply it consistently in all PL/SQL that handles user-supplied identifiers in dynamic SQL.
