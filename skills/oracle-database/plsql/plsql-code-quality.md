...
Producing consistent, readable, and maintainable PL/SQL code depends on established naming conventions, deliberate avoidance of well-known anti-patterns, and automated static analysis. This guide covers style standards, anti-pattern identification, code review practices, and available tooling.

---

## Naming Conventions

Consistent naming underpins readable code. The most widely adopted conventions are derived from Oracle's own internal standards and the Trivadis PL/SQL Coding Guidelines.
...
The [Trivadis PL/SQL and SQL Coding Guidelines](https://trivadis.github.io/plsql-and-sql-coding-guidelines/) represent the most comprehensive publicly available style standard for PL/SQL. Key rules include:
...
**Exception**: `SELECT * ... INTO l_%ROWTYPE` is acceptable when %ROWTYPE is used and every column is genuinely required.
...
PL/SQL Cop is a commercial static analysis tool built on the Trivadis guidelines. It integrates with Maven, Gradle, and CI/CD pipelines.
...
SonarQube combined with the Oracle PL/SQL plugin (included in SonarQube Developer Edition) offers:
...
SQL Developer ships with a built-in code analysis tool:
1. Navigate to **Tools > Code Analysis > Run Analysis**
2. Choose the desired rule sets (PL/SQL Best Practices, Security, etc.)
3. Results are displayed in the Code Analysis panel with associated line numbers

---

## McCabe Cyclomatic Complexity

Cyclomatic complexity quantifies the number of linearly independent paths through a unit of code. Higher complexity translates directly to greater difficulty in testing and maintenance.

**Formula**: Complexity = Number of decision points + 1

Decision points include: `IF`, `ELSIF`, `CASE WHEN`, `LOOP`, `WHILE`, `FOR`, `EXCEPTION WHEN`, and `AND`/`OR` in conditions.
...
When a procedure surpasses these thresholds, extract well-named helper procedures for the inner logic. The original procedure then acts as a coordinator.
