...
Effective PL/SQL debugging draws on a spectrum of techniques ranging from basic output utilities to full-featured interactive debuggers. Knowing the capabilities and constraints of each method allows you to isolate problems more quickly across both development and production environments.

---

## DBMS_OUTPUT

`DBMS_OUTPUT` is the most straightforward debugging tool available — it writes messages to a server-side buffer that the client reads after the PL/SQL block finishes executing.
...
`DBMS_APPLICATION_INFO` populates the `MODULE`, `ACTION`, and `CLIENT_INFO` fields that appear in `V$SESSION`. This is particularly useful for tracking long-running operations in production without making substantial changes to business logic.
...
SQL Developer includes a full interactive debugger supporting breakpoints, watch expressions, and step-by-step execution control.
...
Oracle can generate compile-time warnings that flag potential problems in PL/SQL code before it runs.
...
`UTL_FILE` writes directly to server-side operating system files, making it a practical logging mechanism for batch jobs where `DBMS_OUTPUT` is not usable.
...
`DBMS_TRACE` activates server-side PL/SQL execution tracing. Collected trace data is stored in the `PLSQL_TRACE_EVENTS` and `PLSQL_TRACE_RUNS` tables.
...
Enable SQL trace to record comprehensive SQL and PL/SQL execution information, including wait events and bind variable values.
...
- Call `DBMS_APPLICATION_INFO` in all long-running procedures — the overhead is negligible and it greatly simplifies monitoring.
- Prefix debug messages with timestamps using `TO_CHAR(SYSTIMESTAMP, 'HH24:MI:SS.FF3')` for precision.
- Control debug output through a package-level flag so it can be toggled without touching business logic.
- Always compile with `DEBUG` symbols during development; revert to normal optimization settings before deploying to production.
- Remove or disable `DBMS_OUTPUT` calls on production code paths — replace them with a proper logging mechanism.
- Use SQL trace at level 12 to investigate unexpected SQL execution behaviour, including bind variable values.
- Store `UTL_FILE` log files in a dedicated directory with appropriately restricted permissions.
- Compile periodically with `PLSQL_WARNINGS = 'ENABLE:ALL'` to surface unreachable code and performance-related issues early.
...
| Mistake | Problem | Fix |
|---|---|---|
| Relying on DBMS_OUTPUT in production | Output is not visible to applications | Use proper logging tables backed by autonomous transactions |
| Not checking IS_OPEN before FCLOSE | Raises an exception if the file was never successfully opened | Always guard with `IF UTL_FILE.IS_OPEN(l_file) THEN CLOSE; END IF` |
| Compiling DEBUG symbols for production | Adds performance overhead and exposes source-level information | Compile normally for production using `ALTER PROCEDURE ... COMPILE` |
| Not clearing DBMS_APPLICATION_INFO on exit | Module and Action fields remain set for the next user of a pooled connection | Clear them in both the normal exit path and the exception handler |
| Enabling PLSQL_WARNINGS system-wide without prior testing | Triggers a flood of compile errors against existing code | Enable at session level first, resolve all warnings, then promote to system level |
| Writing large debug strings inside tight loops | Causes measurable memory and CPU overhead | Log conditionally, for example only every N iterations |
...
- **Oracle 11g+**: `DBMS_APPLICATION_INFO.SET_SESSION_LONGOPS` with improved granularity.
- **Oracle 12c+**: Enhanced SQL Developer debugger with better support for 12c-specific features.
- **Oracle 18c+**: Cloud-compatible tracing with Automatic Diagnostic Repository (ADR) improvements.
- **All versions**: `DBMS_OUTPUT.ENABLE(NULL)` for an unlimited output buffer has been available since 10g.
