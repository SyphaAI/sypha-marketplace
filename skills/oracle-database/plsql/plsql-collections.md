...
Collections are the principal data structure for managing in-memory datasets in PL/SQL. Three distinct types exist, each differing in storage semantics, SQL visibility, and initialization requirements. Knowing which type to choose — and how to invoke collection methods safely — is fundamental to both correctness and performance.
...
Declared using `INDEX BY`. No initialization is required. Supports both string and integer key types.
...
**Use case**: Caching lookup data, constructing in-memory hash maps, and any scenario requiring string-keyed access.

---

## Nested Tables

Must be initialized prior to use. Support the full suite of collection methods and the SQL `TABLE()` operator.
...
Maximum size is fixed at type declaration time. Cannot be sparse. Must be initialized before use.
...
**Use case**: Situations where the maximum size is known and constant (months, days of the week, fixed-size lookup arrays stored as database column values).
...
Collections are the mechanism for performing bulk DML through `FORALL` and bulk SELECT operations through `BULK COLLECT`.
...
**Using %ROWTYPE for more concise code**:
...
Schema-level nested table and varray types can be referenced directly in SQL using the `TABLE()` operator.
...
**Defensive pattern**: Always verify with `EXISTS()` before accessing an element by index, particularly after any `DELETE` operation.

---

## Multi-Level Collections

Nested tables can be composed of other collection types, forming nested tables of nested tables.
...
- Prefer **associative arrays** for all in-memory temporary storage in PL/SQL — they require no initialization overhead.
- Choose **nested tables** when SQL access via `TABLE()`, bulk DML, or set operations (`MULTISET UNION`, `MULTISET INTERSECT`) are needed.
- Use **varrays** only when the maximum element count is truly fixed and the type must be persisted as a database column.
- Always guard element access with `EXISTS()` when the element may have been deleted.
- When iterating a potentially sparse collection, use `FIRST`/`NEXT` navigation rather than a numeric FOR loop.
- Always include a `LIMIT` clause with `BULK COLLECT` in loops — unbounded `BULK COLLECT` on large tables will exhaust PGA.
- Anchor collection element types to their source column definitions using `%TYPE` to remain resilient against schema changes.
