...
PL/SQL performance tuning focuses on reducing context switches between the PL/SQL and SQL engines, processing data in sets rather than one row at a time, and caching results where practical. This guide covers the key techniques that deliver the greatest performance improvements in production environments.

---

## Context Switch Cost

Each time PL/SQL dispatches a SQL statement to the SQL engine (or the reverse), a **context switch** takes place. These switches carry overhead: the engines must coordinate, exchange data, and resume execution. In a loop processing 100,000 rows individually, this translates to 100,000 discrete context switches — a leading cause of sluggish PL/SQL code.
...
The objective is to batch work: issue one SQL statement that handles many rows rather than many statements each handling one row.
...
When the transformation cannot be expressed in pure SQL — for example, complex calculations or per-row calls to PL/SQL functions — use bulk operations.

---

## BULK COLLECT with LIMIT Clause

`BULK COLLECT` retrieves multiple rows into a collection in a single context switch. The `LIMIT` clause caps memory consumption.
...
**Choosing LIMIT size**: 100–1000 is a typical range. Too small results in many context switches; too large consumes excessive PGA memory. Profile against your data volume and available PGA.

---

## FORALL with SAVE EXCEPTIONS

`FORALL` submits the entire collection to the SQL engine in a single call, removing per-row context switches for DML operations.
...
Without `SAVE EXCEPTIONS`, the first failing row halts the entire `FORALL` and triggers a rollback. With `SAVE EXCEPTIONS`, failures are recorded and processing continues through the rest of the collection.
...
A pipelined function emits rows one at a time via `PIPE ROW`, letting the caller begin consuming rows as they are produced — analogous to a Unix pipe. This eliminates the need to materialize the entire result set in memory.
...
**Benefits**: Minimal memory footprint for large result sets; the consumer can process rows before the function completes; enables parallel queries against the result.

---

## NOCOPY Hint

By default, `IN OUT` parameters are passed by value — a copy is created on entry and another on exit. For large collections, this copying overhead is significant. `NOCOPY` passes by reference instead, eliminating the copy.
...
**When to use NOCOPY**: Collections containing hundreds or thousands of elements where performance is measurably a bottleneck. Avoid it when partial modification on exception would cause problems.

---

## RESULT_CACHE for Functions

`RESULT_CACHE` stores function output keyed by input parameter values. Subsequent invocations with identical inputs return cached results without re-executing the function body.
...
`RELIES_ON (tax_rates)` instructs Oracle to invalidate cached entries when the `tax_rates` table is modified. In Oracle 11gR2+, `RELIES_ON` is optional — Oracle detects dependencies automatically.
...
`DETERMINISTIC` signals to Oracle that for the same input values the function always returns the same result. This hint permits Oracle to:
1. Cache the function result within a single SQL statement (avoids re-invoking for identical inputs)
...
**Key difference from RESULT_CACHE**: `DETERMINISTIC` is an optimizer hint scoped to a single SQL execution; `RESULT_CACHE` persists results across calls and sessions.

**Warning**: Marking a function as `DETERMINISTIC` when it depends on external state (sequences, `SYSDATE`, session variables) will produce incorrect results. The declaration is not enforced — correctness is entirely the developer's responsibility.

---

## Avoiding Unnecessary Parsing

Hard parsing is costly: the SQL engine must verify syntax, resolve objects, optimize the execution plan, and produce executable code. Soft parsing is far cheaper because it reuses a cached cursor. PL/SQL helps by using static SQL that is bound at compile time.
...
Understanding when the result cache is invalidated prevents hard-to-diagnose data-staleness issues.
...
- **Oracle 12c+**: `PRAGMA UDF` (User Defined Function) hint reduces context switch overhead when a PL/SQL function is called from SQL. Apply when SQL expressions cannot replace the function.
- **All versions**: `BULK COLLECT` and `FORALL` have been available since Oracle 9i and remain the primary bulk operation tools.
