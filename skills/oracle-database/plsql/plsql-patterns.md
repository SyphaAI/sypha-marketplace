...
Well-established design patterns in PL/SQL produce code that is maintainable, performant, and testable. This guide examines the most commonly applied patterns: table APIs, autonomous transactions, pipelined functions, record types, object-relational types, multi-result-set patterns, and cursor reuse.

---

## Table API (TAPI) Pattern

A Table API consolidates all DML operations for a given table into a single package, creating one authoritative point of control for inserts, updates, deletes, and queries. It decouples SQL from business logic and allows the data layer to be refactored without affecting callers.
...
**TAPI tools**: Oracle offers generators such as `tapi_gen`, `OraOpenSource/tapi`, and the SQL Developer data modeler can auto-generate TAPI packages.

---

## Autonomous Transaction Pattern for Audit and Logging

The autonomous transaction pattern guarantees that audit and log records are committed independently of the main transaction — log entries survive a rollback of the outer transaction.
...
**Caution**: Autonomous transactions hold their own locks and operate on their own view of committed data. Keep autonomous transactions narrowly focused on INSERT-and-COMMIT for logging — avoid complex queries inside them.

---

## Pipelined Function for Streaming Results

A pipelined function delivers rows to the caller as they are generated — rather than assembling the complete result in memory first. This makes it well suited for transformation and ETL-style workloads.
...
Oracle object types bring OOP concepts — encapsulation, inheritance, and polymorphism — into the database layer.
...
A procedure with multiple OUT SYS_REFCURSOR parameters can deliver several independent result sets back to the caller simultaneously.
...
When dynamic SQL must be executed repeatedly with varying bind values, parsing once and re-executing avoids the cost of repeated hard parses.
...
For most use cases, `FORALL` with static SQL is simpler and faster than the `DBMS_SQL` parse-once approach. Reserve `DBMS_SQL` for scenarios where the SQL structure itself is dynamic — such as an unknown column count or DDL with variable arguments.

---

## Best Practices

- Use TAPI to centralize all DML — when a table schema changes, only the TAPI requires updates, not every caller.
- Reserve autonomous transactions exclusively for logging and auditing — do not use them as a workaround for general DML.
- Always handle `NO_DATA_NEEDED` in pipelined functions — it indicates that the consumer stopped reading early, not an error condition.
- Prefer `%ROWTYPE` over explicit record types when the structure maps directly to a table — it automatically adapts to schema changes.
- Apply object types to complex domain models that benefit from encapsulation; use record types for straightforward data transfer.
- When returning multiple cursors, clearly document which caller is responsible for closing each one.
- Cache DBMS_SQL cursor handles at the package level for procedures invoked within tight loops.
