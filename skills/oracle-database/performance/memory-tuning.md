...
Oracle database memory is organized into two primary structures:

- **SGA (System Global Area):** Memory shared across all sessions and background processes. Contains the buffer cache, shared pool, large pool, Java pool, streams pool, and redo log buffer.
- **PGA (Program Global Area):** Private memory allocated per session. Handles sort areas, hash join areas, bitmap operations, and session state.

Memory sizing is among the highest-impact tuning decisions available — correct sizing eliminates a wide range of downstream problems, while incorrect sizing leads to excessive I/O, hard parsing, and temp spills.

Oracle supports three memory management modes:
- **Manual Memory Management:** The DBA explicitly sets every SGA component and PGA sort area parameter.
- **ASMM (Automatic Shared Memory Management):** The DBA sets `SGA_TARGET`; Oracle automatically distributes memory across SGA components.
- **AMM (Automatic Memory Management):** The DBA sets `MEMORY_TARGET`; Oracle manages both SGA and PGA automatically.
...
The buffer cache stores copies of data blocks retrieved from disk. It is generally the largest SGA component and the one with the greatest impact on performance.
...
**Target:** Buffer cache hit ratio should exceed 95% for OLTP workloads. Always verify absolute physical read counts as well — a 99% hit ratio combined with 10 million physical reads per minute still represents substantial I/O load.

### Keep and Recycle Pools

Dedicated buffer pools for tailored access patterns:
...
- **Library Cache:** Parsed SQL, PL/SQL code, and execution plans
...
Over time, repeated small allocations and deallocations can fragment the shared pool, producing `ORA-04031: unable to allocate N bytes of shared memory` errors even when aggregate free memory is available.
...
The large pool is an isolated memory region for certain large allocations that would otherwise consume shared pool space:
...
**Rule of thumb:** Calculate the large pool size as expected RMAN parallelism × channel buffer sizes plus parallel query slave count × message buffer size.

### Redo Log Buffer

The redo log buffer is a circular SGA buffer that holds redo records until LGWR flushes them to the online redo log files.
...
**Sizing guidance:** The default value is usually sufficient (3-15 MB). Increase it when `redo log space requests > 0` or the redo buffer hit ratio is poor. Values above 64 MB seldom yield additional benefit.

---

## PGA Memory Management

The PGA is private memory dedicated to each session. Its main consumers are:

- **Sort Area:** `ORDER BY`, `GROUP BY`, `DISTINCT`, and index creation
- **Hash Area:** Hash joins and hash aggregation
- **Bitmap Area:** Bitmap merge operations
- **Session State:** Cursor state, bind variables, and execution context
...
When the work area allocated to a sort or hash join is insufficient, Oracle spills intermediate data to the TEMP tablespace on disk. This causes a dramatic reduction in operation speed.
...
AMM handles both SGA and PGA automatically. Only `MEMORY_TARGET` needs to be configured, with `MEMORY_MAX_TARGET` as an optional ceiling. This mode is suited only to smaller systems where total SGA is under 4 GB.
...
**AMM Pros:** Minimal administrative overhead; Oracle redistributes memory between SGA and PGA dynamically as demand changes.

**AMM Cons:**
- Incompatible with HugePages on Linux, resulting in significant performance degradation on large instances
- Less deterministic; memory allocations can shift in unexpected ways
- Not recommended for the majority of Linux production environments with large memory footprints

### Automatic Shared Memory Management (ASMM)

ASMM automatically manages SGA components. You specify `SGA_TARGET` and, optionally, per-component minimums. PGA is handled independently through `PGA_AGGREGATE_TARGET`.
...
On Linux, allocating the SGA using HugePages (2 MB pages rather than 4 KB) substantially reduces TLB (Translation Lookaside Buffer) pressure and prevents page swapping.
...
**Important:** When HugePages are in use, set `USE_LARGE_PAGES = ONLY` in Oracle initialization parameters. Note that `MEMORY_TARGET` (AMM) **cannot** leverage HugePages — this is the primary reason ASMM is preferred on Linux production systems.
...
- **Prefer ASMM + HugePages on Linux production** over AMM. HugePages deliver significant performance gains for large SGAs but are incompatible with AMM.
- **Set `SGA_MAX_SIZE` >= `SGA_TARGET`** so Oracle can grow dynamically without requiring a restart.
- **Pin frequently-used packages in the shared pool** using `DBMS_SHARED_POOL.KEEP` to limit fragmentation and prevent eviction during memory pressure.
- **Watch `free buffer waits` and `sorts (disk)`** as early indicators of memory pressure.
- **Consult advisory views** (`V$DB_CACHE_ADVICE`, `V$PGA_TARGET_ADVICE`, `V$SHARED_POOL_ADVICE`) before adjusting memory parameters — they quantify the expected benefit.
- **Preserve OS headroom.** Avoid allocating 100% of RAM to Oracle. Keep at least 10-20% reserved for the operating system, file system cache (when not using direct I/O), and other processes.
- **Explicitly size the large pool** when using RMAN with parallel channels or a large number of parallel query slaves.
