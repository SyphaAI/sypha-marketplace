...
The Oracle Cost-Based Optimizer (CBO) depends entirely on statistics to estimate the cost of candidate execution plans. When statistics are accurate, the optimizer selects efficient plans; when they are stale or missing, the result is poor cardinality estimates, incorrect join orders, and suboptimal plan choices.

Statistics encompass:
- **Table statistics:** row count, block count, average row length
- **Column statistics:** number of distinct values (NDV), null count, low/high value, histograms
- **Index statistics:** clustering factor, leaf blocks, index height
- **System statistics:** CPU speed and I/O throughput (multiblock read cost, single-block read cost)

The primary interface for managing statistics is the `DBMS_STATS` package.
...
**Note:** A default database installation includes the most suitable defaults for optimizer statistics gathering. In most situations, using the default parameters for DBMS_STATS calls will produce optimal results.
...
The clustering factor quantifies how closely a table's physical row order aligns with the index key order. A low value (near the block count) is ideal. A high value (near the row count) means the index will generate many random I/O operations.
...
Oracle flags statistics as STALE once more than 10% of rows have changed since the previous statistics collection.
...
Histograms record value distribution for columns with skewed data. Without histograms, the optimizer assumes a uniform distribution, which produces inaccurate estimates for skewed columns.

### When to Use Histograms

- The column exhibits **data skew** (certain values occur far more frequently than others)
- The column appears in `WHERE` predicates
- The column has low-to-medium cardinality with an uneven value distribution
...
- Column values follow a truly uniform distribution (no skew) — a histogram adds collection overhead with no benefit
- The column is never referenced in a `WHERE` predicate
- The column is consistently accessed with bind variables and bind variable peeking is relied upon (histograms can interact poorly with bind peeking in certain scenarios; factor in `CURSOR_SHARING` behavior)

---

## Extended Statistics

Extended statistics enable the optimizer to account for **correlated columns** or **expressions** in predicates. Without them, the optimizer multiplies individual column selectivities independently, leading to under- or over-estimated row counts when column values are correlated.
...
Lock statistics to prevent both automatic and manual regathering from overwriting verified-good statistics on specific objects. This is particularly useful for staging or test data fixtures, and when a known-good execution plan must be preserved.
...
Exporting statistics is useful when you need to:
- Replicate production stats to a test or development environment for realistic query testing
- Preserve known-good stats before a regather, providing a rollback point
- Migrate stats across different database versions
...
Oracle's automatic maintenance framework (Autotask) collects statistics on stale and missing objects during scheduled maintenance windows.
...
Starting with Oracle 11g, newly gathered statistics can be placed in a "pending" state, allowing you to test their effect in isolation before making them available to all sessions. Statistics can then be either published or discarded.
...
- **Rely on defaults unless customization is required** — A default database installation provides the most appropriate settings for optimizer statistics gathering. In most cases, accepting the default DBMS_STATS parameters will yield optimal outcomes.
- **Use `AUTO_SAMPLE_SIZE`** — Oracle's incremental statistics algorithm in 11g+ delivers accurate statistics with minimal overhead. Fixed sample percentages (e.g., 10%) are rarely an improvement.
- **Enable incremental statistics for partitioned tables** — this avoids a full rescan of unchanged partitions during regathers.
...
- **Gather statistics after conventional bulk loads and before executing queries** — a load that inserts millions of rows immediately renders existing statistics stale.
- **Export statistics before major changes** (upgrades, schema changes) so the previous state can be restored if plan regressions occur.
- **Use `METHOD_OPT => 'FOR ALL COLUMNS SIZE AUTO'`** as the standard option — Oracle determines which columns warrant histograms based on workload feedback.
- **Verify `STALE_STATS` before large batch jobs** — running a multi-hour batch against stale statistics is a common but preventable problem.
- **Lock statistics for reference and lookup tables** that change infrequently but participate in many joins.
