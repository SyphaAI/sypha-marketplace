...
ORDS provides a complete OAuth2-based security model for protecting REST endpoints. Access control is defined through **privileges** (which endpoints are protected) and **OAuth2 clients** (which applications or users can access them). ORDS supports the OAuth2 client credentials flow (machine-to-machine), the authorization code flow (user-facing web applications), and the implicit flow. ORDS also supports external identity providers via schema-level or pool-level JWT profiles, enabling integration with Oracle Identity Cloud Service (IDCS), Azure AD, Okta, and other OIDC-compatible providers.

Current ORDS releases use `ORDS_SECURITY` and `ORDS_SECURITY_ADMIN` for OAuth client lifecycle management. The older `OAUTH` and `OAUTH_ADMIN` packages are deprecated and must not be used in new implementations.
...
A **privilege** is a named access control gate attached to one or more URL patterns (REST modules or specific templates). Any request to a protected URL must present a valid OAuth2 Bearer token carrying the required privilege scope.

### Roles

**Roles** are optional labels assignable to privileges. A client or user must hold the required role to receive the privilege. Roles add a second layer of authorization on top of the OAuth2 scope check.

### OAuth Clients

An **OAuth client** represents an application (or user account) that needs to call protected REST APIs. Clients are issued a client ID and client secret, which they exchange for access tokens at the token endpoint.
...
To protect only selected URL patterns rather than an entire module, use `ORDS.DEFINE_PRIVILEGE` with the `p_patterns` array.

---

## OAuth2 Client Credentials Flow (Machine-to-Machine)

This flow is intended for server-to-server API calls where no interactive user is involved. A backend service uses its client ID and secret to obtain an access token.
...
Save the `client_secret` returned by `ROTATE_CLIENT_SECRET` immediately. It may not be queryable later unless explicitly stored.
...
Used when a human user must authenticate and explicitly authorize the application to access data on their behalf.
...
ORDS exposes standard OAuth2 endpoints scoped to each REST-enabled schema:
...
ORDS can validate JWTs issued by external identity providers (Oracle IDCS, Azure AD, Okta, Keycloak, etc.) without requiring the token to be issued by ORDS itself. In current ORDS releases, the JWT profile feature is configurable in one of two mutually exclusive modes:
...
Only one JWT profile can exist per schema. To replace it, delete the existing profile and recreate it. To manage the JWT profile for a different REST-enabled schema, use `ORDS_SECURITY_ADMIN.CREATE_JWT_PROFILE` and `ORDS_SECURITY_ADMIN.DELETE_JWT_PROFILE` with `p_schema`.
...
When ORDS encounters a JWT with `"roles": ["HR_ADMIN"]`, it maps this to the `HR_ADMIN` ORDS role, granting access to privileges that require that role.
...
- **Use client credentials for all service-to-service calls**: Never hardcode DB credentials in client applications. Use OAuth2 client credentials so access can be revoked without changing DB passwords.
- **Set short token expiry for sensitive operations**: The default ORDS token expiry is 1 hour. For high-security contexts, configure a shorter expiry and implement refresh token logic.
- **Scope privileges narrowly**: Create separate privileges for read, write, and admin operations. Clients receive only the minimum privileges required.
- **Use an external IdP for user-facing applications**: Rather than managing users in ORDS, integrate with your organization's identity provider via JWT profile. This enables SSO and centralizes user lifecycle management.
...
- **Audit privilege assignments**: Periodically review `user_ords_clients` and `user_ords_client_roles` to ensure no stale or over-privileged clients remain.

## Common Mistakes

- **Calling the token endpoint for the wrong schema**: ORDS token endpoints are schema-scoped (`/ords/{schema}/oauth/token`). Using the wrong schema path returns a 404.
- **Forgetting to grant roles to clients**: Creating a client and granting a privilege is insufficient if the privilege requires a specific role. Check `user_ords_client_roles`.
- **Sending client credentials in the request body instead of the Authorization header**: The OAuth2 spec allows both, but ORDS requires `Authorization: Basic <base64>` for the `client_credentials` grant. Body-based credentials fail with ORDS.
...
- **Exposing `client_secret` in client-side code**: For browser-based apps (SPAs), use PKCE (if supported by your ORDS version) or a backend-for-frontend pattern. Never embed client secrets in JavaScript.
- **Assuming roles in JWT match ORDS privilege names**: JWT roles are mapped to ORDS roles, and ORDS roles are associated with ORDS privileges. These are distinct levels. Configure the mapping carefully.
