...
ORDS includes built-in support for file upload and download over HTTP. Files are persisted in Oracle Database BLOB columns and delivered back through REST endpoints with the correct MIME type headers. This approach is suited to document management systems, image repositories, report archives, and any application requiring binary content to be stored and retrieved alongside relational metadata.

ORDS manages BLOB data transparently: uploaded file content is bound to PL/SQL handlers as a BLOB via the `:body` implicit parameter, and downloaded content is streamed directly from the database with the correct Content-Type header.
...
For HTML form-based uploads (browsers sending `multipart/form-data`), ORDS can handle the multipart `POST` request natively. Form fields are accessible through `:body_json`, and uploaded files can be processed using `ORDS.BODY_FILE_COUNT` and `ORDS.GET_BODY_FILE`.

Do not dereference both `:body` and `:body_text` in the same PL/SQL block. ORDS permits only one of those stream parameters per handler, and each stream bind may be dereferenced at most once unless first copied to a local variable. If you use `:body` or `:body_text`, you also cannot rely on JSON attribute bind notation from the request payload.
...
**Recommendation**: Native ORDS multipart handling is adequate when a request includes both form fields and one or more files. APEX remains a viable option for browser-based applications, but it is not required for multipart parsing. For service-to-service uploads where the request body contains only the file, prefer the direct binary PUT/POST approach with the file as the raw body.

For upload handlers, set `:forward_location` only when you intentionally want ORDS to execute a follow-up `GET` and return that representation as the upload response. Otherwise, return `201` or `200` without `:forward_location` and let the client fetch the `/content` resource separately.
...
The `media` source type is specifically designed for returning binary or text content. The SQL must return:
...
Specify the output filename on the client side unless you have separate, documented header handling for attachment filenames.
...
When you need to transform the payload before download, perform that work in SQL or in a PL/SQL function called from the `SELECT`, then return the final media type and BLOB through `source_type_media`.
...
Store MIME types in the database alongside the blob. Map file extensions to MIME types at upload time:
...
Control whether browsers render files inline or prompt for a download:
...
For files that exceed available memory, prefer `source_type_media`. ORDS streams the LOB response body automatically, so the handler does not need to implement manual chunking in PL/SQL.

Large uploads are constrained by the ORDS runtime and the fronting HTTP server, not by pagination settings such as `misc.pagination.maxRows`.

In Jetty standalone, review the request-size and JVM memory settings for the deployed ORDS runtime before accepting large files. For very large payloads, consider chunked upload (where the client splits the file into parts, uploads each part, and the server reassembles them) rather than relying on a single request body.
...
- **Use `source_type_media` for downloads when possible**: It is the most straightforward and efficient approach. ORDS handles all streaming and header management automatically.
...
- **Store MIME type in the database**: Do not rely solely on file extension at download time. Store the `Content-Type` from the upload request and use it when serving the file back.
- **Use SECUREFILE LOB storage**: SECUREFILE offers compression, deduplication, and encryption for LOB columns. It is significantly more efficient than BASICFILE for large-scale file storage.
...
- **Dereferencing `:body` or `:body_text` multiple times**: Read the implicit payload bind into a local variable first. If you call `DBMS_LOB.GETLENGTH(:body)` and then reuse `:body` later, the later read may return empty.
- **Not setting `Content-Type` on download**: Without a Content-Type header, browsers guess the type and may display the wrong viewer or launch the wrong application.
- **Forgetting COMMIT in upload handlers**: PL/SQL handlers do not auto-commit. An INSERT/UPDATE of a BLOB without COMMIT leaves the row locked and the data invisible to other sessions.
...
- **Encoding filenames with special characters in headers**: Filenames containing spaces, accents, or non-ASCII characters in Content-Disposition headers must be RFC 5987 encoded. Plain `filename="résumé.pdf"` will be garbled in some clients.
- **Using BASICFILE LOB for new tables**: BASICFILE is legacy. Always specify `STORE AS SECUREFILE` for BLOB columns in new tables to take advantage of better performance and storage options.
