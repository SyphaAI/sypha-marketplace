...
Sessionless transactions in Oracle REST Data Services (ORDS) allow a client to open a transaction on one database session, suspend it, and then resume and commit or roll it back from a separate request by carrying a global transaction identifier (`GTRID`). ORDS exposes database APIs to start, commit, and roll back the transaction, and you convey the `GTRID` in `x-ords-sessionless-transaction-id` when calling REST-Enabled SQL, REST modules, or AutoREST services.

Use this feature when a multi-step workflow needs to keep a single transaction open across several HTTP requests without holding the same database session for the entire duration. Note that sessionless transactions are subject to a timeout, so the workflow must complete or explicitly resume and finalize the transaction before it expires.
...
- Oracle Database support begins with **Oracle AI Database Release 26ai, version 23.6**.
- The control APIs require a client carrying the ORDS **SQL Administrator** or **SQL Developer** role.
...
Before working through these examples, confirm that the target pool is configured for the ORDS Database API. In practice this means `database.api.enabled=true` for the Database API endpoints and `restEnabledSql.active=true` for the current example flow. After changing pool settings, restart ORDS. Also use a REST-enabled schema alias and credentials authorized for the endpoint being called. See [ords-installation.md](./ords-installation.md) and [ords-authentication.md](./ords-authentication.md) for broader setup and authentication context.
...
If you omit `--db-pool`, ORDS applies the setting to the `default` pool. If you leave this value unset, ORDS falls back to a **60-second** timeout. If a suspended sessionless transaction is not resumed within the configured duration, Oracle Database cancels it.
...
The `POST` call returns the `GTRID`, which must be reused on all subsequent requests.

---

## Start a Transaction

Use the database API to open a new sessionless transaction:
...
Store the returned `gtrid` for use throughout the rest of the workflow.

---

## Invoke Services Within the Same Transaction

To run subsequent requests inside the same sessionless transaction, include the `GTRID` in either:
...
As long as you keep supplying the same `GTRID`, ORDS continues the work within the same sessionless transaction context.
...
ORDS explicitly notes that certain REST requests can end the sessionless transaction without using the control APIs:
...
Avoid mixing explicit transaction-control SQL or DDL into a workflow unless ending the transaction is the intended outcome.

---

## Operational Notes

Sessionless transactions decouple the transaction from the session. Once work is suspended, the session or connection can be released and picked up by another client while the transaction remains identified by its `GTRID`.

When selecting the timeout, Oracle documents these tradeoffs:
...
- Configure `jdbc.sessionlesstxn.timeout` explicitly for the pool rather than relying on the default when the workflow spans multiple HTTP calls.
- Treat the returned `GTRID` as essential workflow state and include it on every follow-up request.
- Limit the unit of work to a single Oracle Database only.
- Keep DDL and explicit `COMMIT` or `ROLLBACK` statements out of service calls unless ending the transaction is intentional.

---

## Common Mistakes

- Starting a sessionless transaction and then omitting the `GTRID` on the next request.
- Allowing the workflow to idle longer than the configured timeout after the transaction is suspended.
