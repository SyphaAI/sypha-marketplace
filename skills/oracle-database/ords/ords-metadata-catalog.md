...
For custom-defined REST modules, ORDS supports explicit parameter documentation that enriches the OpenAPI output.
...
Navigate to REST → Modules to view, test, and manage REST APIs through an interactive UI.

### Option 3: Postman Import

Export the OpenAPI spec and import it directly into Postman:
...
Or in Postman UI: File → Import → select the JSON file. Postman generates a collection containing all endpoints, example requests, and documentation.
...
ORDS surfaces its metadata through data dictionary views:
...
- Migrating definitions from DEV to TEST to PROD environments
- Recording REST API changes in source control

---

## Best Practices

- **Always provide `p_comments` on all ORDS objects**: Even brief descriptions significantly improve the quality of the auto-generated OpenAPI spec. Document the expected inputs, outputs, and behavior.
...
- **Automate OpenAPI spec download in CI/CD**: Pull the live metadata from ORDS and diff it against the committed spec. Alert when the live spec diverges from expectations.
- **Test OpenAPI specs with validators**: Use tools like `openapi-generator validate` or `spectral lint` to catch spec issues before consumers encounter them.
...
- **Use `ORDS.DEFINE_PARAMETER` for all query and path params**: This ensures the OpenAPI spec is accurate for Postman/Swagger UI users who depend on documented parameters.

## Common Mistakes

- **Assuming metadata-catalog shows unpublished modules**: Only modules with `p_status => 'PUBLISHED'` appear in the metadata catalog. Modules under development that are set to `NOT_PUBLISHED` are hidden (as intended).
- **Not committing after ORDS.DEFINE_PARAMETER**: Like all ORDS metadata, parameter definitions are stored in DB tables and require an explicit COMMIT.
- **Editing OpenAPI JSON directly**: The metadata-catalog endpoint is generated dynamically from ORDS_METADATA tables. Edits to the JSON output are discarded on the next request. Edit the source definitions instead.
- **Forgetting that metadata-catalog URLs are case-sensitive in module names**: `metadata-catalog/HR.Employees/` and `metadata-catalog/hr.employees/` may return different results. Module names are stored exactly as defined (often lowercase). Be consistent.
- **Using the metadata endpoint as a health check**: The metadata catalog requires DB access. Use `/ords/_/db-api/stable/database/` for health checks instead, as it produces a simpler response with lower overhead.
