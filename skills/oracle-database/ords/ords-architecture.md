...
Oracle REST Data Services (ORDS) is a Java EE application that serves as a bridge between HTTP clients and Oracle Database. It translates REST API calls into database operations, manages connection pooling, handles authentication and authorization, and returns results as JSON or other formats. ORDS supports a broad range of use cases: AutoREST on tables and views, custom PL/SQL-backed REST APIs, Oracle APEX hosting, Database Actions (SQL Developer Web), and Oracle Graph/Spatial REST interfaces.

A solid understanding of ORDS architecture is essential for designing reliable, scalable REST APIs on Oracle Database—selecting the right deployment model, correctly sizing connection pools, understanding URL routing, and maintaining compatibility across Oracle DB and ORDS versions.
...
- **Request Router**: Parses incoming HTTP requests, matches them against registered URL patterns, and dispatches to the appropriate handler.
- **Module/Template/Handler Registry**: An in-memory representation of all REST modules loaded from the database metadata schema. Refreshed on a configurable interval.
...
ORDS stores all REST API definitions within the Oracle Database itself, in two key schemas:
...
Understanding the request lifecycle aids with troubleshooting and performance tuning.
...
**Cons**: Requires Tomcat administration expertise; WAR redeployment is needed on ORDS upgrades.

### 3. Oracle WebLogic Server

Deploy the ORDS WAR into a WebLogic managed server or cluster. Used in enterprise environments already running WebLogic.
...
- ORDS is accessed via the ADB-specific ORDS URL shown in the OCI Console.
...
In ADB, REST-enabling schemas and defining REST APIs follows the same approach as in self-managed ORDS—via `ORDS.ENABLE_SCHEMA`, `ORDS.DEFINE_MODULE`, and so on. The SQL is identical.
...
- **Tune UCP pool size** based on DB connection limits and expected concurrency. The default pool maximum (10) is too small for production.
...
- **Using the SYS or SYSTEM account as the ORDS connection pool user**: This is a serious security risk. Always use a dedicated, low-privilege user.
- **Forgetting to refresh ORDS after metadata changes**: When REST API definitions are modified directly in ORDS_METADATA tables (e.g., via SQL), ORDS must refresh its in-memory cache. Use `ORDS.DEFINE_*` APIs, which trigger a refresh automatically, or restart ORDS.
...
- **Assuming ORDS and APEX require the same version**: ORDS and APEX have independent version dependencies. Check the APEX release notes for the required minimum ORDS version.
