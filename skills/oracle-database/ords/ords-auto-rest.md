...
AutoREST is ORDS's zero-code approach to exposing Oracle Database objects as REST endpoints. With a single PL/SQL call, ORDS automatically generates a complete set of CRUD endpoints for a table or view, including collection GET (with filtering, pagination, and ordering), individual item GET, POST, PUT, and DELETE. AutoREST is well-suited for rapid prototyping, internal tooling, and scenarios where standard CRUD over tables is sufficient. For complex business logic, custom REST APIs (`ORDS.DEFINE_MODULE`) are more appropriate.
...
Maximum limit: ORDS defaults to a maximum of 10,000 rows per request. Override in ORDS config if needed.
...
By default, AutoREST exposes ALL columns of the table or view. To restrict columns, two options are available:

### Option 1: Expose a View Instead of the Table

Create a view containing only the desired columns and AutoREST-enable the view:
...
AutoREST supports object tables (Oracle object-relational types). The JSON representation mirrors the nested object structure. Deeply nested or complex types may require custom handlers to produce clean JSON output.
...
- **Use schema aliases that differ from schema names**: This decouples the public URL from internal DB usernames. If the schema is renamed, the URL remains stable.
- **Enable `p_auto_rest_auth => TRUE`** for any non-public data: Forces authentication on all AutoREST endpoints for the object. Attach an ORDS privilege to grant specific access.
- **Prefer views over direct table exposure**: Views allow you to control columns, apply row-level filters, join data, and rename columns for cleaner APIs—all without REST-specific code.
...
- **Test filter queries in SQL first**: The `q` parameter translates to SQL WHERE clauses. Validate the equivalent SQL in your IDE before implementing in client code.
- **Set sensible `limit` defaults in client code**: ORDS defaults to 25 items per page. Always implement pagination in clients—never assume all records fit in a single response.

## Common Mistakes

- **Forgetting to commit after `ORDS.ENABLE_OBJECT`**: The ORDS metadata tables are standard DB tables. Without a commit, the change is rolled back and the endpoint will not appear.
- **Enabling AutoREST directly on the table when only read access is needed**: This creates POST/PUT/DELETE endpoints that could be exploited. Use a view (which is typically not updatable), attach privileges, or set `p_auto_rest_auth => TRUE`.
...
- **Assuming `{id}` always corresponds to a column named `id`**: The URL segment maps to the primary key, not a column named `id`. If the PK column is `employee_id`, the correct URL is `/employees/101`, not `/employees/?employee_id=101`.
- **Exposing tables with sensitive columns (passwords, PII) via AutoREST**: All columns are exposed by default. Audit your table structure before AutoREST-enabling any table that contains sensitive data.
