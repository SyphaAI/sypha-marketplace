...
Installing Oracle REST Data Services (ORDS) involves downloading the software, preparing the Oracle Database, running the install command to create or update the ORDS metadata schema, configuring database connection pools, and optionally enabling HTTPS. This guide applies to self-managed ORDS deployments — on-premises, OCI Compute, or in containers. For Autonomous Database, ORDS comes pre-installed and this guide does not apply.
...
Check the Java version before proceeding:
...
2. Credentials for SYS or a DBA-privileged account for the initial install.
3. Sufficient tablespace: the ORDS_METADATA schema requires approximately 50MB.
...
ORDS uses a configuration directory — kept separate from the software installation — to store all settings. This directory must be preserved across upgrades.
...
After installation, the config directory has the following structure:
...
For the first install, run the installer interactively:
...
For CI/CD pipelines, automated provisioning, or Ansible/Terraform workflows, run `ords install` without `-i` and supply all required options on the command line. Use `--password-stdin` with either a password file or an inline stdin block. If you include `--proxy-user`, ORDS reads a second password from standard input for the runtime user (by default `ORDS_PUBLIC_USER`).
...
Use `--db-user <name>` only when you need a runtime user other than the default `ORDS_PUBLIC_USER`.
...
When connecting ORDS to Autonomous Database (ATP or ADW), mTLS requires a wallet. This applies to self-managed ORDS running against ADB.
...
Run ORDS on HTTP (port 8080) behind Nginx or Apache HTTPD, which handles TLS termination. ORDS receives plain HTTP internally.
...
Tell ORDS which forwarded header signals that the original client request was HTTPS:
...
ORDS upgrades are two-phase: update the software binary, then upgrade the database metadata schema.
...
The `ords install` command is idempotent — re-running it upgrades the schema when one already exists and creates it fresh when none is present.

---

## Best Practices

- **Separate config directory from software directory**: The config dir should survive software upgrades. Store it at `/opt/oracle/ords/config` (not inside the ORDS software directory).
...
- **Use TNS aliases for ADB**: Wallet-based connections via TNS aliases are easier to maintain than custom JDBC URLs.
...
- **Set `jdbc.MaxLimit` based on DB max sessions**: A limit that is too high can exhaust DB connections. Use `SELECT * FROM v$resource_limit WHERE resource_name = 'sessions'` to check limits.
- **Keep ORDS log directory on fast storage**: High-throughput ORDS servers generate significant log volume. Use SSD-backed volumes for log directories.

## Common Mistakes

- **Not upgrading the DB schema after upgrading ORDS software**: Running a newer ORDS binary against an older schema causes errors. Always run `ords install` after upgrading the binary.
- **Setting `jdbc.MaxLimit` too low (default 10)**: The default is fine for development. Production environments typically need 20-100+ connections depending on concurrency.
- **Using TNS without setting `TNS_ADMIN`**: If the wallet directory path is set but `TNS_ADMIN` is not, Oracle JDBC cannot locate `tnsnames.ora`.
- **Running ORDS as root**: Always run ORDS as a dedicated OS user (`oracle` or `ords`) with no unnecessary privileges.
- **Forgetting to open firewall ports**: ORDS standalone defaults to port 8080 (HTTP) and 8443 (HTTPS). Verify these are permitted through OS firewall rules and cloud security groups.
- **Installing into the CDB root**: Always install ORDS into a PDB, not the CDB root. ORDS_METADATA in the CDB root causes schema resolution issues.
...
- **Restrict database credentials used during installation:**
  - Use the least privileged account sufficient for installation (SYSAS SYSDBA is required for initial install)
...
- **Restrict ORDS_METADATA schema privileges:**
  - The ORDS_METADATA schema requires only specific privileges to function
...
- **Secure ORDS_PUBLIC_USER account:**
  - ORDS_PUBLIC_USER should hold only the CREATE SESSION privilege
  - All actual database access should flow through proxy authentication or specific schema privileges
...
- **Secure database connections during installation:**
  ...
  - If using TCP, ensure the network is trusted and monitored

- **Restrict access to installation ports:**
  - If using ORDS standalone during installation, restrict access to the installation host
...
- **Protect backup configurations:**
  ...
  - Apply the same access controls to backups as to the live configuration
...
- **Secure feature configuration:**
  ...
  - Consider enabling features incrementally and testing security implications at each step
...
- **Separate environments securely:**
  - Use distinct ORDS configurations for development, testing, and production
...
- **PCI-DSS**:
  - Ensure the installation process does not expose cardholder data environments
  - Implement strong access controls for installation credentials
  - Log and monitor all installation activities

- **HIPAA**:
  - Ensure the installation process protects ePHI
  - Implement access controls and audit trails for installation activities
  - Consider encryption of installation logs that reference PHI

- **SOX**:
  - Implement change control procedures for ORDS installation
  - Document and obtain approval for all installation and configuration changes
  - Maintain an audit trail of installation activities

- **GDPR**:
  - Ensure the installation process protects personal data
  - Apply data protection by design during installation
...
- **Perform security testing:**
  - Conduct vulnerability scans on the installed ORDS instance
  - Perform penetration testing (in authorized environments only)
  - Validate authentication and authorization controls

- **Establish security baseline:**
  - Document the secure configuration baseline
  - Implement configuration drift detection
  - Regularly validate against the security baseline
