...
Never expose ORDS over plain HTTP in any non-development environment. Configure ORDS to reject or redirect HTTP requests.
...
CORS (Cross-Origin Resource Sharing) controls which browser origins may call your ORDS endpoints. Misconfigured CORS (e.g., wildcard `*` on authenticated endpoints) is a common vulnerability.
...
For ORDS REST modules, use `ORDS.SET_MODULE_ORIGINS_ALLOWED` to define the allowed origins for a module. `p_module_name` must be the internal module name defined by `ORDS.DEFINE_MODULE`, not the URL path:
...
To clear any existing allowed origins for a module, set `p_origins_allowed` to an empty string:
...
For PL/SQL Gateway external sessions, configure trusted origins using `security.externalSessionTrustedOrigins`:
...
If `security.externalSessionTrustedOrigins` is empty or not configured, CORS requests to the PL/SQL Gateway are blocked.

### CORS Best Practices

- **Configure CORS per REST module**: Use `ORDS.SET_MODULE_ORIGINS_ALLOWED` for REST handlers.
- **Use explicit trusted origins**: Prefer exact origins such as `https://app.example.com` over broad allowlists, particularly for authenticated endpoints.
- **Do not confuse request validation with CORS**: `security.requestValidationFunction` governs whether a PL/SQL Gateway procedure is permitted to execute; it is not the CORS allowlist.
...
Any request to the `hr.employees` module without a valid token receives:
...
If dynamic SQL is genuinely necessary:
...
Column and table names cannot be parameterized as bind variables. Validate them against a strict allowlist:
...
Pool secrets such as `db.password` are stored in an **Oracle Wallet** under `databases/{pool_name}/wallet`. If you use `ords config secret --global`, ORDS stores the secret in `global/wallet`. Secrets never appear in configuration files. Use `ords config secret` to set or rotate them:
...
For cloud deployments, retrieve secrets from OCI Vault at startup and pipe them directly into the wallet — no intermediate file required:
...
ORDS does not include native IP allowlisting. Implement it at the network layer:
...
ORDS does not include built-in rate limiting. Implement it at the reverse proxy or API gateway layer.
...
For OCI deployments, place OCI API Gateway in front of ORDS:
...
Oracle Database Vault (DBV) restricts access to sensitive schemas even from privileged accounts. When combined with ORDS, DBV prevents the ORDS connection account from reaching data it should not access, even if ORDS itself is compromised.
...
This ensures ORDS_PUBLIC_USER (and therefore the general ORDS connection) cannot access `SALARY_DETAILS`, even if an attacker manipulates ORDS handler SQL.
...
- **Enable `p_auto_rest_auth => TRUE` on all AutoREST schemas**: Enforces authentication on all AutoREST endpoints by default. Explicitly whitelist public objects rather than defaulting to open access.
...
- **Run ORDS behind a WAF (Web Application Firewall)**: Placing OCI WAF or AWS WAF in front of ORDS adds protection against common web attacks (OWASP Top 10) that ORDS itself does not mitigate.
- **Audit all ORDS requests**: Enable request logging and route logs to a SIEM. Alert on 401 spikes (credential stuffing), 500 spikes (exploitation attempts), and unusual payload sizes.
- **Keep ORDS patched**: Oracle regularly issues ORDS security patches. Subscribe to Oracle Security Alerts and apply updates promptly.

## Common Mistakes

- **Wildcard CORS on authenticated endpoints**: A `*` CORS policy combined with OAuth2 Bearer tokens undermines token security — any site can issue cross-origin requests. Use explicit origin allowlists.
- **Attempting to write passwords into config files**: ORDS stores secrets in wallet directories, not in config files. Use `ords config secret --password-stdin db.password` for pool credentials. Embedding a password directly in a config file will not work and may break ORDS startup.
...
- **Using the same OAuth client for all services**: If a shared client is compromised, all services lose access. Use one OAuth client per service or application.
- **Not testing authentication on every endpoint after a schema change**: Adding a new module does not automatically protect it. Verify that each new endpoint requires the expected authentication.
- **Ignoring ORA-00942 and ORA-04043 errors in ORDS logs**: These indicate missing objects or privileges — potential signs of misconfiguration or privilege escalation attempts.
