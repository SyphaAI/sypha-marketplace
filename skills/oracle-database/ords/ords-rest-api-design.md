...
While AutoREST delivers zero-code endpoints for tables and views, custom REST API design using `ORDS.DEFINE_MODULE`, `ORDS.DEFINE_TEMPLATE`, and `ORDS.DEFINE_HANDLER` gives you complete control over URL structure, SQL/PL/SQL logic, input validation, output shaping, and HTTP semantics. This is the right approach for production APIs that involve complex business logic, multi-table operations, computed responses, or custom authentication and authorization behavior.
...
Creates the HTTP method handler for a template. Each handler carries a source type and a source (SQL or PL/SQL).
...
ORDS provides special bind variables that are not sourced from the request payload:
...
For read endpoints, prefer SQL source types and let ORDS handle serialization. Use `source_type_collection_feed` for collections and `source_type_collection_item` for individual resources. This keeps pagination, ETag generation, bind handling, and JSON serialization within ORDS rather than building response bodies manually in PL/SQL.

Use SQL/JSON constructors when an endpoint requires a shaped JSON document that does not map cleanly to ordinary columns:
...
For 23ai/26ai-era databases, this SQL-first approach is generally cleaner than looping through cursors in PL/SQL with `HTP.P` or `APEX_JSON`.
...
For `source_type_plsql` handlers, ORDS can serialize implicit result sets and cursor binds. Use manual `HTP` or `APEX_JSON` output only when the handler must perform procedural orchestration that cannot be expressed as SQL, or when maintaining older APEX/mod_plsql-style code. Do not use PL/SQL loops purely to convert rows into JSON.

---

## Pagination in Collection Handlers

For `source_type_collection_feed`, ORDS handles pagination automatically by applying Oracle row-limiting clauses internally to the SQL query. The `:fetch_offset` and `:fetch_size` bind variables are available for manual pagination in PL/SQL:
...
In most situations, the `collection_feed` source type handles this automatically — simply write a plain SELECT without pagination clauses.
...
- **Use `q'[...]'` quoting** for handler source strings: Eliminates the need to escape single quotes in SQL/PL/SQL. The `q'[` syntax treats the content as a literal string.
- **Use `collection_feed` and `collection_item` for read-only endpoints**: These source types automatically manage pagination, ETag generation, and JSON serialization far more effectively than `plsql/block`.
- **Use SQL/JSON constructors for shaped JSON**: On newer database releases, prefer `JSON_OBJECT`, `JSON_ARRAYAGG`, native JSON columns, and views for producing custom JSON documents.
- **Check `SQL%ROWCOUNT` in DML handlers**: Always confirm the DML affected at least one row and return 404 if it did not. Silently returning 200 when nothing changed is misleading.
- **Use `NOT_PUBLISHED` status during development**: Prevents partially-built APIs from being called accidentally.
- **Version your modules**: Include `/v1/`, `/v2/` in the base path. When introducing breaking changes, create a new module rather than altering an existing one.
- **Set `p_mimes_allowed`** on POST/PUT handlers: Declaring accepted content types explicitly causes ORDS to reject requests with the wrong Content-Type early.
...
- **Using the same module name in two schemas**: Module names must be unique per schema but may repeat across schemas. Using identical module names across schemas causes maintenance confusion.
- **Redefining a handler without calling `ORDS.DELETE_HANDLER` first**: If a handler already exists and you call `ORDS.DEFINE_HANDLER` again, it silently overwrites the existing handler. This is generally intended but can cause surprises when the pattern/method combination is wrong.
...
- **Over-using `plsql/block` for SELECT operations**: Many developers reach for PL/SQL blocks with manual cursor loops for simple queries. Use `collection_feed` instead — it is more efficient and provides automatic pagination.
- **Hand-building JSON with `HTP.P` or `APEX_JSON` for simple reads**: On modern Oracle Database releases, use SQL/JSON constructors in SQL handlers and allow ORDS to serialize the response.
