...
Operating ORDS effectively requires visibility into request patterns, connection pool health, slow queries, and error rates. ORDS provides log files, status endpoints, and database views for this purpose. This guide covers log file interpretation, connection pool monitoring, performance tuning parameters, diagnosing common error patterns, and configuring health checks for load balancers and uptime monitors.
...
For production, start with `INFO` or `WARNING`. For troubleshooting, temporarily set `FINE` or `FINER` on the specific logger you are investigating.
...
Access logs are distinct from application logs. To get structured application logs for aggregation or OCI Logging, use a `logging.properties` file with JUL and the ORDS JSON formatter.
...
ORDS logs requests that exceed the slow threshold with additional detail:
...
4. Supply bind variable values from the URL to reproduce the issue exactly
...
- DB CPU cores (do not exceed DB CPU * 10 for pool size)
...
Caches parsed cursors per connection. Higher values reduce soft parse overhead for frequently repeated queries.
...
- `-XX:+UseG1GC`: G1 garbage collector (low-pause, well-suited for server applications)
...
Use `misc.pagination.maxRows` to cap the maximum number of rows ORDS returns from query-based REST services:
...
For a deeper health check that also validates DB connectivity:
...
Returns connection pool status. A 200 response confirms both ORDS and DB connectivity.
...
4. Wrong HTTP method for the template
...
Cause: Unhandled exception in handler SQL/PL/SQL, or a DB error.

Diagnosis: Check the ORDS log for the full ORA- error:
...
Common ORA- errors in the ORDS context:
...
Cause: Connection pool exhausted; all connections are busy.
...
Actions:
1. Increase `jdbc.MaxLimit` (if the DB can handle more connections)
2. Investigate slow queries that are holding connections
...
Run ORDS self-diagnostics to verify configuration before starting:
...
- **Centralize logs in production**: Forward ORDS logs to a centralized log management system (ELK Stack, OCI Logging, Splunk). Query for error patterns across multiple ORDS instances.
- **Alert on pool exhaustion**: Pool exhaustion (503 errors) signals capacity problems. Alert when connection wait time exceeds 1 second.
- **Monitor DB connection count alongside ORDS**: Watch `v$session` counts to correlate ORDS pool utilization with DB session usage.
- **Use the `/ords/` health endpoint, not application endpoints**: Health checks should target the lightweight ORDS root endpoint rather than actual REST APIs that execute DB queries.
- **Track slow request trends over time**: A gradual increase in slow requests often precedes complete outages. Build a dashboard tracking P95/P99 response times.
- **Run `ords validate` after every configuration change**: Catch pool connectivity issues before traffic reaches the instance.

## Common Mistakes

- **Not checking ORDS logs when debugging 404s**: Developers often debug 404 errors by checking the URL and schema configuration in the DB, but overlook the definitive error message in the ORDS log.
- **Ignoring pool exhaustion warnings**: `WARNING Pool: waiting for connection` is frequently treated as a routine event. It is actually a leading indicator of capacity problems that require prompt attention.
- **Setting `jdbc.MaxLimit` higher than DB `sessions` parameter**: ORDS will open connections up to the DB limit successfully, then connections start failing. `jdbc.MaxLimit` should be 20-30% below the DB `sessions` limit.
- **Running with FINE log level in production**: DEBUG/FINE level logging can produce hundreds of MB of log output per hour under load, filling disks and degrading performance.
- **Not monitoring for ORA-28000 (account locked)**: If ORDS_PUBLIC_USER exceeds failed login attempts (for example, due to a password rotation not applied to ORDS), the account locks and all ORDS requests fail with 503.
- **Assuming ORDS health check verifies REST API handlers**: The `/ords/` health check only confirms that ORDS is running and can serve HTTP. It does not verify DB connectivity or that REST modules are functioning. Use `/ords/_/db-api/stable/database/` for full health verification.
