...
The ORDS PL/SQL Gateway connects HTTP requests directly to Oracle PL/SQL stored procedures, packages, and anonymous blocks. It replaces the older Oracle HTTP Server (OHS) `mod_plsql` gateway that was used with Oracle Application Server. For ORDS REST APIs, the `plsql/block` source type grants full access to Oracle's PL/SQL capabilities: calling packages, executing complex business logic, returning custom JSON, setting appropriate HTTP status codes on errors, and streaming CLOB/BLOB content.

Knowing when to use each source type — and how to correctly handle parameters, result sets, and errors — is fundamental to building reliable PL/SQL-backed REST services.

For GET and read-only JSON endpoints, prefer `collection/feed`, `collection/item`, SQL/JSON constructors, native JSON columns, and views over PL/SQL Gateway output. Use `plsql/block` when the endpoint requires procedural orchestration, side effects, package calls, custom status handling, or compatibility with existing PL/SQL Gateway code.
...
ORDS does not automatically serialize PL/SQL OUT parameters. Use these approaches instead:
...
Oracle 12c+ supports **implicit results** (`DBMS_SQL.RETURN_RESULT`), which lets PL/SQL return ref cursors that ORDS automatically serializes as a JSON collection.
...
ORDS detects the implicit result cursor and serializes it as a JSON collection. This is the cleanest approach to returning result sets from packages without exposing direct SQL.

---

## Returning REF CURSORs

An alternative pattern uses a REF CURSOR returned through a bind variable:
...
This eliminates procedural cursor loops and lets the SQL engine handle JSON construction. Use `APEX_JSON` only when you are already inside procedural PL/SQL and cannot express the response cleanly as SQL:
...
For greater control over error structure, catch exceptions and write custom JSON:
...
- **Put business logic in packages, call them from ORDS handlers**: Handlers should serve as thin orchestration layers. All business logic belongs in PL/SQL packages that can be tested independently of REST.
...
- **Never expose ORA- error messages or stack traces to clients**: These reveal schema structure information. Log internally and return a generic message externally.
...
- **Using `DBMS_OUTPUT` to return data**: DBMS_OUTPUT buffers are NOT included in REST responses. Use SQL handlers, SQL/JSON constructors, implicit results, or explicit response output instead.
- **Using PL/SQL loops to build JSON for reads**: Prefer `collection_feed`, `collection_item`, `JSON_OBJECT`, and `JSON_ARRAYAGG` for read endpoints. Reserve PL/SQL output for procedural or legacy cases.
- **Forgetting TO_NUMBER for numeric bind variables**: `WHERE employee_id = :id` works due to implicit conversion, but using `:id` in a calculation fails if `:id` is VARCHAR2 '101' and the operation expects a number.
- **Not setting `p_mimes_allowed` on POST/PUT handlers**: Without this, ORDS accepts requests with any Content-Type (including `text/plain`). JSON body parsing fails silently when the content type is wrong.
- **Setting `:status_code` after writing to HTP**: Status codes must be set before any HTP output. Writing headers (implicitly via HTP) before setting the status code has no effect.
- **Expecting automatic COMMIT in `plsql/block` handlers**: Unlike the `dml` source type, `plsql/block` does NOT auto-commit. Always include an explicit `COMMIT` (or `ROLLBACK` in exception blocks) for DML operations inside PL/SQL handlers.
- **Using `DBMS_SQL.RETURN_RESULT` in a non-plsql/block context**: Implicit results work only with `source_type_plsql`. They are ignored by `collection_feed` and `collection_item` source types.
