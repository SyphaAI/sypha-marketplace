...
Oracle Cloud Infrastructure (OCI) delivers a broad range of database services that span from fully self-managed virtual machine databases to completely autonomous, self-driving options. Choosing the right service tier for a given workload requires understanding the trade-offs in control, automation, cost, and performance that each option presents.

The three main Oracle Database service families on OCI are:
...
ATP is Oracle's fully managed OLTP database service. Oracle handles infrastructure, patching, backup, tuning, and availability. Customers are responsible for schemas, SQL, and application connectivity.
...
ADW exposes three pre-defined services with distinct resource profiles:
...
Autonomous Databases can automatically scale compute resources without downtime when the database is under CPU pressure.
...
Autonomous Databases automatically perform daily backups to OCI Object Storage with a default 60-day retention period.
...
Base Database Service provisions Oracle Database on OCI compute shapes (Virtual Machine or Bare Metal). Oracle manages underlying infrastructure such as OS patching and storage provisioning, while the DBA retains full control over the database itself.
...
ExaCS brings the complete Exadata hardware platform — including Smart Scan, Storage Indexes, and HCC — to OCI. Oracle manages the Exadata hardware and grid infrastructure while the customer manages the database.
...
- **ExaCS**: Exadata hardware is located in OCI data centers. The customer manages the DB; Oracle manages Exadata infrastructure.
- **ExaDB-C@C**: Exadata hardware is installed at the customer's on-premises data center; Oracle manages the infrastructure remotely; the customer manages the DB.
...
OCI allows connections to Autonomous Databases from within a VCN through Private Endpoints, removing internet exposure entirely:
...
Oracle Cloud Free Tier provides two Autonomous Databases (1 OCPU, 20 GB storage each) that are permanently free with no expiration, plus $300 in credits for other services valid for 30 days.
...
Every Autonomous Database instance includes Database Actions (formerly SQL Developer Web), a browser-based SQL and administration IDE accessible at:
...
- **Use the `_medium` service for BI tool connections to ADW.** The `_high` service uses maximum parallelism, which suits a single query well but can create contention when many BI users are active at the same time.
- **Enable auto-scaling during initial deployment and disable it only after load testing.** Discovering that auto-scaling is necessary during testing is far preferable to encountering connection timeouts after a production load spike.
...
- **Right-size DBCS before migrating to Autonomous.** Test the application on ATP with the same data volume and workload profile. Autonomous's automatic optimizations can alter query plans; validate execution plans after migration.
- **Use Data Safe for Autonomous Database security posture management.** Data Safe (included with ADB) provides security assessments, user assessments, data masking, and activity auditing with no additional configuration required.
...
The `_high` service in ATP uses maximum DOP, which introduces parallel query overhead and resource contention for short OLTP transactions. Use `_tp` (low-latency, no parallelism) for OLTP and `_tpurgent` for priority transactions.

### Mistake 2: Not Rotating Wallets After Security Events

Wallets do not expire on their own. Following a security incident or personnel change, download and deploy a new wallet. The old wallet remains valid until it is explicitly invalidated by rotating the database password.
...
Unlike Autonomous Databases, DBCS patches are **not** applied automatically. The DBA must apply quarterly DB patches manually through the OCI Console (one-click patching) or via `dbaascli`. DBCS instances that go unpatched accumulate CVEs over time.

### Mistake 4: Using Autonomous Database for Workloads Requiring Custom Initialization Parameters

ATP and ADW do not permit modification of most `init.ora` parameters. Workloads that depend on specific `optimizer_features_enable` settings, custom `event` parameters, or non-standard memory configurations are not suitable for Autonomous Database. Use DBCS or ExaCS for those cases.

### Mistake 5: Ignoring Egress Costs When Loading Data

Downloading large datasets from Autonomous Database to on-premises incurs OCI egress charges. Use OCI Object Storage as an intermediate stage (egress between ADB and OCI Object Storage within the same region is free) and transfer from Object Storage to on-premises from there.
