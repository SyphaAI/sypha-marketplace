...
The Oracle Database In-Memory (DBIM) option, introduced in Oracle 12c Release 1 (12.1.0.2), adds a columnar memory store alongside the traditional row-format buffer cache. It does not replace the row store — Oracle maintains both formats simultaneously and selects whichever is more efficient for a given operation. OLTP DML continues to operate against the row store; analytic queries that scan large portions of a table read from the columnar In-Memory store.

The principal advantage is analytic query acceleration: columnar storage, vectorized SIMD (Single Instruction Multiple Data) CPU instructions, and in-memory compression together accelerate full-scan analytic queries by 10x–100x compared to reading the same data from disk in row format.
...
- Contains a contiguous row range from one column segment
...
Populating objects into the IMCS is handled by background worker processes (`Wnnn`). These processes read segments from disk (or buffer cache) and populate the IMCS asynchronously. The number of worker processes is controlled by `INMEMORY_MAX_POPULATE_SERVERS`.
...
`FOR QUERY LOW` is the most widely used production choice: meaningful compression with near-zero decompression overhead during scans.

### Selective Column Population

Not every column needs to reside in the IMCS. Excluding frequently-updated or rarely-queried columns reduces the IMCS footprint.
...
The IMCS applies an LRU-like eviction policy when it fills up. Lower-priority objects are evicted to make room for higher-priority ones.
...
The In-Memory Column Store substantially accelerates queries matching these patterns:
...
In-Memory Aggregation (IMA) is a 12c+ feature that transforms GROUP BY operations into SIMD vector operations executed across IMCU data. Rather than processing rows one at a time, Oracle accumulates aggregates across the entire IMCU using vectorized CPU instructions.
...
- No row reconstruction (only required columns are read)
...
Oracle 19c introduced Automatic In-Memory (AIM), which uses the Database Heat Map to automatically populate and evict objects from the IMCS based on real access patterns.
...
- **Start with the most frequently scanned, most column-selective tables.** The IMCS delivers the greatest benefit for wide tables (many columns) where queries access only a subset of columns. A narrow table (5 columns) with full-row access gains little.
- **Use `PRIORITY CRITICAL` for tables critical to SLA.** During database startup, populate workers fill the IMCS in priority order. CRITICAL objects are populated first, ensuring availability before user load arrives.
- **Combine with partitioning for large tables.** Partition by time and keep only the "hot" partitions (current month, last 3 months) in the IMCS. This keeps the IMCS footprint predictable and avoids eviction pressure.
- **Do not enable INMEMORY on tables with heavy UPDATE activity.** The IMCS is eventually consistent with the row store via IMCU journaling. Very high DML rates cause the IMCU journal to fill up, triggering repopulation and consuming populate workers. Reserve IMCS for tables with low-to-moderate DML.
- **Use join groups proactively.** If the same two tables are joined in more than 50% of analytic queries, a join group pre-computes hash values, enabling IMCS-level join processing without extracting and transferring rows to DB server buffers.
- **Monitor `bytes_not_populated` in `V$IM_SEGMENTS`.** A non-zero value means the IMCS ran out of space and could not fully populate a segment. Queries against unpopulated extents fall back to disk reads. Either increase `INMEMORY_SIZE` or reduce the set of in-memory objects.
...
Adding `INMEMORY` to many tables without increasing `INMEMORY_SIZE` causes the IMCS to fill up. New objects fail to populate (or existing ones are evicted), and the query workload falls back to disk reads with no warning beyond a `populate_status = 'OUT OF MEMORY'` flag.
...
For very large tables, the IMCS scan still benefits from parallel query (`PARALLEL` hint or `PARALLEL` clause on the table). Without parallel query, a serial IMCS scan on a 100 GB table remains a single-threaded operation. Pair IMCS with an appropriate degree of parallelism.
...
The IMCS is not updated in real time for every DML operation. Oracle instead uses IMCU journaling — DML changes are recorded in a small journal area within the IMCU, and periodic trickle repopulation merges the journal into the IMCU. Queries see the current committed data via journal merging, but there is a small window of additional CPU cost for highly concurrent DML tables.

Do not use `SELECT ... AS OF TIMESTAMP` against IMCS objects if exact change tracking is required — use the row store query path instead.

### Mistake 4: Using CAPACITY Compression for Frequently Queried Tables

`MEMCOMPRESS FOR CAPACITY HIGH` achieves the highest compression ratio but relies on a slower decompression algorithm. For tables scanned hundreds of times per hour, the decompression CPU overhead becomes significant. Use `FOR QUERY LOW` or `FOR QUERY HIGH` for hot analytical tables.

### Mistake 5: Forgetting to REPOPULATE After Bulk Data Loads

After a large `INSERT /*+ APPEND */` or partition exchange operation, the IMCS copy of that segment is stale. Oracle trickle-repopulates it in the background, but until repopulation completes, queries against the new data read from disk. For time-sensitive workloads, trigger explicit repopulation immediately after the load completes.
