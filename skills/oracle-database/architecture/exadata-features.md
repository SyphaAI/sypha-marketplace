...
Oracle Exadata is an engineered system that co-locates Oracle Database servers with intelligent storage cells running Exadata Storage Server Software (CELLSRV). The central design principle is to push SQL processing — filtering, joining, decompression — down into the storage layer, dramatically cutting the volume of data that must traverse the storage network to reach the database servers.

Exadata is available as a physical on-premises system (X9M, X10M), as Exadata Cloud Service (ExaCS) on OCI, and as Exadata Cloud at Customer (ExaDB-C@C). The Exadata-specific features described in this guide apply across all deployment models.

---

## 1. Smart Scan (Cell Offload Processing)

Smart Scan is the foundational Exadata capability. When Oracle determines that a query can benefit from cell-side processing, it sends predicate, projection, and optional join filter information to each storage cell. Each cell evaluates the data on its local disk and returns only the rows that satisfy the predicates — a process called **cell offload**.
...
The ratio of bytes sent to bytes on disk is called the **cell offload efficiency**. A 90% offload efficiency means 90% of the data never left the storage layer.

### Prerequisites for Smart Scan

Smart Scan activates only when all of the following conditions are satisfied:
...
Storage Indexes are a purely in-memory optimization maintained automatically by CELLSRV. For each 1 MB region of disk, the storage index tracks the minimum and maximum value for up to 8 columns. When a query includes a WHERE clause predicate, the storage cell checks whether any row in each 1 MB region could possibly match. If not, the entire region is skipped without any disk I/O.

Storage Indexes are:
- **Automatic**: no DDL or configuration required
- **In-memory only**: stored in storage cell DRAM, not on disk
- **Lost on cell restart**: rebuilt gradually as data is accessed

### When Storage Indexes Are Most Effective

Storage Indexes deliver the greatest benefit when:
- Data has **natural clustering** by the predicate column (e.g., `ORDER_DATE` in an orders table inserted chronologically)
- Queries filter on **high-cardinality range predicates** (`WHERE order_date BETWEEN ... AND ...`)
- The table is **not randomly distributed** across storage cells
...
`QUERY LOW` and `QUERY HIGH` suit tables that are queried regularly. `ARCHIVE` levels are intended for data that is loaded once and rarely accessed.
...
HCC imposes significant restrictions on DML operations:
...
- **DELETE**: Deleted rows leave gaps in compression units; does not cause migration but wastes space

For this reason, HCC is best suited for append-only or archive tables. Mixed read/write workloads should use Advanced Row Compression instead.
...
IORM is a storage-cell-side resource manager that controls I/O bandwidth allocation among databases, services, and consumer groups sharing the Exadata infrastructure. Unlike CPU-based DBRM (which runs on the DB server), IORM enforces I/O limits at the physical disk level inside CELLSRV.

### IORM Plans

IORM plans are defined at three levels:
1. **Inter-database plan**: Distributes I/O shares across different databases on the same Exadata
2. **Intra-database plan**: Distributes I/O shares across consumer groups within a single database
3. **Database plan with limits**: Caps the I/O bandwidth a specific database is allowed to consume
...
- **Use IORM to protect OLTP latency from large analytics queries.** Without IORM, a parallel Smart Scan consuming all cell I/O bandwidth forces OLTP queries to queue behind it.
- **Keep statistics current on Exadata.** The CBO must recognize that a full table scan is appropriate in order to generate a plan that triggers Smart Scan. Stale statistics can lead the optimizer to choose index lookups that bypass Smart Scan.
...
- **Use `QUERY HIGH` for reporting tables loaded daily and `ARCHIVE HIGH` only for data that will never be updated.** Combining ARCHIVE compression with UPDATE-heavy workflows causes severe row migration and performance degradation.
...
Smart Scan only activates for large segments using direct-path reads. If a table fits in the buffer cache, Oracle uses cached reads and Smart Scan does not activate. The threshold is controlled by `_small_table_threshold`.
...
HCC is fundamentally incompatible with frequent DML. Each UPDATE on an HCC row causes the row to migrate out of the compression unit into OLTP format. Over time, the table becomes a mixture of compressed and uncompressed rows, wasting space and degrading reads.
...
Setting `cell_offload_processing = FALSE` at the system level disables Smart Scan globally. This is sometimes done as a workaround for a specific query bug, but the parameter is then forgotten, leaving the Exadata operating as an ordinary database server.
...
Storage Indexes are rebuilt in memory as data is accessed. After a large bulk load or partition exchange, the Storage Index for the affected regions is cold. The first few query executions against newly loaded data will not benefit from Storage Index skipping. This is expected behavior, not a performance regression.

### Mistake 5: Running Exadata Diagnostics Only at the Database Layer

Exadata performance problems frequently originate in the storage cells (CELLSRV process, Flash Cache, disk I/O). Cell-level diagnostics require `CellCLI` access or Exadata Metrics available through the OCI console. Always inspect cell-level metrics alongside `V$SYSSTAT` before concluding that a performance issue is query-related.
