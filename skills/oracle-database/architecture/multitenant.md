...
Oracle Multitenant, introduced in Oracle 12c, is an architectural model that consolidates multiple databases into a single database engine while preserving their isolation. A **Container Database (CDB)** is the outer shell — one Oracle instance, one set of background processes, one shared SGA. Inside it live **Pluggable Databases (PDBs)**, each presenting itself to applications as a fully independent Oracle database.

The multitenant architecture addresses three longstanding problems with Oracle consolidation:
...
The root container is the administrative hub of the CDB. It holds:
...
User PDBs are the tenant containers where application data resides. Each PDB has:
...
Cloning is among the most powerful PDB operations. A PDB can be cloned locally within the same CDB, remotely from another CDB, or as a hot clone without closing the source.
...
Oracle Database Resource Manager (DBRM) in a CDB context manages CPU and I/O allocation across all PDBs to prevent a single tenant from monopolizing resources.
...
Within each PDB, a standard DBRM consumer group plan can be defined that governs sessions inside that PDB only.
...
User types in a CDB represent one of the most confusing aspects of multitenant. Misunderstanding this distinction is a frequent source of security and connectivity problems.

### Common Users

Common users are created in CDB$ROOT and exist (with the same identity) in all containers. Oracle enforces the `C##` prefix to distinguish them.
...
Local users exist in exactly one PDB and are invisible to other PDBs and CDB$ROOT.
...
Application containers provide a second level of containment inside a CDB. An Application Container acts as a root for a set of Application PDBs, allowing application-specific master data (seed data, reference tables, even PL/SQL objects) to be shared across all application PDBs.
...
- **Always use Local Undo mode (12.2+).** Shared Undo (the 12.1 default) places all undo for all PDBs in a single undo tablespace, making PDB-level point-in-time recovery far more difficult. Local undo gives each PDB its own undo tablespace.
- **Use `SAVE STATE` for PDBs that should auto-open.** Without `SAVE STATE`, PDBs close when the CDB instance restarts and remain closed until opened manually.
- **Set storage limits on PDBs early.** Without `STORAGE (MAXSIZE ...)`, one runaway PDB can fill all shared disk, impacting every other PDB.
...
- **Apply patches at the CDB level.** One `opatch apply` patches all PDBs at once — a major operational advantage over non-CDB environments.
...
- **Use separate CDBs for different lifecycle stages.** Place DEV PDBs in a DEV CDB and PROD PDBs in a PROD CDB. The CDB-level patch level and parameter settings affect all contained PDBs.
...
Because CDB$ROOT is open by default and easy to connect to as SYSDBA, developers sometimes create their application tables in the root container. These objects become shared metadata objects — they behave unpredictably and are nearly impossible to clean up.

**Fix:** Enforce a policy that application schemas are only created inside designated PDBs. Use `PDB_LOCKDOWN` profiles to block non-administrative connections to CDB$ROOT.

### Mistake 2: Not Using `CONTAINER = ALL` When Granting Common Privileges

A common user granted a privilege with `CONTAINER = CURRENT` (while logged into CDB$ROOT) holds that privilege only in CDB$ROOT, not in any PDB. This is a frequent source of "privilege not granted" errors when the common user connects directly to a PDB.
...
When PDBs need to register with different listeners (e.g., on different ports for different applications), each PDB must configure its own `LOCAL_LISTENER` parameter.
...
Data Pump defaults to the CDB root when invoked from the OS. Always connect through the PDB service or use the appropriate connection string.
...
Plugging a PDB from a lower-version CDB into a higher-version CDB without running `DBMS_PDB.CHECK_PLUG_COMPATIBILITY` and resolving violations produces an incompatible PDB that either fails to open or opens with errors. Always review `PDB_PLUG_IN_VIOLATIONS` before opening a newly plugged PDB.
