...
Oracle Real Application Clusters (RAC) is a shared-disk cluster database technology that enables multiple Oracle instances to mount and open the same database at the same time. Every node runs its own Oracle instance (memory structures and background processes), but all nodes share a common set of datafiles stored on shared storage (ASM or a cluster file system). RAC delivers high availability through instance survivability and horizontal scalability by spreading workload across nodes.

RAC forms the cornerstone of Oracle's Maximum Availability Architecture (MAA) and is frequently combined with Data Guard to provide both HA within a site and DR across sites.
...
All RAC nodes access the same datafiles, control files, redo log groups, and archived logs. Oracle Automatic Storage Management (ASM) is the recommended storage layer. ASM delivers:
...
Each node runs its own ASM instance (`+ASM1`, `+ASM2`, etc.) as a separate Oracle instance type.
...
Each RAC instance includes background processes not present in single-instance Oracle:
...
Cache Fusion is Oracle's protocol for moving data blocks between instances across the private interconnect rather than writing them to and re-reading from disk. It is the defining technology that makes shared-disk RAC viable.

### How Cache Fusion Works

1. Instance 1 reads block 42 from disk into its buffer cache.
2. Instance 2 needs block 42. Rather than performing a disk I/O, Oracle's Global Cache Service (GCS) determines that Instance 1 holds block 42.
3. GCS directs Instance 1 to send block 42 directly to Instance 2 over the private interconnect.
4. Instance 2 receives the current version of the block without accessing disk.

This block transfer is called a **CR (Consistent Read) transfer** when the receiver requires an older image for read consistency, or a **Current Block transfer** when the receiver needs the most recently committed version to modify it.
...
GCS tracks the state of all data blocks across every instance. Each block has a master instance responsible for monitoring its state. The GCS maintains a distributed lock called a **PCM (Parallel Cache Management) lock** for every block.
...
- **NULL** — the instance no longer requires the block in current mode

### Global Enqueue Service (GES)

GES governs non-cache-fusion resources that span instances, such as dictionary locks, DML locks, sequences, and DDL locks. GES ensures that when Instance 1 acquires a lock on a row, Instance 2 cannot acquire a conflicting lock on the same row.
...
RAC services are the primary mechanism for directing workload to particular nodes and enabling transparent failover. A service is a named entity that clients connect to, rather than connecting directly to an instance.
...
Node affinity is the practice of binding particular workloads or schemas to specific nodes in the cluster. This is critical for reducing Cache Fusion traffic: when the same data is consistently accessed by the same node, cross-instance block transfers are eliminated.
...
With this topology, OLTP data blocks reside primarily in Node 1/2 buffer caches and reporting data resides primarily in Node 3/4 caches, reducing cross-instance transfers.

### Table Partitioning and Affinity

In policy-managed databases with multiple server pools, partitioned tables can be arranged so that different partitions are accessed predominantly by different node sets. This is referred to as **partition affinity**.
...
RAC introduces a collection of wait events that do not exist in single-instance Oracle. Elevated wait times on these events indicate interconnect or cache contention problems.
...
FCF relies on Oracle Notification Service (ONS) and is configured in JDBC Thin drivers or Universal Connection Pool (UCP). FCF is superior to TAF for Java applications:

- The connection pool receives an ONS event (via the GI event notification system) immediately when a service goes down
- Stale connections are proactively purged from the pool before applications attempt to use them
...
Application Continuity (introduced in Oracle Database 12c Release 1) extends TAF concepts to transparently replay in-flight transactions, including DML, after a recoverable error. TAC (introduced in 18c, expanded in 19c) accomplishes this without any application configuration by using `failover_type => 'AUTO'` on the service (instead of `'TRANSACTION'` for standard AC).
...
CVU (`cluvfy`) is Oracle's pre-installation and post-installation diagnostic tool for cluster environments. It validates network configuration, OS parameters, shared storage, and cluster software health.
...
- **Use SCAN (Single Client Access Name) for all client connections.** SCAN gives clients a single address regardless of how many nodes are in the cluster. Never hardcode VIP addresses in application connection strings.
- **Run Grid Infrastructure and Database homes on separate file systems.** GI patching and DB patching are independent cycles; separate homes prevent unintended outages during either process.
- **Size the interconnect for peak block transfer load.** Track `gc cr blocks received` and `gc current blocks received` rates to project bandwidth requirements. A saturated interconnect is the leading cause of RAC performance degradation.
- **Define services before deployment.** Services should map 1:1 to application workload types (OLTP, reporting, batch). Mixing workloads within a single service makes diagnosis and isolation difficult.
...
- **Prefer policy-managed databases for new deployments.** Server pools let Oracle dynamically rebalance instances across nodes during node failure without requiring manual intervention.
- **Enable Cluster Health Monitor (CHM/OS Watcher).** CHM collects OS-level metrics (CPU, memory, network, disk I/O) per node at 1-second granularity, which is invaluable when performing post-mortem analysis of node evictions.
...
If `/etc/hosts` or DNS lacks a proper private hostname entry, Oracle may fall back to using the public network for interconnect traffic. This saturates the public NIC and causes severe GCS latency.

**Fix:** Always confirm the interconnect IP is on the private network both before and after installation.
...
RAC requires one UNDO tablespace **per instance**. A single shared UNDO tablespace is not supported in RAC.
...
With a single sequence shared across all instances, the right-hand leaf block of the index becomes a hot block that every instance contends to update. This generates heavy `gc buffer busy` waits.
...
Node evictions (where a hung or slow node is forcibly removed from the cluster) are frequently treated as hardware failures when the actual causes are:
...
Always examine `cssd.log`, `alert_<sid>.log`, and CHM data together before concluding the fault is hardware-related.

### Mistake 5: Applying Patches Without Running `opatchauto`

In a RAC environment, GI and RAC patches must be applied using `opatchauto` in a rolling manner. Manually applying patches without `opatchauto` can leave GI and DB homes in an inconsistent state across nodes, potentially triggering split-brain scenarios.
