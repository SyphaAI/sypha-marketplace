...
Oracle Data Guard is Oracle's native solution for high availability, disaster recovery, and data protection. It maintains one or more synchronized copies of a production database (the **primary**) known as **standby databases**. When the primary becomes unavailable, a standby can be promoted to take over, keeping downtime and data loss to a minimum.

Data Guard is a built-in feature of Oracle Database **Enterprise Edition**. Standard Edition is not supported. A primary can ship redo directly to as many as 30 standby databases; with cascading, configurations can grow beyond 30 total standbys.

The two fundamental services are:
- **Redo Transport Services** — automates the transfer of redo data from the primary to standby destinations; detects and resolves archive log gaps automatically.
- **Apply Services** — automatically applies received redo on the standby to keep it synchronized.
...
A block-for-block identical replica of the primary database. Redo Apply (Media Recovery via the MRP0 process) keeps it in sync.
...
- Most efficient apply method — bypasses the SQL layer entirely.

### Logical Standby

Maintained via **SQL Apply** (LogMiner-based): redo is translated into SQL statements and executed on an open read-write database.

- Open for read-write during apply; SQL Apply-maintained tables are shielded from user DML by logical standby guard rules.
- Can carry additional indexes, partitioning, and materialized views absent from the primary.
...
A physical standby temporarily converted to read-write for testing or development purposes.

- Receives and archives redo from the primary but does **not** apply it.
- Local writes are discarded upon conversion back; Flashback Database is used internally.
...
- Achieves zero data loss at geographically distant standbys without requiring direct synchronous transmission over a long-distance link.
- Cannot be opened; cannot run redo apply; cannot assume the primary role.
...
Protection modes govern the trade-off between data protection and primary database performance.
...
- Potential data loss equals the transport lag.
...
- Transactions commit after redo has been received at the standby and acknowledged.
- Uses **SYNC** transport.
- If all synchronized standbys become unreachable, automatically reverts to Maximum Performance (primary remains up).
- Two sub-variants:
  - `SYNC AFFIRM` — waits for redo written to disk on standby (maximum durability)
  - `SYNC NOAFFIRM` (FastSync) — waits only for redo received in standby memory (better performance; small risk during simultaneous power failures at both sites)
...
- Can only be set from Maximum Availability mode with at least one synchronized standby already in place.
- Oracle recommends a minimum of two standby databases to prevent a single standby failure from shutting down the primary.
...
Specifies where redo is archived and shipped. Supports up to 31 destinations (`LOG_ARCHIVE_DEST_1` through `LOG_ARCHIVE_DEST_31`).
...
| `REOPEN=<secs>` | Minimum seconds between reconnect attempts on failure |
...
Required when `DB_UNIQUE_NAME` is used in `LOG_ARCHIVE_DEST_n`:
...
Broker centralizes and automates configuration, monitoring, and role transitions. Using Broker is strongly recommended over manual `LOG_ARCHIVE_DEST_n` management.
...
A planned, graceful role reversal — both databases remain intact with no data loss. Typically used for planned maintenance, patching, or DR testing.
...
An emergency operation used when the primary is unavailable and cannot be recovered quickly. May involve data loss unless all redo was received.
...
After failover, Flashback Database rolls the old primary back so it can be re-synchronized as a standby:
...
FSFO allows an observer process to automatically trigger failover when the primary is unavailable, without requiring DBA intervention. Requires Broker.
...
Active Data Guard enables a physical standby to remain **read-only** while simultaneously applying redo. Requires a separate Active Data Guard license.
...
- **Use Data Guard Broker (DGMGRL)** for all operations. Manual `LOG_ARCHIVE_DEST_n` management is error-prone and difficult to keep consistent.
...
- **Use Maximum Availability** with SYNC transport for OLTP workloads where the network round-trip to the standby is acceptable (typically < 5ms).
...
- **Monitor lag actively.** A standby with 4 hours of apply lag provides 4 hours of recovery time, not instant failover capability.
- **Back up from the standby** with RMAN to offload backup I/O from the primary.
- **Set `STANDBY_FILE_MANAGEMENT=AUTO`** to eliminate manual datafile management when the primary adds new datafiles.
...
Every database must have a unique `DB_UNIQUE_NAME` value. They may share the same `DB_NAME`.
...
Always verify gap status before a manual failover:
```sql
SELECT * FROM V$ARCHIVE_GAP;
```

**Not enabling Flashback Database before failover**
Without Flashback, reinstating the old primary requires a complete rebuild from backup.

**Using deprecated syntax**
The `USING CURRENT LOGFILE` clause (deprecated 12.1) and `V$MANAGED_STANDBY` (deprecated 12.2) still function but should not appear in new scripts. Use `V$DATAGUARD_PROCESS` instead.
