...
Oracle Advanced Queuing (AQ) is a message queuing facility integrated directly into the Oracle database engine. Unlike external messaging systems, AQ stores messages in standard database tables, meaning messages participate fully in Oracle transactions, inherit the database's reliability guarantees, and can be queried using ordinary SQL.

Oracle Database 21c rebranded and substantially extended AQ as **Transactional Event Queues (TQ/TEQ)**, introducing high-throughput partitioned storage, Kafka-compatible APIs, and greater scalability while preserving full backward compatibility with the original AQ APIs.

**When to use Oracle AQ/TQ:**
- Applications already operating on Oracle that require guaranteed message delivery
- Scenarios that demand transactional consistency between database writes and message publication
- Systems where message data must be queryable or reportable
- Environments where introducing an external broker (Kafka, RabbitMQ) adds undesirable operational overhead
...
Each message is dequeued by exactly one consumer. This is the most straightforward model and corresponds directly to a point-to-point (P2P) messaging pattern.

**Multi-Consumer Queues**
Multiple named subscribers each receive a copy of every message, corresponding to a publish/subscribe (pub/sub) pattern. Each subscriber independently tracks its own logical position in the queue.

**Exception Queues**
Every queue is paired with an exception queue. Messages that cannot be delivered — for example, because the maximum retry count was exceeded — are moved there automatically, preventing queue poisoning.
...
A **queue table** is the underlying database table that holds messages. A **queue** is a logical object built on top of a queue table. A single queue table can host multiple queues. Queue tables have dedicated storage options and index structures that are managed by `DBMS_AQADM`.

---

## DBMS_AQADM — Administration Package

`DBMS_AQADM` manages the lifecycle of queue infrastructure: creating queue tables, creating queues, starting and stopping queues, and adding or removing subscribers.
...
Oracle AQ supports automatic **propagation** of messages between queues, including queues in remote databases accessed through database links. This enables distributed messaging without requiring application-level forwarding logic.
...
Oracle AQ implements the Java Message Service (JMS) specification via the Oracle AQ JMS provider. This lets Java EE applications consume standard JMS APIs while the underlying storage is Oracle database queues.
...
TEQ extends AQ with a **partitioned, high-throughput storage engine** purpose-built for event streaming workloads.
...
- **Use `ON_COMMIT` visibility** for both enqueue and dequeue in OLTP systems. `IMMEDIATE` visibility can make messages visible before the producing transaction finishes, causing consumers to observe partial data.
- **Always handle `DBMS_AQ.TIME_OUT` and `DBMS_AQ.NO_MESSAGE_FOUND`** exceptions in dequeue loops. Omitting these handlers results in silent consumer failures.
- **Set `max_retries` and `retry_delay`** on every queue. Without `max_retries`, a persistently failing message blocks processing indefinitely. With it, messages are moved to the exception queue once the limit is reached.
- **Separate queue tables by payload type and consumer pattern.** Mixing single-consumer and multi-consumer queues in the same table is not permitted, and mixing unrelated payload types complicates monitoring.
- **Monitor exception queues regularly.** Set up an alert or scheduled job that reports whenever a message lands in an exception queue.
- **Use content-based routing rules on subscribers** rather than creating a separate queue for each use case. This keeps queue table count under control.
- **Grant only the necessary privileges.** Use `DBMS_AQADM.GRANT_QUEUE_PRIVILEGE` instead of granting direct table access to the underlying queue table.
- **Purge stale messages.** Use `DBMS_AQADM.PURGE_QUEUE_TABLE` with a purge condition to remove processed messages and keep table size in check.
...
**Mistake 1: Committing after enqueue but omitting the dequeue commit**
When a dequeue uses `ON_COMMIT` visibility and the session rolls back rather than commits, the message is returned to the queue. This is intentional and expected behavior — do not try to work around it. Structure dequeue code so that business logic and the final COMMIT form a single atomic unit.

**Mistake 2: Creating too many queue tables**
Each queue table produces several internal Oracle objects (IOTs, indexes, etc.). Proliferating queue tables for minor variations wastes resources. Use subscriber rules and correlation IDs to distinguish message streams within a single queue.

**Mistake 3: Using `DBMS_AQ.FOREVER` wait in high-volume applications**
A session blocking indefinitely with `FOREVER` wait holds a database connection for an unlimited duration. In connection-pool environments, this starves other users. Use a bounded wait (e.g., 30–60 seconds) and loop instead.

**Mistake 4: Not testing exception queue behavior**
Teams configure `max_retries` but rarely verify what happens once it is exceeded. Simulate a failure scenario during development to confirm that messages land in the exception queue and that monitoring alerts fire as expected.

**Mistake 5: Dropping a queue without stopping it first**
Always invoke `DBMS_AQADM.STOP_QUEUE` before `DBMS_AQADM.DROP_QUEUE`, and drop queues before dropping the queue table. Skipping these steps produces ORA-24005 and related errors.
...
Propagation schedules silently accumulate failure counts whenever the remote database or network is unreachable. Configure a job that queries `DBA_QUEUE_SCHEDULES.FAILURES > 0` and alerts the operations team.
