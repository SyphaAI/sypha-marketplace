...
AI profiles link Oracle Database to external LLM providers for `DBMS_CLOUD_AI` and SELECT AI workflows.

This guide covers the Oracle 26ai AI profile APIs and provider attributes. `DBMS_VECTOR` and `DBMS_VECTOR_CHAIN` share the same providers but do not consume `DBMS_CLOUD_AI` profiles directly; instead they rely on vector credentials and per-call JSON parameters.
...
Credentials hold API keys encrypted within the Oracle credential store.
...
When the database is configured for OCI resource principal access, reference the reserved credential name `OCI$RESOURCE_PRINCIPAL` in the profile. This credential is not created using `DBMS_CLOUD.CREATE_CREDENTIAL`.
...
`DBMS_VECTOR` and `DBMS_VECTOR_CHAIN` do not consume `DBMS_CLOUD_AI` profiles. Create a vector credential and then reference it from the JSON `params` supplied to vector package calls.
...
Confirm end-to-end connectivity right after creating a profile, before depending on it in application code.
...
Use a failover function to attempt the primary profile and automatically switch to a secondary profile when the primary provider is unreachable (ORA-20000 / ORA-29273).
...
OCI-backed profiles accept a `region` attribute and, where required, a `provider_endpoint` override.
...
Leave the `name` field out of an `object_list` entry to include every object in a schema. Keep the list narrowed to the tables that are relevant to the use case — large lists (more than 50 tables) slow down NL-to-SQL generation because the model must process a larger schema context.
...
API providers enforce rate limits (tokens per minute, requests per minute) — Oracle imposes no throttling on its side. For high-volume workloads, handle ORA-29273 with HTTP 429 by introducing a delay between retries. Prefer `UTL_TO_EMBEDDINGS` (batch mode) over per-row calls to reduce total request count.
...
AI profiles have no built-in versioning. Adopt a naming convention (e.g., `MYAPP_AI_V1`, `MYAPP_AI_V2`) and capture the current settings before making changes to a profile. To roll back, recreate the previous profile under a new name and switch the session to it, or maintain a `_STABLE` profile that is only updated after thorough validation.
...
- Store all API keys through `DBMS_CLOUD.CREATE_CREDENTIAL` — never hardcode keys in SQL or PL/SQL
- Grant `EXECUTE ON DBMS_CLOUD` only to the users who require credential creation
- Restrict `object_list` to only the tables needed — the LLM receives table and column names plus comments, not data values
- Rotate API keys using `DBMS_CLOUD.UPDATE_CREDENTIAL` without needing to drop and recreate the credential
- On OCI, prefer `OCI$RESOURCE_PRINCIPAL` wherever it is available to eliminate API key management overhead
- For vector package calls, manage and rotate credentials with `DBMS_VECTOR.CREATE_CREDENTIAL` or `DBMS_VECTOR_CHAIN.CREATE_CREDENTIAL` rather than assuming a `DBMS_CLOUD_AI` profile is applicable
