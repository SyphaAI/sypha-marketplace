...
Oracle 26ai ships three PL/SQL package families for AI vector operations:

- **`DBMS_VECTOR`** — utility functions for generating embeddings, converting vectors, and managing vector indexes
- **`DBMS_VECTOR_CHAIN`** — composable pipeline functions for end-to-end RAG workflows: chunking text, generating embeddings, storing, searching, summarizing, and producing responses
- **`DBMS_HYBRID_VECTOR`** — hybrid search functions and helpers for retrieval paths that combine vector and full-text search

The vector packages rely on vector credentials and per-call JSON parameters rather than `DBMS_CLOUD_AI` profiles.
...
Substantially more efficient than invoking `UTL_TO_EMBEDDING` inside a loop.
...
`DBMS_VECTOR_CHAIN` delivers composable pipeline steps designed for RAG workflows.
...
A full end-to-end pipeline: ingest documents → chunk → embed → store → query → generate.
...
`VECTOR_EMBEDDING` produces embeddings inline within SQL without needing PL/SQL. The model must be loaded into Oracle using `DBMS_VECTOR.LOAD_ONNX_MODEL` or stored as an ONNX model in a database directory. For API-based models such as OpenAI or Cohere, use `UTL_TO_EMBEDDING` in PL/SQL instead.
...
Manage HTTP 429 (rate limit) errors from embedding API calls using exponential back-off.
...
Persist source attribution alongside embeddings so that chunks can be cited and filtered by document.
...
Select a chunking approach based on content type. Inspect the chunk size distribution after ingestion to verify your settings are appropriate.
...
VECTOR columns are dimensionally typed; stored embeddings from one model cannot share a column with embeddings produced by a model of a different dimension. Follow this migration path when switching to a new model.
...
`UTL_TO_EMBED_AND_GENERATE` consolidates embedding, vector search, and generation into a single pipeline call. This function was introduced in 26ai and is not available in 23ai — use the multi-step pattern from the End-to-End RAG Pipeline section for 23ai environments.
...
`DBMS_HYBRID_VECTOR` targets hybrid retrieval over hybrid vector indexes, not standard vector SQL. Use it when a query requires both semantic search and Oracle Text-style keyword constraints within the same database-managed search path.
...
Use `GET_SQL` or `GET_HYBRID_SQL` when you need to examine the SQL generated by a hybrid search pipeline.

## Best Practices

- Prefer `UTL_TO_EMBEDDINGS` (batch) over `UTL_TO_EMBEDDING` (single) for bulk ingestion — far fewer API calls are required
- A chunk size of 150–300 words with 10–20% overlap performs well for most document types
- Persist `CHUNK_OFFSET` and `CHUNK_LENGTH` so the original source passage can be retrieved later
- Build the vector index (HNSW) after the initial bulk ingestion, not before — indexes build faster on tables that already contain data
- Reserve `DBMS_HYBRID_VECTOR` for hybrid retrieval; use standard vector SQL or `DBMS_VECTOR` for pure vector operations
- Cache embeddings for frequently queried text; re-embedding the same content is wasteful and adds API cost
- Use `temperature: 0` for production generation — deterministic output is simpler to test and validate

## Common Mistakes

**Chunking after embedding** — always chunk first and then embed each chunk. Embedding an entire document yields a single vector that blends all content together, losing specific detail.

**Embedding model mismatch** — query embeddings and stored embeddings must be generated by the same model. Running `text-embedding-3-small` queries against `text-embedding-ada-002` chunks produces meaningless results.

**Not storing chunk source metadata** — without `doc_id`, `page_number`, or `source_url` stored alongside the chunk, you have no way to cite sources or filter results by document.

**Re-chunking on every query** — chunking belongs at ingestion time. Store the resulting chunks in the database and never rechunk at query time.

**Using `DBMS_HYBRID_VECTOR` for pure vector search** — this package is designed for hybrid indexes and hybrid search execution. Use `VECTOR_DISTANCE`, shorthand operators, or `DBMS_VECTOR` for non-hybrid vector work.
