...
description: >-
  Oracle Database guidance covering SQL, PL/SQL, SQLcl, ORDS, administration,
  app development, performance, security, migrations, and agent-safe database
  workflows. Use when the user needs to write, edit, rewrite, review, format,
  debug, tune, or explain SQL; create or refactor PL/SQL; work with SQLcl,
  Liquibase, ORDS, JDBC, node-oracledb, Python, Java, .NET, or database
  frameworks; troubleshoot queries, sessions, locks, waits, indexes, optimizer
  plans, AWR, ASH, migrations, schemas, users, roles, privileges, backup,
  recovery, Data Guard, RAC, multitenant, containers, monitoring, auditing,
  encryption, VPD, or safe agent database operations.
metadata:
  category: data
  source:
...
This domain provides Oracle Database skills covering administration, SQL and PL/SQL development, performance tuning, security, ORDS, SQLcl, migrations, frameworks, OCR container guidance, and agent-safe database workflows.

## How to Use This Domain

1. Begin with the routing table below.
2. Read only the specific file or category relevant to your task.
