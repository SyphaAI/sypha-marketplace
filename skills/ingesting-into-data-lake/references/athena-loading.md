...
An alternative approach for straightforward one-time data loads when Glue ETL is not available or not needed.

## Step 1: Create External Table for Source

Set up a temporary external table that points to the source files in S3.
...
For type casting rules, date parsing, null handling, and boolean conversion patterns, see [type-transformations.md](type-transformations.md).
...
When Athena hits the 30-minute timeout limit:

1. **Batch by partition**: Load one month or day at a time
2. **Switch to Glue ETL**: The better option for datasets > 1GB — supports more workers, provides built-in monitoring and retries
