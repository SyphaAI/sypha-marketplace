...
Reference guide for building AWS Glue ETL jobs that ingest data from external databases into S3 Tables.

## Overview

Glue ETL jobs leverage PySpark to connect to external databases through Glue connections, read data incrementally via watermark columns, apply transformations, and write results to Iceberg tables in S3 Tables.

## PySpark Script Structure

### Basic Incremental Append Template

For immutable data (transactions, events, logs) where only new records need to be appended:
...
For mutable data (customer profiles, product catalogs) where existing records may be updated:
...
Use this template when users want to filter or transform data at the source using a custom SQL query:
...
For small dimension tables or when the source does not support watermark-based filtering:
