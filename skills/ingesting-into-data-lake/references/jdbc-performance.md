...
Reference for identifying and fixing performance bottlenecks, incremental load failures, IAM/permissions problems, and monitoring gaps in external data import pipelines.
...
1. **Missing indexes** - Source query performs a full table scan
2. **Too much data** - Entire table is loaded rather than an incremental slice
3. **Network bandwidth** - Insufficient throughput between the database and Glue
4. **Source database load** - Database is under high concurrent load
...
2. **Monitor source database load:**
   - Review CPU, memory, and I/O utilization
   - Inspect slow query logs
   - Identify competing concurrent queries

3. **Measure network throughput:**
   - Review data transfer rates in CloudWatch metrics
...
1. **Timeout too short** - The data volume demands more processing time
...
- Job completes successfully but loads 0 records on every run
- Watermark file holds the same value after each execution

**Root causes:**

1. **No new data in source** - There are genuinely no changes
2. **Timezone mismatch** - Source uses local time while the watermark uses UTC
...
- Target table contains duplicate records sharing the same primary key

**Root causes:**

1. **Using append instead of upsert** - Appropriate only for immutable data
2. **Job retry** - A failed job restarted from the same watermark
...
**Root cause:**
The Glue job IAM role is missing the required S3 permissions.

**Solution:**

Attach S3 permissions to the Glue role:
...
**Root cause:**
The Glue job role does not have Glue Data Catalog permissions.
...
- `glue.driver.aggregate.recordsRead` - Number of records read from the source
- `glue.driver.aggregate.bytesRead` - Volume of bytes read from the source
- `glue.driver.aggregate.elapsedTime` - Total job duration
- `glue.driver.aggregate.numCompletedTasks` - Count of completed tasks
- Job run state (SUCCEEDED, FAILED, TIMEOUT)

## Troubleshooting Checklist

When a job fails, use this systematic approach:
...
- The last successful log entry before the failure
...
2. **Check CloudWatch logs** - Review error messages and stack traces
3. **Test connectivity** - Confirm network paths, security groups, and credentials
4. **Validate source data** - Query the source database directly
5. **Review job configuration** - Examine worker count, timeout, and arguments
6. **Monitor metrics** - Configure CloudWatch alarms for proactive alerting
7. **Document resolution** - Maintain a runbook of recurring issues and their fixes

With a systematic troubleshooting approach and robust monitoring, external data import pipelines run reliably with minimal manual intervention.
