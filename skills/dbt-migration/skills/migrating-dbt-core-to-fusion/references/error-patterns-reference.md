...
Full catalog of dbt-core to Fusion migration error patterns, grouped by type.
...
dbt-autofix relocates unrecognized config keys into `meta:`. However, some may be misspelled versions of genuine config keys (e.g. `materailized` instead of `materialized`). Check whether any key inside `meta:` closely resembles a known Fusion config key. If it is a typo: move it back out of `meta:` using the correct spelling. If it is truly custom: leave it in `meta:` and update macro references to use `config.meta_get('key')`.
...
**Note**: After updating package versions, delete `package-lock.yml` and the `dbt_packages/` directory, then run `dbt deps`. If errors continue, run `dbt-autofix deprecations --include-packages`.

**Note**: Fivetran `_source` packages have been consolidated into main packages (e.g. `fivetran/microsoft_ads_source` is now `fivetran/microsoft_ads`).
...
**Important**: Only `config` objects support `meta_get()` and `meta_require()`. Plain dicts use `.get()`.
...
Fusion enforces exact name matching. dbt-core was permissive with spaces versus underscores.
...
> **Tip**: These errors can frequently be caught early by running `dbt debug` (see Step 0).

## Fusion Engine Gaps (Category D)

These require Fusion engine updates. Alternatives may be suggested with appropriate caveats about risks and fragility.
