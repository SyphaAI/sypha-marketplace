...
name: gcp-iam
description: >-
  Handles identity and access control for Google Cloud resources through IAM
  policies and roles.
metadata:
  upstream:
    cluster: cloud-gcp
...
This skill routes all GCP provisioning and operational actions to the official Google Cloud Python client libraries.
...
Use the Google Cloud Python SDK for all GCP provisioning and operational tasks. This skill covers architecture guidance, cost modeling, and pre-flight requirements — execution is handled by the SDK.
...
- Pre-flight IAM and Workload Identity Federation requirements
- IaC approach decisions (Terraform AzureRM vs Deployment Manager vs Config Connector)
- Integration patterns with Google Workspace and other GCP services
- Multi-agent workflow design using Vertex AI Agent Engine
