...
description: >-
  Assists with Microsoft Entra ID app registration, OAuth 2.0 authentication,
  and MSAL integration. USE FOR: create app registration, register Azure AD app,
...
Microsoft Entra ID (formerly Azure Active Directory) is Microsoft's cloud-based identity and access management platform. App registrations enable applications to authenticate users and securely access Azure resources.
...
Create an app registration through the Azure portal or via the Azure CLI.
...
Using IaC to manage Entra app registrations is strongly recommended if your project already uses IaC, if you need a scalable approach to managing many registrations, or if you require a detailed audit history of configuration changes.

### Step 2: Configure Authentication

Configure authentication settings according to your application type.

- **Web Apps**: Add redirect URIs, enable ID tokens if required
- **SPAs**: Add redirect URIs, enable implicit grant flow if necessary
- **Mobile/Desktop**: Use `http://localhost` or a custom URI scheme
- **Services**: No redirect URI is needed for the client credentials flow

### Step 3: Configure API Permissions

Grant your application the permissions required to access Microsoft APIs or your own APIs.
...
For confidential client applications (web apps, services), generate a client secret, certificate, or federated identity credential.

**Client Secret:**
- Navigate to "Certificates & secrets"
- Create a new client secret
- Copy the value immediately (it is displayed only once)
- Store it securely (Key Vault is recommended)

**Certificate:** In production environments, prefer certificates over secrets for stronger security. Upload the certificate through the "Certificates & secrets" section.

**Federated Identity Credential:** Used to dynamically authenticate the confidential client against the Entra platform.

### Step 5: Implement OAuth Flow

Incorporate the OAuth flow into your application code.
...
Guide the user through their first app registration step by step.
...
Build a .NET/Python/Node.js console app that handles user authentication.
...
Establish daemon/service authentication without requiring user interaction.
...
MSAL is the preferred library for integrating with the Microsoft identity platform.
