...
This document illustrates the OAuth 2.0 authentication flows supported by Microsoft Entra ID.

**Note:** All implementation steps below are provided for illustrative purposes. Using a library to manage the authentication flow is always strongly recommended.
...
The user is redirected to the Microsoft login page, completes authentication, and grants consent.
...
**Validation:**
- Confirm `state` matches the value you originally sent
...
PKCE (Proof Key for Code Exchange) strengthens security for public clients that are unable to securely store a client secret.
...
**Parameters:**
- `scope`: Use the `{resource}/.default` format
  - For Microsoft Graph: `https://graph.microsoft.com/.default`
  - For your own API: `api://{api_app_id}/.default`
...
**Use for:** Devices that lack a browser (IoT devices, CLIs) and headless environments
...
**Poll at 5-second intervals (use the `interval` value from the response)**
...
**Use for:** Obtaining new access tokens after expiry without requiring re-authentication

### When to Refresh

- Access tokens generally expire after 1 hour
- Refresh tokens have a longer lifespan (14-90 days)
- Refresh proactively before the access token expires to ensure a seamless user experience
...
**Note:** `client_secret` is only required for confidential clients
...
**Important:** A new refresh token is issued with each response; use it for the next refresh
...
- Single-use (a new one is issued with each refresh)

### ID Token

- Contains user identity information
- Used by the app to identify the authenticated user
- Format: JWT
