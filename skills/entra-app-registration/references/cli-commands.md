...
This document serves as a complete reference for managing Microsoft Entra app registrations with the Azure CLI.
...
**⚠️ Important:** Resetting client credentials removes all existing credentials.
**⚠️ Important:** The secret value is displayed only once. Store it in a secure location (e.g., Azure Key Vault).
...
**Note:** Specify `Scope` for delegated permissions and `Role` for application permissions.
...
**Note:** Admin consent is needed for application permissions and for certain delegated permissions.
