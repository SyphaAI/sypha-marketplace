...
description: >-
  Build and deploy Lakeflow Jobs on Databricks using DABs, Python SDK, or the
  CLI. Use when authoring data engineering jobs with notebooks, Python wheels,
...
Lakeflow Jobs coordinate data workflows through multi-task DAGs, versatile triggers, and full-featured monitoring. Jobs accommodate a wide variety of task types and can be administered via Asset Bundles (DABs), Python SDK, or CLI.
...
After scaffolding, create `AGENTS.md` in the project directory to supply agents with project guidance. Use this content:
...
Do not pipe a remote installer directly into a shell. Confirm the installed binary with `databricks -v`.
...
Jobs support DAG-based task dependency graphs:
...
- `ALL_SUCCESS` (default) — executes only when all dependencies succeed
- `ALL_DONE` — executes when all dependencies finish, regardless of outcome
- `AT_LEAST_ONE_SUCCESS` — executes when at least one dependency succeeds
- `NONE_FAILED` — executes when no dependency has failed
- `ALL_FAILED` — executes only when all dependencies have failed
- `AT_LEAST_ONE_FAILED` — executes when at least one dependency has failed
...
Define shareable cluster configurations that tasks can reuse:
...
For notebook and Python tasks, leave out the cluster configuration to use serverless:
...
Parameters declared at the job level are forwarded to ALL tasks automatically (no need to repeat them per task):
...
- `CAN_VIEW` — read access to the job and its run history
- `CAN_MANAGE_RUN` — ability to view, trigger, and cancel runs
- `CAN_MANAGE` — complete control including editing and deleting the job
...
- **databricks-dabs** — DABs configuration patterns shared between jobs and pipelines
- **databricks-pipelines** — SDP (formerly DLT) pipelines invoked via `pipeline_task`
