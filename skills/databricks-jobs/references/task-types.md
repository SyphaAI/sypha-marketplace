...
Execute Databricks notebooks. This is the most frequently used task type.
...
Execute Python files directly on a Spark cluster.
...
Execute Python packages that are distributed as wheels.
...
Execute SQL queries or files, and refresh dashboards or alerts.
...
Execute dbt projects on Databricks.
...
Invoke DLT or Spark Declarative Pipelines.
...
Execute Scala/Java JAR files on Spark.
...
Launch another job as a task, enabling job chaining.
...
Iterate over a collection and execute a nested task for every element.
...
In the generating notebook, publish values using task values:
...
Inside the nested task, retrieve the current item via:
- In parameters: `{{input}}`
- In notebook: Read the value supplied through `base_parameters`

---

## Task Libraries

Attach libraries to tasks to satisfy their dependencies.
...
Declare reusable Python environments for serverless tasks that require custom pip dependencies.

> **IMPORTANT:** The `client` field is **required** in the environment `spec`. It identifies the
> base serverless environment version. Set it to `"4"`. Without it, the API returns:
