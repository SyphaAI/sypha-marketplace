...
Schedule jobs to run based on a cron expression.
...
Execute jobs at fixed intervals — a simpler alternative to cron.
...
Start jobs automatically when new files appear in cloud storage.
...
Fire jobs whenever Unity Catalog tables are modified.
...
- Tables must reside in Unity Catalog
- The job identity requires `SELECT` permission on the monitored tables
- Compatible with Delta tables, both managed and external

---

## Continuous Jobs

Perpetually running jobs that restart automatically.
...
- The job begins running as soon as it is created or unpaused
- Restarts automatically following completion or failure
- Keeps exactly one active run at any moment
- Set `pause_status: PAUSED` to halt execution
...
Execute jobs on demand without any automatic trigger.

### No Trigger Configuration

Leave out `schedule`, `trigger`, and `continuous`:
...
A job may define multiple trigger types, each evaluated independently:
...
- The job queues additional runs when `max_concurrent_runs > 1`
- Otherwise, subsequent trigger events are skipped while a run is in progress
