...
Deliver email alerts for job lifecycle events.
...
Dispatch HTTP webhooks for job events (Slack, PagerDuty, custom endpoints).

### Create Notification Destination First

Before using webhooks, set up a notification destination in the workspace:
...
Track job health metrics and trigger alerts when thresholds are breached.
...
- Once the timeout is reached, the task or job is cancelled
- Partial results may not be preserved
- The `on_failure` notifications are triggered as a result
...
- Retries apply only when a task fails
- Each retry initiates a fresh task attempt
- The retry count is reset at the start of each job run
- Downstream dependencies wait until all retries have concluded

---

## Run Queue Settings

Govern concurrent run behavior.
