...
DB2 pureScale provides shared-data, active-active clustering for DB2 LUW. All members read from and write to the same data concurrently — no data partitioning is needed. It delivers continuous availability and transparent horizontal scaling.

### Architecture Components

- **Members** — DB2 database engine instances that process SQL. All members are active and capable of handling read/write workloads.
- **Cluster Caching Facility (CF)** — A centralized shared memory facility responsible for global buffer pool management, lock management, and group buffer pool coordination. Typically deployed on dedicated hosts. Configure a primary and secondary CF for redundancy.
- **GPFS / IBM Spectrum Scale** — The required shared filesystem. All members and CFs access the same storage through GPFS.
- **Cluster interconnect** — A low-latency network (InfiniBand or 10GbE+) linking members and CFs for lock and page transfers.
...
For most RHEL deployments, HADR with automatic client reroute is the standard HA approach. pureScale is suited to workloads that demand active-active read/write scaling or sub-second failover without requiring application-level awareness.
