...
The instance profile is loaded at login: `~db2inst1/sqllib/db2profile`
...
Most registry variable changes require a `db2stop` / `db2start` cycle before they take effect.
...
DB2 provides three tablespace types: SMS (System Managed Space), DMS (Database Managed Space), and Automatic Storage (recommended).
