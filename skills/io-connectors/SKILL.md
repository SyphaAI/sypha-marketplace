...
name: io-connectors
description: >-
  Provides guidance on developing and using I/O connectors in Apache Beam.
  Apply when building with I/O connectors, implementing new connectors, or
  troubleshooting data source and sink problems.
metadata:
  category: data
  source:
...
## Overview
I/O connectors allow pipelines to read from and write to external data sources. Beam ships with 51+ Java I/O connectors and a number of Python connectors.
...
Resides in the `it/` directory:
- `it/common/` - Shared test utilities
...
TestPipeline:
- Blocks on execution by default (on TestDataflowRunner)
- Has a 15-minute default timeout
- Reads options from the `beamTestPipelineOptions` system property
...
## Cross-language I/O
Beam allows I/O connectors from one SDK to be used in another through the expansion service.
...
For detailed guidance on building new I/O connectors, see the [Developing new I/O connectors SKILL](../developing-new-io-connectors/SKILL.md).
