---
name: dd-pup
description: Datadog CLI (Rust). OAuth2 authentication with token refresh support.
metadata:
  version: 1.0.1
  author: datadog-labs
...
The Pup CLI for Datadog API operations. Supports both OAuth2 and API key authentication.
...
Install pup by following the [setup instructions](https://github.com/datadog-labs/agent-skills/tree/main?tab=readme-ov-file#setup-pup).

## Required Input Resolution

For commands that require specific scope values (`<env>`, `<service-name>`, `<team-id>`, resource IDs), follow this order:

1. Check context first (conversation history, prior command output, saved variables).
2. If a value is missing, run a discovery command first (list/search) to obtain valid values.
3. If still missing or ambiguous, ask the user to confirm the exact value.
4. Then execute the target command.
5. Never run commands with unresolved placeholders such as `<env>` or `<monitor-id>`.
...
**Tokens expire (~1 hour)**. If a command fails with 401/403 during a conversation:
...
If Chrome opens the wrong profile or window, copy the one-time OAuth URL printed by `pup auth login`
(`If the browser doesn't open, visit: ...`) and open it manually in the correct account session.
...
Refer to [Setup Pup](https://github.com/datadog-labs/agent-skills/tree/main?tab=readme-ov-file#setup-pup) for installation instructions.
