...
description: >-
  Build Lakeflow Spark Declarative Pipelines (formerly Delta Live Tables) on
  Databricks. Use when constructing batch or streaming data pipelines with
  Python or SQL. Invoke BEFORE beginning implementation.
...
Use this tree to identify the correct dataset type and applicable features. Multiple features can be applied to the same dataset — for example, a Streaming Table can combine Auto Loader for ingestion, Append Flows for fan-in, and Expectations for data quality. Select the dataset type first, then layer on the relevant features.
...
- **Names** → SDP = LDP = Lakeflow Declarative Pipelines = (formerly) DLT. All are interchangeable when the user references them.
- **"Create a table" without specifying type** → ask whether the source is streaming or batch. Streaming source → Streaming Table; batch source → Materialized View. Mismatched pairs fail at validation.
- **Aggregation over a streaming source** → use a Materialized View with a batch read (`spark.read.table` / `SELECT FROM` without `STREAM`). Streaming Tables are append-only and do not recompute aggregates when source rows change; Materialized Views do.
- **Intermediate logic** → default to a Temporary View, even for shared logic reused by multiple downstream tables. Use a Private MV/ST (`private=True` / `CREATE PRIVATE ...`) only when materializing once avoids significant reprocessing. For preprocessing before Auto CDC, the temp view is required — the CDC flow reads from `STREAM(view_name)` (SQL) or `spark.readStream.table("view_name")` (Python).
- **Union of streams** → use multiple Append Flows instead. UNION across streaming sources is an anti-pattern.
- **Changing dataset type** → converting ST→MV or MV→ST in place is not possible. A full refresh does NOT fix this. Drop the existing table manually or give the new dataset a different name.
- **`CREATE OR REFRESH` vs `CREATE`** → both parse for SQL datasets, but `CREATE OR REFRESH` is the idiomatic form. For PRIVATE datasets: `CREATE OR REFRESH PRIVATE STREAMING TABLE` / `... MATERIALIZED VIEW`.
- **Kafka/Event Hubs sink serialization** → the `value` column is required; serialize the row using `to_json(struct(*)) AS value`. See [sink-python.md](references/sink-python.md).
...
- **Auto CDC TRUNCATE** (SCD Type 1 only) → SQL: `APPLY AS TRUNCATE WHEN condition`. Python: `apply_as_truncates=expr("condition")`. Do NOT state that truncate is unsupported.
- **Python-only features** → Sinks, ForEachBatch Sinks, CDC from snapshots, and custom data sources require Python. When the user is working in SQL, explain this limitation and recommend switching to Python.
- **Recommend ONE clear approach** → present a single recommended path. Avoid listing anti-patterns or inferior alternatives — they cause confusion. Mention alternatives only when they offer genuinely different trade-offs.

## Common Issues

Error-to-cause/fix mappings that agents encounter frequently. For deploy issues specific to DAB-bundle vs CLI-iteration workflows, see the corresponding reference files.
...
Each pipeline has a **default catalog and schema** configured in its settings. All datasets are written there unless overridden.

- **Fully-qualified names**: Supply `catalog.schema.table` in the dataset name to write to a catalog/schema other than the pipeline default. The pipeline creates the dataset there directly — no Sink is required.
- **USE CATALOG / USE SCHEMA**: SQL commands that switch the current catalog/schema for all subsequent definitions in the same file.
- **LIVE prefix**: Deprecated. It is ignored in the default publishing mode.
- When reading or defining datasets within the pipeline, use only the dataset name — do NOT use fully-qualified names unless the pipeline already does so or the user explicitly asks for a different target catalog/schema.

## API Reference

**Read the linked reference file before writing pipeline code for any feature.** Each table below maps a feature to its exact API and to the detail file for that (feature, language) combination.

Some features depend on others — read both relevant files:

- **Auto Loader** / **Auto CDC** / **Sinks** write to a streaming table → also read [streaming-table-python.md](references/streaming-table-python.md) / [streaming-table-sql.md](references/streaming-table-sql.md).
- **Expectations** are attached to a dataset → also read the dataset definition file (streaming-table / materialized-view / temporary-view).
...
The tables above cover **only the modern API**. If any of the following appears in user code, it is the legacy DLT syntax — **always migrate it to the modern form**, never extend it. Read [references/dlt-migration.md](references/dlt-migration.md) before recommending changes to ensure the conversion is correct (especially around `apply_changes` → `create_auto_cdc_flow` semantics and `partition_cols` → `cluster_by`).
...
Make this decision before scaffolding — it determines which template files (`.py` vs `.sql`) and reference docs to use. Both languages can coexist, but choose a primary one. When in doubt, default to SQL for simplicity.
...
If the intent is unclear, ask. Stay with the chosen language unless the user explicitly requests a switch.

## Choose Your Workflow

Three project shapes are available — choose before scaffolding. Default to A for production-bound work and C for exploration or demo scaffolding.

- **A: Standalone new pipeline project (DAB)** — the pipeline IS the project, with no existing `databricks.yml`. Scaffold with `databricks pipelines init --output-dir . --config-file init-config.json`. → [1-project-initialization-with-dab.md](references/1-project-initialization-with-dab.md)
- **B: Pipeline in an existing bundle (DAB)** — `databricks.yml` already exists. Add a `resources/<name>.pipeline.yml` that points at `src/`. → [1-project-initialization-with-dab.md#workflow-b-pipeline-in-existing-bundle](references/1-project-initialization-with-dab.md#workflow-b-pipeline-in-existing-bundle)
- **C: Rapid CLI iteration (no bundle)** — for prototyping. Use `databricks pipelines create / start-update / list-pipeline-events`; convert to a bundle later if the work moves to production. → [2-rapid-iteration-with-cli.md](references/2-rapid-iteration-with-cli.md)

## Pipeline Structure

- Follow the medallion pattern (Bronze → Silver → Gold) unless the user specifies otherwise. Keep the structure simple by default — a few tables is enough.
- Place one dataset per file, named after the dataset. Transformation files belong in `src/` or `transformations/`.
- **Gold layer: retain the key business dimensions.** When aggregating into Gold, preserve the dimensions analysts need to filter and slice by (location, department, product line, customer segment, time period). Over-aggregating discards information that cannot be recovered downstream. If a dashboard is mentioned, every filter it uses must appear as a column in the Gold table. It is easier to aggregate further in queries than to reconstruct lost dimensions.


## Running a Pipeline

The correct run command depends on which workflow was selected above.

- **Workflow A / B (DAB)** — Code changes only become effective after running `databricks bundle deploy`. Always deploy before any run, dry run, or selective refresh.
  ```bash
...
- **Workflow C (CLI, no bundle)** — Upload files to the workspace and drive the pipeline directly. Re-upload after each code change.
  ```bash
...
- **Selective refresh** is the preferred option when only one table needs to run. Its dependencies must already be materialized.
- **Full refresh** is the most expensive and risky option and **can cause data loss** (it reprocesses streaming sources from the beginning, destroying streaming state). Use it only when truly necessary. Always raise it as a follow-up action that the user must explicitly approve.

**Always poll the update**, not the top-level pipeline state — see the rationale in [2-rapid-iteration-with-cli.md#step-4-start-an-update-and-poll-that-update](references/2-rapid-iteration-with-cli.md#step-4-start-an-update-and-poll-that-update). This rule applies equally to bundle runs.
