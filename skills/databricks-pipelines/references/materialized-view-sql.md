...
Clause notes (the same semantics apply as with streaming tables — see [streaming-table-sql.md](streaming-table-sql.md) for the detailed treatment of `PRIVATE`, `MASK`, `WITH ROW FILTER`, and informational table constraints):
...
- Generated columns are supported via `col TYPE GENERATED ALWAYS AS (expr)`.
...
MVs apply incremental refresh automatically when possible, falling back to full recompute otherwise.
