...
Handles CDC from streaming events (`dp.create_auto_cdc_flow`) or periodic snapshots (`dp.create_auto_cdc_from_snapshot_flow`). Both approaches write into a pre-created streaming table.

Choose streaming when CDC events arrive continuously (transaction logs, Kafka, Delta change feeds). Choose snapshot when the source is a full dump that is compared against the previous state (daily extracts, batch exports).

The legacy aliases `dp.apply_changes()` / `dp.apply_changes_from_snapshot()` still parse but should be migrated (see [SKILL.md Legacy DLT Syntax](../SKILL.md#legacy-dlt-syntax--always-migrate)).

For querying SCD Type 2 history tables, see [scd-2-querying.md](scd-2-querying.md).

## `dp.create_auto_cdc_flow(...)`

Call at the top level — does NOT return a value.
...
`source` must be a table/view identifier (string) — NOT a DataFrame. To filter the source before CDC processing, define a `@dp.temporary_view()` and reference its name. Avoid materializing a streaming table solely for filtering — a temp view is the preferred approach.
...
`source` takes either a string (the most common form — the name of a table holding the latest snapshot) or a callable for replaying historical snapshots:
...
The version must be a comparable scalar type (`int`, `str`, `float`, `bytes`, `datetime`, `date`, or `Decimal`).
...
Use `track_history_except_column_list=[...]` for the opposite behavior.
...
- Always create the target first with `dp.create_streaming_table()`.
- `dp.create_auto_cdc_flow()` does NOT return a value — invoke it at the top level.
- `source` must be a string table/view name, never a DataFrame. Pre-process using `@dp.temporary_view()`.
- SCD Type 2 appends `__START_AT` / `__END_AT` columns whose type matches `sequence_by`. Include them if you provide an explicit target schema.
- `sequence_by` accepts either a string column name or `col("ts")` — both work. Use `struct("ts", "id")` for multi-column ordering.
