...
Two DAB-based workflows for building Spark Declarative Pipelines:

- **Workflow A**: Standalone new project where the pipeline *is* the entire project.
- **Workflow B**: Adding a pipeline to an existing bundle (the pipeline is one component of a larger app, jobs, and dashboards setup).

For prototyping without a bundle, refer to [2-rapid-iteration-with-cli.md](2-rapid-iteration-with-cli.md).

---

## Workflow A: Standalone Bundle (`pipelines init`)

Use when the user needs a new project where the pipeline *is* the project (no existing `databricks.yml`).
...
Both produce DAB-structured projects; `pipelines init` is the newer, more targeted command.
...
**Key rule**: Pipeline transformations are plain `.sql` / `.py` files. Notebooks belong in `explorations/` for ad-hoc work only.

### Customize and deploy

1. Replace the `sample_*` files in `transformations/` with your actual datasets (one dataset per file).
2. Edit `databricks.yml` to define per-target catalog/schema variables and the workspace host.
3. Update `resources/<name>_etl.pipeline.yml` for pipeline-level settings (serverless is enabled by default).
4. Run `databricks bundle validate` → `databricks bundle deploy [-t <target>]` → `databricks bundle run <pipeline_name>`.
...
Python projects include a standard `pyproject.toml`. Runtime dependencies go in `[project].dependencies`; development-only dependencies go in `[project.optional-dependencies].dev` (e.g. `databricks-connect>=15.4,<15.5`, `pytest`, `ruff`). The `--editable ${workspace.file_path}` entry in the pipeline resource installs the package on serverless compute when the bundle is deployed.
...
Use when `databricks.yml` already exists for a larger project (app, jobs, and dashboards) and a pipeline is being added to it.
...
The pipeline inherits the bundle's existing targets, variables, and permissions.

---

## Running a Pipeline (Workflow A / B)

**A deployment is required before running.** During local development, code changes only take effect after `databricks bundle deploy`. Always deploy before any run, dry run, or selective refresh.
...
For the reasoning behind polling the update (not the pipeline) and the FAILED-extraction `jq` pattern, see [2-rapid-iteration-with-cli.md#step-4-start-an-update-and-poll-that-update](2-rapid-iteration-with-cli.md#step-4-start-an-update-and-poll-that-update). This applies to bundle runs as well.

### Refresh modes

- **Selective refresh** is the preferred approach when only one table needs to be updated. Its dependencies must already be materialized.
- **Full refresh** is the most costly option and **can cause data loss** — it reprocesses streaming sources from scratch and wipes streaming state. Use it only when truly needed, and always present it as a follow-up that the user must explicitly approve. CLI flags: `databricks bundle run <pipeline_name> --full-refresh-all` or `--refresh <table>` for selective.

### Editing pipeline code

Edit `.sql` / `.py` files under `src/`, then run `databricks bundle deploy` followed by `databricks bundle run`. The bundle deploy uploads changed files as raw `FILE` entries. Do not use `databricks workspace import --format SOURCE` for a bundle-managed pipeline — that creates a NOTEBOOK entry, causing subsequent bundle deploys to fail with `type mismatch (asked: FILE, actual: NOTEBOOK)`.

---

## Migrating from a Manual Folder Structure

When the user already has `bronze/`, `silver/`, `gold/` folders without a bundle, migrate to Workflow A by adding a `databricks.yml` and a pipeline resource that points to the existing folders using a `glob`. No file moves are needed — the medallion folders work as-is under `transformations/**`.
