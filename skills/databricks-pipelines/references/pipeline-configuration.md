...
JSON field reference for `databricks pipelines create --json '{...}'` and `databricks pipelines update <id> --json '{...}'`, with variant snippets covering the most common configurations.

Defaults to **serverless + Unity Catalog**. Avoid setting `serverless: false` unless the user specifically requires R, Spark RDD APIs, or JAR / Maven libraries.

## Canonical Create (dev / iteration defaults)

For development, demos, and iterative work, include these fields every time:
...
> **Tuned for demo / iteration.** The `pipelines.*RetryAttempts: "0"` overrides turn off retries so a failing update surfaces quickly (~30s) rather than spinning on the same root cause for 10+ minutes. For production, **remove these overrides** so the platform's retry defaults (5 update / 2 flow) can absorb transient infra failures.

Per-field rationale:

- **`continuous: false`** — triggered runs. `true` auto-restarts failed updates indefinitely (`cause: RETRY_ON_FAILURE`), wasting cost and locking up polling loops. Only set to `true` when the user explicitly asks for always-on streaming.
- **`development: true`** — enables faster startup, relaxed validation, and no retry-on-failure. Mandatory for any edit/re-run loop.
- **`pipelines.numUpdateRetryAttempts: "0"` + `maxFlowRetryAttempts: "0"`** — belt-and-suspenders guard against retries. Even in `development` mode, some configurations still retry. Remove for prod.
- **`channel: "PREVIEW"`** — grants access to the latest features. Use `"CURRENT"` (the default) for production stability.

The variant snippets below show only the **deltas** to add or replace in the canonical JSON.
...
Every key defined here is also reachable from pipeline code via `spark.conf.get("key")` — leverage this to parameterize transformations.
...
For continuous pipelines, defines the 5-hour window during which daily restarts may be scheduled:
...
The canonical create above is optimized for iterative work. For production, **remove** `"development": true` and both `pipelines.*RetryAttempts` overrides so the platform's retry defaults (5 / 2) can handle transient infra failures. Add ownership tags:
...
Set `"channel"` to `"CURRENT"` for stable runtime behavior.

### Non-serverless / dedicated cluster

Only needed for R, Spark RDD APIs, or JAR/Maven libraries.
...
`update` accepts the same JSON shape as `create`:
...
Then kick off a new run with `databricks pipelines start-update <pipeline_id> [--full-refresh]`. See [2-rapid-iteration-with-cli.md](2-rapid-iteration-with-cli.md#step-4-start-an-update-and-poll-that-update) for the polling pattern — never poll top-level `pipelines get` state to determine run completion.

---

## Multi-Schema Patterns

**Preferred: one pipeline, multiple schemas** via fully-qualified table names — simpler than operating multiple pipelines. For straightforward cases where all tables share a single schema, use name prefixes (`bronze_*`, `silver_*`, `gold_*`).

Set the pipeline default schema to one layer (e.g. bronze) and pull the rest from `configuration`:
...
For cross-catalog references: supply a three-part `f"{cat}.{schema}.{table}"` in `name=`. SQL uses the identical fully-qualified form in `CREATE OR REFRESH ...`.
