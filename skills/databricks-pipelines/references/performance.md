...
Use `CLUSTER BY (AUTO)` / `cluster_by=["AUTO"]` while exploring the workload, prototyping, or when access patterns are uncertain. Switch to manually chosen keys in production once query patterns are well understood.

### Cluster key data types

**Numeric, string, date, or timestamp only.** `BOOLEAN`, `ARRAY`, `MAP`, `STRUCT`, `BINARY` fail at first write with `DELTA_CLUSTERING_COLUMNS_DATATYPE_NOT_SUPPORTED` (no data-skipping stats). Low-cardinality flags also provide no benefit from clustering — leave them out.
...
Typical wins: 20–50% query improvement, no small-file problem, automatic optimization. **Keep `PARTITION BY` only for**: regulatory physical separation, data lifecycle requiring `DROP PARTITION`, DBR < 13.3 compatibility, or large existing tables where migration cost exceeds the benefit.
...
Short retention windows break time-travel queries that reach beyond the window — apply this only to high-volume tables where storage cost is the dominant concern.
...
MVs apply incremental refresh automatically where possible. Requirements:
...
Falls back to full recompute when any requirement is not satisfied.

---

## State Management for Streaming

Higher cardinality produces more state. Monitor the combinations used in `GROUP BY`.
...
Small static dimensions broadcast automatically — no additional configuration required.
...
Always bound by an event-time interval. Without bounds, state grows without limit.
...
**Filter early** — push filters into the streaming read so downstream MV inputs remain small. The anti-pattern is wide-open silver tables filtered later in gold MVs — every row gets processed twice.
...
**Avoid `SELECT *`** once the schema is stable. Narrow projections enable Delta column pruning and reduce wire/state size for stateful operations.

---

## Pre-Aggregation

When the same coarse aggregation is queried repeatedly, materialize it.
...
Querying `orders_monthly` is significantly cheaper than re-aggregating the source table each time.
...
**Default to serverless.** Move to classic only when R, Spark RDD APIs, JAR/Maven libraries, or other serverless-incompatible features are needed — see [pipeline-configuration.md](pipeline-configuration.md#serverless-limitations-force-classic-clusters).
...
Watch for slow streaming tables (high processing lag), large state operations (memory pressure), expensive joins (long batch durations), and small-file accumulation (raise auto-optimize).
