...
Reference for reading SCD Type 2 history tables produced by Auto CDC: covers current-state views, point-in-time queries, change analysis, and joining facts with historical dimensions. SQL is shown as the canonical form; Python equivalents translate via `spark.read.table(...).filter(F.col("__END_AT").isNull())` and similar.
...
Both columns share the same type as the `SEQUENCE BY` / `sequence_by` column (typically `TIMESTAMP`).

**Rule of thumb**: `WHERE __END_AT IS NULL` selects only current rows. This is the most frequently used filter — embed it in a materialized view if you need it repeatedly.
...
To look up a single entity: `WHERE customer_id = '12345' AND __END_AT IS NULL`.

## Point-in-Time Queries

Retrieves the state as it existed on a specific date. **Boundary convention**: `[__START_AT, __END_AT)` — start inclusive, end exclusive. Getting this wrong will either drop the seam row or introduce double-counting.
...
For each fact row, select the dimension version that was active at the transaction's event time.
...
Suitable when attributes are *labels* (always-current product name, region label) rather than values that factor into calculations.
...
**When to use which**: use as-of-time for revenue, billing, and audit scenarios; use current-dim for operational dashboards where attributes serve as display labels.

## Optimization

**Push repeated filters into MVs** when querying history tables frequently:
...
**Cluster the history table on lookup key + time**: `CLUSTER BY (product_id, __START_AT)`. This speeds up both entity lookups and point-in-time scans. See [performance.md#cluster-key-selection-by-layer](performance.md#cluster-key-selection-by-layer).

## Best Practices

1. **Filter `__END_AT IS NULL` for "current"** — never compare `__START_AT` against `MAX(__START_AT)` per entity. That approach is slower and breaks under concurrent updates.
2. **Inclusive-lower / exclusive-upper** for point-in-time joins (`__START_AT <= D AND (__END_AT > D OR __END_AT IS NULL)`).
3. **Materialize repeated filters.** A `dim_*_current` MV is more efficient than re-filtering the history table on every downstream read.
4. **High-precision `SEQUENCE BY`.** Sub-second collisions produce non-deterministic ordering — use microsecond timestamps or `STRUCT(ts, tiebreaker)`.
5. **`TRACK HISTORY ON` only columns that require versioning** on wide tables (remaining columns receive Type-1 in-place updates without generating new history rows).
