...
description: >-
  ...
  with automatic compaction and snapshot management. Handles table bucket,
  namespace, table, schema, Glue catalog registration, partitioning, and IAM
  access control. Triggers on: create table, data lake table, analytics table,
  ...
  querying existing tables (use querying-data-lake), or locating an existing
  table (use finding-data-lake-assets).
metadata:
  upstream:
    version: 1
...
Amazon S3 Tables delivers managed Iceberg tables with automatic compaction and snapshot management, queryable through Athena and Iceberg-compatible engines.

## Common Tasks

You MUST use AWS MCP server tools when connected — they provide command validation, sandboxed execution, and audit logging. Fall back to AWS CLI if MCP is unavailable.

## Decision Guide

**Before creating, You MUST check what exists:**

You MUST run `aws glue get-tables --database-name <NAME>` when the user references a database.
...
- **Existing data in S3**: Create an empty table (Steps 1-8), then use the `ingesting-into-data-lake` skill.
- **Glue ETL pipeline**: Read `references/table-creation-glue-etl.md` first, then follow Steps 1-6.
...
- You MUST check whether AWS MCP server tools or the AWS CLI are available and notify the user if either is missing
- You MUST confirm the target AWS region and validate credentials with `aws sts get-caller-identity`

### 2. Understand the Schema

- **Explicit schema**: Validate Iceberg types.
- **Loose description**: Ask for columns, types, and grain. Propose a schema and confirm.
- **Existing S3 data**: Infer schema from file headers only. Create an empty table first, then use the `ingesting-into-data-lake` skill.

**Constraints:**

- You MUST read `references/best-practices.md` for Iceberg type mapping, partitioning, and naming conventions.
- You MUST request all required parameters upfront: table name, columns, types, partition strategy. For schema evolution, see `references/athena-ddl-path.md`.
...
- You SHOULD recommend partition columns based on query access patterns.
...
Capture the `table-bucket-arn`. Encryption (SSE-S3 default, SSE-KMS) and storage class (STANDARD, INTELLIGENT_TIERING) are set at creation. See `references/best-practices.md`.

**Constraints:**

- You MUST list existing buckets with `aws s3tables list-table-buckets` and ask the user to select one or create a new one.
- When SSE-KMS is used, the KMS key policy MUST allow the S3 Tables maintenance service principal to read data. Search AWS docs for `"S3 Tables KMS key policy"` for the required policy.
- If bucket creation fails, consult `references/best-practices.md` for common errors.
...
- You MUST list existing namespaces first and suggest reusing one when appropriate
- You MUST use lowercase names with no hyphens

### 5. Create Glue Data Catalog Integration

Check whether `s3tablescatalog` exists (create once per region per account):

```bash
aws glue get-catalog --catalog-id s3tablescatalog
```

If it is not found, create it (requires `glue:CreateCatalog`, `glue:passConnection`):
...
S3 Tables uses the `s3tables:*` IAM namespace (not `s3:*`).
...
You MUST scope policies to the correct ARN patterns. You MUST read `references/access-control.md` for exact resource ARNs.

**Constraints:**

- You MUST ask the user for the querying principal ARN
- You MUST NOT grant permissions broader than necessary
- You MUST NOT create IAM roles automatically; verify existing roles and guide the user
...
You MUST verify the table with `aws s3tables get-table` and confirm queryability by running `DESCRIBE <table_name>` via Athena with `--query-execution-context '{"Catalog":"s3tablescatalog/<BUCKET_NAME>","Database":"<NAMESPACE>"}'`. Do NOT place the catalog name in the SQL. Present a summary: bucket ARN, namespace, table, schema, partitions.
...
- [best-practices.md](references/best-practices.md) -- Iceberg types, partitioning, naming conventions, common errors
...
- Loading data: use the `ingesting-into-data-lake` skill
