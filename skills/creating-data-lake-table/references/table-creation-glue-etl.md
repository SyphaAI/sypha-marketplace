...
Use this reference when building Glue ETL pipelines that create and write to S3 Tables. Tables created through the S3 Tables API (`aws s3tables create-table --metadata`) are also accessible by Spark.
...
- The table bucket and namespace MUST exist before the job runs

## Static Config Gotcha (Most Common Failure)

In Glue 5.x, catalog configs are **static** and MUST be provided as `--conf` job arguments. Using `spark.conf.set()` raises:

```
AnalysisException: Cannot modify the value of a static config: spark.sql.extensions
```

**Rule:** All `spark.sql.catalog.*` configuration belongs in `--conf`, never in the PySpark script.

**Rule:** Catalog and database names that contain hyphens MUST be backtick-escaped in Spark SQL (e.g., `` `my-catalog`.`my-db`.my_table ``). Without backticks, Spark returns `INVALID_IDENTIFIER`.
...
The `--conf` argument is a single string of space-separated `--conf key=value` pairs:

```json
"--conf": "spark.sql.catalog.s3tables=org.apache.iceberg.spark.SparkCatalog --conf spark.sql.catalog.s3tables.type=rest --conf ..."
```

The first key-value pair has no `--conf` prefix. Use `--cli-input-json file://config.json` to sidestep shell escaping issues.
...
Because catalog config lives in `--conf`, the script remains clean:
...
The Glue service role requires: `AWSGlueServiceRole` plus `s3tables:GetTableBucket`, `s3tables:GetNamespace`, `s3tables:ListNamespaces`, `s3tables:CreateTable`, `s3tables:GetTable`, `s3tables:ListTables`, `s3tables:UpdateTableMetadataLocation`, `s3tables:GetTableMetadataLocation`, `s3tables:GetTableData`, `s3tables:PutTableData`, and `glue:GetCatalog`, `glue:GetDatabase`, `glue:GetTable`, `glue:passConnection`. The table bucket, namespace, and Glue catalog MUST be created before the Glue job runs (Steps 3-5 in SKILL.md). For precise resource ARN scoping, see `access-control.md`.
...
For the latest Glue ETL guidance, search AWS docs for `"Running ETL jobs on Amazon S3 tables with AWS Glue"`.
