...
**Schema changes are unavoidable, but uncontrolled changes produce schema drift** — documents within the same collection end up with inconsistent structures, resulting in application errors and query failures. Use `schemaVersion` fields for controlled migration and schema validation to guard against unintended drift.

### The problem: schema drift

MongoDB's flexible schema is a strength, but careless field additions force code to accommodate many different document shapes.
...
Add a `schemaVersion` field to every document. Application code inspects the version and handles both the old and new formats. This lets old and new documents coexist, allows new code to be deployed ahead of the data migration, permits gradual migration during low-traffic windows, and keeps rollback straightforward.
...
- Adding new optional fields (existing code simply ignores them)
- Adding new indexes (invisible to the application)
- Relaxing validation (converting a required field to optional)
...
- Removing fields that existing code still reads
...
- **Truly polymorphic data**: Event logs containing different event types may require flexible schemas — use `pattern-polymorphic` instead.
- **Early prototyping**: Defer validation during the exploration phase; introduce it before moving to production.
- **User-defined fields**: Some applications intentionally permit custom metadata fields.
- **Small datasets with a downtime window**: When the entire dataset can be migrated in minutes during a maintenance period.
- **Additive-only changes**: If all modifications are limited to adding optional fields, versioning is unnecessary overhead.
