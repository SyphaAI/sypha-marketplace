...
**Every index carries a write cost.** On insert, update, and delete operations, MongoDB must update ALL indexes on the collection. Unused or redundant indexes degrade write performance without providing any query benefit, and they occupy RAM in the WiredTiger cache that would otherwise serve working set data. Atlas Performance Advisor explicitly flags "Redundant Index" and "Unused Index" conditions.
...
Atlas automatically surfaces:
- **"Redundant Index"** — a prefix subset of an existing compound index
- **"Unused Index"** — zero query usage during the observed window

**When NOT to use this pattern:**

- **New collections with anticipated queries**: If known queries are on the way, creating indexes in advance is perfectly reasonable.
- **Indexes supporting infrequent but critical operations**: A compliance or backup query that runs once a month may show low usage statistics but is still necessary.
- **TTL indexes**: These fulfill data lifecycle objectives regardless of whether they are used by application queries.
