...
**Time series collections are designed specifically for append-only measurement workloads.** MongoDB handles bucketing, compression, and indexing of time series data automatically, delivering high ingest rates with significantly less storage and index overhead compared to a regular collection. They are well-suited for IoT sensor readings, application metrics, financial data, and event logs.

**MongoDB 8.0 Performance:** The block processing engine introduced in MongoDB 8.0 can substantially accelerate eligible analytical pipelines (for example, `$match` + `$sort` on the time field + `$group`). In some scenarios throughput gains exceed 200%. This optimization applies automatically for qualifying queries.
...
- **Not time-based data**: The primary access pattern is not time-range queries.
- **Frequent updates/deletes**: Time series collections are optimized for append-only workloads; modifying historical data is slow.
- **Very low volume**: A few hundred events gain nothing from bucketing.
- **Need transactional writes**: Time series collections do not support writes inside transactions (reads are supported).
- **Complex queries on measurements**: When most queries filter primarily on non-time fields, regular collections are likely a better fit.
