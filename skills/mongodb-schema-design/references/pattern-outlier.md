...
**Separate atypical documents with oversized arrays to protect typical query performance.** When a small fraction of documents is substantially larger than the majority, those outliers can monopolize memory, index space, and query processing. Move overflow data to a separate collection and mark the document with a flag.

**Problem scenario:**

A typical book might have 50 customers in an embedded array, while a bestseller like Harry Potter can accumulate 50,000 (~2.5MB). Because queries return the entire document, the outlier consumes a disproportionate share of memory and network bandwidth. A multikey index on that array generates 50,000 index entries for a single document.

**Correct (outlier pattern):**

Standard documents retain their full embedded array and carry `hasExtras: false`. Outlier documents cap the embedded array at a threshold (e.g. 50), set `hasExtras: true`, store a denormalized `customerCount`, and place remaining items into batched documents in a separate collection (e.g. `{ bookId, customers: [...], batch: 1, count: 950 }`). Application code inspects the `hasExtras` flag to determine whether overflow batches need to be loaded.
...
- **Uniform distribution**: If all documents have comparable array sizes, there are no outliers to isolate.
- **Always need full data**: If the application always displays all 50,000 customers, the pattern provides no benefit.
- **Write-heavy outliers**: The complexity of the update logic may not justify the read-path optimization.
- **Small outliers**: If outliers are 200 items versus a typical 50, simply raising the threshold is sufficient.
