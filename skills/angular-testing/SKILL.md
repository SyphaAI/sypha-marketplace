...
description: >-
  Construct unit and integration tests for Angular v20+ applications using Vitest or
  Jasmine with TestBed and modern testing patterns. Use when writing tests for
  components with signals, OnPush change detection, services that depend on
  inject(), and HTTP interactions. Activates when creating tests, testing
  signal-based components, mocking dependencies, or setting up test
  infrastructure. Does not cover E2E testing via Cypress or Playwright, or
  testing JavaScript/TypeScript code outside Angular.
metadata:
...
Write tests for Angular v20+ applications with Vitest (the preferred option) or Jasmine, targeting signal-based components and current patterns.

## Vitest Setup (Angular v20+)

Angular v20+ includes built-in Vitest support via the `@angular/build` package.
...
For migrating from Jasmine to Vitest and for advanced configuration options, refer to [references/vitest-migration.md](references/vitest-migration.md).
...
For OnPush components, change detection must be triggered manually:
...
More advanced testing approaches — including component harnesses, router testing, form testing, and directive testing — are documented in [references/testing-patterns.md](references/testing-patterns.md).

Instructions for migrating from Jasmine to Vitest can be found in [references/vitest-migration.md](references/vitest-migration.md).
