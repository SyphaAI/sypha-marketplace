...
Every option is supplied to `ZSchema.create(options)`. This file documents each option along with its type, default value, and behavior.
...
These are processed by `ZSchema.create()` and control which validator variant is returned. They are not retained on the instance.
...
Both can be combined: `ZSchema.create({ async: true, safe: true })`.
...
`'none'` bypasses meta-schema version detection — schemas are validated according to whatever `$schema` declares.
...
These options instruct `validateSchema()` and `validate()` to reject schemas that are missing certain keywords.
...
Setting `strictMode: true` activates all of the following simultaneously:
...
These are supplied as the third argument to `validate()` / `validateSafe()`, not to `ZSchema.create()`.
