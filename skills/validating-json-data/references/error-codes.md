...
Full list of error codes emitted by z-schema. Each error is reported in `SchemaErrorDetail.code`.
...
These errors generate nested sub-errors in `detail.inner`.
...
These occur during `validateSchema()` calls, not during data validation.
