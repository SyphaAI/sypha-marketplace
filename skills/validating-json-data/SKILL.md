...
description: >-
  Validates JSON data against JSON Schema using the z-schema library. Invoke
  when the user needs to validate JSON, check data against a schema, handle
  validation errors, work with custom format validators, target JSON Schema
  drafts 04 through 2020-12, set up z-schema in a project, compile schemas with
  cross-references, resolve remote $ref, tune validation options, or examine
  error details. Covers sync/async modes, safe error handling, schema
...
z-schema checks JSON data against JSON Schema (draft-04, draft-06, draft-07, draft-2019-09, draft-2020-12). Default draft: **draft-2020-12**.
...
z-schema offers four modes controlled by two toggles: `async` and `safe`. Select whichever one fits your use case.
...
Alternatively, call `.validateSafe()` on a regular (throwing) validator to get the same result shape:
...
Necessary when using async format validators.
...
`ValidateError` exposes `.details` — an array of `SchemaErrorDetail`:
...
Combinator keywords (`anyOf`, `oneOf`, `not`) generate nested `inner` errors:
...
Supply `ValidateOptions` as the third argument to include or exclude particular error codes:
...
For the complete error code list, see [references/error-codes.md](references/error-codes.md).

## Schema pre-compilation

Pre-compile schemas at startup for improved runtime performance and to resolve cross-references:
...
Must return `Promise<boolean>`. Requires `{ async: true }`.
...
Specify the draft explicitly when the schema is written for a particular version:
...
For a side-by-side feature comparison across drafts, see [references/draft-comparison.md](references/draft-comparison.md).
...
Scope validation to a specific path within a schema:
...
All types are available as named exports from the package:
...
- [references/error-codes.md](references/error-codes.md) — Complete error code list with descriptions and examples
- [references/options.md](references/options.md) — Full options reference with defaults
- [references/draft-comparison.md](references/draft-comparison.md) — Feature comparison across JSON Schema drafts

## Important conventions

- Always use `ZSchema.create(options?)` — never `new ZSchema()`. The factory method returns the correctly typed variant.
- Error details are on `.details` (not `.errors`).
- Import types with `import type { ... }` and values with `import { ValidateError }`.
- The default draft is `draft2020-12`. Specify the version explicitly when targeting an older draft.
