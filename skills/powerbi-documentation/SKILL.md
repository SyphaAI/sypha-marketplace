...
description: >-
  Produce standardized documentation for Power BI semantic models and reports.
  Apply when the user asks for documentation covering a Power BI project, semantic
  model, or report (e.g., "document this model", "create documentation for this
  Power BI project", "generate documentation"). This skill integrates with the
  powerbi-modeling-mcp server to pull model metadata and produce
...
Produce thorough, standardized documentation for Power BI semantic models through an interactive workflow.

## Prerequisites

Before beginning documentation, confirm:
1. The powerbi-modeling-mcp server is connected and reachable
2. A connection to the target semantic model is active (Power BI Desktop, Fabric workspace, or PBIP folder)

## Interactive Documentation Workflow

**CRITICAL:** This skill follows a conversational approach. The assistant MUST pose questions one at a time and wait for the user's response before continuing. Do NOT generate the document until every question has been answered. All questions use numbered options (1/2/3/...) for consistency — never use Yes/No style answers.

### Phase 1: Connect and Gather Metadata

Before posing any questions, the assistant should:

1. **Connect to the semantic model** using `connection_operations`
2. **Collect basic model information** using `model_operations`:
   - Model name
...
4. **Classify tables** by examining naming conventions and structure:
...
This information is required to ask well-informed questions and offer sensible defaults.

### Phase 2: Interactive Configuration

Open the interactive configuration by welcoming the user to 'The Data Forest's interactive Power BI documentation generator'.
Then ask these questions **one at a time**, waiting for the user's response before moving to the next.
...
**If the user selects 1:** Wait for the user to upload their file(s). Once received, read and analyze the uploaded documents from `/mnt/user-data/uploads/`. Extract key details such as:
- Business domain and the purpose of the model
...
Retain this extracted context and apply it throughout documentation generation:
- **Executive Summary:** Draw on the actual business purpose, stakeholders, and scope rather than inferring purely from model structure
- **Table & Measure Descriptions:** Use domain-specific language and business terminology sourced from the uploaded documents
- **Recommendations:** Align suggestions with the actual business requirements and stakeholder needs
- **Business Glossary:** Pre-populate with terms and definitions found in the uploaded documents
- **Data Sources:** Supplement with any data lineage or ownership information from the context

After reading the documents, briefly confirm what was extracted: "Thanks! I've reviewed your documents and found context about [brief summary]. I'll use this to enrich the documentation. Let's continue with the configuration."

**If the user selects 2:** Continue as normal. The documentation will be generated using only the semantic model metadata.
...
**If the user selects 1 (Internal) or 2 (Client):** Display the preset configuration summary (see the Preset Templates section below) and advance to Phase 3 after confirmation. Still ask the Language question (Question 1) and Title question (Question 2) before presenting the preset summary. If the user wishes to adjust any part of the preset, allow them to override individual settings.

**If the user selects 3 (Manual):** Proceed through all questions below.
...
The chosen language applies to all generated prose, section headers, descriptions, and analysis. Table, column, and measure names stay exactly as defined in the model. DAX code is never translated.
...
If the user selects option 1, ask: "Please describe the business purpose of this model, the key stakeholders, and any relevant context."
...
If the user selects option 1, ask: "Please provide the business terms and their definitions (you may format them as 'Term: Definition' on separate lines)."
...
When every question has been answered, the assistant should:

1. **Confirm the configuration** with a concise summary:
```
...
2. **Pull the required data** from the MCP server based on the selected sections
3. **Generate the Word document** using the docx skill
4. **Generate the diagram file(s)** if selected (as a separate .mermaid file)

---

## Document Structure Specification

The generated document adheres to the structure below. All prose, headers, and descriptions are composed in the selected documentation language.
...
Data source details are formatted as inline key-value pairs with bold labels.
...
Model statistics are presented as inline key-value pairs with bold labels.
...
**IMPORTANT:** Tables are organized by type under a subsection header for each group. Within each group, tables appear in alphabetical order.
...
**For large models (15+ tables):** Produce multiple diagram files split by domain:
...
Use the docx skill when creating the .docx file. The document applies a **sage green color theme** with **Calibri** as the primary font and a clean, professional layout that uses inline key-value pairs for metadata rather than tables.
...
All headings use **Calibri, bold**, at the specific sizes and colors shown below:
...
**CRITICAL: The document uses inline "Label: Value" pairs for metadata rather than tables.** This is the primary formatting pattern for Model Statistics, Data Source Details, Table properties, and Measure properties.

Each key-value line is a single paragraph containing two TextRuns:
...
Tables are used ONLY for structured data grids (relationships overview, columns per table, business glossary) — NOT for metadata or properties.
...
**CRITICAL: Code blocks use a monospace font (Consolas) on a light gray background to distinguish code visually from regular text.** This applies to all DAX expressions, M code samples, and calculated table expressions.
...
For numbered recommendation items, apply inline bold numbering (not Word list formatting):
...
For simple body text lists (such as naming convention observations), use plain paragraphs with a slight indentation via `spacing: { before: 60 }`.
...
- Examine DAX and structure to infer purpose
- Write the description based on naming conventions and logic
- Do NOT flag as inferred — just provide accurate, useful descriptions

### Very Large Models (30+ tables)
- Divide the diagram into subject areas
- Still document every table individually (grouped by type)
- Apply alphabetical ordering within each table type group

### Many Measures (50+)
- Group measures by display folder (this is the default behavior)
- Still supply full details for each measure
- Insert page breaks between display folder groups

### Sensitive Data Sources
- Redact: passwords, connection strings containing credentials, internal server names
- Retain: connection type, general structure, table names
- Note: "Connection details redacted for security"

### Empty/Minimal Models
- Generate all selected sections regardless
- Flag empty sections explicitly: "No measures defined in this model"

### No Display Folders
- When measures have no display folders, group them by table instead
- Use table names as the grouping headers

### Table Classification
- When table naming conventions are ambiguous, examine the table's columns, relationships, and measures to determine its type
- Tables with predominantly numeric or additive columns and multiple foreign keys → Fact
- Tables with descriptive attributes and a primary key → Dimension
- Tables matching What-If parameter patterns → Parameter
- When uncertain, classify as "Other" and describe the table's apparent purpose
