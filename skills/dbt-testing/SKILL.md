...
description: >-
  ...
  generic tests, and singular tests. Apply this skill when implementing data
  quality checks, adding primary/foreign key constraints, building custom tests,
...
Equip AI agents with deep expertise in dbt testing strategies, offering guidance on implementing
comprehensive data quality checks through database-enforced constraints, generic tests, and custom
singular tests to maintain data integrity across all layers.
...
The `dbt_constraints` package delivers database-level enforcement (not just dbt tests):
...
database constraints that are enforced around the clock.
...
**Limitation**: Validates only during `dbt test` runs and does not prevent bad data from entering through other sources.
...
**Keep it simple** - verify source data integrity only.
...
**Add business logic validation** - confirm that calculated fields produce sensible results.
...
**Maximum enforcement** - apply all constraint types to guarantee production data quality.
...
- `error`: Test failure halts dbt execution (exit code 1)
- `warn`: Test failure logs a warning but execution continues (exit code 0)

**Use `warn` for**:

- Data quality checks that should not block deployment
...
Add tests as models are built, not after they are deployed.
...
- Continuous enforcement (not just during dbt runs)
- Performance optimization
- Stronger integration with BI tools
...
- Avoid over-testing trivial columns
- Concentrate on business-critical fields
- Use sampling for very large tables when necessary
...
- Begin with dbt_constraints for primary/foreign keys
...
- Database-level enforcement (continuous, not just during dbt runs)
...
- Use `--store-failures` to persist failing records
- Query the test failure table
- Inspect the actual data that failed the test
- Add more targeted tests to isolate the problem
...
**Goal**: Equip AI agents to act as expert dbt testers, implementing comprehensive, database-enforced
data quality checks that safeguard data integrity across all layers and access patterns.
