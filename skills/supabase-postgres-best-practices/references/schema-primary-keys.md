...
The primary key you choose has direct consequences for insert throughput, index size, and replication
efficiency.
...
- Single-database systems: `bigint identity` (sequential, 8 bytes, SQL-standard)
- Distributed or externally exposed IDs: UUIDv7 (requires pg_uuidv7) or ULID (time-ordered, no
  fragmentation)
- `serial` is functional but `identity` is the SQL standard and should be used in new
  applications
- Do not use random UUIDs (v4) as primary keys on large tables — they cause index
  fragmentation
