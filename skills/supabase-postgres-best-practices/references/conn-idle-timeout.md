...
Connections left idle consume resources unnecessarily. Set timeout values so the server automatically reclaims them.
...
When using a connection pooler, apply the timeout settings at that layer:
