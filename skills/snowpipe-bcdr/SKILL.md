...
name: snowpipe-bcdr
description: >-
  Apply when designing or operating Snowpipe disaster recovery on Azure ADLS Gen2
  — selecting among dual-pipe, RA-GRS, active-active, or Failover Group
  patterns, and executing failover/failback/catchup procedures. Triggers: snowpipe BCDR,
...
This skill assists developers and data engineers in designing, implementing, and operating Snowpipe business continuity / disaster recovery on **Azure ADLS Gen2**. Six patterns are covered — select one based on RPO, RTO, Snowflake edition, and cost tolerance.
...
Options 2–6 require the **Business Critical Edition** for Failover Groups. On lower editions, only Option 1 is available.
...
This skill covers **Azure ADLS Gen2 only**. AWS S3 / GCS variants are not in scope.
...
Every option follows the same structure: storage integration → stage → pipe (with `AUTO_INGEST=TRUE` and notification integration) → monitoring views → catchup procedure.
...
Watch for pending files (`>1000` for `>15 min`), error rate from `SNOWFLAKE.ACCOUNT_USAGE.COPY_HISTORY` (`>5%/hr`), and load latency (`>30 min`).
...
Catchup options: `ALTER PIPE … REFRESH` (≤7 days), `COPY INTO … FROM @stage` (any file age), or a cursor procedure that compares `DIRECTORY(@stage)` against `COPY_HISTORY` to produce an audit trail.
...
- Step 2 — obtain approval before executing `CREATE`/`ALTER` statements for storage integrations, pipes, and failover groups.
