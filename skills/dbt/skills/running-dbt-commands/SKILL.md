...
description: Constructs and executes dbt CLI commands, selects the appropriate dbt executable, and structures command parameters. Use when running models, tests, builds, compiles, or show queries via the dbt CLI. Use when unsure which dbt executable to use or how to format command parameters.
...
1. **Use MCP tools if available** (`dbt_build`, `dbt_run`, `dbt_show`, etc.) - these automatically manage paths, timeouts, and output formatting
2. **Use `build` instead of `run` or `test`** - `test` does not refresh the model, so validating a model change requires `build`. `build` performs both a `run` and a `test` for each node (model, seed, snapshot) following DAG order
3. **Always use `--quiet`** combined with `--warn-error-options '{"error": ["NoNodesForSelectionCriteria"]}'` to reduce output noise while still surfacing selector typos
4. **Always use `--select`** - never execute the entire project without explicit user approval
...
Three CLIs are available. **Ask the user which one they're using if unsure.**
...
**Common setup:** Core installed in a venv with Fusion at `~/.local/bin`. Invoking `dbt` uses Core. Use `dbtf` or `~/.local/bin/dbt` to target Fusion.

## Selectors

**A selector is always required.** Graph operators:
...
Use `dbt list` to preview the selection before executing. Useful for verifying complex selectors.
...
Preview data using `dbt show`. Use `--inline` for ad-hoc SQL queries.
...
**Important:** Use the `--limit` flag rather than a SQL `LIMIT` clause.

## Variables

Pass as a STRING, not a dict. No special characters (`\`, `\n`).
...
After a dbt command completes, inspect `target/run_results.json` for detailed execution information:
...
Point to production data instead of materializing upstream models:
...
Disable or relax SQL analysis for models with dynamic SQL or unrecognized UDFs:
