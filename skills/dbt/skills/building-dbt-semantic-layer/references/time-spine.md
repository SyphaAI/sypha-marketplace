...
A time spine is required for time-based joins and aggregations in MetricFlow. It underpins time-based metrics and dimensions.
...
**Using the `dbt.date_spine` macro** (when your adapter supports it):
...
> **Note**: `dbt.date_spine()` is not supported by every adapter. When the macro is unavailable, fall back to raw SQL using `generate_series` or the equivalent date generation function for your warehouse.
...
Query using metrics (if metrics have been defined):
...
If your project already has a date dimension model, register it as a time spine:
