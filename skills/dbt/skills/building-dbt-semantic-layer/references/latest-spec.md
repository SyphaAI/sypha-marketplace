...
This guide covers authoring with the **latest** dbt Semantic Layer YAML spec, available in **dbt Core 1.12+** and **Fusion** (always).

In the latest spec, semantic models are expressed as metadata annotations attached to your dbt models rather than declared as standalone top-level resources. Measures are superseded by simple metrics defined inline within models.
...
Add a `semantic_model:` block to the model's YAML with `enabled: true`. Set `agg_time_dimension` at the model level to point to the primary time column. If the model has no time column, notify the user that time-based metrics are unavailable. Ask whether the user wants to create a derived time dimension instead.
...
Locate the primary key column (look for an `_id` suffix, uniqueness tests, or an explicit config). Add an `entity:` block to that column's entry. For foreign keys, declare each as `entity: type: foreign`.
...
Review columns for dimension candidates:
- Time columns -> `dimension: type: time` with `granularity` set at the column level
- Categorical columns (strings, booleans) -> `dimension: type: categorical`

Show the proposed dimensions to the user and request confirmation before proceeding.
...
Add simple metrics to the model. For each metric, gather: name, description, label, aggregation type, and expression. Supported aggregation types: `sum`, `min`, `max`, `average`, `median`, `count`, `count_distinct`, `percentile`, `sum_boolean`.
...
Use the `derived_semantics` block for dimensions or entities that do not map 1:1 to a physical column. The `expr` field is mandatory.
...
Simple metrics declared inside a model act as building blocks. Advanced metrics that reference simple metrics _within the same model_ belong under the model's `metrics:` key. Advanced metrics that reference metrics _from different models_ belong under the top-level `metrics:` key.

#### Derived Metrics

Produce a new metric by combining multiple metrics with an expression.
...
Accumulate a metric across a running window or grain-to-date period. Requires a [time spine](time-spine.md).
...
Divide one metric by another to form a ratio. Both numerator and denominator accept either a plain string (metric name) or a dict (with `name`, `filter`, `alias`).
...
Track how frequently one event is followed by another for a given entity within a defined time window.
...
With constant properties (enforce matching dimension values between base and conversion events):
...
When a metric depends on more than one semantic model, declare it at the top-level `metrics:` key:
...
For slowly changing dimension tables, apply `validity_params` to time dimensions and use the `natural` entity type:
...
SCD Type II semantic models do not support simple metrics.
