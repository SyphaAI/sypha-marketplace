...
This guide covers authoring with the **legacy** dbt Semantic Layer YAML spec, available in **dbt Core 1.6 through 1.11**. dbt Core 1.12+ continues to support this spec, but the [latest spec](latest-spec.md) is preferred for new projects.

In the legacy spec, semantic models are declared as standalone top-level resources, independent of dbt model definitions. Measures are a fundamental concept that serve as the foundation for metrics.
...
Create a top-level `semantic_models:` entry that references the dbt model through `ref()`. Set `defaults.agg_time_dimension` to the primary time column. If the model has no time column, alert the user that time-based measures and metrics are not supported. Ask whether the user would like to create a derived time dimension.
...
Identify the primary key column. Add it to the `entities:` array with `type: primary`. When the entity name differs from the column name, use `expr` to point to the actual column. For foreign keys, declare them as `type: foreign`.
...
When the dimension name differs from the column name, use `expr` to reference the actual column.
...
Add a `measures:` array to the semantic model for aggregations. Then declare top-level `metrics:` that refer to those measures through `type_params`.

Supported aggregation types: `sum`, `min`, `max`, `average`, `sum_boolean`, `count_distinct`, `median`, `percentile`.
...
For measures such as account balances or MRR that must not be summed across time:
...
All metrics are declared at the top-level `metrics:` key and reference measures through `type_params`.
...
Produce a result by combining multiple metrics with an expression.
...
Accumulate a measure over a running window or grain-to-date period. Requires a [time spine](time-spine.md).
...
Express one metric divided by another. Numerator and denominator accept either a string (metric name) or a dict (with `name`, `filter`, `alias`).
...
Quantify how frequently one event is followed by another for a given entity within a time window.
...
For slowly changing dimension tables, place `validity_params` inside `type_params` and use the `natural` entity type:
...
SCD Type II semantic models do not support measures.
