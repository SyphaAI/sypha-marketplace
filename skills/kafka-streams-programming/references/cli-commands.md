...
Exact CLI commands for every operation. Copy these directly rather than guessing syntax.
...
**IMPORTANT: CLI authentication and App authentication are SEPARATE mechanisms.**
- The `confluent` CLI relies on your **login session** for management operations (creating topics, consuming messages, listing resources, etc.).
- The **API key/secret** in `.env` is used by the **application** to authenticate via SASL_SSL at runtime.
- Do NOT attempt to pass `--api-key` to topic management commands — that flag does not exist.
- `confluent api-key store` is only relevant if you wish to associate a key with a cluster for future reference — it has no effect on CLI login authentication.
...
**Create with config (CC):** Append `--config retention.ms=604800000`
...
```
Drop `--value-format` and the SR flags when consuming plain text.
...
> The `--key-deserializer` override is necessary because these schema-aware consumers default to `KafkaAvroDeserializer` (or the equivalent) for the key. Our topologies write string keys using `Serdes.String()`, and a raw UTF-8 key carries no `0x00` magic byte or 4-byte schema ID — the schema-aware deserializer therefore fails on the very first record.
...
**Do not use plain `kafka-console-producer` for schematized topics** — it emits raw strings without the Schema Registry magic byte, which triggers `Unknown magic byte!` deserialization errors.
