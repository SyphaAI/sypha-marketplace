...
description: >-
  Design, build, and troubleshoot Kafka Streams apps (JVM-embedded stream
  processing). Activate when user references KStream, KTable, topology,
  TopologyTestDriver, StreamsBuilder, interactive queries, GlobalKTable,
  joins/windows/aggregations, or debugging symptoms (rebalancing, state stores,
  lag, deserialization errors). Also activate when user wants to optimize Kafka
...
Before responding in any mode (Architect, Build, Debug), verify the target environment if the user has not already stated it: **Apache Kafka | Confluent Platform | Confluent Cloud | WarpStream**. Versions and auth methods shape every recommendation — KIP-1071 support, SASL config, ACL model, transactional-id expiry, and CLI tool names all differ based on this choice. Omit the question only when the user has already named the environment.
...
Identify the user's intent and enter the corresponding mode:
...
When intent is unclear, default to **Architect** — understand the problem before writing code.
...
Design the correct topology. Map the user's data problem to KS primitives.
...
Match the problem to a pattern (read `references/topology-patterns.md` only for the specific pattern needed). Present: why it fits, data flow in plain English, KS primitives involved, tradeoffs/alternatives.
...
Once the user confirms, proceed to Build Mode.

---

## Build Mode

Produce a complete, runnable Kafka Streams project.
...
Present the plan: topics to create (source/output/DLQ), schemas to register. Changelog/repartition topics are auto-created by KS. **If the user states that input topics already exist, exclude them from `create-topics.sh` — the script should only create new topics (typically output + DLQ).**
...
If the user wants sample data: generate SampleDataProducer.java and a `produce` task.
...
Explain the topology, config choices, how to run the app, and what to monitor. Note that `group.protocol=streams` (KIP-1071) delivers 50-80% faster rebalancing (requires AK 4.2+/CP 8.2+).
...
**Confluent Cloud:**
You usually cannot run end-to-end yourself because the cluster + SR API keys belong to the user. Do as much as possible without them, then hand off the rest:
1. If `.env` has real CC creds (the user has configured a real `.env`): run the app locally pointed at CC (`./gradlew run` auto-loads `.env`) and follow steps 3–5 above. Don't skip just because it's CC — if you have creds, run it.
...
**WarpStream:**
You usually cannot run end-to-end yourself because the WarpStream cluster and credentials belong to the user. Follow the same approach as Confluent Cloud:
...
In the handoff, clearly state which of the above steps you performed. If you ran the app and observed `RUNNING`, say so. If you only compiled, say only that. Do not imply a runtime verification that you did not carry out.
...
Confirm the target environment first (see preamble) — most debug paths branch on it. Then request: error message, config, KS/Java versions, new app or regression?

### Step 3: Diagnose and Fix

Read the relevant section in `references/debugging.md` for the identified category. Provide the fix with a clear explanation.
