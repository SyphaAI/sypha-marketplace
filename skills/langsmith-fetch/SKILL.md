...
description: >-
  Debug LangChain and LangGraph agents by retrieving execution traces from
  LangSmith Studio. Use when debugging agent behavior, investigating errors,
  analyzing tool calls, inspecting memory operations, or evaluating agent
  performance. Automatically retrieves recent traces and analyzes execution
  patterns. Requires the langsmith-fetch CLI to be installed.
metadata:
...
Retrieve execution traces directly from LangSmith Studio in your terminal to debug LangChain and LangGraph agents.
...
1. 🎯 What the agent was attempting to accomplish
2. 🛠️ Which tools were called, in order
3. ✅ Tool outcomes (success or failure)
...
**Analyze and report on:**
1. 📊 Total number of errors found
2. ❌ Error types and their frequency
3. 🕐 When errors occurred
4. 🎯 Which agents and tools failed
...
**Steps:**
1. Verify that traces exist:
   ```bash
   langsmith-fetch traces --last-n-minutes 5 --limit 5
   ```

2. **If NO traces are found:**
   - Tracing may be disabled
   - Confirm `LANGCHAIN_TRACING_V2=true` is present in the environment
   - Confirm `LANGCHAIN_API_KEY` is set
   - Verify the agent actually executed

3. **If traces are found:**
   - Examine them for errors
   - Check execution time (is the agent hanging?)
   - Confirm that tool calls completed
...
**Steps:**
1. Retrieve the specific trace
2. Review which tools were available at execution time
3. Examine the agent's reasoning behind the tool selection
4. Inspect tool descriptions and instructions
5. Propose improvements to the prompt or tool configuration
...
**Steps:**
1. Search traces for memory operations:
   ```bash
   langsmith-fetch traces --last-n-minutes 10 --limit 20 --format raw | grep -i "memory\|recall\|store"
   ```

2. Investigate:
   - Were memory tools invoked?
   - Did recall operations return results?
   - Were memories successfully stored?
   - Are retrieved memories being passed into the agent context?
...
**Possible causes:**
1. No agent activity within the selected timeframe
...
### Environment variables are not persisting
...
2. Save it to the `error-cases/` folder
3. Record what went wrong in a README
...
- Confirm that `langsmith-fetch` is installed before executing any commands
- Verify that the required environment variables are set
- Use `--format pretty` for human-readable output
- Use `--format json` when parsing or analyzing data programmatically
- When exporting sessions, maintain an organized folder structure
- Always deliver clear analysis with actionable recommendations
- When commands fail, assist with diagnosing and resolving configuration issues
