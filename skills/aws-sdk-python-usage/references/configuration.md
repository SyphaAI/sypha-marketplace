...
Every configuration option goes through `botocore.config.Config`, and several configs may be merged together:
...
Favor `total_max_attempts` over the legacy `max_attempts`.  Note that
`max_attempts` excludes the initial attempt (it really counts the retry
attempts).
...
These can alternatively be set through the `AWS_MAX_ATTEMPTS` and `AWS_RETRY_MODE` env vars or `~/.aws/config`.
...
Every client keeps its own urllib3 connection pool. When issuing parallel requests (e.g. with `concurrent.futures`), align `max_pool_connections` with your concurrency level so you avoid connection churn.
...
A default client config can also be established on a botocore Session:
