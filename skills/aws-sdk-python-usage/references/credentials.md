...
boto3 looks up credentials in the following order:
...
Usually you should rely on the default chain to resolve credentials instead of hardcoding them.
...
Create explicit sessions whenever a single process needs multiple credential sets or profiles.
...
To have credentials refreshed automatically when the assumed role expires, define a profile with `role_arn` in `~/.aws/config`:
...
Handy for checking which credentials are actually in effect:
