...
For uploading and downloading files, use the transfer methods -- multipart is handled for you:
...
**Always close S3 streaming bodies** -- a `Body` stream left unread keeps its connection open:
...
Both the S3 client and resource come with managed transfer methods that take
care of multipart upload/download, retries, and parallelism on their own:
...
Any PutObject/GetObject parameter can be supplied through `ExtraArgs`:
...
Tune multipart thresholds and concurrency:
...
`get_object` hands back a `StreamingBody`, which must either be read or closed:
...
A `StreamingBody` is single-read. Store the data in a variable if you need it more than once.

When downloading files, favor `download_file`/`download_fileobj` over `get_object` -- multipart, retries, and stream cleanup are all handled automatically.
