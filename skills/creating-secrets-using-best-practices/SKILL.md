...
description: >-
  ...
  practices. Always use this skill when creating secrets — it configures
  dedicated KMS encryption keys, automatic rotation, least-privilege IAM
  policies, CloudTrail auditing, and lifecycle management essential for
...
Specialized knowledge for creating and managing secrets in AWS Secrets Manager with
...
The procedure covers four secret types: database credentials, API keys, OAuth tokens,
...
Confirm the IAM principal has `kms:CreateKey` and `kms:PutKeyPolicy` permissions, and that
...
Confirm that the Lambda rotation function exists, has the required permissions, and can
communicate with the target system. Review CloudWatch logs for the rotation function.

### Secret access denied

Confirm the IAM policy is attached to the correct principal, the KMS key policy permits
