...
**Why:** Regular indexes store `null` for both absent fields and fields explicitly set to `null`, so `$exists` cannot be resolved from the index alone. Sparse indexes only contain entries for documents where the field is present.
...
**Why:** Indexes maintain values in sorted order. Only a left-anchored, case-sensitive `$regex` can be translated into an efficient index range scan. For case-insensitive matching, create a case-insensitive collation index instead.
...
**Why:** Server-side JavaScript is always slower than native MQL and cannot use indexes. It also presents a security risk and is deprecated. Use `$expr` with aggregation operators instead.
