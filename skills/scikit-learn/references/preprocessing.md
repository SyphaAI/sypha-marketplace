...
Data preprocessing converts raw data into a format that machine learning models can work with effectively. This covers scaling, encoding, handling missing values, and feature engineering.
...
- Rescales features to a specified range (default [0, 1])
- Formula: X_scaled = (X - X.min) / (X.max - X.min)
- Use when: Bounded values are needed, data is not normally distributed
...
- Normalizes each sample individually to unit norm
- Common norms: 'l1', 'l2', 'max'
- Use when: Each sample must be normalized independently (e.g., text features)
- Example:
```python
...
- Range: [-1, 1]
- Does not shift or center data (preserves sparsity)
...
- Generates binary columns for each category
...
- Represents categories as integers
...
**QuantileTransformer**
- Maps features to follow a uniform or normal distribution
- Robust to outliers
- Use when: You want to reduce the influence of outliers
- Example:
```python
...
**IterativeImputer**
- Models each feature with missing values as a function of the remaining features
...
**KNNImputer**
- Imputes values using k-nearest neighbors
...
- Use when: Non-linear features are needed for linear models
- Example:
```python
...
**KBinsDiscretizer**
- Divides continuous features into discrete intervals
...
**Binarizer**
- Converts features to binary (0 or 1) values based on a threshold
- Example:
```python
...
- Transforms text into a token count matrix
...
- Outperforms CountVectorizer on most tasks
- Example:
```python
...
**HashingVectorizer**
- Applies the hashing trick for memory efficiency
- No fitting required; cannot reverse-transform
...
- Repeatedly removes features based on model importance scores
...
- RFE with cross-validation to determine the optimal number of features
- Example:
```python
...
**SelectFromModel**
- Selects features based on model coefficients or importances
...
Always fit transformers exclusively on training data:
```python
# Correct
scaler = StandardScaler()
X_train_scaled = scaler.fit_transform(X_train)
