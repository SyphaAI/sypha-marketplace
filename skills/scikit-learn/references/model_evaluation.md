...
A complete guide for evaluating models, tuning hyperparameters, and selecting the best model using scikit-learn's model selection tools.
...
- Divides data into k consecutive folds
```python
...
**StratifiedKFold**
- Maintains class distribution within each fold
...
**GroupKFold**
- Prevents samples from the same group appearing in both train and validation sets
- Use when samples are not independent of one another
```python
...
**cross_val_predict**
- Obtains predictions for each sample when it was part of the validation set
...
**GridSearchCV**
- Exhaustive search across a parameter grid
...
**RandomizedSearchCV**
- Draws random combinations from parameter distributions
...
- Progressively narrows candidates using successive halving
...
Always apply stratified splitting for classification tasks:
```python
X_train, X_test, y_train, y_test = train_test_split(
...
### Cross-Validation
- Use 5 or 10-fold CV for most situations
