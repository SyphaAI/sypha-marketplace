...
Supervised learning algorithms learn from labeled training data to generate predictions on new, unseen data. Scikit-learn provides thorough implementations for both classification and regression tasks.
...
- Use when: Linear relationships exist, interpretability matters
- Example:
```python
...
- Use when: Multicollinearity is present, regularization is needed
- Example:
```python
...
- Use when: Sparse models are desired, feature selection is required
...
- Use when: Both feature selection and regularization are needed
- Example:
```python
...
- Use when: Probabilistic predictions and interpretability are required
...
- Use when: Datasets are very large (>10^4 samples)
- Example:
```python
...
**SVR (`sklearn.svm.SVR`)**
- Regression using kernel methods
...
- Non-parametric model that learns decision rules from data
- Key parameters:
  - `max_depth`: Maximum tree depth (prevents overfitting)
  - `min_samples_split`: Minimum samples required to split a node
  - `min_samples_leaf`: Minimum samples required in a leaf
  - `criterion`: 'gini', 'entropy' for classification; 'squared_error', 'absolute_error' for regression
- Use when: Interpretable model is needed, non-linear relationships exist, features are mixed types
...
- Key parameters:
  ...
  - `max_features`: Features considered for each split ('sqrt', 'log2', or int)
  - `min_samples_split`, `min_samples_leaf`: Control tree growth
- Use when: High accuracy is needed and computation is not a constraint
...
- Sequential ensemble that builds trees on residuals
- Key parameters:
  - `n_estimators`: Number of boosting stages
  - `learning_rate`: Shrinks the contribution of each tree
  - `max_depth`: Depth of individual trees (typically 3-5)
  - `subsample`: Fraction of samples used to train each tree
- Use when: High accuracy is the priority and training time is acceptable
- Frequently achieves best-in-class performance
- Example:
```python
...
- Faster gradient boosting using a histogram-based algorithm
- Natively supports missing values and categorical features
- Key parameters: Similar to GradientBoosting
- Use when: Working with large datasets or requiring faster training
- Example:
```python
...
**AdaBoost**
- Adaptive boosting that focuses on misclassified samples
- Key parameters: `n_estimators`, `learning_rate`, `estimator` (base estimator)
- Use when: A straightforward boosting approach is sufficient
- Example:
```python
...
- Use when: You want to ensemble different model types
- Example:
```python
...
- Trains a meta-model on outputs from base models
...
- Use when: Dataset is small, a simple baseline is needed
...
- Probabilistic classifiers grounded in Bayes' theorem
- Fast to train and predict
- GaussianNB: Continuous features (assumes Gaussian distribution)
- MultinomialNB: Count features (text classification)
- BernoulliNB: Binary features
- Use when: Text classification, fast baseline, probabilistic outputs are needed
- Example:
```python
...
- Use when: Complex non-linear patterns exist and dataset is large
