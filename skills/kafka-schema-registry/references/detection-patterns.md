...
Patterns used to find Kafka applications, dependencies, producers, consumers, and serializers.
...
- Have a `serialize(String topic,` method
- Rely on `ObjectMapper`, `Gson`, `Jackson`, `org.json`, or hand-built JSON construction → JSON format
- Rely on `GenericDatumWriter`, `SpecificDatumWriter`, `BinaryEncoder` → AVRO format
- Rely on `com.google.protobuf`, `toByteArray()` → PROTOBUF format
- Make NO reference to `schema.registry.url` or `SchemaRegistryClient`
