...
**When to use:**
- `repartition(n)`: Increasing partitions, requiring even distribution, or partitioning by column
- `coalesce(n)`: Decreasing partitions only (no shuffle overhead)
- `repartitionByRange()`: Requiring sorted partitions for range-based queries
...
**Spark UI Check:** When AQE is active, look for the "Adaptive" badge in the SQL tab and review coalesced partition counts in stage details.
...
**Cache when:**
- A DataFrame is accessed multiple times within the same job
- Computing the DataFrame is costly (complex joins or aggregations)
- Running iterative algorithms such as ML training loops
- Performing interactive exploration in notebooks

**Do NOT cache when:**
- The DataFrame is consumed only once
- The data volume exceeds available cluster memory
- The source is already fast to read (local SSD, columnar formats)
- The chosen storage level triggers excessive garbage collection
...
**Spark UI Check:** The Storage tab lists cached RDDs and DataFrames. Watch "Fraction Cached" — a value below 100% signals insufficient memory.
...
- Small lookup tables smaller than 200MB
- Dimension tables that are joined against large fact tables
- Configuration data that must be available across all tasks
- Eliminating shuffle overhead in map-side joins
...
**Warning:** Broadcasting tables larger than 200MB risks driver OOM errors and degraded broadcast performance. Use a sort-merge join instead.
...
**Note:** Bucketing requires a Hive metastore and saveAsTable; it is not compatible with direct file writes.
...
- Verify that cached data displays "(cached)" in the DAG
- Watch for skipped stages, which indicate cached data is being reused

### Stages Tab

- **Shuffle Write Size**: High values suggest opportunities to repartition
- **Shuffle Read Size**: Should be consistent across tasks with no skew
- **Task Duration Distribution**: High variance points to partition imbalance

### Storage Tab

- **Size in Memory**: The actual volume of cached data
- **Size on Disk**: Volume that has spilled to disk
- **Fraction Cached**: Should reach 100% when memory is adequate

### SQL Tab

- "BroadcastExchange" entries indicate a broadcast join is in use
- "ShuffleExchange" entries signal data movement between partitions
- Review "Rows Output" at each stage to understand data flow
...
1. **Target 128-256MB partitions** - Avoid sizes that are too small (scheduling overhead) or too large (OOM risk)
2. **Use 2-4 partitions per core** - Sustain full parallelism across the cluster
3. **Enable AQE in Spark 3.x** - Delegates partition sizing to the runtime optimizer
4. **Cache only reused DataFrames** - Quantify the benefit before caching indiscriminately
5. **Use MEMORY_AND_DISK** - A reliable default storage level for most workloads
6. **Broadcast tables < 200MB** - Removes shuffle overhead for small dimension tables
7. **Coalesce after filters** - Trim partition count once data volume has shrunk
8. **Repartition before joins** - Co-locate related tables to prevent unnecessary shuffles
9. **Partition writes by filter columns** - Allows readers to skip irrelevant partitions
10. **Monitor the Storage tab** - Confirm the cache fits entirely in available memory
