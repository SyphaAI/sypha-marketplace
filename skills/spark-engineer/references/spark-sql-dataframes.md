...
**Use DataFrames when:**
- Processing structured or semi-structured data in formats such as JSON, Parquet, CSV, or Avro
- Performing SQL-style operations including joins, aggregations, and filters
- Leveraging Catalyst optimizer capabilities like predicate pushdown and column pruning
- Working with columnar file formats to benefit from better compression ratios

**Use RDDs when:**
- Requiring precise control over how data is physically distributed across the cluster
- Handling unstructured data such as raw text or custom binary formats
- Implementing custom partitioning logic that the DataFrame API cannot express
- Migrating legacy code (migrate to DataFrames whenever feasible)
...
**Spark UI Check:** In the SQL tab, compare "BroadcastHashJoin" and "SortMergeJoin" entries. A broadcast join performs a fast exchange, whereas a sort-merge join triggers a full shuffle.
...
Predicate pushdown allows filters to be evaluated at the storage layer before data is loaded into Spark, significantly reducing I/O.
...
Selecting only the columns needed lets Spark skip unnecessary data at read time, particularly beneficial with columnar formats.
...
When filtering on partition columns, Spark skips entire directory segments rather than scanning the full dataset.
...
1. **Duration** - Prolonged stages reveal opportunities for query optimization
2. **Input Size** - Confirms that partition pruning has reduced the volume of data scanned
3. **Shuffle Write/Read** - Excessive shuffle points to join or aggregation inefficiencies
4. **Spill (Memory/Disk)** - Spill to disk signals memory pressure; consider increasing executor memory
...
1. **Always define explicit schemas** - Schema inference has no place in production pipelines
2. **Use built-in functions** - Reach for UDFs only when no built-in equivalent exists
3. **Broadcast small tables** - Any table under 200MB qualifies for a broadcast join
4. **Filter early** - Apply row and column reductions before joins and aggregations
5. **Select only needed columns** - Allows Spark to skip unneeded column data at read time
6. **Partition by common filter columns** - Enables Spark to prune entire partitions from scans
7. **Cache strategically** - Persist only DataFrames that are genuinely reused within the job
8. **Monitor Spark UI** - Regularly review shuffle volume, disk spill, and GC time
9. **Enable AQE in Spark 3.x** - Offloads partition and skew handling to the runtime optimizer
10. **Test with production data volume** - Query performance can shift significantly as data scales
