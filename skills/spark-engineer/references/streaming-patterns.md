...
**Use when:**
- Consuming continuous data streams (Kafka, files, sockets)
- Exactly-once processing guarantees are required
- Building real-time analytics and dashboards
- Designing event-driven architectures
- Running incremental ETL from streaming sources

**Consider alternatives when:**
- Batch processing meets the need (less complexity)
- Sub-second latency is required (consider Flink)
- Event processing is very simple (Kafka Streams may be enough)
...
Watermarks specify the maximum allowable delay for data before it is discarded. They allow Spark to:
- Reclaim old state (bounded memory usage)
- Produce results at the correct time
- Process out-of-order events correctly
...
1. **Always use watermarks** - Avoids unbounded state accumulation
2. **Choose appropriate output mode** - Append for ETL, Update for aggregations
3. **Set checkpoint locations** - Necessary for fault tolerance
4. **Use foreachBatch over foreach** - Achieves better performance for custom sinks
5. **Monitor state size** - Track memory growth through progress metrics
6. **Tune trigger intervals** - Strike the right balance between latency and throughput
7. **Match Kafka partitions to parallelism** - Consumer task count equals Kafka partition count
8. **Use stream-static joins when possible** - Less complex than stream-stream joins
9. **Test with production data rates** - Throughput characteristics change with volume
10. **Enable structured streaming UI** - Exposes detailed metrics in the Spark UI
