...
**Rules of thumb:**
- 5 cores per executor is the sweet spot (prevents HDFS I/O bottleneck)
- Reserve 1 core per node for the OS and YARN
- Reserve 1GB per node for system overhead
...
In the Stages tab, review:
- **Shuffle Write Size**: Total volume of data written during shuffle
- **Shuffle Read Size**: Total volume of data consumed from shuffle
- **Shuffle Read Blocked Time**: Time spent waiting for shuffle data to arrive
- **Shuffle Spill (Memory)**: Data that overflowed into memory spill buffers
- **Shuffle Spill (Disk)**: Data written to disk during spill (problematic — increase memory)
...
**Spark UI indicators:**
- A small number of tasks running significantly longer than the rest
- The task duration histogram exhibits a long tail
- Certain partitions holding substantially more data than others
...
- **Job Duration**: Pinpoint slow-running jobs
- **Stages**: Stage count correlates directly with shuffle frequency
- **DAG Visualization**: Trace the logical flow of data through the pipeline
...
- **Storage Memory**: Current cache utilization per executor
- **Shuffle Read/Write**: I/O patterns across the cluster
- **GC Time**: Garbage collection overhead per executor
- **Failed Tasks**: Counts of executor-level task failures

### SQL Tab

- **Duration**: Elapsed time for query execution
- **Details**: Physical plan breakdown for the query
- **Metrics**: Row counts at input and output of each operator

### Storage Tab

- **Cached RDDs/DataFrames**: Sizes and partition distribution of cached objects
- **Fraction Cached**: Should reach 100% when memory is sufficient
...
1. **Size executors appropriately** - 5 cores and 16GB memory is a common starting point
2. **Enable AQE (Spark 3.x)** - Lets the runtime optimize partitions and handle skew automatically
3. **Tune shuffle partitions** - Drive the setting from actual data size, not the default of 200
4. **Address data skew** - Apply key salting or rely on AQE's built-in skew handling
5. **Monitor Spark UI** - Inspect shuffle volume, disk spill, and GC overhead regularly
6. **Use broadcast joins** - Appropriate for any table smaller than 200MB
7. **Filter and select early** - Trim rows and columns before any shuffle operations
8. **Avoid UDFs** - Built-in functions are typically 10-100x faster
9. **Cache strategically** - Persist only DataFrames that are reused, and unpersist promptly
10. **Test at scale** - Observed performance can differ substantially as data volume grows
