...
**Use RDDs when:**
- Handling unstructured data such as raw text or custom binary formats
- Requiring fine-grained control over physical data placement
- Implementing custom partitioning logic tailored to specific access patterns
- Maintaining legacy Spark code that cannot easily be migrated
- Constructing custom data structures that cannot be expressed as DataFrames

**Prefer DataFrames when:**
- Dealing with structured or semi-structured data
- Carrying out SQL-like operations such as joins, filters, and aggregations
- Benefiting from the Catalyst optimizer's query planning
- Reading or writing standard file formats like Parquet, JSON, or ORC
...
Transformations produce a new RDD but are not evaluated until an action triggers execution.
...
**When to use:**
- `repartition(n)`: When you need to increase partitions or ensure an even distribution
- `coalesce(n)`: When reducing partition count after a filter has shrunk the dataset

---

## Pair RDD Operations

Pair RDDs — RDDs of key-value tuples — unlock a rich set of additional transformations.
...
**Spark UI Check:** Compare shuffle write sizes between operations. `reduceByKey` should produce substantially less shuffle data than `groupByKey` for an equivalent computation.

---

## Actions (Trigger Execution)

Actions either return computed values to the driver or write results to external storage.
...
**Caution:** Accumulators can be incremented more than once when tasks are retried. Restrict their use to debugging and monitoring purposes — do not embed them in business logic.
...
1. **Uneven partition sizes**: Tasks with runtimes far exceeding their peers indicate imbalanced partitions
2. **Data skew**: A single partition holding far more data than the rest
3. **Straggler tasks**: A handful of tasks completing 10x slower than the median
...
1. **Prefer DataFrames** - Fall back to RDDs only when the DataFrame API cannot express the logic
2. **Use reduceByKey over groupByKey** - Local combining before the shuffle drastically cuts data movement
3. **Preserve partitioning** - Apply mapValues or flatMapValues to retain the existing partitioner
4. **Minimize shuffles** - Co-partition related RDDs and broadcast small datasets
5. **Use mapPartitions** - Amortizes expensive initialization such as database connections across a whole partition
6. **Avoid collect on large data** - Use take, takeSample, or foreachPartition to limit driver-side data
7. **Broadcast lookup tables** - Prevents a shuffle when joining against small reference data
8. **Use accumulators for observability only** - Do not rely on them for business logic
9. **Inspect partition distribution** - Use custom partitioners to prevent skew
10. **Profile with Spark UI** - Diagnose shuffle size, disk spill, and GC overhead
