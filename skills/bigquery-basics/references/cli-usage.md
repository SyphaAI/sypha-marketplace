...
The `bq` command-line tool provides an interface to BigQuery for resource
...
- `--location`: Sets the geographic location for the job or resource.

- `--project_id`: Overrides the default project used by the command.

- `--format`: Adjusts the output format (e.g., `prettyjson`, `sparse`, `csv`).
